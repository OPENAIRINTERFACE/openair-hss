...
static uint32_t                         eNB_app_register (
  const uint32_t enb_id_start,
  const uint32_t enb_id_end,
  const Enb_properties_array_t * enb_properties);
static void                            *eNB_app_task (
  void *args_p);
void                                    mme_test_s1_start_test (
  instance_t instance);
  ...
  for (enb_id = enb_id_start; (enb_id < enb_id_end); enb_id++) {
    {
      ...
      for (mme_id = 0; mme_id < s1ap_register_eNB->nb_mme; mme_id++) {
        ...
        strncpy (s1ap_register_eNB->mme_ip_address[mme_id].ipv4_address, enb_properties->properties[enb_id]->mme_ip_address[mme_id].ipv4_address, sizeof (s1ap_register_eNB->mme_ip_address[0].ipv4_address));
        strncpy (s1ap_register_eNB->mme_ip_address[mme_id].ipv6_address, enb_properties->properties[enb_id]->mme_ip_address[mme_id].ipv6_address, sizeof (s1ap_register_eNB->mme_ip_address[0].ipv6_address));
      }
...
//------------------------------------------------------------------------------
{
  ...
  AssertFatal (enb_nb <= enb_properties_p->number, "Number of eNB is greater than eNB defined in configuration file (%d/%d)!", enb_nb, enb_properties_p->number);
  /*
  ...
  do {
    ...
    case S1AP_DEREGISTERED_ENB_IND:
      LOG_W (ENB_APP, "[eNB %d] Received %s: associated MME %d\n", instance, msg_name, S1AP_DEREGISTERED_ENB_IND (msg_p).nb_mme);
      /*
...
//------------------------------------------------------------------------------
{
  s1ap_eNB_itti_send_sctp_data_req (instance,
                                    ...
                                    s1ap_scenarios[s1c_test.scenario_index][s1c_test.tx_next_message_index].buf_len, s1ap_scenarios[s1c_test.scenario_index][s1c_test.tx_next_message_index].sctp_stream_id);
}
