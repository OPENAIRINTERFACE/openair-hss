...
def perform_flattening(tag):
    ...
    if cli == 'docker':
      ...
      cmd += ' --change "ENV PATH=/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin" '
    cmd += ' --change "WORKDIR /openair-hss" '
    cmd += ' --change "EXPOSE 9042/tcp" '
    cmd += ' --change "EXPOSE 5868/tcp" '
    cmd += ' --change "EXPOSE 9080/tcp" '
    cmd += ' --change "EXPOSE 9081/tcp" '
    cmd += ' --change "CMD [\\"/openair-hss/bin/oai_hss\\", \\"-j\\", \\"/openair-hss/etc/hss_rel14.json\\", \\"--reloadkey\\", \\"true\\"]" '
    cmd += ' --change "ENTRYPOINT [\\"/bin/bash\\", \\"/openair-hss/bin/entrypoint.sh\\"]" '
