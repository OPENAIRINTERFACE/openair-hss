...
static int s1ap_send_init_sctp(void) {
  ...
  if (message_p) {
    ...
    if (memcmp(&mme_config.ip.s1_mme_v6.s6_addr, (void *)&in6addr_any,
               sizeof(mme_config.ip.s1_mme_v6.s6_addr)) != 0) {
      message_p->ittiMsg.sctpInit.nb_ipv6_addr = 1;
      memcpy(message_p->ittiMsg.sctpInit.ipv6_address[0].s6_addr,
             mme_config.ip.s1_mme_v6.s6_addr, 16);
    }
    /*
...
//------------------------------------------------------------------------------
static void s1ap_remove_enb(void **enb_ref) {
...
//------------------------------------------------------------------------------
static bool s1ap_enb_compare_by_tac_cb(__attribute__((unused))
  ...
  if (*tac_p == enb_ref->tai_list.partial_tai_list[0]
    ...
    switch (ITTI_MSG_ID(received_message_p)) {
      ...
      case NAS_DOWNLINK_DATA_REQ: {
        ...
        s1ap_generate_downlink_nas_transport(
            NAS_DOWNLINK_DATA_REQ(received_message_p).enb_ue_s1ap_id,
      ...
      case S1AP_E_RAB_MODIFY_REQ: {
        s1ap_generate_s1ap_e_rab_modify_req(
            &S1AP_E_RAB_MODIFY_REQ(received_message_p));
      } break;

      case S1AP_E_RAB_RELEASE_REQ: {
        s1ap_generate_s1ap_e_rab_release_req(
            &S1AP_E_RAB_RELEASE_REQ(received_message_p));
      } break;
      ...
      case SCTP_DATA_IND: {
        ...
        if (s1ap_mme_decode_pdu(
                &pdu, SCTP_DATA_IND(received_message_p).payload) < 0) {
      ...
      // Handover messages from MME_APP after validation or rejection from nas
      // and S11/SAE-GW --> the respective handover method will be checked
      // inside
      case S1AP_PATH_SWITCH_REQUEST_FAILURE: {
        s1ap_handle_path_switch_request_failure(
            &S1AP_PATH_SWITCH_REQUEST_FAILURE(received_message_p));
      } break;
      case S1AP_HANDOVER_PREPARATION_FAILURE: {
        s1ap_handle_handover_preparation_failure(
            &S1AP_HANDOVER_PREPARATION_FAILURE(received_message_p));
      } break;
      case S1AP_HANDOVER_REQUEST: {
        s1ap_handle_handover_request(
            &S1AP_HANDOVER_REQUEST(received_message_p));
      } break;

      case S1AP_HANDOVER_CANCEL_ACKNOWLEDGE: {
        s1ap_handle_handover_cancel_acknowledge(
            &S1AP_HANDOVER_CANCEL_ACKNOWLEDGE(received_message_p));
      } break;

      case S1AP_PATH_SWITCH_REQUEST_ACKNOWLEDGE: {
        s1ap_handle_path_switch_req_ack(
            &S1AP_PATH_SWITCH_REQUEST_ACKNOWLEDGE(received_message_p));
      } break;
      case S1AP_HANDOVER_COMMAND: {
        s1ap_handle_handover_command(
            &S1AP_HANDOVER_COMMAND(received_message_p));
      } break;

      case S1AP_MME_STATUS_TRANSFER: {
        s1ap_handle_mme_status_transfer(
            &S1AP_MME_STATUS_TRANSFER(received_message_p));
      } break;
      ...
      case S1AP_CONFIGURATION_TRANSFER: {
        s1ap_mme_configuration_transfer(
      ...
      case TIMER_HAS_EXPIRED: {
        ...
        if (received_message_p->ittiMsg.timer_has_expired.arg != NULL) {
          enb_s1ap_id_key_t enb_s1ap_id_key = (enb_s1ap_id_key_t)(
              received_message_p->ittiMsg.timer_has_expired.arg);
          enb_ue_s1ap_id_t enb_ue_s1ap_id =
              MME_APP_ENB_S1AP_ID_KEY2ENB_S1AP_ID(enb_s1ap_id_key);
          uint32_t enb_id = ((enb_s1ap_id_key >> 24) & 0xFFFFFFFFFF);

          /** Check if the UE still exists. */
          ue_ref_p =
              s1ap_is_enb_ue_s1ap_id_in_list_per_enb(enb_ue_s1ap_id, enb_id);
          if (!ue_ref_p) {
            OAILOG_WARNING(
                LOG_S1AP,
                "Timer with id 0x%lx expired but no associated UE context!\n",
                received_message_p->ittiMsg.timer_has_expired.timer_id);
            break;
          }
          OAILOG_WARNING(LOG_S1AP,
                         "Processing expired timer with id 0x%lx for "
                         "ueId " MME_UE_S1AP_ID_FMT
                         " with s1ap_ue_context_rel_timer_id 0x%lx !\n",
                         received_message_p->ittiMsg.timer_has_expired.timer_id,
          ...
          if (received_message_p->ittiMsg.timer_has_expired.timer_id ==
            ...
            s1ap_mme_handle_ue_context_rel_comp_timer_expiry(ue_ref_p);
                     ...
                     ue_ref_p->s1ap_handover_completion_timer.id) {
...
int s1ap_mme_init(void) {
  ...
  hash_table_ts_t *h = hashtable_ts_init(
      &g_s1ap_enb_coll, mme_config.max_s1_enbs, NULL, s1ap_remove_enb,
      bs1); /**< Use a better removal handler. */
  bdestroy_wrapper(&bs1);
  if (!h) return RETURNerror;

  bstring bs2 = bfromcstr("s1ap_mme_id2assoc_id_coll");
  h = hashtable_ts_init(&g_s1ap_mme_id2assoc_id_coll, mme_config.max_ues, NULL,
                        hash_free_int_func, bs2);
  ...
  OAILOG_DEBUG(LOG_S1AP,
               "Initializing S1AP interface: DONE, but not reachable yet (wait "
               "for MME<->HSS CER procedure)\n");
  return RETURNok;
}
//------------------------------------------------------------------------------
void s1ap_mme_exit(void) {
  ...
  if (hashtable_ts_destroy(&g_s1ap_enb_coll) != HASH_TABLE_OK) {
    OAILOG_ERROR(LOG_S1AP,
                 "An error occured while destroying s1 eNB hash table. \n");
  }
  if (hashtable_ts_destroy(&g_s1ap_mme_id2assoc_id_coll) != HASH_TABLE_OK) {
    OAILOG_ERROR(LOG_S1AP,
                 "An error occured while destroying assoc_id hash table. \n");
  }
...
void s1ap_dump_enb_list(void) {
  hashtable_ts_apply_callback_on_elements(&g_s1ap_enb_coll,
                                          s1ap_dump_enb_hash_cb, NULL, NULL);
}
...
void s1ap_dump_enb(const enb_description_t *const enb_ref) {
  ...
  hashtable_ts_apply_callback_on_elements(
      (hash_table_ts_t *const) & enb_ref->ue_coll, s1ap_dump_ue_hash_cb, NULL,
      NULL);
  indent--;
  eNB_LIST_OUT("");
#else
...
//------------------------------------------------------------------------------
bool s1ap_enb_compare_by_enb_id_cb(__attribute__((unused))
                                   const hash_key_t keyP,
                                   void *const elementP, void *parameterP,
                                   void **resultP) {
...
enb_description_t *s1ap_is_enb_id_in_list(const uint32_t enb_id) {
  ...
  hashtable_ts_apply_callback_on_elements(
      (hash_table_ts_t *const) & g_s1ap_enb_coll, s1ap_enb_compare_by_enb_id_cb,
      (void *)enb_id_p, (void **)&enb_ref);
  return enb_ref;
}

//------------------------------------------------------------------------------
void s1ap_is_tac_in_list(const tac_t tac, int *num_enbs,
  ...
  hashtable_ts_apply_list_callback_on_elements(
      (hash_table_ts_t *const) & g_s1ap_enb_coll, s1ap_enb_compare_by_tac_cb,
      (void *)tac_p, &ea);
  OAILOG_DEBUG(
      LOG_S1AP,
      "Found %d matching enb references based on the received tac " TAC_FMT
      ". \n",
      ea.num_elements, tac);
...
enb_description_t *s1ap_is_enb_assoc_id_in_list(
  ...
  enb_description_t *enb_ref = NULL;
  hashtable_ts_get(&g_s1ap_enb_coll, (const hash_key_t)sctp_assoc_id,
                   (void **)&enb_ref);
  return enb_ref;
}
...
bool s1ap_ue_compare_by_enb_ue_s1ap_id_cb(__attribute__((unused))
                                          const hash_key_t keyP,
...
ue_description_t *s1ap_is_ue_enb_id_in_list(
    enb_description_t *enb_ref, const enb_ue_s1ap_id_t enb_ue_s1ap_id) {
  ue_description_t *ue_ref = NULL;
  hashtable_ts_get((hash_table_ts_t *const) & enb_ref->ue_coll,
                   (const hash_key_t)enb_ue_s1ap_id, (void **)&ue_ref);
  return ue_ref;
}

//------------------------------------------------------------------------------
ue_description_t *s1ap_is_enb_ue_s1ap_id_in_list_per_enb(
    const enb_ue_s1ap_id_t enb_ue_s1ap_id, const uint32_t enb_id) {
  ue_description_t *ue_ref = NULL;
  enb_description_t *enb_ref = NULL;
...
//------------------------------------------------------------------------------
bool s1ap_enb_find_ue_by_mme_ue_id_cb(__attribute__((unused))
                                      const hash_key_t keyP,
                                      void *const elementP, void *parameterP,
                                      void **resultP) {
  enb_description_t *enb_ref = (enb_description_t *)elementP;
...
//------------------------------------------------------------------------------
bool s1ap_ue_compare_by_s11_sgw_teid_cb(__attribute__((unused))
                                        const hash_key_t keyP,
                                        void *const elementP, void *parameterP,
                                        void **resultP) {
...
//------------------------------------------------------------------------------
bool s1ap_enb_find_ue_by_s11_sgw_teid_cb(__attribute((unused))
                                         const hash_key_t keyP,
                                         void *const elementP, void *parameterP,
                                         void **resultP) {
  enb_description_t *enb_ref = (enb_description_t *)elementP;
  ...
  hashtable_ts_apply_callback_on_elements(
      (hash_table_ts_t *const) & enb_ref->ue_coll,
      s1ap_ue_compare_by_s11_sgw_teid_cb, parameterP, resultP);
  if (*resultP) {
    return true;
  }
  return false;
...
ue_description_t *s1ap_is_s11_sgw_teid_in_list(const s11_teid_t teid) {
  ...
  hashtable_ts_apply_callback_on_elements(&g_s1ap_enb_coll,
                                          ...
                                          (void *)teid_id_p, (void **)&ue_ref);
  return ue_ref;
}

//------------------------------------------------------------------------------
void s1ap_notified_new_ue_mme_s1ap_id_association(
    const sctp_assoc_id_t sctp_assoc_id, const enb_ue_s1ap_id_t enb_ue_s1ap_id,
    const mme_ue_s1ap_id_t mme_ue_s1ap_id) {
  ...
  if (enb_ref) {
    ...
    if (ue_ref) {
      ...
      hashtable_rc_t h_rc = hashtable_ts_insert(
          &g_s1ap_mme_id2assoc_id_coll, (const hash_key_t)mme_ue_s1ap_id,
...
enb_description_t *s1ap_new_enb(void) {
  ...
  hashtable_ts_init(&enb_ref->ue_coll, mme_config.max_ues, NULL, free_wrapper,
                    bs);
  bdestroy_wrapper(&bs);
...
ue_description_t *s1ap_new_ue(const sctp_assoc_id_t sctp_assoc_id,
  ...
  MSC_LOG_EVENT(MSC_S1AP_MME,
                " Associating ue  (enb_ue_s1ap_id: " ENB_UE_S1AP_ID_FMT
                ") to eNB %s",
                ue_ref->mme_ue_s1ap_id, enb_ref->enb_name);
  return ue_ref;
}

//------------------------------------------------------------------------------
void s1ap_set_tai(enb_description_t *enb_ref, S1AP_SupportedTAs_t *ta_list) {
  ...
  enb_ref->tai_list.partial_tai_list[0].typeoflist =
      TRACKING_AREA_IDENTITY_LIST_ONE_PLMN_NON_CONSECUTIVE_TACS;
  TBCD_TO_PLMN_T(plmn_i, &enb_ref->tai_list.partial_tai_list[0]
  ...
  for (int i = 0; i < ta_list->list.count && i < 3; i++) {
    ...
    enb_ref->tai_list.partial_tai_list[0]
        .u.tai_one_plmn_non_consecutive_tacs.tac[i] = tac_value;
...
void s1ap_remove_ue(ue_description_t *ue_ref) {
  ...
  if (ue_ref->s1ap_ue_context_rel_timer.id != S1AP_TIMER_INACTIVE_ID) {
    if (timer_remove(ue_ref->s1ap_ue_context_rel_timer.id, NULL)) {
      OAILOG_ERROR(LOG_S1AP,
                   "Failed to stop s1ap ue context release complete timer for "
                   "UE id  %d \n",
  ...
  if (ue_ref->s1ap_handover_completion_timer.id != S1AP_TIMER_INACTIVE_ID) {
    if (timer_remove(ue_ref->s1ap_handover_completion_timer.id, NULL)) {
      OAILOG_ERROR(LOG_S1AP,
                   "Failed to stop s1ap handover completion timer for UE "
  ...
  /** We will try to remove the SCTP association too, but it will anyways be set
...
// bool
// s1ap_add_bearer_context_to_list (__attribute__((unused))const hash_key_t
// keyP,
//               void * const bearer_ctx_void,
//               void *parameterP_bearer_list,
//               void __attribute__((unused)) **unused_resultP)
//{
//  const bearer_context_t * const bearer_ctxt_p = (const bearer_context_t
//  *)bearer_ctx_void; if (bearer_ctxt_p == NULL) {
//    return false;
//  }
//
//  const S1ap_E_RABToBeSetupListHOReqIEs_t * const e_RABToBeSetupListHOReq_p =
//  (const S1ap_E_RABToBeSetupListHOReqIEs_t *)parameterP_bearer_list; if
//  (e_RABToBeSetupListHOReq_p == NULL) {
//    return false;
//  }
//  S1ap_E_RABToBeSetupItemHOReq_t          e_RABToBeSetupHO = {0}; // yes,
//  alloc on stack
//
//  if(s1ap_generate_bearer_context_to_setup(bearer_ctxt_p, &e_RABToBeSetupHO)
//  != RETURNok){
