...
typedef struct ue_description_s {
  ...
  struct s1ap_timer_t
      s1ap_handover_completion_timer;  // todo: not TXXXX value found for this.
} ue_description_t;
...
typedef struct enb_description_s {
  ...
  tai_list_t tai_list;  ///< Tracking Area Identifiers signaled by the eNB (for
                        ///< each cell - used for paging.).
  /*@}*/
 ...
 * @returns NULL if no eNB matchs the eNB id, or reference to the eNB element in
 *list if matches
 **/
 ...
 * @returns NULL if no eNB matchs the sctp assoc id, or reference to the eNB
 *element in list if matches
 **/
 ...
 * @returns NULL if no UE matchs the ue_enb_id, or reference to the ue element
 *in list if matches
 **/
...
/** \brief Look for given ue enb s1ap id in the list of UEs for a particular
 *enb. \param enb_id The unique ue_enb_id to search in list
 * @returns NULL if no UE matchs the ue_enb_id, or reference to the ue element
 *in list if matches
 **/
...
/** \brief associate mainly 2(3) identifiers in S1AP layer: {mme_ue_s1ap_id_t,
 *sctp_assoc_id (,enb_ue_s1ap_id)}
 **/
 ...
 * Calls s1ap_dump_enb
 **/
bool s1ap_dump_enb_hash_cb(const hash_key_t keyP, void* const enb_void,
                           void* unused_parameterP, void** unused_resultP);
 ...
 * Calls s1ap_dump_ue
 **/
bool s1ap_dump_ue_hash_cb(const hash_key_t keyP, void* const ue_void,
                          void* unused_parameterP, void** unused_resultP);
...
bool s1ap_enb_compare_by_enb_id_cb(const hash_key_t keyP, void* const elementP,
                                   void* parameterP,
