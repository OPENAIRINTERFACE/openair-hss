...
extern hash_table_ts_t g_s1ap_enb_coll;  // contains eNB_description_s, key is
                                         // eNB_description_s.assoc_id

static const char *const s1_enb_state_str[] = {"S1AP_INIT", "S1AP_RESETTING",
                                               "S1AP_READY", "S1AP_SHUTDOWN"};
...
s1ap_message_decoded_callback s1ap_messages_callback[][3] = {
    ...
    {0,
     s1ap_mme_handle_handover_resource_allocation_response, /* HandoverResourceAllocation
                                                             */
    ...
    {s1ap_mme_handle_erab_release_indication, 0,
     0}, /* E_RABReleaseIndication */
...
int s1ap_mme_handle_message(const sctp_assoc_id_t assoc_id,
   ...
   */
  if (s1ap_messages_callback[pdu->choice.initiatingMessage.procedureCode]
                            [pdu->present - 1] == NULL) {
  ...
  int ret =
      (*s1ap_messages_callback[pdu->choice.initiatingMessage.procedureCode]
                              [pdu->present - 1])(assoc_id, stream, pdu);
  ASN_STRUCT_FREE_CONTENTS_ONLY(asn_DEF_S1AP_S1AP_PDU, pdu);
  return ret;
}
...
int s1ap_mme_handle_s1_setup_request(const sctp_assoc_id_t assoc_id,
  ...
  if (hss_associated) {
    ...
    OAILOG_MESSAGE_START(OAILOG_LEVEL_DEBUG, LOG_S1AP, (&context),
    ...
    if (ie->value.choice.Global_ENB_ID.eNB_ID.present ==
      ...
      if (ie->value.choice.Global_ENB_ID.eNB_ID.choice.homeENB_ID.size != 28) {
        ...
        OAILOG_DEBUG(
            LOG_S1AP, "S1-Setup-Request homeENB_ID.size %lu (should be 28)\n",
      ...
      if (ie->value.choice.Global_ENB_ID.eNB_ID.choice.macroENB_ID.size != 20) {
        ...
        OAILOG_DEBUG(
            LOG_S1AP, "S1-Setup-Request macroENB_ID.size %lu (should be 20)\n",
            ie->value.choice.Global_ENB_ID.eNB_ID.choice.macroENB_ID.size);
      }
    ...
    if (nb_enb_associated >= max_s1_enb_connected) {
      OAILOG_ERROR(LOG_S1AP,
                   "There is too much eNB connected to MME, rejecting the "
                   "association\n");
       ...
       */
      rc = s1ap_mme_generate_s1_setup_failure(
          assoc_id, S1AP_Cause_PR_misc,
          S1AP_CauseMisc_control_processing_overload, S1AP_TimeToWait_v20s);
      OAILOG_FUNC_RETURN(LOG_S1AP, rc);
    }

    if (s1ap_mme_compare_gummei(&global_enb_id->pLMNidentity) != RETURNok) {
      OAILOG_ERROR(LOG_S1AP,
                   "No Common GUMMEI with eNB, generate_s1_setup_failure\n");
    ...
    if ((enb_association = s1ap_is_enb_id_in_list(enb_id)) == NULL) {
      ...
      if ((enb_association = s1ap_is_enb_assoc_id_in_list(assoc_id)) == NULL) {
        OAILOG_ERROR(LOG_S1AP,
                     "Ignoring s1 setup from unknown assoc %u and enbId %u",
                     assoc_id, enb_id);
        OAILOG_FUNC_RETURN(LOG_S1AP, RETURNerror);
      } else {
        ...
        s1ap_set_tai(enb_association,
                     &ie_supported_tas->value.choice.SupportedTAs);
        if (enb_name != NULL) {
          memcpy(enb_association->enb_name,
                 ie_enb_name->value.choice.ENBname.buf,
                 ie_enb_name->value.choice.ENBname.size);
          enb_association->enb_name[ie_enb_name->value.choice.ENBname.size] =
              '\0';
        }
      }
    } else {
      ...
      /*
       * eNB has been fount in list, consider the s1 setup request as a reset
       * connection,
       * * * * reseting any previous UE state if sctp association is != than the
       * previous one
       */
      if (enb_association->sctp_assoc_id != assoc_id) {
        OAILOG_ERROR(LOG_S1AP,
                     "Rejecting s1 setup request as eNB id %d is already "
                     "associated to an active sctp association"
                     "Previous known: %d, new one: %d\n",
                     enb_id, enb_association->sctp_assoc_id, assoc_id);
        rc = s1ap_mme_generate_s1_setup_failure(
            assoc_id, S1AP_Cause_PR_misc, S1AP_CauseMisc_unspecified,
            -1); /**< eNB should attach again. */

        /** Also remove the old eNB. */
        OAILOG_INFO(LOG_S1AP,
                    "Rejecting the old eNB connection for eNB id %d and old "
                    "assoc_id: %d\n",
                    enb_id, assoc_id);
        s1ap_dump_enb_list();
      ...
      OAILOG_INFO(LOG_S1AP,
                  "We found the eNB id %d in the current list of enbs with "
                  "matching sctp associations:"
                  "assoc: %d\n",
     ...
     */
    OAILOG_ERROR(LOG_S1AP,
                 "Rejecting s1 setup request Can not process the request, MME "
                 "is not connected to HSS\n");
    rc = s1ap_mme_generate_s1_setup_failure(assoc_id, S1AP_Cause_PR_misc,
...
//------------------------------------------------------------------------------
int s1ap_mme_handle_ue_cap_indication(__attribute__((unused))
   ...
   */
  {
    ...
    DevCheck(ue_cap_ind_p->radio_capabilities_length <
                 S1AP_UE_RADIOCAPABILITY_MAX_SIZE,
             S1AP_UE_RADIOCAPABILITY_MAX_SIZE,
             ue_cap_ind_p->radio_capabilities_length, 0);
...
int s1ap_mme_handle_initial_context_setup_response(
  ...
  if ((ue_ref_p = s1ap_is_ue_mme_id_in_list((uint32_t)mme_ue_s1ap_id)) ==
      NULL) {
    OAILOG_DEBUG(LOG_S1AP,
                 "No UE is attached to this mme UE s1ap id: " MME_UE_S1AP_ID_FMT
                 " %u(10)\n",
  ...
  if (ue_ref_p->s1_ue_state == S1AP_UE_WAITING_CRR &&
    ...
    OAILOG_ERROR(LOG_S1AP,
                 "A context release procedure is goingoing for ueRerefernce "
                 "with ueId " MME_UE_S1AP_ID_FMT
                 " and enbUeId " ENB_UE_S1AP_ID_FMT
                 ". Ignoring received Setup Response. \n",
  ...
  memset((void *)&message_p->ittiMsg.mme_app_initial_context_setup_rsp, 0,
         sizeof(itti_mme_app_initial_context_setup_rsp_t));

  itti_mme_app_initial_context_setup_rsp_t *initial_context_setup_rsp = NULL;
  initial_context_setup_rsp =
      &message_p->ittiMsg.mme_app_initial_context_setup_rsp;
  ...
  for (int item = 0; item < ie->value.choice.E_RABSetupListCtxtSURes.list.count;
    ...
    initial_context_setup_rsp->bcs_to_be_modified.bearer_context[item2]
        .eps_bearer_id = e_rab_setup_item_ctxt_su_res->e_RAB_ID;
    initial_context_setup_rsp->bcs_to_be_modified.bearer_context[item2]
        .s1_eNB_fteid.teid =
        htonl(*((uint32_t *)e_rab_setup_item_ctxt_su_res->gTP_TEID.buf));
    bstring transport_address =
        blk2bstr(e_rab_setup_item_ctxt_su_res->transportLayerAddress.buf,
  ...
  if (ie) {
    ...
    for (int index = 0; index < s1ap_e_rab_list->list.count; index++) {
      ...
      initial_context_setup_rsp->e_rab_release_list
          .item[initial_context_setup_rsp->e_rab_release_list.no_of_items]
  ...
  //	  initial_context_setup_rsp->e_rab_release_list.item[0].cause.present =
  // S1AP_Cause_PR_radioNetwork;
  //	  initial_context_setup_rsp->e_rab_release_list.item[0].cause.choice.radioNetwork
...
//------------------------------------------------------------------------------
int s1ap_mme_handle_ue_context_release_request(__attribute__((unused))
                                               const sctp_assoc_id_t assoc_id,
  ...
  switch (cause_type) {
    case S1AP_Cause_PR_radioNetwork:
      ...
      OAILOG_DEBUG(LOG_S1AP,
                   "UE CONTEXT RELEASE REQUEST with Cause_Type = Radio Network "
                   "and Cause_Value = %ld\n",
                   cause_value);
      break;
    ...
    case S1AP_Cause_PR_nas:
      ...
      OAILOG_DEBUG(LOG_S1AP,
                   "UE CONTEXT RELEASE REQUEST with Cause_Type = NAS and "
                   "Cause_Value = %ld\n",
                   cause_value);
      break;
    ...
    case S1AP_Cause_PR_misc:
      ...
      OAILOG_DEBUG(LOG_S1AP,
                   "UE CONTEXT RELEASE REQUEST with Cause_Type = MISC and "
                   "Cause_Value = %ld\n",
                   cause_value);
      break;
  ...
  */
  ...
  if ((ue_ref_p = s1ap_is_ue_mme_id_in_list(mme_ue_s1ap_id)) == NULL) {
     ...
     */
    OAILOG_DEBUG(LOG_S1AP,
                 "UE_CONTEXT_RELEASE_REQUEST ignored cause could not get "
                 "context with mme_ue_s1ap_id " MME_UE_S1AP_ID_FMT
...
//------------------------------------------------------------------------------
static int s1ap_mme_generate_ue_context_release_command(
    ue_description_t *ue_ref_p, mme_ue_s1ap_id_t mme_ue_s1ap_id,
    enum s1cause cause, enb_description_t *enb_ref_p) {
  uint8_t *buffer = NULL;
  uint32_t length = 0;
  S1AP_S1AP_PDU_t pdu;
  ...
  switch (cause) {
    ...
    case S1AP_HANDOVER_CANCELLED:
      ...
      if (!ue_ref_p) expect_release_complete = false;
      break;
    ...
    case S1AP_SUCCESSFUL_HANDOVER:
      ...
      break;
    /**
     * If the cause is an S1AP_NETWORK_ERROR or an S1AP_SYSTEM_FAILURE, we send
     * just a UE-Context-Release-Request (and dismiss the complete). No UE
     * Reference will exist at that point.
     */
    case S1AP_NETWORK_ERROR:
    case S1AP_SYSTEM_FAILURE:
      cause_type = S1AP_Cause_PR_transport;
  ...
  rc = s1ap_mme_itti_send_sctp_request(
      &b, enb_ref_p->sctp_assoc_id,
      (ue_ref_p) ? ue_ref_p->sctp_stream_send : enb_ref_p->next_sctp_stream,
      mme_ue_s1ap_id);
  if (ue_ref_p) {
    ...
    if (!expect_release_complete) {
      OAILOG_DEBUG(LOG_S1AP,
                   "UE_CONTEXT_RELEASE_REQUEST will be sent for UE with "
                   "mmeUeS1apId " MME_UE_S1AP_ID_FMT
                   " with directy UE-Reference removal "
      ...
      OAILOG_DEBUG(LOG_S1AP,
                   "UE_CONTEXT_RELEASE_REQUEST will be sent for UE with "
                   "mmeUeS1apId " MME_UE_S1AP_ID_FMT
                   ". Waiting for RELEASE-COMPLETE to remove the UE-Reference "
                   "due reason %d. \n",
                   ue_ref_p->mme_ue_s1ap_id, cause);
      // Start timer to track UE context release complete from eNB
      enb_s1ap_id_key_t enb_ue_s1ap_id_key = INVALID_ENB_UE_S1AP_ID_KEY;
      MME_APP_ENB_S1AP_ID_KEY(enb_ue_s1ap_id_key, enb_ref_p->enb_id,
                              ue_ref_p->enb_ue_s1ap_id);
      if (timer_setup(ue_ref_p->s1ap_ue_context_rel_timer.sec, 0, TASK_S1AP,
                      INSTANCE_DEFAULT, TIMER_ONE_SHOT,
                      (void *)enb_ue_s1ap_id_key,
        ...
        OAILOG_DEBUG(LOG_S1AP,
                     "Started S1AP UE context release timer for UE id  %d and "
                     "timer id 0x%lx  \n",
                     ue_ref_p->mme_ue_s1ap_id,
    ...
    OAILOG_DEBUG(LOG_S1AP,
                 "No UE reference exists for UE id " MME_UE_S1AP_ID_FMT
                 ". Continuing with release complete acknowledge. \n",
                 mme_ue_s1ap_id);
    message_p =
    ...
    /** Notify the MME_APP layer that error handling context removals can
     * continue. */
...
int s1ap_handle_ue_context_release_command(
  ...
  if (!enb_ref_p) {
    OAILOG_DEBUG(LOG_S1AP,
                 "No enbRef could be found for enb_id %d for releasing the "
                 "context of ueId " MME_UE_S1AP_ID_FMT
                 " and enbUeS1apId " ENB_UE_S1AP_ID_FMT ". \n",
                 ue_context_release_command_pP->enb_id,
                 ue_context_release_command_pP->mme_ue_s1ap_id,
                 ue_context_release_command_pP->enb_ue_s1ap_id);
    OAILOG_FUNC_RETURN(LOG_S1AP, RETURNerror);
  }

  if ((ue_ref_p = s1ap_is_enb_ue_s1ap_id_in_list_per_enb(
           ue_context_release_command_pP->enb_ue_s1ap_id,
           ue_context_release_command_pP->enb_id)) == NULL) {
    OAILOG_DEBUG(
        LOG_S1AP,
        "No UE reference could be found for enbUeS1apId " ENB_UE_S1AP_ID_FMT
        " and enbId %d. \n",
        ue_context_release_command_pP->enb_ue_s1ap_id,
        ue_context_release_command_pP->enb_id);
  ...
  if (ue_ref_p) {
    OAILOG_DEBUG(LOG_S1AP,
                 "UE reference for enbUeS1apId " ENB_UE_S1AP_ID_FMT
                 " and enbId %d is %p. \n",
                 ue_context_release_command_pP->enb_ue_s1ap_id,
                 ue_context_release_command_pP->enb_id, ue_ref_p);
  }

  /**
   * Check the cause. If it is implicit detach or sctp reset/shutdown no need to
   * send UE context release command to eNB. Free UE context locally.
   */
  if (ue_context_release_command_pP->cause == S1AP_IMPLICIT_CONTEXT_RELEASE ||
    ...
    OAILOG_DEBUG(LOG_S1AP,
                 "Removed UE reference for ueId " MME_UE_S1AP_ID_FMT
                 " implicitly. Continuing with release complete. \n",
                 ue_context_release_command_pP->mme_ue_s1ap_id);
    message_p =
        itti_alloc_new_message(TASK_S1AP, S1AP_UE_CONTEXT_RELEASE_COMPLETE);
    AssertFatal(message_p != NULL, "itti_alloc_new_message Failed");
    ...
    /** We send the mme_ue_s1ap_id explicitly, since it may be 0 in some
     * handover complete procedures. */
    rc = s1ap_mme_generate_ue_context_release_command(
        ue_ref_p, ue_context_release_command_pP->mme_ue_s1ap_id,
        ue_context_release_command_pP->cause, enb_ref_p);
  }
...
//------------------------------------------------------------------------------
int s1ap_mme_handle_ue_context_release_complete(__attribute__((unused))
                                                const sctp_assoc_id_t assoc_id,
                                                __attribute__((unused))
                                                const sctp_stream_id_t stream,
                                                S1AP_S1AP_PDU_t *pdu) {
  ...
  if ((ue_ref_p = s1ap_is_ue_enb_id_in_list(enb_ref, enb_ue_s1ap_id)) == NULL) {
    /*
     * MME doesn't know the MME UE S1AP ID provided.
     * This implies that UE context has already been deleted on the expiry of
     * timer! Ignore this message. The timer will trigger the cleanup in the
     * MME_APP. EMM is again independent.
     */
    OAILOG_DEBUG(LOG_S1AP,
                 " UE Context Release commplete:No S1 context. Ignore the "
                 "message for ueid " MME_UE_S1AP_ID_FMT "\n",
                 (uint32_t)mme_ue_s1ap_id);
    OAILOG_FUNC_RETURN(LOG_S1AP, RETURNok);
  }
  ...
  S1AP_UE_CONTEXT_RELEASE_COMPLETE(message_p).sctp_assoc_id =
      ue_ref_p->enb->sctp_assoc_id;
  if (ue_ref_p->s1_ue_state != S1AP_UE_WAITING_CRR) {
    OAILOG_ERROR(LOG_S1AP,
                 " UE " MME_UE_S1AP_ID_FMT
                 " with enb_ue_s1ap_id " ENB_UE_S1AP_ID_FMT
                 " not in WAITING_CRR state, but %d.. continuing \n",
                 ue_ref_p->mme_ue_s1ap_id, ue_ref_p->enb_ue_s1ap_id,
...
int s1ap_mme_handle_initial_context_setup_failure(
  ...
  if (ue_ref_p->enb_ue_s1ap_id != (enb_ue_s1ap_id & ENB_UE_S1AP_ID_MASK)) {
    ...
    OAILOG_DEBUG(LOG_S1AP,
                 "INITIAL_CONTEXT_SETUP_FAILURE ignored, mismatch "
                 "enb_ue_s1ap_id: ctxt " ENB_UE_S1AP_ID_FMT
                 " != received " ENB_UE_S1AP_ID_FMT " ",
                 (uint32_t)ue_ref_p->enb_ue_s1ap_id,
                 (uint32_t)(enb_ue_s1ap_id & ENB_UE_S1AP_ID_MASK));
  ...
  switch (cause_type) {
    case S1AP_Cause_PR_radioNetwork:
      cause_value = ie->value.choice.Cause.choice.radioNetwork;
      OAILOG_DEBUG(LOG_S1AP,
                   "INITIAL_CONTEXT_SETUP_FAILURE with Cause_Type = Radio "
                   "Network and Cause_Value = %ld\n",
                   cause_value);
      break;
    ...
    case S1AP_Cause_PR_transport:
      cause_value = ie->value.choice.Cause.choice.transport;
      OAILOG_DEBUG(LOG_S1AP,
                   "INITIAL_CONTEXT_SETUP_FAILURE with Cause_Type = Transport "
                   "and Cause_Value = %ld\n",
                   cause_value);
      break;
    ...
    case S1AP_Cause_PR_protocol:
      cause_value = ie->value.choice.Cause.choice.protocol;
      OAILOG_DEBUG(LOG_S1AP,
                   "INITIAL_CONTEXT_SETUP_FAILURE with Cause_Type = Protocol "
                   "and Cause_Value = %ld\n",
                   cause_value);
      break;
...
//------------------------------------------------------------------------------
int s1ap_mme_handle_path_switch_request(__attribute__((unused))
                                        const sctp_assoc_id_t assoc_id,
                                        __attribute__((unused))
                                        const sctp_stream_id_t stream,
                                        S1AP_S1AP_PDU_t *pdu) {
  ...
  //  capabilities received from the eNB are the same as the UE security
  //  capabilities that the MME has stored. If there is a mismatch, the MME may
  //  log the event and may take additional measures, such as raising an alarm.
  ...
  if ((ue_ref_p = s1ap_is_ue_mme_id_in_list(mme_ue_s1ap_id)) == NULL) {
     ...
     * * * * MME replies with a PATH SWITCH REQUEST FAILURE message and start
     * operation
     * * * * as described in TS 36.413 [11].
     * * * * TODO
     */
    OAILOG_DEBUG(LOG_S1AP,
                 "MME UE S1AP ID provided by eNB doesn't point to any valid "
                 "UE: " MME_UE_S1AP_ID_FMT "\n",
                 enb_ue_s1ap_id);
    s1ap_send_path_switch_request_failure(assoc_id, mme_ue_s1ap_id,
                                          enb_ue_s1ap_id, S1AP_Cause_PR_nas);
  } else {
    /** The enb_ue_s1ap_id will change! **/
    OAILOG_DEBUG(LOG_S1AP,
                 "Removed old ue_reference before handover for MME UE S1AP "
    ...
    if ((ue_ref_p = s1ap_new_ue(assoc_id, enb_ue_s1ap_id)) == NULL) {
      ...
      OAILOG_ERROR(LOG_S1AP,
                   "S1AP:Initial UE Message- Failed to allocate S1AP UE "
                   "Context, eNBUeS1APId:" ENB_UE_S1AP_ID_FMT "\n",
                   enb_ue_s1ap_id);
      OAILOG_FUNC_RETURN(LOG_S1AP, RETURNerror);
    }
    ...
    ue_ref_p->s1ap_handover_completion_timer.sec =
        S1AP_HANDOVER_COMPLETION_TIMER;
    ...
    AssertFatal(message_p != NULL, "itti_alloc_new_message Failed");
    memset((void *)&message_p->ittiMsg.s1ap_path_switch_request, 0,
           sizeof(itti_s1ap_path_switch_request_t));
    /*
     * Bad, very bad cast...
     */
    ...
    path_switch_request->bcs_to_be_modified.num_bearer_context =
        e_rab_to_be_switched_dl_list->list.count;
    for (int item = 0; item < e_rab_to_be_switched_dl_list->list.count;
         item++) {
      /*
       * Bad, very bad cast...
       */
      ...
      S1AP_E_RABToBeSwitchedDLItem_t *s1ap_e_rab_to_be_switched_dl_item =
          &eRABToBeSwitchedDlItemIEs_p->value.choice.E_RABToBeSwitchedDLItem;

      path_switch_request->bcs_to_be_modified.bearer_context[item]
          .eps_bearer_id = s1ap_e_rab_to_be_switched_dl_item->e_RAB_ID;
      path_switch_request->bcs_to_be_modified.bearer_context[item]
          .s1_eNB_fteid.teid =
          htonl(*((uint32_t *)s1ap_e_rab_to_be_switched_dl_item->gTP_TEID.buf));
      bstring transport_address = blk2bstr(
          s1ap_e_rab_to_be_switched_dl_item->transportLayerAddress.buf,
          s1ap_e_rab_to_be_switched_dl_item->transportLayerAddress.size);
      /** Set the IP address from the FTEID. */
      if (4 == blength(transport_address)) {
        path_switch_request->bcs_to_be_modified.bearer_context[item]
            .s1_eNB_fteid.ipv4 = 1;
        memcpy(&path_switch_request->bcs_to_be_modified.bearer_context[item]
                    .s1_eNB_fteid.ipv4_address,
               transport_address->data, blength(transport_address));
      } else if (16 == blength(transport_address)) {
        path_switch_request->bcs_to_be_modified.bearer_context[item]
...
int s1ap_mme_handle_handover_preparation(const sctp_assoc_id_t assoc_id,
  ...
  OAILOG_DEBUG(LOG_S1AP,
               "Handover Required message received from eNB UE S1AP "
               "ID: " ENB_UE_S1AP_ID_FMT "\n",
               enb_ue_s1ap_id);
  ...
  if ((ue_ref_p = s1ap_is_ue_mme_id_in_list(mme_ue_s1ap_id)) == NULL) {
    /**
     * The MME UE S1AP ID provided by eNB doesn't point to any valid UE.
     * * * * MME replies with a HANDOVER PREPARATION FAILURE message and start
     * operation.
     * * * * as described in TS 36.413 [11].
     */
    OAILOG_ERROR(LOG_S1AP,
                 "MME UE S1AP ID provided by source eNB doesn't point to any "
                 "valid UE: " MME_UE_S1AP_ID_FMT "\n",
                 enb_ue_s1ap_id);
    rc = s1ap_handover_preparation_failure(assoc_id, mme_ue_s1ap_id,
                                           enb_ue_s1ap_id, S1AP_Cause_PR_misc);
    OAILOG_FUNC_RETURN(LOG_S1AP, rc);
  }
  /*
   * No timers to be started. Also, the state does not need to be changed from
   * UE_CONNECTED, since TS 36.413 8.4.1.2 specifies that MME may/can initiate
   * some E-RAB Modification signaling towards the source eNB before the
   * Handover Procedure is completed. Handover restart won't be triggered by
   * eNB. Source eNB may restart the Handover. If the UE is still connected
   * (Handover dismissed or still going on, it will send a
   * HO-Preparation-Failure.
   */
  if (ue_ref_p->s1_ue_state != S1AP_UE_CONNECTED) {
    OAILOG_ERROR(LOG_S1AP,
                 "UE: " MME_UE_S1AP_ID_FMT
                 " is not in connected mode, instead %d \n",
  ...
  if (!target_id->choice.targeteNB_ID.selected_TAI.pLMNidentity.buf ||
      !target_id->choice.targeteNB_ID.selected_TAI.tAC.buf ||
      !target_id->choice.targeteNB_ID.global_ENB_ID.pLMNidentity.buf) {
    OAILOG_ERROR(LOG_S1AP,
                 "Handover Required message for UE: " MME_UE_S1AP_ID_FMT
                 " has no target_tai or enb_id information. \n",
                 enb_ue_s1ap_id);
    rc = s1ap_handover_preparation_failure(assoc_id, mme_ue_s1ap_id,
                                           enb_ue_s1ap_id, S1AP_Cause_PR_misc);
    OAILOG_FUNC_RETURN(LOG_S1AP, rc);
  ...
  TBCD_TO_PLMN_T(&target_id->choice.targeteNB_ID.selected_TAI.pLMNidentity,
                 &target_tai.plmn);
  target_tai.tac =
      htons(*((uint16_t *)target_id->choice.targeteNB_ID.selected_TAI.tAC.buf));
  ...
  TBCD_TO_PLMN_T(&target_id->choice.targeteNB_ID.global_ENB_ID.pLMNidentity,
                 &enb_id_plmn);
  ...
  /** Get the enb id. */
  uint32_t enb_id = 0;
  if (target_id->choice.targeteNB_ID.global_ENB_ID.eNB_ID.present ==
      S1AP_ENB_ID_PR_homeENB_ID) {
    /** Target is a home eNB Id. */
    // Home eNB ID = 28 bits
    uint8_t *enb_id_buf = target_id->choice.targeteNB_ID.global_ENB_ID.eNB_ID
                              .choice.homeENB_ID.buf;
    if (target_id->choice.targeteNB_ID.global_ENB_ID.eNB_ID.choice.homeENB_ID
            .size != 28) {
      // TODO: handle case were size != 28 -> notify ? reject ?
    }
    enb_id = (enb_id_buf[0] << 20) + (enb_id_buf[1] << 12) +
    ...
    uint8_t *enb_id_buf = target_id->choice.targeteNB_ID.global_ENB_ID.eNB_ID
                              .choice.macroENB_ID.buf;
    if (target_id->choice.targeteNB_ID.global_ENB_ID.eNB_ID.choice.macroENB_ID
            .size != 20) {
      // TODO: handle case were size != 28 -> notify ? reject ?
    }
    enb_id = (enb_id_buf[0] << 12) + (enb_id_buf[1] << 4) +
  ...
  OAILOG_DEBUG(LOG_S1AP,
               "Successfully decoded targetID IE in Handover Required. \n");
  ...
  switch (s1ap_cause_present) {
    case S1AP_Cause_PR_radioNetwork:
      ...
      S1AP_HANDOVER_REQUIRED(message_p).f_cause_value =
          ie->value.choice.Cause.choice.radioNetwork;
      break;
    default:
      OAILOG_WARNING(
          LOG_S1AP,
          "Cause type %d is not evaluated. Setting cause to radio.. \n",
          s1ap_cause_present);
      S1AP_HANDOVER_REQUIRED(message_p).f_cause_type =
          S1AP_Cause_PR_radioNetwork;
   ...
   * todo:
   * We don't need to purge the bstring's, etc in MME_APP (of the newly
   * allocated itti). Since it will automatically be purged with:
   * s1ap_free_mme_decode_pdu. All allocated messages will be purged, so either
   * allocated new into bstring's or copy into stacked parameters.
   */
  // Mandatory
  S1AP_FIND_PROTOCOLIE_BY_ID(
      S1AP_HandoverRequiredIEs_t, ie, container,
      S1AP_ProtocolIE_ID_id_Source_ToTarget_TransparentContainer, true);
  const S1AP_Source_ToTarget_TransparentContainer_t
      *const source_totarget_transparentcontainer =
          &ie->value.choice.Source_ToTarget_TransparentContainer;
  S1AP_HANDOVER_REQUIRED(message_p).eutran_source_to_target_container =
      blk2bstr(source_totarget_transparentcontainer->buf,
               source_totarget_transparentcontainer->size);

  //  OCTET_STRING_fromBuf (&S1AP_HANDOVER_REQUIRED
  //  (message_p).transparent_container,
  //      (char
  //      *)(handoverRequired_p->source_ToTarget_TransparentContainer.buf),
...
int s1ap_mme_handle_handover_cancel(const sctp_assoc_id_t assoc_id,
  ...
  OAILOG_DEBUG(LOG_S1AP,
               "Handover Cancel message received from eNB UE S1AP "
               "ID: " ENB_UE_S1AP_ID_FMT ". \n",
               enb_ue_s1ap_id);
  if ((ue_ref_p = s1ap_is_ue_mme_id_in_list(mme_ue_s1ap_id)) == NULL) {
    /*
    ...
    OAILOG_ERROR(LOG_S1AP,
                 "MME UE S1AP ID provided by source eNB doesn't point to any "
                 "valid UE: " MME_UE_S1AP_ID_FMT
                 ". Continuing with the HO-Cancellation (impl. detach). \n",
                 mme_ue_s1ap_id);
    OAILOG_FUNC_RETURN(LOG_S1AP, RETURNerror);
  }
  ...
  AssertFatal(message_p != NULL, "itti_alloc_new_message Failed");
  memset((void *)&message_p->ittiMsg.s1ap_handover_cancel, 0,
         sizeof(itti_s1ap_handover_cancel_t));
...
int s1ap_mme_handle_handover_notification(const sctp_assoc_id_t assoc_id,
  ...
  OAILOG_DEBUG(LOG_S1AP,
               "Handover Notify message received from eNB UE S1AP "
               "ID: " ENB_UE_S1AP_ID_FMT "\n",
               enb_ue_s1ap_id);
  /**
   * Search the UE via the ENB-ID and the ENB_UE_S1AP_ID.
   * The MME_UE_S1AP_ID will not be registered with the UE_REFERECNCE until
   * HANDOVER_NOTIFY is completed.
   */
  enb_description_t *enb_ref = s1ap_is_enb_assoc_id_in_list(assoc_id);
  DevAssert(enb_ref);
  if ((ue_ref_p = s1ap_is_enb_ue_s1ap_id_in_list_per_enb(
           enb_ue_s1ap_id, enb_ref->enb_id)) == NULL) {
    /*
     * The MME UE S1AP ID provided by eNB doesn't point to any valid UE.
     * * * * MME replies with a PATH SWITCH REQUEST FAILURE message and start
     * operation
    ...
    OAILOG_ERROR(LOG_S1AP,
                 ...
                 " provided by target eNB %d doesn't point to any valid UE. \n",
                 enb_ue_s1ap_id, enb_ref->enb_id);
    /**
     * Wait for the S10 Relocation Timer to run up or trigger a cancel by the
     * source side. Nothing extra done in this case.
     */
    OAILOG_FUNC_RETURN(LOG_S1AP, RETURNerror);
  }
  /** Not changing any ue_reference properties. The MME_APP layer will
   * eventually stop the S10_HANDOVER_RELOCATION TIMER. */
  ...
  // Currently just use this message send the MBR to SAE-GW and inform the
  // source MME
  message_p = itti_alloc_new_message(TASK_S1AP, S1AP_HANDOVER_NOTIFY);
  AssertFatal(message_p != NULL, "itti_alloc_new_message Failed");
  memset((void *)&message_p->ittiMsg.s1ap_handover_notify, 0,
         sizeof(itti_s1ap_handover_notify_t));
  /*
   * Bad, very bad cast...
   */
  S1AP_HANDOVER_NOTIFY(message_p).mme_ue_s1ap_id = mme_ue_s1ap_id;
  S1AP_HANDOVER_NOTIFY(message_p).enb_ue_s1ap_id = enb_ue_s1ap_id;
  S1AP_HANDOVER_NOTIFY(message_p).tai =
      tai; /**< MME will not check if thats the correct target eNB. Just update
              the UE Context. */
...
int s1ap_mme_handle_handover_resource_allocation_response(
  ...
  OAILOG_DEBUG(LOG_S1AP,
               "Handover Request Acknowledge received from eNB UE S1AP "
               "ID: " ENB_UE_S1AP_ID_FMT "\n",
               enb_ue_s1ap_id);

  /** Will return NULL if an UE-Reference already exists in the UE_MAP of the
   * target-eNB. This UE eNB Id has currently no known s1 association.
   * * * * Create new UE Reference. Associate in the MME_APP to the new
   * mme_ue_s1ap_id.
   * * * * Update eNB UE list.
   * * * * Forward message to NAS.
   */
  ...
  if (enb_ref == NULL) {
    OAILOG_ERROR(LOG_S1AP,
                 "S1AP:Handover Request Message- Failed to get eNB-Reference "
                 "for assocId: %d. \n",
                 assoc_id);
    ...
    AssertFatal(message_p != NULL, "itti_alloc_new_message Failed");
    itti_s1ap_handover_failure_t *handover_failure_p =
        &message_p->ittiMsg.s1ap_handover_failure;
    memset((void *)&message_p->ittiMsg.s1ap_handover_failure, 0,
           sizeof(itti_s1ap_handover_failure_t));
    /** Fill the S1AP Handover Failure elements per hand. */
    handover_failure_p->mme_ue_s1ap_id = mme_ue_s1ap_id;
    //     handover_failure_p->enb_ue_s1ap_id = enb_ue_s1ap_id; /**< Set it from
    //     the invalid context. */
    handover_failure_p->assoc_id =
        assoc_id; /**< Set it from the invalid context. */
    /**
     * Set it to S1AP_SYSTEM_FAILURE for the invalid context. Not waiting for
     * RELEASE-COMPLETE from target-eNB. It may remove the sctp association to
     * the correct MME_UE_S1AP_ID, too. For that, to continue with the handover
     * fail case, to reach the source-ENB, the MME_APP has to re-notify the
     * MME_UE_S1AP_ID/SCTP association.
     */
  ...
  if ((ue_ref_p = s1ap_new_ue(assoc_id, enb_ue_s1ap_id)) == NULL) {
    OAILOG_ERROR(LOG_S1AP,
                 "S1AP:Handover Request Message- Failed to allocate S1AP UE "
                 "Context, mmeUeS1APId:" MME_UE_S1AP_ID_FMT "\n",
                 mme_ue_s1ap_id);
    /** This is a SYSTEM_FAILURE. No UE_REFERENCE for the target-ENB exists. */
    message_p = itti_alloc_new_message(TASK_S1AP, S1AP_HANDOVER_FAILURE);
    AssertFatal(message_p != NULL, "itti_alloc_new_message Failed");
    ...
    handover_failure_p->cause = S1AP_SYSTEM_FAILURE;
    itti_send_msg_to_task(TASK_MME_APP, INSTANCE_DEFAULT, message_p);
    itti_s1ap_ue_context_release_command_t *ue_context_release_cmd_p =
        &((itti_s1ap_ue_context_release_command_t){
            .enb_ue_s1ap_id = ue_ref_p->enb_ue_s1ap_id,
            .enb_id = ue_ref_p->enb->enb_id,
            .cause = S1AP_SYSTEM_FAILURE});
    /** Remove the UE-Reference to the target-eNB implicitly. Don't need to wait
     * for the UE_CONTEXT_REMOVAL_COMMAND_COMPLETE. */
    s1ap_handle_ue_context_release_command(
        ue_context_release_cmd_p); /**< Send a removal message and remove the
                                      context also directly. */
  ...
  S1AP_FIND_PROTOCOLIE_BY_ID(S1AP_HandoverRequestAcknowledgeIEs_t, ie,
                             ...
                             false);
  const S1AP_E_RABAdmittedList_t *e_rab_admitted_list =
      NULL;  // List of s1ap_E_RABAdmittedItem
  if (ie) {
  ...
  if (!(e_rab_admitted_list) || (e_rab_admitted_list->list.count < 1)) {
     ...
     * the correct MME_UE_S1AP_ID, too. For that, to continue with the handover
     * fail case, to reach the source-ENB, the MME_APP has to re-notify the
     * MME_UE_S1AP_ID/SCTP association (S1 association).
     */
    handover_failure_p->cause = S1AP_SYSTEM_FAILURE;
    itti_send_msg_to_task(TASK_MME_APP, INSTANCE_DEFAULT, message_p);
    itti_s1ap_ue_context_release_command_t *ue_context_release_cmd_p =
  ...
  /**
   * Only create a new UE_REFERENCE!Set the (new) ue_reference to
   * S1AP_UE_HANDOVER state. todo: not setting to this state if intra-MME S1AP
   * handover is being done.
   */
  // Will be allocated by NAS
  ...
  ue_ref_p->sctp_stream_recv = stream;
  /**
   * todo: Lionel: this (if no run condition occurs= should be the same stream
   * where we sent the HANDOVER_REQUEST... might not be if run-condition occurs!
   */
  ue_ref_p->sctp_stream_send = ue_ref_p->enb->next_sctp_stream;

  /*
   * Increment the sctp stream for the eNB association.
   * If the next sctp stream is >= instream negociated between eNB and MME, wrap
   * to first stream.
   * TODO: search for the first available stream instead.
   */
   ...
   */
  message_p =
      itti_alloc_new_message(TASK_S1AP, S1AP_HANDOVER_REQUEST_ACKNOWLEDGE);
  AssertFatal(message_p != NULL, "itti_alloc_new_message Failed");
  memset((void *)&message_p->ittiMsg.s1ap_handover_request_acknowledge, 0,
         sizeof(itti_s1ap_handover_request_acknowledge_t));

  S1AP_HANDOVER_REQUEST_ACKNOWLEDGE(message_p).mme_ue_s1ap_id = mme_ue_s1ap_id;
  S1AP_HANDOVER_REQUEST_ACKNOWLEDGE(message_p).enb_ue_s1ap_id =
      ue_ref_p->enb_ue_s1ap_id;

  itti_s1ap_handover_request_acknowledge_t *s1ap_handover_request_acknowledge =
      NULL;
  s1ap_handover_request_acknowledge =
      &message_p->ittiMsg.s1ap_handover_request_acknowledge;
  s1ap_handover_request_acknowledge->bcs_to_be_modified.num_bearer_context =
      e_rab_admitted_list->list.count;
  for (int item = 0; item < e_rab_admitted_list->list.count; item++) {
    /*
     * Bad, very bad cast...
     */
    const S1AP_E_RABAdmittedItem_t *const s1ap_e_rabadmitteditem =
        &(((S1AP_E_RABAdmittedItemIEs_t *)e_rab_admitted_list->list.array[item])
              ->value.choice.E_RABAdmittedItem);
    s1ap_handover_request_acknowledge->bcs_to_be_modified.bearer_context[item]
        .eps_bearer_id = s1ap_e_rabadmitteditem->e_RAB_ID;
    s1ap_handover_request_acknowledge->bcs_to_be_modified.bearer_context[item]
        .s1_eNB_fteid.teid =
        htonl(*((uint32_t *)s1ap_e_rabadmitteditem->gTP_TEID.buf));
    bstring transport_address =
        blk2bstr(s1ap_e_rabadmitteditem->transportLayerAddress.buf,
                 s1ap_e_rabadmitteditem->transportLayerAddress.size);
    /** Set the IP address from the FTEID. */
    if (4 == blength(transport_address)) {
      s1ap_handover_request_acknowledge->bcs_to_be_modified.bearer_context[item]
          .s1_eNB_fteid.ipv4 = 1;
      memcpy(&s1ap_handover_request_acknowledge->bcs_to_be_modified
                  .bearer_context[item]
                  .s1_eNB_fteid.ipv4_address,
             transport_address->data, blength(transport_address));
    } else if (16 == blength(transport_address)) {
  ...
  S1AP_FIND_PROTOCOLIE_BY_ID(
      ...
      S1AP_ProtocolIE_ID_id_E_RABFailedToSetupListHOReqAck, false);
  if (ie) {
    const S1AP_E_RABFailedtoSetupListHOReqAck_t
        *const e_rab_failed_to_setup_list =
            &ie->value.choice.E_RABFailedtoSetupListHOReqAck;
    /** Get the failed bearers. */
    for (int index = 0; index < e_rab_failed_to_setup_list->list.count;
         index++) {
      const S1AP_E_RABFailedtoSetupItemHOReqAckIEs_t
          *const e_rab_failed_to_setup_item_ies =
              (const S1AP_E_RABFailedtoSetupItemHOReqAckIEs_t *const)
                  e_rab_failed_to_setup_list->list.array[index];
      const S1AP_E_RABFailedToSetupItemHOReqAck_t
          *const e_rab_failed_to_setup_item =
              &e_rab_failed_to_setup_item_ies->value.choice
                   .E_RABFailedToSetupItemHOReqAck;
      s1ap_handover_request_acknowledge->e_rab_release_list.item[index]
          .e_rab_id = e_rab_failed_to_setup_item->e_RAB_ID;
      s1ap_handover_request_acknowledge->e_rab_release_list.item[index].cause =
          e_rab_failed_to_setup_item->cause;
   ...
   * The method
   * ASN_STRUCT_FREE_CONTENTS_ONLY(asn_DEF_S1AP_Target_ToSource_TransparentContainer,
   * &s1ap_HandoverRequestAcknowledgeIEs->target_ToSource_TransparentContainer)
  ...
  const S1AP_Target_ToSource_TransparentContainer_t
      *const target_to_source_transparent_container =
          &ie->value.choice.Target_ToSource_TransparentContainer;

  S1AP_HANDOVER_REQUEST_ACKNOWLEDGE(message_p)
      .target_to_source_eutran_container =
      blk2bstr((void *)target_to_source_transparent_container->buf,
               target_to_source_transparent_container->size);
  // todo @ Lionel: do we need to run this method? Message deallocated later?
  // free_wrapper((void**)
  // &(handover_request_acknowledge_pP->transparent_container.buf));
...
int s1ap_mme_handle_handover_resource_allocation_failure(
  ...
  /**
   * No UE Reference to the target-eNB expected @ Handover Failure. Only will be
   * created to the target eNB with Handover Request Acknowledge. Send Handover
   * Failure back (manually) to the MME. This will trigger an implicit detach if
   * the UE is not REGISTERED yet (single MME S1AP HO), or will just send a
   * HO-PREP-FAILURE to the MME (if the cause is not S1AP-SYSTEM-FAILURE).
   */
  message_p = itti_alloc_new_message(TASK_S1AP, S1AP_HANDOVER_FAILURE);
  AssertFatal(message_p != NULL, "itti_alloc_new_message Failed");
  itti_s1ap_handover_failure_t *handover_failure_p =
  ...
  switch (cause_type) {
    case S1AP_Cause_PR_radioNetwork:
      cause_value = ie->value.choice.Cause.choice.radioNetwork;
      OAILOG_DEBUG(LOG_S1AP,
                   "HANDOVER_FAILURE with Cause_Type = Radio Network and "
                   "Cause_Value = %ld\n",
                   cause_value);
      break;
    ...
    case S1AP_Cause_PR_transport:
      cause_value = ie->value.choice.Cause.choice.transport;
      OAILOG_DEBUG(LOG_S1AP,
                   "HANDOVER_FAILURE with Cause_Type = Transport and "
                   "Cause_Value = %ld\n",
                   cause_value);
      break;
    ...
    case S1AP_Cause_PR_nas:
      ...
      OAILOG_DEBUG(
          LOG_S1AP,
          "HANDOVER_FAILURE with Cause_Type = NAS and Cause_Value = %ld\n",
          cause_value);
      break;
    ...
    case S1AP_Cause_PR_protocol:
      ...
      OAILOG_DEBUG(
          LOG_S1AP,
          "HANDOVER_FAILURE with Cause_Type = Protocol and Cause_Value = %ld\n",
          cause_value);
      break;
    ...
    case S1AP_Cause_PR_misc:
      ...
      OAILOG_DEBUG(
          LOG_S1AP,
          "HANDOVER_FAILURE with Cause_Type = MISC and Cause_Value = %ld\n",
          cause_value);
      break;
   ...
   * Set it to anything else thatn SYSTEM_FAILURE, not to trigger any implicit
   * detach due cause (only based on the handover type).
   */
...
/**
 * ENB Status Transfer.
 * Contains Transparent Data which is forwarded to the target eNB without state
 * change.
 */
int s1ap_mme_handle_enb_status_transfer(const sctp_assoc_id_t assoc_id,
  ...
  OAILOG_DEBUG(LOG_S1AP,
               "Enb Status Transfer received from source eNodeB for UE with "
               "eNB UE S1AP ID: " ENB_UE_S1AP_ID_FMT "\n",
               enb_ue_s1ap_id);

  /** In the single-MME case as well, the MME_UE_S1AP_ID should still provide
   * the source eNB. */
  if ((ue_ref_p = s1ap_is_ue_mme_id_in_list(mme_ue_s1ap_id)) == NULL) {
    /*
     * The MME UE S1AP ID provided by eNB doesn't point to any valid UE.
     * Since the message is optional, just disregard the message.
     * No need to remove the UE implicitly, and if it is a intra-MME HO, the
     * s10-relocation completion timer will purge it anyway.
     */
    OAILOG_ERROR(LOG_S1AP,
                 "MME UE S1AP ID provided by eNB doesn't point to any valid "
                 "UE: " MME_UE_S1AP_ID_FMT
                 " for the ENB_STATUS_TRANFER message. \n",
                 mme_ue_s1ap_id);
    OAILOG_FUNC_RETURN(LOG_S1AP, RETURNerror);
  }
  /** UE resources will not be released yet. */
  /** ENB Status Transfer message sent to MME_APP layer. It will decide if the
   * target MME lies in the same MME or different MME. */
  message_p = itti_alloc_new_message(TASK_S1AP, S1AP_ENB_STATUS_TRANSFER);
  AssertFatal(message_p != NULL, "itti_alloc_new_message Failed");
  itti_s1ap_status_transfer_t *enb_status_transfer_p =
      &message_p->ittiMsg.s1ap_enb_status_transfer;
  ...
  /** Set the E-UTRAN container. The S1AP octet string should be purged in the
   * outer method. */

  enb_status_transfer_p->status_transfer_bearer_list =
      calloc(1, sizeof(status_transfer_bearer_list_t));
  // Mandatory
  S1AP_FIND_PROTOCOLIE_BY_ID(
      S1AP_ENBStatusTransferIEs_t, ie, container,
      S1AP_ProtocolIE_ID_id_eNB_StatusTransfer_TransparentContainer, true);
  const S1AP_ENB_StatusTransfer_TransparentContainer_t
      *const enb_status_transfer_transparent_container =
  ...
  enb_status_transfer_p->status_transfer_bearer_list->num_bearers =
      enb_status_transfer_transparent_container
          ->bearers_SubjectToStatusTransferList.list.count;

  for (int i = 0; i < enb_status_transfer_transparent_container
                          ->bearers_SubjectToStatusTransferList.list.count;
       i++) {
    const S1AP_Bearers_SubjectToStatusTransfer_ItemIEs_t
        *const bearers_subject_to_status_transfer_item_ies =
            (const S1AP_Bearers_SubjectToStatusTransfer_ItemIEs_t *const)
                enb_status_transfer_transparent_container
                    ->bearers_SubjectToStatusTransferList.list.array[i];
    enb_status_transfer_p->status_transfer_bearer_list
        ->bearerStatusTransferList[i]
        .bsc_ul.hfn_count =
        bearers_subject_to_status_transfer_item_ies->value.choice
            .Bearers_SubjectToStatusTransfer_Item.uL_COUNTvalue.hFN;
    enb_status_transfer_p->status_transfer_bearer_list
        ->bearerStatusTransferList[i]
        .bsc_ul.pdcp_count =
        bearers_subject_to_status_transfer_item_ies->value.choice
            .Bearers_SubjectToStatusTransfer_Item.uL_COUNTvalue.pDCP_SN;
    enb_status_transfer_p->status_transfer_bearer_list
        ->bearerStatusTransferList[i]
        .bsc_dl.hfn_count =
        bearers_subject_to_status_transfer_item_ies->value.choice
            .Bearers_SubjectToStatusTransfer_Item.dL_COUNTvalue.hFN;
    enb_status_transfer_p->status_transfer_bearer_list
        ->bearerStatusTransferList[i]
        .bsc_dl.pdcp_count =
        bearers_subject_to_status_transfer_item_ies->value.choice
            .Bearers_SubjectToStatusTransfer_Item.dL_COUNTvalue.pDCP_SN;
    enb_status_transfer_p->status_transfer_bearer_list
        ->bearerStatusTransferList[i]
        .ebi = bearers_subject_to_status_transfer_item_ies->value.choice
                   .Bearers_SubjectToStatusTransfer_Item.e_RAB_ID;
    if (bearers_subject_to_status_transfer_item_ies->value.choice
            .Bearers_SubjectToStatusTransfer_Item.receiveStatusofULPDCPSDUs) {
      OAILOG_WARNING(
          LOG_S1AP, "ENB_STATUS Transfer Bearer Item optionals not decoded!\n");
      //		  enb_status_transfer_p->status_transfer_bearer_list->bearerStatusTransferList[i].receiveStatusofULPDCPSDU
      //=
      // blk2bstr(bearers_subject_to_status_transfer_item_ies->value.choice.Bearers_SubjectToStatusTransfer_Item.receiveStatusofULPDCPSDUs->buf,
...
static bool s1ap_send_enb_deregistered_ind(__attribute__((unused))
  ...
  if (ue_ref_p) {
    ...
    OAILOG_INFO(LOG_S1AP,
                "Triggering eNB deregistration indication for UE with "
                "enbId " ENB_UE_S1AP_ID_FMT ". \n",
                ue_ref_p->enb_ue_s1ap_id);

    AssertFatal(arg->current_ue_index < S1AP_ITTI_UE_PER_DEREGISTER_MESSAGE,
                "Too many deregistered UEs reported in "
                "S1AP_ENB_DEREGISTERED_IND message ");
    ...
    if (arg->current_ue_index == 0 && arg->handled_ues > 0) {
      ...
      MSC_LOG_TX_MESSAGE(
          MSC_S1AP_MME, MSC_NAS_MME, NULL, 0,
    ...
    arg->current_ue_index =
        arg->handled_ues % S1AP_ITTI_UE_PER_DEREGISTER_MESSAGE;
...
//------------------------------------------------------------------------------
static bool construct_s1ap_mme_full_reset_req(__attribute__((unused))
                                              const hash_key_t keyP,
                                              void *const dataP, void *argP,
                                              void **resultP) {
  ...
  if (ue_ref_p) {
    ...
    S1AP_ENB_INITIATED_RESET_REQ(arg->message_p)
        .ue_to_reset_list[i]
        .enb_ue_s1ap_id =
        (enb_ue_s1ap_id_t *)ue_ref_p; /**< Should be the address of the id. */
    arg->current_ue_index++;
    *resultP = arg->message_p;
  } else {
    OAILOG_TRACE(LOG_S1AP, "No valid UE provided in callback: %p\n", ue_ref_p);
...
int s1ap_handle_sctp_disconnection(const sctp_assoc_id_t assoc_id, bool reset) {
  ...
  hashtable_ts_apply_callback_on_elements(
      ...
      (void **)&message_p); /**< Just releasing the procedure. */

  /** Release the S1AP UE references implicitly. */
  OAILOG_DEBUG(
      LOG_S1AP,
      "Releasing all S1AP UE references related to the ENB with assocId %d. \n",
      assoc_id);

  //  if ( (!(arg.handled_ues % S1AP_ITTI_UE_PER_DEREGISTER_MESSAGE)) &&
  //  (arg.handled_ues) ){
  ...
  MSC_LOG_TX_MESSAGE(MSC_S1AP_MME, MSC_NAS_MME, NULL, 0,
                     "0 S1AP_ENB_DEREGISTERED_IND num ue to deregister %u",
                     S1AP_ENB_DEREGISTERED_IND(message_p).nb_ue_to_deregister);
  itti_send_msg_to_task(TASK_MME_APP, INSTANCE_DEFAULT, message_p);
  message_p = NULL;
...
int s1ap_handle_new_association(sctp_new_peer_t *sctp_new_peer_p) {
  ...
  if ((enb_association =
    ...
    hashtable_rc_t hash_rc = hashtable_ts_insert(
        &g_s1ap_enb_coll, (const hash_key_t)enb_association->sctp_assoc_id,
    ...
    OAILOG_WARNING(LOG_S1AP,
                   "Received new association request on an association that is "
                   "being %s, ignoring",
...
int s1ap_mme_handle_erab_modify_response(const sctp_assoc_id_t assoc_id,
  ...
  S1AP_FIND_PROTOCOLIE_BY_ID(S1AP_E_RABModifyResponseIEs_t, ie, container,
                             S1AP_ProtocolIE_ID_id_E_RABModifyListBearerModRes,
                             false);
  if (ie) {
    const S1AP_E_RABModifyListBearerModRes_t
        *const e_rab_modify_list_bearer_mod_res =
            &ie->value.choice.E_RABModifyListBearerModRes;

    int num_erab = e_rab_modify_list_bearer_mod_res->list.count;
    for (int index = 0; index < num_erab; index++) {
      const S1AP_E_RABModifyItemBearerModResIEs_t
          *const e_rab_setup_item_bearer_su_res_ies =
              (S1AP_E_RABModifyItemBearerModResIEs_t *)
                  e_rab_modify_list_bearer_mod_res->list.array[index];
      const S1AP_E_RABModifyItemBearerModRes_t *const erab_item =
          &e_rab_setup_item_bearer_su_res_ies->value.choice
               .E_RABModifyItemBearerModRes;
      S1AP_E_RAB_MODIFY_RSP(message_p).e_rab_modify_list.item[index].e_rab_id =
          erab_item->e_RAB_ID;
  ...
  S1AP_FIND_PROTOCOLIE_BY_ID(S1AP_E_RABModifyResponseIEs_t, ie, container,
                             ...
                             false);
  if (ie) {
    const S1AP_E_RABList_t *const e_rab_failed_list =
        &ie->value.choice.E_RABList;

    int num_erab = e_rab_failed_list->list.count;
    for (int index = 0; index < num_erab; index++) {
      const S1AP_E_RABItemIEs_t *const erab_item_ies =
          (S1AP_E_RABItemIEs_t *)e_rab_failed_list->list.array[index];
      const S1AP_E_RABItem_t *const erab_item =
          (S1AP_E_RABItem_t *)&erab_item_ies->value.choice.E_RABItem;
      S1AP_E_RAB_MODIFY_RSP(message_p)
          .e_rab_failed_to_modify_list.item[index]
          .e_rab_id = erab_item->e_RAB_ID;
      S1AP_E_RAB_MODIFY_RSP(message_p)
          .e_rab_failed_to_modify_list.item[index]
          .cause = erab_item->cause;
      S1AP_E_RAB_MODIFY_RSP(message_p)
          .e_rab_failed_to_modify_list.no_of_items += 1;
    }
  }
...
int s1ap_mme_handle_erab_release_response(const sctp_assoc_id_t assoc_id,
  ...
  //  AssertFatal (message_p != NULL, "itti_alloc_new_message Failed");
  //  S1AP_E_RAB_RELEASE_RSP (message_p).mme_ue_s1ap_id =
  //  ue_ref_p->mme_ue_s1ap_id; S1AP_E_RAB_RELEASE_RSP
  //  (message_p).enb_ue_s1ap_id = ue_ref_p->enb_ue_s1ap_id;
  //  S1AP_E_RAB_RELEASE_RSP (message_p).e_rab_setup_list.no_of_items = 0;
  //  S1AP_E_RAB_RELEASE_RSP (message_p).e_rab_failed_to_setup_list.no_of_items
  //  = 0;
  //
  //  if (s1ap_E_RABSetupResponseIEs_p->presenceMask &
  //  S1AP_E_RABSETUPRESPONSEIES_E_RABSETUPLISTBEARERSURES_PRESENT) {
  //    int num_erab =
  //    s1ap_E_RABSetupResponseIEs_p->e_RABSetupListBearerSURes.s1ap_E_RABSetupItemBearerSURes.count;
  //    for (int index = 0; index < num_erab; index++) {
  ...
  //      S1AP_E_RAB_SETUP_RSP (message_p).e_rab_setup_list.item[index].e_rab_id
  //      = erab_setup_item->e_RAB_ID; S1AP_E_RAB_SETUP_RSP
  //      (message_p).e_rab_setup_list.item[index].transport_layer_address =
  //          blk2bstr(erab_setup_item->transportLayerAddress.buf,
  ...
  //    int num_erab =
  //    s1ap_E_RABSetupResponseIEs_p->e_RABFailedToSetupListBearerSURes.s1ap_E_RABItem.count;
  //    for (int index = 0; index < num_erab; index++) {
  //      S1AP_E_RABItem_t * erab_item = (S1AP_E_RABItem_t
  //      *)s1ap_E_RABSetupResponseIEs_p->e_RABFailedToSetupListBearerSURes.s1ap_E_RABItem.array[index];
  //      S1AP_E_RAB_SETUP_RSP
  //      (message_p).e_rab_failed_to_setup_list.item[index].e_rab_id =
  //      erab_item->e_RAB_ID; S1AP_E_RAB_SETUP_RSP
  //      (message_p).e_rab_failed_to_setup_list.item[index].cause =
  //      erab_item->cause; S1AP_E_RAB_SETUP_RSP
  //      (message_p).e_rab_failed_to_setup_list.no_of_items += 1;
  //    }
  //  }
  //
  //  rc =  itti_send_msg_to_task (TASK_MME_APP, INSTANCE_DEFAULT, message_p);
...
int s1ap_mme_handle_erab_release_indication(const sctp_assoc_id_t assoc_id,
  ...
  container =
      &pdu->choice.initiatingMessage.value.choice.E_RABReleaseIndication;

  S1AP_FIND_PROTOCOLIE_BY_ID(S1AP_E_RABReleaseIndicationIEs_t, ie, container,
                             S1AP_ProtocolIE_ID_id_MME_UE_S1AP_ID, true);
  mme_ue_s1ap_id = ie->value.choice.MME_UE_S1AP_ID;
  ...
  for (int index = 0; index < num_erab; index++) {
    ...
    const S1AP_E_RABItem_t *const erab_item =
        (S1AP_E_RABItem_t *)&erab_item_ies->value.choice.E_RABItem;
    S1AP_E_RAB_RELEASE_IND(message_p).e_rab_release_list.item[index].e_rab_id =
        erab_item->e_RAB_ID;
    S1AP_E_RAB_RELEASE_IND(message_p).e_rab_release_list.item[index].cause =
        erab_item->cause;
    S1AP_E_RAB_RELEASE_IND(message_p).e_rab_release_list.erab_bitmap |=
        (0x01 << (erab_item->e_RAB_ID - 1));
...
//------------------------------------------------------------------------------
void s1ap_mme_handle_ue_context_rel_comp_timer_expiry(void *arg) {
  MessageDef *message_p = NULL;
  OAILOG_FUNC_IN(LOG_S1AP);
  DevAssert(arg != NULL);
  ...
  S1AP_UE_CONTEXT_RELEASE_COMPLETE(message_p).mme_ue_s1ap_id =
      ue_ref_p->mme_ue_s1ap_id;
  MSC_LOG_TX_MESSAGE(
      MSC_S1AP_MME, MSC_MMEAPP_MME, NULL, 0,
...
//------------------------------------------------------------------------------
void s1ap_mme_handle_mme_mobility_completion_timer_expiry(void *arg) {
  OAILOG_FUNC_IN(LOG_S1AP);
  DevAssert(arg != NULL);

  ue_description_t *ue_reference_p = (ue_description_t *)arg;
  OAILOG_INFO(LOG_S1AP,
              "Expired- MME Mobility Completion timer for "
              "enbUeS1apId " ENB_UE_S1AP_ID_FMT " \n",
              ue_reference_p->enb_ue_s1ap_id);

  /** Inactivate the timer either here in the timeout or manually when removing
   * the S1AP UE_REFERECE. */
  ue_reference_p->s1ap_handover_completion_timer.id = S1AP_TIMER_INACTIVE_ID;

  /** Perform an UE_CONTEXT_RELEASE and inform the MME_APP afterwards, which
   * will check if the CLR flag is set or not. */
  if (s1ap_mme_generate_ue_context_release_command(
          ue_reference_p, ue_reference_p->mme_ue_s1ap_id,
          S1AP_NAS_NORMAL_RELEASE, ue_reference_p->enb) == RETURNerror) {
    OAILOG_DEBUG(LOG_S1AP,
                 "Error removing S1AP_UE_REFERENCE FOR "
                 "ENB_UE_S1AP_ID " ENB_UE_S1AP_ID_FMT
                 " and mmeUeS1apId  " MME_UE_S1AP_ID_FMT " \n",
                 (uint32_t)ue_reference_p->enb_ue_s1ap_id,
                 (uint32_t)ue_reference_p->mme_ue_s1ap_id);
  } else {
    OAILOG_DEBUG(
        LOG_S1AP,
        "Removed S1AP_UE_REFERENCE FOR ENB_UE_S1AP_ID " ENB_UE_S1AP_ID_FMT
        " and mmeUeS1apId  " MME_UE_S1AP_ID_FMT "\n",
        (uint32_t)ue_reference_p->enb_ue_s1ap_id,
        (uint32_t)ue_reference_p->mme_ue_s1ap_id);
  }
...
int s1ap_mme_handle_error_ind_message(const sctp_assoc_id_t assoc_id,
  ...
  if (ie) mme_ue_s1ap_id = ie->value.choice.MME_UE_S1AP_ID;
  ...
  if (!ue_ref) {
    ...
    OAILOG_WARNING(LOG_S1AP,
                   "No UE reference exists for enbId %d and "
                   "enb_ue_s1ap_id " ENB_UE_S1AP_ID_FMT
                   ". Dropping error indication. \n",
   ...
   * Just forward to the MME_APP layer, depending if there is a handover
   * procedure running or not the current handover procedure might be
   * disregarded.
   */
  message_p = itti_alloc_new_message(TASK_S1AP, S1AP_ERROR_INDICATION);
  AssertFatal(message_p != NULL, "itti_alloc_new_message Failed");
  memset((void *)&message_p->ittiMsg.s1ap_error_indication, 0,
         sizeof(itti_s1ap_error_indication_t));
  ...
  if (ie) {
    ...
    switch (cause_type) {
      case S1AP_Cause_PR_radioNetwork:
        cause_value = ie->value.choice.Cause.choice.radioNetwork;
        OAILOG_DEBUG(LOG_S1AP,
                     "S1AP_ERROR_INDICATION with Cause_Type = Radio Network "
                     "and Cause_Value = %ld\n",
                     cause_value);
        break;
      ...
      case S1AP_Cause_PR_transport:
        cause_value = ie->value.choice.Cause.choice.transport;
        OAILOG_DEBUG(LOG_S1AP,
                     "S1AP_ERROR_INDICATION with Cause_Type = Transport and "
                     "Cause_Value = %ld\n",
                     cause_value);
        break;
      ...
      case S1AP_Cause_PR_nas:
        cause_value = ie->value.choice.Cause.choice.nas;
        OAILOG_DEBUG(LOG_S1AP,
                     "S1AP_ERROR_INDICATION with Cause_Type = NAS and "
                     "Cause_Value = %ld\n",
                     cause_value);
        break;
      ...
      case S1AP_Cause_PR_protocol:
        cause_value = ie->value.choice.Cause.choice.protocol;
        OAILOG_DEBUG(LOG_S1AP,
                     "S1AP_ERROR_INDICATION with Cause_Type = Protocol and "
                     "Cause_Value = %ld\n",
                     cause_value);
        break;
      ...
      case S1AP_Cause_PR_misc:
        cause_value = ie->value.choice.Cause.choice.misc;
        OAILOG_DEBUG(LOG_S1AP,
                     "S1AP_ERROR_INDICATION with Cause_Type = MISC and "
                     "Cause_Value = %ld\n",
                     cause_value);
        break;
      ...
      default:
        OAILOG_ERROR(LOG_S1AP,
                     "S1AP_ERROR_INDICATION with Invalid Cause_Type = %d\n",
                     cause_type);
        OAILOG_FUNC_RETURN(LOG_S1AP, RETURNerror);
    }
  }
...
int s1ap_mme_handle_enb_configuration_transfer(const sctp_assoc_id_t assoc_id,
  ...
  if (!ie) {
    ...
    OAILOG_ERROR(
        LOG_S1AP,
        "eNB configuration transfer currently only implementd for 4G. \n");
    OAILOG_FUNC_RETURN(LOG_S1AP, rc);
  }
  ...
  if (!sonConfigurationTransfer->targeteNB_ID.global_ENB_ID.pLMNidentity.buf ||
    ...
    OAILOG_ERROR(LOG_S1AP,
                 "eNB configuration transfer message for has no target_tai or "
                 "enb_id information. \n");
    OAILOG_FUNC_RETURN(LOG_S1AP, rc);
  }
  memset((void *)&target_tai, 0, sizeof(tai_t));
  ...
  TBCD_TO_PLMN_T(
      &sonConfigurationTransfer->targeteNB_ID.selected_TAI.pLMNidentity,
      &target_tai.plmn);
  target_tai.tac = htons(*(
      (uint16_t *)sonConfigurationTransfer->targeteNB_ID.selected_TAI.tAC.buf));
  ...
  if (!sonConfigurationTransfer->sourceeNB_ID.global_ENB_ID.pLMNidentity.buf ||
    ...
    OAILOG_ERROR(LOG_S1AP,
                 "eNB configuration transfer message for has no source_tai or "
                 "enb_id information. \n");
    OAILOG_FUNC_RETURN(LOG_S1AP, rc);
  }
  ...
  TBCD_TO_PLMN_T(
      &sonConfigurationTransfer->sourceeNB_ID.selected_TAI.pLMNidentity,
      &source_tai.plmn);
  source_tai.tac = htons(*(
      (uint16_t *)sonConfigurationTransfer->sourceeNB_ID.selected_TAI.tAC.buf));

  /** Target Global-ENB-ID. */
  ecgi_t target_global_enb_id = {0};
  memset(&enb_id_plmn, 0, sizeof(plmn_t));
  TBCD_TO_PLMN_T(
      &sonConfigurationTransfer->targeteNB_ID.global_ENB_ID.pLMNidentity,
      &enb_id_plmn);
  ...
  memset(&enb_id_plmn, 0, sizeof(plmn_t));
  TBCD_TO_PLMN_T(
      &sonConfigurationTransfer->sourceeNB_ID.global_ENB_ID.pLMNidentity,
      &enb_id_plmn);
  ...
  if (sonConfigurationTransfer->targeteNB_ID.global_ENB_ID.eNB_ID.present ==
      S1AP_ENB_ID_PR_homeENB_ID) {
    /** Target is a home eNB Id. */
    // Home eNB ID = 28 bits
    uint8_t *enb_id_buf = sonConfigurationTransfer->targeteNB_ID.global_ENB_ID
                              .eNB_ID.choice.homeENB_ID.buf;
    if (sonConfigurationTransfer->targeteNB_ID.global_ENB_ID.eNB_ID.choice
            .homeENB_ID.size != 28) {
      // TODO: handle case were size != 28 -> notify ? reject ?
    }
  ...
  if (sonConfigurationTransfer->sourceeNB_ID.global_ENB_ID.eNB_ID.present ==
      S1AP_ENB_ID_PR_homeENB_ID) {
    /** Source is a home eNB Id. */
    // Home eNB ID = 28 bits
    uint8_t *enb_id_buf = sonConfigurationTransfer->sourceeNB_ID.global_ENB_ID
                              .eNB_ID.choice.homeENB_ID.buf;
    if (sonConfigurationTransfer->sourceeNB_ID.global_ENB_ID.eNB_ID.choice
            .homeENB_ID.size != 28) {
      // TODO: handle case were size != 28 -> notify ? reject ?
    }
    source_enb_id = (enb_id_buf[0] << 20) + (enb_id_buf[1] << 12) +
                    (enb_id_buf[2] << 4) + ((enb_id_buf[3] & 0xf0) >> 4);
    ...
    source_enb_id = (enb_id_buf[0] << 12) + (enb_id_buf[1] << 4) +
                    ((enb_id_buf[2] & 0xf0) >> 4);
    //    OAILOG_MESSAGE_ADD (context, "macro eNB id: %05x", target_enb_id);
  ...
  OAILOG_DEBUG(
      LOG_S1AP,
      "Successfully decoded sourceID IE in ENB Configuration Transfer. \n");

  enb_conf_reply_t *conf_reply = NULL;
  if (sonConfigurationTransfer->sONInformation.present ==
      S1AP_SONInformation_PR_NOTHING) {  // todo: check.. recompile..
    OAILOG_WARNING(LOG_S1AP,
                   "Received invalid S1AP SON information type %d. \n",
                   sonConfigurationTransfer->sONInformation.present);
    OAILOG_FUNC_RETURN(LOG_S1AP, RETURNok);
  } else if (sonConfigurationTransfer->sONInformation.present ==
    ...
    struct S1AP_X2TNLConfigurationInfo *x2TNLConfigurationInfo =
        sonConfigurationTransfer->sONInformation.choice.sONInformationReply
            .x2TNLConfigurationInfo;
    if (x2TNLConfigurationInfo) {
      conf_reply = calloc(1, sizeof(enb_conf_reply_t));
      conf_reply->reply_count =
          x2TNLConfigurationInfo->eNBX2TransportLayerAddresses.list.count;
      for (int item = 0;
           item <
           x2TNLConfigurationInfo->eNBX2TransportLayerAddresses.list.count;
           item++) {
         ...
         * Bad, very bad cast...
         */
        S1AP_TransportLayerAddress_t *s1ap_transportLayerAddress_p =
            (S1AP_TransportLayerAddress_t *)x2TNLConfigurationInfo
                ->eNBX2TransportLayerAddresses.list.array[item];
        /** Set the IP address from the FTEID. */
        conf_reply->addresses[item] =
            blk2bstr(s1ap_transportLayerAddress_p->buf,
                     s1ap_transportLayerAddress_p->size);
      }
      OAILOG_INFO(LOG_S1AP,
                  "Successfully decoded X2 configuration transfer reply. \n");
    } else {
      OAILOG_WARNING(
          LOG_S1AP,
          "No X2 reply object is received in the eNB status reply.\n");
    }
  } else {
  ...
  /** Selected-TAI. */
  itti_s1ap_configuration_transfer_t *enb_configuration_transfer_p =
  ...
  memcpy((void *)&enb_configuration_transfer_p->target_global_enb_id,
         &target_global_enb_id, sizeof(target_global_enb_id));
  memcpy((void *)&enb_configuration_transfer_p->source_global_enb_id,
         &source_global_enb_id, sizeof(source_global_enb_id));
...
int s1ap_mme_handle_enb_reset(const sctp_assoc_id_t assoc_id,
  ...
  if (enb_association->s1_state != S1AP_READY) {
    // Ignore the message
    OAILOG_INFO(LOG_S1AP,
                "S1 setup is not done.Invalid state.Ignoring ENB Initiated "
                "Reset.eNB Id = %d , S1AP state = %d \n",
  ...
  if (s1ap_reset_type != RESET_ALL) {
    ...
    s1_sig_conn_id_t
        ue_to_reset_list[num_ue_assoc]; /**< Create a stacked array. */
    ...
    for (item = 0; item < num_ue_assoc; item++) {
      ...
      //			  OAILOG_ERROR (LOG_S1AP, "Incorrect element in
      // Bearers_SubjectToStatusTransferList: %d\n",
      // resetType->choice.partOfS1_Interface.list.array[i]->id);
      //			  OAILOG_FUNC_RETURN (LOG_S1AP, RETURNerror);
      //		  }
      S1AP_UE_associatedLogicalS1_ConnectionItemResAck_t *s1_sig_conn_p =
          (S1AP_UE_associatedLogicalS1_ConnectionItemResAck_t *)ie->value.choice
              .ResetType.choice.partOfS1_Interface.list.array[item];
      if (!s1_sig_conn_p) {
        OAILOG_ERROR(LOG_S1AP,
      ...
      S1AP_UE_associatedLogicalS1_ConnectionItem_t *s1_sig_conn_item =
          &s1_sig_conn_p->value.choice.UE_associatedLogicalS1_ConnectionItem;
      if (s1_sig_conn_item->mME_UE_S1AP_ID != NULL) {
        mme_ue_s1ap_id_t mme_ue_s1ap_id =
            (mme_ue_s1ap_id_t) * (s1_sig_conn_item->mME_UE_S1AP_ID);
        if ((ue_ref_p = s1ap_is_ue_mme_id_in_list(mme_ue_s1ap_id)) != NULL) {
          if (s1_sig_conn_item->eNB_UE_S1AP_ID != NULL) {
            enb_ue_s1ap_id_t enb_ue_s1ap_id =
                (enb_ue_s1ap_id_t) * (s1_sig_conn_item->eNB_UE_S1AP_ID);
            enb_ue_s1ap_id &= ENB_UE_S1AP_ID_MASK;
            if (ue_ref_p->enb_ue_s1ap_id == enb_ue_s1ap_id) {
              ue_to_reset_list[item].mme_ue_s1ap_id =
                  &(ue_ref_p->mme_ue_s1ap_id);
              ue_to_reset_list[item].enb_ue_s1ap_id =
                  (enb_ue_s1ap_id_t *)ue_ref_p;
            } else {
              ...
              OAILOG_ERROR(LOG_S1AP,
                           "Partial Reset Request:enb_ue_s1ap_id mismatch "
                           "between id %d sent by eNB and id %d stored in epc "
                           "for mme_ue_s1ap_id %d \n",
                           enb_ue_s1ap_id, ue_ref_p->enb_ue_s1ap_id,
                           mme_ue_s1ap_id);
            }
          } else {
        ...
        if (s1_sig_conn_item->eNB_UE_S1AP_ID != NULL) {
          enb_ue_s1ap_id_t enb_ue_s1ap_id =
              (enb_ue_s1ap_id_t) * (s1_sig_conn_item->eNB_UE_S1AP_ID);
          enb_ue_s1ap_id &= ENB_UE_S1AP_ID_MASK;
          if ((ue_ref_p = s1ap_is_ue_enb_id_in_list(enb_association,
                                                    enb_ue_s1ap_id)) != NULL) {
            ue_to_reset_list[item].enb_ue_s1ap_id =
                (enb_ue_s1ap_id_t *)ue_ref_p;
    ...
    OAILOG_INFO(LOG_S1AP,
                "After iterating all %d partial UEs, informing the MME_APP "
                "layer for the received Partial Reset Request.\n",
                num_ue_assoc);
    ...
    S1AP_ENB_INITIATED_RESET_REQ(message_p).ue_to_reset_list =
        (s1_sig_conn_id_t *)calloc(num_ue_assoc, sizeof(s1_sig_conn_id_t));
    memcpy(S1AP_ENB_INITIATED_RESET_REQ(message_p).ue_to_reset_list,
           ue_to_reset_list, sizeof(ue_to_reset_list));
  } else {
    /** Generating full reset request. */
    OAILOG_INFO(
        LOG_S1AP,
        "Received a full reset request. Informing the MME_APP layer.\n");
     ...
     */
    S1AP_ENB_INITIATED_RESET_REQ(message_p).ue_to_reset_list =
        (s1_sig_conn_id_t *)calloc(enb_association->nb_ue_associated,
                                   sizeof(s1_sig_conn_id_t));
    hashtable_ts_apply_callback_on_elements(&enb_association->ue_coll,
...
void s1ap_mme_generate_erab_modification_confirm(
  ...
  bstring b = blk2bstr(buffer_p, length);
  free(buffer_p);
  s1ap_mme_itti_send_sctp_request(&b, ue_ref->enb->sctp_assoc_id,
