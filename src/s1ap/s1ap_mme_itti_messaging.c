...
int s1ap_mme_itti_nas_uplink_ind(const mme_ue_s1ap_id_t ue_id,
  ...
  *payload = NULL;
  ...
  MSC_LOG_TX_MESSAGE(MSC_S1AP_MME, MSC_NAS_MME, NULL, 0,
                     "0 NAS_UPLINK_DATA_IND ue_id " MME_UE_S1AP_ID_FMT
                     " len %u",
                     NAS_UPLINK_DATA_IND(message_p).ue_id,
                     blength(NAS_UPLINK_DATA_IND(message_p).nas_msg));
...
int s1ap_mme_itti_nas_downlink_cnf(const mme_ue_s1ap_id_t ue_id,
  ...
  MSC_LOG_TX_MESSAGE(
      ...
      "0 NAS_DL_DATA_CNF ue_id " MME_UE_S1AP_ID_FMT " err_code %u",
    ...
    const void *const opt_relay_node_indicator)       // unused
{
  ...
  MSC_LOG_TX_MESSAGE(
      ...
      "0 S1AP_INITIAL_UE_MESSAGE eNB ue_id " ENB_UE_S1AP_ID_FMT
      " as cause %u tai:%c%c%c.%c%c%c:%u len %u ",
      enb_ue_s1ap_id,
      ...
      (9 < S1AP_INITIAL_UE_MESSAGE(message_p).tai.mnc_digit3)
          ? ' '
          : (char)(S1AP_INITIAL_UE_MESSAGE(message_p).tai.mnc_digit3 + 0x30),
...
void s1ap_mme_itti_nas_non_delivery_ind(const mme_ue_s1ap_id_t ue_id,
  ...
  MSC_LOG_TX_MESSAGE(MSC_S1AP_MME, MSC_NAS_MME, NULL, 0,
                     "0 NAS_DOWNLINK_DATA_REJ ue_id " MME_UE_S1AP_ID_FMT
                     " len %u",
                     ue_id, NAS_DL_DATA_REJ(message_p).nas_msg->slen);
