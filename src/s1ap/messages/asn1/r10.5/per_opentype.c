...
int uper_open_type_put(asn_TYPE_descriptor_t *td,
  ...
  for (bptr = buf, toGo = size; toGo;) {
    ...
    ASN_DEBUG("Prepending length %d to %s and allowing to save %d", (int)size,
              td->name, (int)maySave);
    if (maySave < 0) break;
  ...
  ASN_DEBUG("Open type put %s of length %ld + overhead (1byte?)", td->name,
            (long)size);

  return 0;
}

int aper_open_type_put(asn_TYPE_descriptor_t *td,
                       asn_per_constraints_t *constraints, void *sptr,
                       asn_per_outp_t *po) {
  void *buf;
  void *bptr;
...
#define UPDRESTOREPD                                          \
  ...
  if (pd->nboff != pd->nbits) {
    ASN_DEBUG("Open type %s overhead pd%s old%s", td->name, per_data_string(pd),
