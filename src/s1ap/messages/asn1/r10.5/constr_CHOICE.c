  ...
  switch (ctx->phase) {
    ...
    case 2:
      ...
      do {
         ...
         */
        rval = elm->type->ber_decoder(opt_codec_ctx, elm->type, memb_ptr2, ptr,
                                      LEFT, elm->tag_mode);
        switch (rval.code) {
          case RC_OK:
            break;
...
int CHOICE_constraint(asn_TYPE_descriptor_t *td, const void *sptr,
  ...
  if (present > 0 && present <= td->elements_count) {
    ...
    _ASN_CTFAIL(app_key, td, sptr, "%s: no CHOICE element given (%s:%d)",
...
asn_dec_rval_t CHOICE_decode_aper(asn_codec_ctx_t *opt_codec_ctx,
                                  asn_TYPE_descriptor_t *td,
                                  asn_per_constraints_t *constraints,
                                  void **sptr, asn_per_data_t *pd) {
  asn_CHOICE_specifics_t *specs = (asn_CHOICE_specifics_t *)td->specifics;
...
asn_enc_rval_t CHOICE_encode_uper(asn_TYPE_descriptor_t *td,
  ...
  if (ct && ct->range_bits >= 0) {
    ...
    return elm->type->uper_encoder(elm->type, elm->per_constraints, memb_ptr,
                                   po);
  } else {
    asn_enc_rval_t rval;
...
asn_enc_rval_t CHOICE_encode_aper(asn_TYPE_descriptor_t *td,
                                  asn_per_constraints_t *constraints,
                                  void *sptr, asn_per_outp_t *po) {
  asn_CHOICE_specifics_t *specs = (asn_CHOICE_specifics_t *)td->specifics;
  asn_TYPE_member_t *elm; /* CHOICE's element */
  ...
  if (ct && ct->range_bits >= 0) {
    ...
    return elm->type->aper_encoder(elm->type, elm->per_constraints, memb_ptr,
                                   po);
  } else {
    asn_enc_rval_t rval;
    if (specs->ext_start == -1) _ASN_ENCODE_FAILED;
...
asn_comp_rval_t *CHOICE_compare(asn_TYPE_descriptor_t *td1, const void *sptr1,
