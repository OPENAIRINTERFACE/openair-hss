...
static void test_uncipher_ctr(const struct nettle_cipher *cipher,
                              const uint8_t *key, unsigned key_length,
                              const uint8_t *cipheredtext, unsigned length,
                              const uint8_t *cleartext, const uint8_t *ictr) {
...
START_TEST(doit_test_aes128_ctr_decrypt) {
  /*
...
Suite *suite(void) {
  ...
  tcase_add_test(tc_core, doit_test_aes128_ctr_decrypt);
