...
int aia_cb(struct msg **msg, struct avp *paramavp, struct session *sess,
...
int fd_init_dict_objs(void) {
   ...
   */
  CHECK_FD_FCT(
      fd_disp_register(aia_cb, DISP_HOW_CC, &when, NULL, &s6a_fd_cnf.aia_hdl));
...
//------------------------------------------------------------------------------
int parse_experimental_result(struct avp *avp, s6a_experimental_result_t *ptr) {
  struct avp_hdr *hdr;
...
//------------------------------------------------------------------------------
int parse_rand(struct avp_hdr *hdr, uint8_t *rand_p) {
  int ret = 0;
...
//------------------------------------------------------------------------------
int parse_xres(struct avp_hdr *hdr, res_t *xres) {
  int ret = 0;
...
//------------------------------------------------------------------------------
int parse_autn(struct avp_hdr *hdr, uint8_t *autn) {
  int ret = 0;
...
//------------------------------------------------------------------------------
int parse_kasme(struct avp_hdr *hdr, uint8_t *kasme) {
  int ret = 0;
...
static inline int parse_e_utran_vector(struct avp *avp_vector,
...
//------------------------------------------------------------------------------
int parse_authentication_info_avp(struct avp *avp_auth_info,
  ...
  while (avp) {
    ...
    switch (hdr->avp_code) {
      case AVP_CODE_E_UTRAN_VECTOR: {
        ...
        CHECK_FCT(
            parse_e_utran_vector(avp, &authentication_info->eutran_vector[0]));
...
void auth_write_results(char *fileout, authentication_info_t *ai) {
  ...
  for (int i = 0; i < ai->nb_of_vectors; i++) {
    fprintf(fp,
            "RAND[%u]=\"%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%"
            "02x%02x%02x\"\n",
    ...
    fprintf(fp, "\"\n");
    fprintf(fp,
            "AUTN[%u]=\"%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%"
            "02x%02x%02x\"\n",
            i, DISPLAY_128BITS(ai->eutran_vector[i].autn));
    fprintf(fp,
            "KASME[%u]=\"%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%"
            "02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%"
            "02x%02x%02x\"\n",
            i, KASME_DISPLAY_1(ai->eutran_vector[i].kasme),
            KASME_DISPLAY_2(ai->eutran_vector[i].kasme));

    uint8_t kenb[AUTH_KENB_SIZE] = {0};
    derive_keNB(ai->eutran_vector[i].kasme, gul_nas_count, kenb);
    fprintf(fp,
            "KENB[%u]=\"%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%"
            "02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%"
            "02x%02x%02x\"\n",
    ...
    derive_key_nas(NAS_INT_ALG, gintegrity, ai->eutran_vector[i].kasme,
                   knas_int);
    derive_key_nas(NAS_ENC_ALG, gencryption, ai->eutran_vector[i].kasme,
                   knas_enc);
    fprintf(fp,
            "KNAS_INT[%u]=\"%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%"
            "02x%02x%02x%02x\"\n",
            i, DISPLAY_128BITS(knas_int));
    fprintf(fp,
            "KNAS_ENC[%u]=\"%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%"
            "02x%02x%02x%02x\"\n",
...
int aia_cb(struct msg **msg, struct avp *paramavp, struct session *sess,
  ...
  if (skip_auth_res == 0) {
    ...
    if (avp) {
      CHECK_FCT(
          parse_authentication_info_avp(avp, &s6a_auth_info_ans_p->auth_info));
    } else {
      DevMessage(
...
//------------------------------------------------------------------------------
int generate_authentication_info_req(s6a_auth_info_req_t *air_p) {
  struct avp *avp;
  struct msg *msg;
  struct session *sess;
...
//------------------------------------------------------------------------------
int config_parse_opt_line(int argc, char *argv[]) {
  int c;
   ...
   */
  AssertFatal((gfd_config_file),
              "Mandatory argument -c freeDiameter config file");
