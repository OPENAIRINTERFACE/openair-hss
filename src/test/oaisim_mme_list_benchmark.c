...
static char infoNAS[] = {
    0x07, 0x42, 0x01, 0xE0, 0x06, 0x00, 0x00, 0xF1, 0x10, 0x00, 0x01, 0x00,
    0x2C, 0x52, 0x01, 0xC1, 0x01, 0x09, 0x10, 0x03, 0x77, 0x77, 0x77, 0x07,
    0x61, 0x6E, 0x72, 0x69, 0x74, 0x73, 0x75, 0x03, 0x63, 0x6F, 0x6D, 0x05,
    0x01, 0x0A, 0x01, 0x20, 0x37, 0x27, 0x0E, 0x80, 0x80, 0x21, 0x0A, 0x03,
    0x00, 0x00, 0x0A, 0x81, 0x06, 0x0A, 0x00, 0x00, 0x01, 0x50, 0x0B, 0xF6,
...
int recv_callback(uint32_t assocId, uint32_t stream, uint8_t *buffer,
                  uint32_t length);
...
int s1ap_test_generate_initial_ue_message(uint32_t eNB_UE_S1AP_ID,
  ...
  return s1ap_eNB_encode_initial_ue_message(initialUEmessageIEs_p, buffer,
                                            length);
}

int s1ap_test_generate_initial_setup_resp(uint32_t eNB_UE_S1AP_ID,
                                          uint32_t mme_UE_S1AP_ID,
                                          uint8_t eRAB_id, uint32_t teid,
                                          uint8_t **buffer, uint32_t *length) {
...
int recv_callback(uint32_t assocId, uint32_t stream, uint8_t *buffer,
  ...
  if (message.procedureCode == ProcedureCode_id_S1Setup &&
    ...
    s1ap_test_generate_initial_setup_resp(
        ...
        message.msg.initialContextSetupRequestIEs.mme_ue_s1ap_id, 0x5, 0x1,
        &buffer2, &len);
...
int sctp_connected(void *args, uint32_t assocId, uint32_t instreams,
  ...
  if (sctp_send_msg(assocId, 0, buffer, len) < 0) {
    free(buffer);
    fprintf(stderr,
            "sctp_send_msg returned status < 0. Something bad happened on SCTP "
            "layer\n");
    exit(0);
  }
