...
/*! \file msc.c
   \brief Message chart generator logging utility (generate files to be
   processed by a script to produce a mscgen input file for generating a
   sequence diagram document) \author  Lionel GAUTHIER \date 2015 \email:
...
int msc_init(const msc_env_t envP, const int max_threadsP) {
  ...
  rv = snprintf(msc_filename, NAME_MAX, "/tmp/openair.msc.%u.log",
                envP);  // TODO NAME
  ...
  AssertFatal(g_msc_fd != NULL, "Could not open MSC log file %s : %s",
              msc_filename, strerror(errno));

  for (i = MIN_MSC_PROTOS; i < MAX_MSC_PROTOS; i++) {
    switch (i) {
      case MSC_NAS_UE:
        rv = snprintf(&g_msc_proto2str[i][0], MSC_MAX_PROTO_NAME_LENGTH,
                      "NAS_UE");
...
void msc_end(void) {
  ...
  if (NULL != g_msc_fd) {
    ...
    if (rv != 0) {
      OAI_FPRINTF_ERR("Error while flushing stream of MSC log file: %s",
                      strerror(errno));
    }
  ...
  if ((MIN_MSC_PROTOS <= protoP) && (MAX_MSC_PROTOS > protoP)) {
    ...
    if (NULL != new_item_p) {
      rv = bassignformat(new_item_p->bstr, "%" PRIu64 " [PROTO] %d %s\n",
                         __sync_fetch_and_add(&g_message_number, 1), protoP,
                         &g_msc_proto2str[protoP][0]);

      if (BSTR_ERR == rv) {
        OAI_FPRINTF_ERR("Error while declaring new protocol in MSC: %d",
                        protoP);
...
void msc_log_event(const msc_proto_t protoP, char *format, ...) {
  ...
  if (NULL != new_item_p) {
    ...
    if (BSTR_ERR == rv) {
      OAI_FPRINTF_ERR("Error while logging MSC event : %s",
                      &g_msc_proto2str[protoP][0]);
      return;
    }
...
//------------------------------------------------------------------------------
void msc_log_message(const char *const message_operationP,
                     const msc_proto_t proto1P, const msc_proto_t proto2P,
                     uint8_t *bytesP, const unsigned int num_bytes,
                     char *format, ...) {
  va_list args;
  ...
  if (NULL != new_item_p) {
    struct timeval elapsed_time;
    shared_log_get_elapsed_time_since_start(&elapsed_time);
    rv = bassignformat(new_item_p->bstr,
                       "%" PRIu64 " [MESSAGE] %d %s %d %" PRIu64 " %04ld:%06ld",
                       __sync_fetch_and_add(&g_message_number, 1), proto1P,
                       message_operationP, proto2P, mac, elapsed_time.tv_sec,
                       elapsed_time.tv_usec);

    if (BSTR_ERR == rv) {
      OAI_FPRINTF_ERR("Error while logging MSC message : %s/%s",
