...
#define LOG_CONFIG_STRING_MME_SCENARIO_PLAYER_LOG_LEVEL \
...
typedef struct log_config_s {
  ...
  log_level_t spgw_app_log_level; /*!< \brief SP-GW ITTI task log level starting
                                     ...
                                     MAX_LOG_LEVEL (no log) */
  log_level_t
      s10_log_level; /*!< \brief S10 ITTI task log level starting from
                        OAILOG_LEVEL_EMERGENCY up to MAX_LOG_LEVEL (no log) */
  log_level_t
  ...
  log_level_t
      msc_log_level; /*!< \brief MSC utility log level starting from
                        OAILOG_LEVEL_EMERGENCY up to MAX_LOG_LEVEL (no log) */
  log_level_t
      xml_log_level; /*!< \brief XML dump/load of messages (mainly for MME
                        scenario player) log level starting from
                        OAILOG_LEVEL_EMERGENCY up to MAX_LOG_LEVEL (no log) */
  log_level_t
      mme_scenario_player_log_level;  /*!< \brief scenario player log level
...
int log_init(const log_env_t envP, const log_level_t default_log_levelP,
...
void log_message_add(struct shared_log_queue_item_s* contextP, char* format,
...
void log_message(log_thread_ctxt_t* const thread_ctxtP,
                 ...
                 const char* const source_fileP, const unsigned int line_numP,
                 char* format, ...) __attribute__((format(printf, 6, 7)));
...
#define OAILOG_MESSAGE_START(lOgLeVeL, pRoTo, cOnTeXt, ...)               \
  do {                                                                    \
    log_message_start(NULL, lOgLeVeL, pRoTo, cOnTeXt, __FILE__, __LINE__, \
                      ##__VA_ARGS__);                                     \
  } while (0) /*!< \brief when need to log only 1 message with many char  \
                 messages, ex formating a dumped struct */
#define OAILOG_MESSAGE_ADD(cOnTeXt, ...)                              \
  do {                                                                \
    log_message_add(cOnTeXt, ##__VA_ARGS__);                          \
...
#define OAILOG_STREAM_HEX_ARRAY(pRoTo, mEsSaGe, sTrEaM, sIzE)           \
  do {                                                                  \
    log_stream_hex_array(OAILOG_LEVEL_TRACE, pRoTo, __FILE__, __LINE__, \
                         mEsSaGe, sTrEaM, sIzE);                        \
  } while (0) /*!< \brief trace buffer content with indexes */
#endif
#endif
