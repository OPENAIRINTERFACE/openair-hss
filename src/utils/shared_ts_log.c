...
void *shared_log_task(__attribute__((unused)) void *args_p) {
  ...
  timer_setup(LOG_FLUSH_PERIOD_SEC, LOG_FLUSH_PERIOD_MICRO_SEC,
              TASK_SHARED_TS_LOG, INSTANCE_DEFAULT, TIMER_ONE_SHOT, NULL,
              &timer_id);
    ...
    if (received_message_p != NULL) {
      switch (ITTI_MSG_ID(received_message_p)) {
        case TIMER_HAS_EXPIRED: {
          ...
          timer_setup(LOG_FLUSH_PERIOD_SEC, LOG_FLUSH_PERIOD_MICRO_SEC,
                      TASK_SHARED_TS_LOG, INSTANCE_DEFAULT, TIMER_ONE_SHOT,
                      NULL, &timer_id);
        } break;
...
void shared_log_flush_messages(void) {
  ...
  while ((rv = lfds710_queue_bmm_dequeue(&g_shared_log.log_message_queue, NULL,
                                         (void **)&item_p)) == 1) {
