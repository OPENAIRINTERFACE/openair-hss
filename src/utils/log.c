...
void *log_task(__attribute__((unused)) void *args_p) {
  ...
  log_start_use();
  timer_setup(LOG_FLUSH_PERIOD_SEC, LOG_FLUSH_PERIOD_MICRO_SEC, TASK_LOG,
      ...
      switch (ITTI_MSG_ID(received_message_p)) {
        case TIMER_HAS_EXPIRED: {
          ...
          if (LOG_TCP_STATE_NOT_CONNECTED == g_oai_log.tcp_state) {
            ...
            timer_setup(LOG_CONNECT_PERIOD_SEC, LOG_CONNECT_PERIOD_MICRO_SEC,
                        TASK_LOG, INSTANCE_DEFAULT, TIMER_ONE_SHOT, NULL,
                        &timer_id);
          }
        } break;
        ...
        case TERMINATE_MESSAGE: {
          ...
          rc = itti_send_msg_to_task(TASK_SHARED_TS_LOG, INSTANCE_DEFAULT,
                                     terminate_message_p);
...
void log_set_config(const log_config_t *const config) {
  if (config) {
    ...
    if ((MAX_LOG_LEVEL > config->s10_log_level) &&
        (MIN_LOG_LEVEL <= config->s10_log_level))
      g_oai_log.log_level[LOG_S10] = config->s10_log_level;
    ...
    if ((MAX_LOG_LEVEL > config->secu_log_level) &&
        (MIN_LOG_LEVEL <= config->secu_log_level))
      g_oai_log.log_level[LOG_SECU] = config->secu_log_level;
    ...
    if ((MAX_LOG_LEVEL > config->msc_log_level) &&
        (MIN_LOG_LEVEL <= config->msc_log_level))
      g_oai_log.log_level[LOG_MSC] = config->msc_log_level;
    if ((MAX_LOG_LEVEL > config->xml_log_level) &&
        (MIN_LOG_LEVEL <= config->xml_log_level))
      g_oai_log.log_level[LOG_XML] = config->xml_log_level;
    if ((MAX_LOG_LEVEL > config->mme_scenario_player_log_level) &&
        (MIN_LOG_LEVEL <= config->mme_scenario_player_log_level))
      g_oai_log.log_level[LOG_MME_SCENARIO_PLAYER] =
          config->mme_scenario_player_log_level;
    ...
    if (config->output) {
      ...
      if (1 ==
      ...
      } else if (1 == biseqcstrcaseless(config->output,
                                        LOG_CONFIG_STRING_OUTPUT_SYSLOG)) {
        ...
        if (('.' == bchar(config->output, 0)) ||
          ...
          AssertFatal(NULL != g_oai_log.log_fd,
                      "Could not open log file %s : %s", bdata(config->output),
                      strerror(errno));
          g_oai_log.is_output_is_fd = true;
        } else {
...
//------------------------------------------------------------------------------
int log_init(__attribute__((unused)) const log_env_t envP,
             const log_level_t default_log_levelP, const int max_threadsP) {
  int i = 0;
  ...
  thread_ctxt->tid = p;
  hashtable_rc_t hash_rc = hashtable_ts_insert(g_oai_log.thread_context_htbl,
  ...
  snprintf(&g_oai_log.log_proto2str[LOG_SPGW_APP][0], LOG_MAX_PROTO_NAME_LENGTH,
           "SPGW-APP");
  snprintf(&g_oai_log.log_proto2str[LOG_S10][0], LOG_MAX_PROTO_NAME_LENGTH,
           "S10");
  ...
  snprintf(&g_oai_log.log_proto2str[LOG_CONFIG][0], LOG_MAX_PROTO_NAME_LENGTH,
           "CONFIG");
  snprintf(&g_oai_log.log_proto2str[LOG_MSC][0], LOG_MAX_PROTO_NAME_LENGTH,
           "MSC");
  snprintf(&g_oai_log.log_proto2str[LOG_XML][0], LOG_MAX_PROTO_NAME_LENGTH,
           "XML");
  snprintf(&g_oai_log.log_proto2str[LOG_MME_SCENARIO_PLAYER][0],
           LOG_MAX_PROTO_NAME_LENGTH, "MME-TEST");
...
void log_stream_hex(const log_level_t log_levelP, const log_proto_t protoP,
  ...
  if (messageP) {
    log_message_start(thread_ctxt, log_levelP, protoP, &message, source_fileP,
                      line_numP, "hex stream ");
    if (!message) return;
    rv = bformata(message->bstr, "%s", messageP);
  } else {
    log_message_start(thread_ctxt, log_levelP, protoP, &message, source_fileP,
                      line_numP, "hex stream (%ld bytes):", sizeP);
    if (!message) return;
  }
  ...
  if (streamP) {
    for (octet_index = 0; octet_index < sizeP; octet_index++) {
      if ((octet_index % 16) == 0) {
        ...
        log_message_start(thread_ctxt, log_levelP, protoP, &message,
                          source_fileP, line_numP, " %04ld |", octet_index);
      }

      /*
       * Print every single octet in hexadecimal form
       */
      log_message_add(message, " %02x",
  ...
  if (*messageP) {
    ...
    if (filename_length > LOG_DISPLAYED_FILENAME_MAX_LENGTH) {
      ...
      rv = bformata(
          ...
          &g_oai_log.log_proto2str[protoP][0],
          LOG_DISPLAYED_FILENAME_MAX_LENGTH, LOG_DISPLAYED_FILENAME_MAX_LENGTH,
          source_fileP, line_numP, thread_ctxt->indent, " ");
    }
