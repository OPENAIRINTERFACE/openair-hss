...
/*
   ...
   hash_free_func() is used when this hashtable is used to store generic values
   as data (pointer = value).
*/
...
/*
   ...
   hashtable_destroy().
*/
hash_table_t *hashtable_create(const hash_size_t sizeP,
                               hash_size_t (*hashfuncP)(const hash_key_t),
                               void (*freefuncP)(void **),
                               bstring display_name_pP) {
...
//------------------------------------------------------------------------------
hashtable_rc_t hashtable_is_key_exists(const hash_table_t *const hashtblP,
...
hashtable_rc_t hashtable_apply_callback_on_elements(
    ...
    bool funct_cb(hash_key_t keyP, void *dataP, void *parameterP,
                  void **resultP),
...
// criteria different than the single key The compare criteria in implemented in
// the funct_cb function
hashtable_rc_t hashtable_ts_apply_list_callback_on_elements(
    hash_table_ts_t *const hashtblP,
    bool funct_cb(const hash_key_t keyP, void *const dataP, void *parameterP,
                  void **resultP),
    void *parameterP, hashtable_element_array_t *ea) /**< Stacked list. */
{
  hash_node_t *node = NULL;
  unsigned int i = 0;
  unsigned int num_elements = 0;
  ...
  while ((ea->num_elements <= hashtblP->num_elements) &&
         (num_elements < hashtblP->num_elements) && (i < hashtblP->size)) {
    pthread_mutex_lock(&hashtblP->lock_nodes[i]);
    if (hashtblP->nodes[i] != NULL) {
      node = hashtblP->nodes[i];
...
*/
hashtable_rc_t hashtable_free(hash_table_t *const hashtblP,
                              const hash_key_t keyP) {
...
*/
hashtable_rc_t hashtable_remove(hash_table_t *const hashtblP,
                                const hash_key_t keyP, void **dataP) {
  hash_node_t *node, *prevnode = NULL;
  hash_size_t hash = 0;
...
*/
hashtable_rc_t hashtable_get(const hash_table_t *const hashtblP,
                             const hash_key_t keyP, void **dataP) {
  hash_node_t *node = NULL;
  hash_size_t hash = 0;
...
hashtable_rc_t hashtable_resize(hash_table_t *const hashtblP,
                                const hash_size_t sizeP) {
  hash_table_t newtbl;
  hash_size_t n;
...
hashtable_rc_t hashtable_ts_resize(hash_table_ts_t *const hashtblP,
                                   const hash_size_t sizeP) {
