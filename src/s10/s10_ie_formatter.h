 ...
 * 3GPP TS 29.274 #8.38
 */
nw_rc_t s10_ue_mm_eps_context_ie_set(nw_gtpv2c_msg_handle_t* msg,
                                     const mm_context_eps_t* ue_eps_mm_context);
 ...
 * 3GPP TS 29.274 #8.51
 */
int s10_target_identification_ie_set(
...
/* Bearer Contexts to Create Information Element as part of Create Session
 * Request 3GPP TS 29.274 Table 7.2.1-2.
 */
// nw_rc_t s10_bearer_context_to_be_created_ie_get (uint8_t ieType, uint16_t
// ieLength, uint8_t ieInstance, uint8_t * ieValue, void *arg);
...
nw_rc_t s10_fteid_ie_get(uint8_t ieType, uint16_t ieLength, uint8_t ieInstance,
                         uint8_t* ieValue, void* arg);
...
int s10_pdn_address_ie_set(nw_gtpv2c_msg_handle_t* msg,
