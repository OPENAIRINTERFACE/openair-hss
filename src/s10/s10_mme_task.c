...
//------------------------------------------------------------------------------
static nw_rc_t s10_mme_log_wrapper(
...
//------------------------------------------------------------------------------
static nw_rc_t s10_mme_ulp_process_stack_req_cb(nw_gtpv2c_ulp_handle_t hUlp,
    ...
    case NW_GTPV2C_ULP_API_INITIAL_REQ_IND:
      ...
      switch (pUlpApi->u_api_info.initialReqIndInfo.msgType) {
        case NW_GTP_FORWARD_RELOCATION_REQ:
          ret = s10_mme_handle_forward_relocation_request(&s10_mme_stack_handle,
                                                          pUlpApi);
          break;
        case NW_GTP_FORWARD_ACCESS_CONTEXT_NTF:
          ret = s10_mme_handle_forward_access_context_notification(
              &s10_mme_stack_handle, pUlpApi);
          break;

        case NW_GTP_FORWARD_RELOCATION_COMPLETE_NTF:
          ret = s10_mme_handle_forward_relocation_complete_notification(
              &s10_mme_stack_handle, pUlpApi);
          break;
        ...
        case NW_GTP_RELOCATION_CANCEL_REQ:
          ret = s10_mme_handle_relocation_cancel_request(&s10_mme_stack_handle,
                                                         pUlpApi);
          break;
    ...
    case NW_GTPV2C_ULP_API_TRIGGERED_RSP_IND:
      ...
      switch (pUlpApi->u_api_info.triggeredRspIndInfo.msgType) {
        case NW_GTP_FORWARD_RELOCATION_RSP:
          ret = s10_mme_handle_forward_relocation_response(
              &s10_mme_stack_handle, pUlpApi);
          break;

        case NW_GTP_FORWARD_ACCESS_CONTEXT_ACK:
          ret = s10_mme_handle_forward_access_context_acknowledge(
              &s10_mme_stack_handle, pUlpApi);
          break;

        case NW_GTP_FORWARD_RELOCATION_COMPLETE_ACK:
          ret = s10_mme_handle_forward_relocation_complete_acknowledge(
              &s10_mme_stack_handle, pUlpApi);
          break;
        ...
        case NW_GTP_RELOCATION_CANCEL_RSP:
          ret = s10_mme_handle_relocation_cancel_response(&s10_mme_stack_handle,
                                                          pUlpApi);
          break;
    ...
    case NW_GTPV2C_ULP_API_TRIGGERED_ACK_IND:
      ...
      switch (pUlpApi->u_api_info.triggeredAckIndInfo.msgType) {
        case NW_GTP_CONTEXT_ACK:
          ret = s10_mme_handle_context_acknowledgement(&s10_mme_stack_handle,
                                                       pUlpApi);
          break;
      }
      break;
...
//------------------------------------------------------------------------------
static nw_rc_t s10_mme_send_udp_msg(nw_gtpv2c_udp_handle_t udpHandle,
...
//------------------------------------------------------------------------------
static nw_rc_t s10_mme_start_timer_wrapper(
  ...
  if (tmrType == NW_GTPV2C_TMR_TYPE_REPETITIVE) {
    ret = timer_setup(timeoutSec, timeoutUsec, TASK_S10, INSTANCE_DEFAULT,
...
//------------------------------------------------------------------------------
static nw_rc_t s10_mme_stop_timer_wrapper(
...
static void *s10_mme_thread(void *args) {
    ...
    switch (ITTI_MSG_ID(received_message_p)) {
      ...
      case S10_FORWARD_RELOCATION_REQUEST: {
        s10_mme_forward_relocation_request(
            &s10_mme_stack_handle,
            &received_message_p->ittiMsg.s10_forward_relocation_request);
      } break;

      case S10_FORWARD_RELOCATION_RESPONSE: {
        s10_mme_forward_relocation_response(
            &s10_mme_stack_handle,
            &received_message_p->ittiMsg.s10_forward_relocation_response);
      } break;

      case S10_FORWARD_ACCESS_CONTEXT_NOTIFICATION: {
        s10_mme_forward_access_context_notification(
            &s10_mme_stack_handle,
      ...
      case S10_FORWARD_ACCESS_CONTEXT_ACKNOWLEDGE: {
        s10_mme_forward_access_context_acknowledge(
            &s10_mme_stack_handle,
            &received_message_p->ittiMsg
      ...
      case S10_CONTEXT_REQUEST: {
        s10_mme_context_request(
            &s10_mme_stack_handle,
            &received_message_p->ittiMsg.s10_context_request);
      } break;

      case S10_CONTEXT_RESPONSE: {
        s10_mme_context_response(
            &s10_mme_stack_handle,
            &received_message_p->ittiMsg.s10_context_response);
      } break;

      case S10_CONTEXT_ACKNOWLEDGE: {
        s10_mme_context_acknowledge(
            &s10_mme_stack_handle,
            &received_message_p->ittiMsg.s10_context_acknowledge);
        OAILOG_WARNING(LOG_S10,
                       "S10 Context ACK not sent due memory reasons. \n");
      } break;

      case S10_RELOCATION_CANCEL_REQUEST: {
        s10_mme_relocation_cancel_request(
            &s10_mme_stack_handle,
            &received_message_p->ittiMsg.s10_relocation_cancel_request);
      } break;

      case S10_RELOCATION_CANCEL_RESPONSE: {
        s10_mme_relocation_cancel_response(
            &s10_mme_stack_handle,
            &received_message_p->ittiMsg.s10_relocation_cancel_response);
      } break;

      /** New Internal Messages. */

      /**
       * Use this message in case of an error to remove the S10 Local Tunnel
       * endpoints. No response to MME_APP is sent/expected.
       */
      case S10_REMOVE_UE_TUNNEL: {
        s10_mme_remove_ue_tunnel(
            &s10_mme_stack_handle,
            &received_message_p->ittiMsg.s10_remove_ue_tunnel);
      } break;
      ...
      case UDP_DATA_IND: {
        ...
        rc = nwGtpv2cProcessUdpReq(
            s10_mme_stack_handle, udp_data_ind->msgBuf,
...
//------------------------------------------------------------------------------
static int s10_send_init_udp(struct in_addr *address, struct in6_addr *address6,
...
//------------------------------------------------------------------------------
int s10_mme_init(const mme_config_t *mme_config_p) {
  int ret = 0;
  ...
  DevAssert(NW_OK ==
            nwGtpv2cSetLogLevel(s10_mme_stack_handle, NW_LOG_LEVEL_DEBG));
  /** Create 2 sockets, one for 2123 (received initial requests), another high
   * port. */
  s10_send_init_udp(&mme_config.ip.s10_mme_v4, &mme_config.ip.s10_mme_v6,
