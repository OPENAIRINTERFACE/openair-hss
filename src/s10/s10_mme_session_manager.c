...
//  uint16_t                                offendingIeLength;
//  itti_s10_forward_relocation_request_t  *req_p;
//  itti_s10_forward_relocation_request_t  *req_p1;
//  MessageDef                             *message_p;
//  MessageDef                             *message_p1;
//  nw_gtpv2c_msg_parser_t                     *pMsgParser;
//  nw_gtpv2c_msg_parser_t                     *pMsgParser2;
//
//  DevAssert (stack_p );
//  /** Allocating the Signal once at the sender (MME_APP --> S10) and once at the receiver (S10-->MME_APP). */
//  message_p = itti_alloc_new_message (TASK_S10, S10_FORWARD_RELOCATION_REQUEST);
...
//  memset(req_p, 0, sizeof(*req_p));
//
//  message_p1 = itti_alloc_new_message (TASK_S10, S10_FORWARD_RELOCATION_REQUEST);
//  req_p1 = &message_p1->ittiMsg.s10_forward_relocation_request;
//  memset(req_p1, 0, sizeof(*req_p));
//
//
//  req_p->teid = nwGtpv2cMsgGetTeid(pUlpApi->hMsg);
//  req_p->trxn = (void *)pUlpApi->u_api_info.initialReqIndInfo.hTrxn;
//
//
//  req_p1->teid = nwGtpv2cMsgGetTeid(pUlpApi->hMsg);
//  req_p1->trxn = (void *)pUlpApi->u_api_info.initialReqIndInfo.hTrxn;
//
//
//  /** Check the destination TEID is 0. */
//  if(req_p->teid != (teid_t)0){
...
//  /** SECOND UE. */
//
//  rc = nwGtpv2cMsgParserNew (*stack_p, NW_GTP_FORWARD_RELOCATION_REQ, s10_ie_indication_generic, NULL, &pMsgParser2);
//  DevAssert (NW_OK == rc);
//
//   /*
//    * Sender (Source MME) FTEID for CP IE
//    */
//   rc = nwGtpv2cMsgParserAddIe (pMsgParser2, NW_GTPV2C_IE_FTEID, NW_GTPV2C_IE_INSTANCE_ZERO, NW_GTPV2C_IE_PRESENCE_MANDATORY,
//       gtpv2c_fteid_ie_get, &req_p1->s10_source_mme_teid);
//   DevAssert (NW_OK == rc);
//
//   /*
//    * Sender (Source SAE-GW) FTEID for CP IE
//    */
//   rc = nwGtpv2cMsgParserAddIe (pMsgParser2, NW_GTPV2C_IE_FTEID, NW_GTPV2C_IE_INSTANCE_ONE, NW_GTPV2C_IE_PRESENCE_MANDATORY,
//       gtpv2c_fteid_ie_get, &req_p1->s11_sgw_teid);
//   DevAssert (NW_OK == rc);
//
//   /*
//    * IMSI IE
//    */
//   rc = nwGtpv2cMsgParserAddIe (pMsgParser2, NW_GTPV2C_IE_IMSI, NW_GTPV2C_IE_INSTANCE_ZERO, NW_GTPV2C_IE_PRESENCE_CONDITIONAL,
//       gtpv2c_imsi2_ie_get, &req_p1->imsi);
//   DevAssert (NW_OK == rc);
//
//   /*
//    * PDN Connection IE : Several can exist
//    * todo: multiple pdn connection IEs can exist with instance 0.
//    */
//   req_p1->pdn_connections = calloc(1, sizeof(mme_ue_eps_pdn_connections_t));
//   rc = nwGtpv2cMsgParserAddIe (pMsgParser2, NW_GTPV2C_IE_PDN_CONNECTION, NW_GTPV2C_IE_INSTANCE_ZERO, NW_GTPV2C_IE_PRESENCE_CONDITIONAL,
//        s10_pdn_connections_ie_get, req_p1->pdn_connections);
//   DevAssert (NW_OK == rc);
//
//   /*
//    * MME UE MM Context.
//    */
//   req_p1->ue_eps_mm_context = calloc(1, sizeof(mm_context_eps_t));
//   rc = nwGtpv2cMsgParserAddIe (pMsgParser2, NW_GTPV2C_IE_MM_EPS_CONTEXT, NW_GTPV2C_IE_INSTANCE_ZERO, NW_GTPV2C_IE_PRESENCE_MANDATORY,
//        s10_mm_ue_context_ie_get, req_p1->ue_eps_mm_context);
//   DevAssert (NW_OK == rc);
//
//   /**
//    * E-UTRAN container (F-Container) Information Element.
//    * Instance Zero is E-UTRAN. Only E-UTRAN will be supported.
//    */
//   rc = nwGtpv2cMsgParserAddIe (pMsgParser2, NW_GTPV2C_IE_F_CONTAINER, NW_GTPV2C_IE_INSTANCE_ZERO, NW_GTPV2C_IE_PRESENCE_CONDITIONAL,
//        s10_f_container_ie_get, &req_p1->f_container);
//   DevAssert (NW_OK == rc);
//
//   /**
//    * Target Identifier Information Element.
//    */
//   rc = nwGtpv2cMsgParserAddIe (pMsgParser2, NW_GTPV2C_IE_TARGET_IDENTIFICATION, NW_GTPV2C_IE_INSTANCE_ZERO, NW_GTPV2C_IE_PRESENCE_CONDITIONAL,
//       gtpv2c_target_identification_ie_get, &req_p1->target_identification);
//   DevAssert (NW_OK == rc);
//
//   // todo: F-CAUSE not handled!
//   /*
//    * Run the parser
//    */
//   rc = nwGtpv2cMsgParserRun (pMsgParser2, (pUlpApi->hMsg), &offendingIeType, &offendingIeInstance, &offendingIeLength);
//
////  rc = nwGtpv2cMsgParserDelete (*stack_p, pMsgParser);
////  DevAssert (NW_OK == rc);
////  rc = nwGtpv2cMsgDelete (*stack_p, (pUlpApi->hMsg));
////  DevAssert (NW_OK == rc);
//
//  return itti_send_msg_to_task (TASK_MME_APP, INSTANCE_DEFAULT, message_p1);
//}
