...
typedef uint32_t
    tmsi_t; /*!< \brief  Since the TMSI has only local significance (i.e. within
               ...
               meet local needs.
                         The TMSI consists of 4 octets. It can be coded using a
...
/*! \struct  imeisv_t
 * \brief Structure containing an International Mobile station Equipment
 * Identity, BCD structure.
 * The IMEISV is composed of the following elements (each element shall
 * consist of decimal digits only):
 *        - Type Allocation Code (TAC). Its length is 8 digits;
 *        - Serial Number (SNR) is an individual serial number uniquely
 *          identifying each equipment within each TAC. Its length is 6
 *          digits;
 *        - Software Version Number (SVN) identifies the software version number
 *          of the mobile equipment. Its length is 2 digits. Regarding updates
 *          of the IMEISV: The security requirements of 3GPP TS 22.016 [32]
 *          apply only to the TAC and SNR, but not to the SVN part of the
 * IMEISV.
 */
...
// 15 Identification of Multimedia
