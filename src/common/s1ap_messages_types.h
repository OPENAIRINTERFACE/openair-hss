...
#define S1AP_PATH_SWITCH_REQUEST_ACKNOWLEDGE(mSGpTR) \
...
#define S1AP_ENB_STATUS_TRANSFER(mSGpTR) \
  (mSGpTR)->ittiMsg.s1ap_enb_status_transfer
#define S1AP_MME_STATUS_TRANSFER(mSGpTR) \
  (mSGpTR)->ittiMsg.s1ap_mme_status_transfer
/** Handover Notify. */
...
#define S1AP_HANDOVER_PREPARATION_FAILURE(mSGpTR) \
  (mSGpTR)->ittiMsg.s1ap_handover_preparation_failure

#define S1AP_HANDOVER_CANCEL(mSGpTR) (mSGpTR)->ittiMsg.s1ap_handover_cancel
#define S1AP_HANDOVER_CANCEL_ACKNOWLEDGE(mSGpTR) \
  (mSGpTR)->ittiMsg.s1ap_handover_cancel_acknowledge
...
#define S1AP_HANDOVER_REQUEST_ACKNOWLEDGE(mSGpTR) \
  (mSGpTR)->ittiMsg.s1ap_handover_request_acknowledge
...
#define S1AP_CONFIGURATION_TRANSFER(mSGpTR) \
  (mSGpTR)->ittiMsg.s1ap_configuration_transfer
...
typedef struct itti_s1ap_configuration_transfer_s {
...
typedef struct itti_s1ap_handover_command_s {
  ...
  /** Since no inner NAS structure is present (message does not travel NAS
   * layer) set the UE_IDs manually. */
  mme_ue_s1ap_id_t mme_ue_s1ap_id;
  enb_ue_s1ap_id_t enb_ue_s1ap_id : 24;
...
typedef struct itti_s1ap_handover_request_acknowledge_s {
  ...
  bstring target_to_source_eutran_container; /**< Target-ToSource Transparent
                                                Container. */
...
typedef struct itti_s1ap_paging_s {
  ...
  //  tai_t                   tai;               /* Indicating the Tracking Area
  //  from which the UE has sent the NAS message.                         */
