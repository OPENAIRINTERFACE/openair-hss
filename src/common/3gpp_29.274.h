...
#define MSG_DELETE_BEARER_REQUEST_MAX_BEARER_CONTEXTS 11
  ...
  bearer_context_to_be_removed_t
      bearer_context[MSG_DELETE_BEARER_REQUEST_MAX_BEARER_CONTEXTS];
} bearer_contexts_to_be_removed_t;

/** Method called inside the timeout. */
typedef esm_cause_t (*esm_ll_handler_cb_t)(const mme_ue_s1ap_id_t, const ebi_t,
                                           const bool retry,
                                           const int retx_count, bearer_qos_t*,
                                           bstring data);
...
typedef struct esm_timeout_ll_cb_arg_s {
  ...
  /* This parameter is received only if the QoS parameters have been modified */
  bearer_qos_t bearer_level_qos;  ///< Bearer QoS, Mandatory CSR
  /** Method called inside the timeout to transmit lower layer data. */
  uintptr_t esm_proc_timeout;
  esm_ll_handler_cb_t ll_handler;
} esm_timeout_ll_cb_arg_t;
...
#define MSG_MODIFY_BEARER_REQUEST_MAX_BEARER_CONTEXTS 11
  uint8_t num_bearer_context;
  bearer_context_to_be_modified_t
      bearer_context[MSG_MODIFY_BEARER_REQUEST_MAX_BEARER_CONTEXTS];
...
#define MSG_MODIFY_BEARER_RESPONSE_MAX_BEARER_CONTEXTS 11
  uint8_t num_bearer_context;
  bearer_context_modified_t
      bearer_context[MSG_MODIFY_BEARER_RESPONSE_MAX_BEARER_CONTEXTS];
...
typedef struct bearer_context_within_create_bearer_response_s {
  ...
  fteid_t
      s5_s8_u_pgw_fteid;  ///< This IE shall be sent on the S5/S8 interfaces.
                          ///< It shall be used to correlate the bearers with
                          ///< those in the Create Bearer Request.
  ...
  fteid_t s2b_u_pgw_fteid;   ///<  C This IE shall be sent on the S2b interface.
                             ///<  It shall be used
                             ///< to correlate the bearers with those in the
                             ///< Create Bearer
                             ///<   Request.
  protocol_configuration_options_t
...
#define MSG_CREATE_BEARER_RESPONSE_MAX_BEARER_CONTEXTS 11
  uint8_t num_bearer_context;
  bearer_context_within_create_bearer_response_t
      bearer_context[MSG_CREATE_BEARER_RESPONSE_MAX_BEARER_CONTEXTS];
...
typedef struct mm_context_eps_s {
  ...
  uint8_t num_quit : 3;
  uint8_t num_quad : 3;
  // todo: osci 0 --> old stuff (everything from s to s+64 in 29.274 --> 8-38.5
  // not present
...
typedef struct mbms_abs_time_data_transfer_s {
  uint32_t sec_since_epoch; /**< Seconds since 1970 (2208988800L removed from
                               value). */
...
typedef struct mbms_flags_s {
   ...
   * to release the MBMS Bearer Context locally in the MME/SGSN (see 3GPP
   * TS 23.007 [13]).
   */
  unsigned lmri : 1;
} mbms_flags_t;
