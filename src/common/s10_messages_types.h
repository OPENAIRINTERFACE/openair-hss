...
#define S10_FORWARD_RELOCATION_REQUEST(mSGpTR) \
  (mSGpTR)->ittiMsg.s10_forward_relocation_request
#define S10_FORWARD_RELOCATION_RESPONSE(mSGpTR) \
  (mSGpTR)->ittiMsg.s10_forward_relocation_response
#define S10_FORWARD_ACCESS_CONTEXT_NOTIFICATION(mSGpTR) \
  (mSGpTR)->ittiMsg.s10_forward_access_context_notification
#define S10_FORWARD_ACCESS_CONTEXT_ACKNOWLEDGE(mSGpTR) \
  (mSGpTR)->ittiMsg.s10_forward_access_context_acknowledge
#define S10_FORWARD_RELOCATION_COMPLETE_NOTIFICATION(mSGpTR) \
  (mSGpTR)->ittiMsg.s10_forward_relocation_complete_notification
#define S10_FORWARD_RELOCATION_COMPLETE_ACKNOWLEDGE(mSGpTR) \
  (mSGpTR)->ittiMsg.s10_forward_relocation_complete_acknowledge
...
#define S10_CONTEXT_ACKNOWLEDGE(mSGpTR) \
  (mSGpTR)->ittiMsg.s10_context_acknowledge
#define S10_RELOCATION_CANCEL_REQUEST(mSGpTR) \
  (mSGpTR)->ittiMsg.s10_relocation_cancel_request
#define S10_RELOCATION_CANCEL_RESPONSE(mSGpTR) \
  (mSGpTR)->ittiMsg.s10_relocation_cancel_response
...
typedef struct itti_s10_forward_relocation_request_s {
  ...
  target_identification_t
      target_identification; /**< Making them pointer to decouple them. */

  mme_ue_eps_pdn_connections_t* pdn_connections;
  ///< MME/SGSN UE EPS PDN Connections
  ///< Several IEs with this type and instance values shall be PDN Connection
  ///< included as necessary to represent a list of PDN Connections
  ...
  // todo: indication flags --> start with 0 to indicate no indirect data
  // tunneling
  F_Container_t f_container;  // E-UTRAN transparent container

  /** Source-To-Target Transparent Container. */
  // todo: if this is an buffer, how is it freed? does everything needs to be
  // stacked
...
typedef struct itti_s10_forward_relocation_response_s {
  ...
  gtpv2c_cause_t cause;  ///< If the MME could successfully establish the UE
 ...
 *
 * The Forward Access Context Notification Acknowledge will be sent on S10
 * interface as part of these procedures:
 * - E-UTRAN Tracking Area Update with MME Change
 * - S1-based Handover with MME change
 * - todo: also sent at attach with MME change with GUTI!! (without getting
 * security context from HSS by new MME)
...
typedef struct itti_s10_context_request_s {
  ...
  Complete_Request_Message_t
      complete_request_message; /**< Could be a TAU or an attach request
                                   message. */
  /* S11 stack specific parameter. Not used in standalone epc mode */

  void* trxn;  ///< Transaction identifier
  //  struct sockaddr                 peer_ip;             ///< MME ipv4 address
  //  for S-GW or S-GW ipv4 address for MME
  union {
    struct sockaddr_in
        addr_v4;  ///< MME ipv4 address for S-GW or S-GW ipv4 address for MME
    struct sockaddr_in6
...
typedef struct itti_s10_context_response_s {
  ...
  /* S11 stack specific parameter. Not used in standalone epc mode */
  void* trxnId;  ///< Transaction identifier (received) or the transaction
                 ///< itself (when sent)
