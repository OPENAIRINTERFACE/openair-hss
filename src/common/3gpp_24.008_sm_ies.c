  ...
  while (flag <= TRAFFIC_FLOW_TEMPLATE_FLOW_LABEL_FLAG) {
    switch (packetfilter->packetfiltercontents.flags & flag) {
      case TRAFFIC_FLOW_TEMPLATE_IPV4_REMOTE_ADDR_FLAG:
        ...
        for (j = encoded; j < TRAFFIC_FLOW_TEMPLATE_IPV4_ADDR_SIZE + encoded;
             j++) {
          *((uint8_t *)(buffer + j)) =
              packetfilter->packetfiltercontents.ipv4remoteaddr[j - encoded]
                  .addr;
          *((uint8_t *)(buffer + j + TRAFFIC_FLOW_TEMPLATE_IPV4_ADDR_SIZE)) =
              packetfilter->packetfiltercontents.ipv4remoteaddr[j - encoded]
                  .mask;
        }
      ...
      case TRAFFIC_FLOW_TEMPLATE_IPV6_REMOTE_ADDR_FLAG:
        ...
        for (j = encoded; j < TRAFFIC_FLOW_TEMPLATE_IPV6_ADDR_SIZE + encoded;
             j++) {
          *((uint8_t *)(buffer + j)) =
              packetfilter->packetfiltercontents.ipv6remoteaddr[j - encoded]
                  .addr;
          *((uint8_t *)(buffer + j + TRAFFIC_FLOW_TEMPLATE_IPV6_ADDR_SIZE)) =
              packetfilter->packetfiltercontents.ipv6remoteaddr[j - encoded]
                  .mask;
        }
...
//  2) When the TFT operation is "Create a new TFT" or "Add packet filters to
//  existing TFT" or "Replace packet filters in existing TFT" and two or more
//  packet filters in all TFTs associated with this PDP address and APN would
//  have identical packet filter precedence values.

//------------------------------------------------------------------------------
static bool check_if_tft_is_rendered_empty(
    traffic_flow_template_t *tft_original, traffic_flow_template_t *tft) {
  /** Check if all TFTs are removed, if there are any more existing and the
   * existing once have at least one DL/BD packet filter. */
  long int removed_pfs = tft_original->packet_filter_identifier_bitmap &
                         tft->packet_filter_identifier_bitmap;
  long int remaining_pfs =
      tft_original->packet_filter_identifier_bitmap - removed_pfs;

  /** Check if there is at least 1 PF with a downlink filter. */
  if (!remaining_pfs) return true;

  /** There may be more packet filters, but most could be invalid. So, #pf's not
   * an indicator for emptiness. */
  //  DevAssert(tft_original->numberofpacketfilters >
  //  tft->numberofpacketfilters);

  /** Check if all TFTs are removed, if there are any more existing and the
   * existing once have at least one DL/BD packet filter. */
  for (int num_pf = 0; num_pf < tft_original->numberofpacketfilters; num_pf++) {
    if (tft_original->packetfilterlist.createnewtft[num_pf].direction &
        (TRAFFIC_FLOW_TEMPLATE_DOWNLINK_ONLY |
         TRAFFIC_FLOW_TEMPLATE_BIDIRECTIONAL)) {
      /** Found a DL packet filter, check if it is empty or not. */
      bool pf_removed =
          (1
           << tft_original->packetfilterlist.createnewtft[num_pf].identifier) &
          tft->packet_filter_identifier_bitmap;
      if (!pf_removed) {
        /** Found a DL packet filter which is not removed. */
        return false;
      }
    }
  }
  /** All downlink packet filters are removed. */
  return true;
}
 ...
 */
#include "esm_cause.h"
esm_cause_t verify_traffic_flow_template(
    traffic_flow_template_t *tft, traffic_flow_template_t *tft_original) {
  if (!tft) {
  ...
  switch (tft->tftoperationcode) {
    case TRAFFIC_FLOW_TEMPLATE_OPCODE_CREATE_NEW_TFT: {
      ...
      for (int num_pf = 0; num_pf < tft->numberofpacketfilters; num_pf++) {
        ...
        if (tft->packet_filter_identifier_bitmap &
            (0x01 << tft->packetfilterlist.createnewtft[num_pf].identifier +
                         1)) {
          /** Packet Filter Identifier already set. */
          return ESM_CAUSE_SYNTACTICAL_ERROR_IN_PACKET_FILTER;
        }
        if (tft->precedence_set[tft->packetfilterlist.createnewtft[num_pf]
                                    .eval_precedence]) {
          /** Packet Filter Identifier already set. */
          return ESM_CAUSE_SYNTACTICAL_ERROR_IN_PACKET_FILTER;
        }
        ...
        tft->packet_filter_identifier_bitmap |=
            (0x01 << tft->packetfilterlist.createnewtft[num_pf].identifier + 1);
        tft->precedence_set[tft->packetfilterlist.createnewtft[num_pf]
                                .eval_precedence] =
            tft->packetfilterlist.createnewtft[num_pf].identifier + 1;
    ...
    case TRAFFIC_FLOW_TEMPLATE_OPCODE_ADD_PACKET_FILTER_TO_EXISTING_TFT: {
      ...
      for (int num_pf = 0; num_pf < tft->numberofpacketfilters; num_pf++) {
        ...
        if (tft_original
                ->precedence_set[tft->packetfilterlist.addpacketfilter[num_pf]
                                     .eval_precedence] ||
            tft->precedence_set[tft->packetfilterlist.addpacketfilter[num_pf]
                                    .eval_precedence]) {
          /** Precedence already exists in the original or already in the new
           * TFT. */
          return ESM_CAUSE_SYNTACTICAL_ERROR_IN_PACKET_FILTER;
        }
        ...
        tft->precedence_set[tft->packetfilterlist.addpacketfilter[num_pf]
                                .eval_precedence] =
            tft->packetfilterlist.addpacketfilter[num_pf].identifier + 1;
      }
      /** Successfully checked addition TFTs. */
      return ESM_CAUSE_SUCCESS;
    }
    case TRAFFIC_FLOW_TEMPLATE_OPCODE_REPLACE_PACKET_FILTERS_IN_EXISTING_TFT: {
      ...
      for (int num_pf = 0; num_pf < tft->numberofpacketfilters; num_pf++) {
         ...
         */
        if (tft->precedence_set[tft->packetfilterlist
        ...
        tft->precedence_set[tft->packetfilterlist.replacepacketfilter[num_pf]
                                .eval_precedence] =
      ...
      for (int num_pf = 0; num_pf < tft->numberofpacketfilters; num_pf++) {
        /** Successfully validated packet filter syntax. */
        if (tft_original->precedence_set[tft->packetfilterlist
                                             .replacepacketfilter[num_pf]
                                             .eval_precedence] &&
            tft_original->precedence_set[tft->packetfilterlist
                                             .replacepacketfilter[num_pf]
                                             .eval_precedence] !=
                (tft->packetfilterlist.replacepacketfilter[num_pf].identifier +
                 1)) {
          /** Found in the original TFT a packet filter with same precedence but
           * other identifier. For this identifier, a new precedence also should
           * exist (check only one layer). */
          uint8_t ow_ident =
              tft_original
                  ->precedence_set[tft->packetfilterlist
                                       .replacepacketfilter[num_pf]
                                       .eval_precedence]; /**< The one added. */
          if (!(tft->packet_filter_identifier_bitmap & (0x01 << ow_ident))) {
            /** No replacement for packet filter whose precedence is
             * overwritten.. */
            return ESM_CAUSE_SYNTACTICAL_ERROR_IN_THE_TFT_OPERATION;
          }
        }
    ...
    case TRAFFIC_FLOW_TEMPLATE_OPCODE_DELETE_PACKET_FILTERS_FROM_EXISTING_TFT: {
      ...
      /*
       * Check that the packet filters exist.
       * Always skim the received TFT.
       */
      esm_cause_t esm_cause = ESM_CAUSE_SUCCESS;
      for (int num_pf = 0; num_pf < tft->numberofpacketfilters; num_pf++) {
        /**
         * Find the packet filter in the original TFT.
         * Not checking if multiple with same identifier.
         */
        if (!(tft_original->packet_filter_identifier_bitmap &
              (0x01
               << tft->packetfilterlist.deletepacketfilter[num_pf].identifier +
                      1))) {
          esm_cause =
              ESM_CAUSE_SYNTACTICAL_ERROR_IN_THE_TFT_OPERATION; /**< Continue to
                                                                   iterate. */
        }
        if (tft->packet_filter_identifier_bitmap &
            (0x01
      ...
      /** Use the received TFT list, its more important than syntactical errors.
       */
      if (check_if_tft_is_rendered_empty(tft_original, tft)) {
        return ESM_CAUSE_SEMANTIC_ERROR_IN_THE_TFT_OPERATION;
      }
      return esm_cause;
      /** Added for Bearer Resource Modification Request from UE. */
    }
