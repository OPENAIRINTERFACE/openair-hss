...
//------------------------------------------------------------------------------
static
void mce_app_reset_mbsfn_area_ids(m2_enb_id, assoc_id) { // todo: mce_app_recalculate_mcs(mbsfn_service_area)!! should be counted
...
//------------------------------------------------------------------------------
static
mbsfn_area_id_t mce_app_update_mbsfn_area(const mbsfn_area_id_t mbsfn_area_id, const mbms_service_area_id_t mbms_service_area_id, const sctp_assoc_id_t assoc_id, const uint32_t m2_enb_id) {
	...
	// todo: unlock
	OAILOG_FUNC_RETURN(LOG_MCE_APP, (mbsfn_area) ? mbsfn_area->mbsfn_area_id: INVALID_MBSFN_AREA_ID);
}
		...
		mbsfn_areas_t * const mbsfn_areas)
{
	...
	/** Reset all MBSFN service areas for the given eNB Id. */
	mce_app_reset_mbsfn_area_ids(m2_enb_id, assoc_id); // todo: mce_app_recalculate_mcs(mbsfn_service_area)!! should be counted
	...
	for(int num_mbms_area = 0; num_mbms_area < mbms_service_areas->num_service_area; num_mbms_area ++){
		...
		mbsfn_area = mce_app_update_mbsfn_area(mbsfn_area_id, mbms_service_areas->serviceArea[num_mbms_area], assoc_id, m2_enb_id);
		if(mbsfn_area != NULL){
			OAILOG_INFO(LOG_MCE_APP, "Found an MBSFN Area " MBSFN_AREA_ID_FMT " for MBMS SAI " MBMS_SERVICE_AREA_ID_FMT " and updated it with the eNB information. \n",
					mbsfn_area->mbms_service_area_id, mbms_service_areas->serviceArea[num_mbms_area]);
			if(mbsfn_areas->mbsfn_bitmap & (0x01) << mbsfn_area->mbsfn_area_id){
				OAILOG_WARNING(LOG_MCE_APP, "MBSFN Area " MBSFN_AREA_ID_FMT " was already existing. Not incrementing count. \n", mbsfn_area->mbsfn_area_id);
			} else {
				mbsfn_areas->num_mbsfn_areas++;
				mbsfn_areas->mbsfn_bitmap  |= ((0x01) << mbsfn_area->mbsfn_area_id);
			}
		} else {
...
//			mbsfn_area = mce_app_create_mbsfn_area(mbms_service_areas->serviceArea[num_mbms_area], assoc_id, m2_enb_id);
			if(!mbsfn_area) {
				OAILOG_ERROR(LOG_MCE_APP, "No MBSFN Area could be created for MBMS Service Area " MBMS_SERVICE_AREA_ID_FMT ". \n",
						mbms_service_areas->serviceArea[num_mbms_area]);
				OAILOG_FUNC_OUT(LOG_MCE_APP);
			} else {
				...
				if(mbsfn_areas->mbsfn_bitmap & (0x01) << mbsfn_area->mbsfn_area_id){
					DevMessage("MBSFN Area " + mbsfn_area->mbsfn_service_area_id + " was already existing for newly created MBSFN Area.");
				} else {
					mbsfn_areas->num_mbsfn_areas++;
					mbsfn_areas->mbsfn_bitmap  |= ((0x01) << mbsfn_area->mbsfn_area_id);
				}
		...
		/** Check that the corresponding MBSFN area for the given eNB Id is equal to the created one. */
//		mce_app_app_mbsfn_areas_reset_enb(m2_enb_id, mbsfn_area->mbsfn_area_id);
	}
