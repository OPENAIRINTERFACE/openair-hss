...
//------------------------------------------------------------------------------
int get_gateway_and_iface(bstring *gw, bstring *iface)
{
    ...
    for ( ; NLMSG_OK(nlh, received_bytes); nlh = NLMSG_NEXT(nlh, received_bytes))
    {
        ...
        if ((*gateway_address) && (*interface)) {
            *gw = bfromcstr(gateway_address);
            if (iface) {
              *iface = bfromcstr(interface);
            }
            break;
        } else {
          rv = RETURNerror;
...
int get_inet_addr_from_iface(bstring if_name, struct in_addr * const inet_addr) {
  ...
  strncpy(ifr.ifr_name, (const char *)if_name->data, IFNAMSIZ-1);
  if (ioctl(fd, SIOCGIFADDR, &ifr)) {
    close(fd);
    OAILOG_CRITICAL(LOG_SPGW_APP, "Failed to probe %s inet addr: error %s\n", bdata(if_name), strerror(errno));
    return RETURNerror;
  }
  close(fd);
...
int get_mtu_from_iface(bstring if_name, uint32_t * const mtu) {
  ...
  if (ioctl(fd, SIOCGIFMTU, &ifr)) {
    close(fd);
    OAILOG_CRITICAL(LOG_SPGW_APP, "Failed to probe %s MTU: error %s\n", bdata(if_name), strerror(errno));
    return RETURNerror;
  }
  close(fd);
  *mtu = ifr.ifr_mtu;
