...
int s6a_clr_cb(struct msg **msg, struct avp *paramavp, struct session *sess,
  ...
  if (avp_p) {
    ...
    if (hdr_p->avp_value->os.len > IMSI_LENGTH) {
      ...
      memcpy(s6a_cancel_location_req_p->imsi, hdr_p->avp_value->os.data,
   ...
   */
  CHECK_FCT(
      fd_msg_search_avp(qry, s6a_fd_cnf.dataobj_s6a_cancellation_type, &avp_p));
  ...
  if (avp_p) {
     ...
     * * * * The user may be disallowed to use the specified RAT, check the
     * access
     * * * * restriction bit mask received from DB.
     */
    //    if ((hdr->avp_value->u32 != 1004) || (FLAG_IS_SET
    //    (mysql_ans.access_restriction, E_UTRAN_NOT_ALLOWED))) {
    //      experimental = 1;
    //      FPRINTF_ERROR ( "access_restriction
    //      DIAMETER_ERROR_RAT_NOT_ALLOWED\n"); result_code =
    //      DIAMETER_ERROR_RAT_NOT_ALLOWED; goto out;
    //    }
  } else {
...
err:
  ...
  OAILOG_DEBUG(
      LOG_S6A,
      "Sending S6A_CANCEL_LOCATION_REQUEST to task MME_APP - error case\n");
  return RETURNok;
}
