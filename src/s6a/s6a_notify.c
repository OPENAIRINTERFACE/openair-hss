...
int s6a_na_cb(struct msg **msg, struct avp *paramavp, struct session *sess,
  ...
  //  CHECK_FCT (fd_msg_search_avp (qry, s6a_fd_cnf.dataobj_s6a_user_name,
  //  &avp));
  //
  //  if (avp) {
  ...
  if (avp) {
    ...
    MSC_LOG_TX_MESSAGE(MSC_S6A_MME, MSC_NAS_MME, NULL, 0, "0 S6A_NOTIFY_ANS %s",
                       retcode_2_string(s6a_notif_ans_p->result.choice.base));
    ...
    if (hdr->avp_value->u32 != ER_DIAMETER_SUCCESS) {
      ...
      OAILOG_DEBUG(LOG_S6A, "Received S6A Result code %u:%s\n",
                   s6a_notify_ans_p->result.choice.base,
                   retcode_2_string(s6a_notify_ans_p->result.choice.base));
    }
  } else {
    /*
    ...
    if (avp) {
      ...
      s6a_parse_experimental_result(
          avp, &s6a_notify_ans_p->result.choice.experimental);
      MSC_LOG_TX_MESSAGE(MSC_S6A_MME, MSC_NAS_MME, NULL, 0,
                         "0 S6A_NOTIFY_ANS %s", s6a_notify_ans_p->imsi,
                         experimental_retcode_2_string(
                             s6a_notify_ans_p->result.choice.experimental));
    } else {
      /*
      ...
      MSC_LOG_TX_MESSAGE_FAILED(MSC_S6A_MME, MSC_NAS_MME, NULL, 0,
                                "0 S6A_NOTIFY_ANS", s6a_notify_ans_p->imsi);
...
int s6a_generate_notify_req(s6a_notify_req_t *nr_p) {
  struct avp *avp;
  struct msg *msg;
  struct session *sess;
   ...
   */
  {
    ...
    PLMN_T_TO_TBCD(
        ...
        mme_config_find_mnc_length(
            nr_p->visited_plmn.mcc_digit1, nr_p->visited_plmn.mcc_digit2,
            nr_p->visited_plmn.mcc_digit3, nr_p->visited_plmn.mnc_digit1,
            nr_p->visited_plmn.mnc_digit2, nr_p->visited_plmn.mnc_digit3));
    value.os.data = plmn;
    value.os.len = 3;
    CHECK_FCT(fd_msg_avp_setvalue(avp, &value));
    CHECK_FCT(fd_msg_avp_add(msg, MSG_BRW_LAST_CHILD, avp));
    ...
    OAILOG_DEBUG(LOG_S6A, "%s NR- new visited_plmn: %02X%02X%02X\n",
                 __FUNCTION__, value.os.data[0], value.os.data[1],
                 value.os.data[2]);
  }
