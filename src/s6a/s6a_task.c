...
#define FD_LOG_MAX_MESSAGE_LENGTH 8192
  ...
  rv1 = snprintf(buffer, 8192,
                 "...... %05ld:%06ld %08lX LEVEL S6A --no source file-- ",
                 elapsed_time.tv_sec, elapsed_time.tv_usec, pthread_self());
...
int s6a_init(const mme_config_t *mme_config_p) {
   ...
   * fd_core_version(), free_wrapper_DIAMETER_MINIMUM_VERSION); return
   * RETURNerror; } else { S6A_DEBUG("Freediameter version %s\n",
   * fd_core_version());
   * }
   */
