...
int libgtpnl_init(struct in_addr *ue_net, struct in_addr *ue_netmask, int mtu,
                  int *fd0, int *fd1u) {
  ...
  *fd1u = socket(AF_INET, SOCK_DGRAM, 0);
  ...
  OAILOG_NOTICE(
      LOG_GTPV1U,
      "GTP kernel configured (marking required for dedicated bearers)\n");
...
int libgtpnl_add_tunnel(struct in_addr ue, struct in_addr enb, uint32_t i_tei,
