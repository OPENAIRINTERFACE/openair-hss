...
int s11_mme_downlink_data_notification_acknowledge(
  ...
  MSC_LOG_TX_MESSAGE(MSC_S11_MME, MSC_SGW, NULL, 0,
                     "0 DOWNLINK_DATA_NOTIFICATION_ACK S11 teid " TEID_FMT
                     " cause %d",
...
int s11_mme_handle_release_access_bearer_response(
  ...
  NW_GTPV2C_IE_INSTANCE_ZERO, NW_GTPV2C_IE_PRESENCE_OPTIONAL, s11_fteid_ie_get,
                  &resp_p->recovery);
  DevAssert (NW_OK == rc);*/
...
//------------------------------------------------------------------------------
int s11_mme_handle_delete_bearer_failure_indication(
    nw_gtpv2c_stack_handle_t *stack_p, nw_gtpv2c_ulp_api_t *pUlpApi) {
  nw_rc_t rc = NW_OK;
  uint8_t offendingIeType, offendingIeInstance;
  uint16_t offendingIeLength;
  ...
  DevAssert(stack_p);
  message_p =
      itti_alloc_new_message(TASK_S11, S11_DELETE_BEARER_FAILURE_INDICATION);
   ...
   * Create a new message parser
   */
  rc = nwGtpv2cMsgParserNew(*stack_p, NW_GTP_DELETE_BEARER_FAILURE_IND,
                            s11_ie_indication_generic, NULL, &pMsgParser);
  DevAssert(NW_OK == rc);
  ...
  rc = nwGtpv2cMsgParserAddIe(
      pMsgParser, NW_GTPV2C_IE_CAUSE, NW_GTPV2C_IE_INSTANCE_ZERO,
      NW_GTPV2C_IE_PRESENCE_MANDATORY, gtpv2c_cause_ie_get, &ind_p->cause);
  DevAssert(NW_OK == rc);
  ...
  if (rc != NW_OK) {
    MSC_LOG_RX_DISCARDED_MESSAGE(
        MSC_S11_MME, MSC_SGW, NULL, 0,
        "0 DELETE_BEARER_FAILURE_INDICATION local S11 teid " TEID_FMT " ",
        ind_p->teid);
    /*
     * TODO: handle this case
     */
  ...
  MSC_LOG_RX_MESSAGE(
      MSC_S11_MME, MSC_SGW, NULL, 0,
      "0 DELETE_BEARER_FAILURE_INDICATION local S11 teid " TEID_FMT " cause %u",
...
//------------------------------------------------------------------------------
int s11_mme_modify_bearer_request(nw_gtpv2c_stack_handle_t *stack_p,
  ...
  for (int i = 0; i < req_p->bearer_contexts_to_be_modified.num_bearer_context;
    ...
    rc =
        gtpv2c_bearer_context_to_be_modified_within_modify_bearer_request_ie_set(
            &(ulp_req.hMsg),
            &req_p->bearer_contexts_to_be_modified.bearer_context[i]);
    DevAssert(NW_OK == rc);
  }
  ...
  for (int i = 0; i < req_p->bearer_contexts_to_be_removed.num_bearer_context;
    ...
    rc =
        gtpv2c_bearer_context_to_be_removed_within_modify_bearer_request_ie_set(
            &(ulp_req.hMsg),
            &req_p->bearer_contexts_to_be_removed.bearer_context[i]);
    DevAssert(NW_OK == rc);
  }

  MSC_LOG_TX_MESSAGE(MSC_S11_MME, MSC_SGW, NULL, 0,
                     "0 MODIFY_BEARER_REQUEST local S11 teid " TEID_FMT
                     " num bearers ctx %u",
                     req_p->local_teid,
...
//------------------------------------------------------------------------------
int s11_mme_handle_modify_bearer_response(nw_gtpv2c_stack_handle_t *stack_p,
  ...
  //	  /** Set the cause. */
  //	  resp_p->cause.cause_value = SYSTEM_FAILURE; /**< Would mean that this
  // message either did not come at all or could not be dealt with properly. */
  //	  rc = nwGtpv2cMsgDelete (*stack_p, (pUlpApi->hMsg));
  //	  DevAssert (NW_OK == rc);
  //	  return itti_send_msg_to_task (TASK_MME_APP, INSTANCE_DEFAULT,
  // message_p);
  //  }
  ...
  if (rc != NW_OK) {
    MSC_LOG_RX_DISCARDED_MESSAGE(
        MSC_S11_MME, MSC_SGW, NULL, 0,
        "0 MODIFY_BEARER_RESPONSE local S11 teid " TEID_FMT " ", resp_p->teid);
    /*
     * TODO: handle this case
     */
    itti_free(ITTI_MSG_ORIGIN_ID(message_p), message_p);
  ...
  if (resp_p->cause.cause_value == LATE_OVERLAPPING_REQUEST) {
    ...
    OAILOG_WARNING(LOG_S11,
                   "Received a late overlapping request (MBR). Not forwarding "
                   "message to MME_APP layer. \n");
    itti_free(ITTI_MSG_ORIGIN_ID(message_p), message_p);
    message_p = NULL;
    return RETURNok;
  }
...
//------------------------------------------------------------------------------
int s11_mme_delete_bearer_command(nw_gtpv2c_stack_handle_t *stack_p,
  ...
  DevAssert(NW_OK == rc);
  MSC_LOG_TX_MESSAGE(MSC_S11_MME, MSC_SGW, NULL, 0,
                     "0 DELETE_BEARER_COMMAND local S11 teid " TEID_FMT " ",
                     cmd_p->local_teid);
...
//------------------------------------------------------------------------------
int s11_mme_bearer_resource_command(nw_gtpv2c_stack_handle_t *stack_p,
   ...
   * Prepare a new Delete Session Request msg
   */
  rc = nwGtpv2cMsgNew(*stack_p, true, NW_GTP_BEARER_RESOURCE_CMD, cmd_p->teid,
                      0, &(ulp_req.hMsg));
  ulp_req.u_api_info.initialReqInfo.edns_peer_ip = &cmd_p->edns_peer_ip;
  ulp_req.u_api_info.initialReqInfo.teidLocal = cmd_p->local_teid;
  ...
  gtpv2c_ebi_ie_set(&(ulp_req.hMsg), cmd_p->linked_ebi,
                    NW_GTPV2C_IE_INSTANCE_ZERO);
   ...
   * Set the TAD.
   */
  gtpv2c_tad_ie_set(&(ulp_req.hMsg),
                    (traffic_flow_aggregate_description_t *)&cmd_p->tad);
...
//------------------------------------------------------------------------------
int s11_mme_handle_bearer_resource_failure_indication(
    nw_gtpv2c_stack_handle_t *stack_p, nw_gtpv2c_ulp_api_t *pUlpApi) {
  nw_rc_t rc = NW_OK;
  uint8_t offendingIeType, offendingIeInstance;
  uint16_t offendingIeLength;
  ...
  DevAssert(stack_p);
  message_p =
      itti_alloc_new_message(TASK_S11, S11_BEARER_RESOURCE_FAILURE_INDICATION);
   ...
   * Create a new message parser
   */
  rc = nwGtpv2cMsgParserNew(*stack_p, NW_GTP_BEARER_RESOURCE_FAILURE_IND,
                            s11_ie_indication_generic, NULL, &pMsgParser);
  DevAssert(NW_OK == rc);
  ...
  rc = nwGtpv2cMsgParserAddIe(
      ...
      NW_GTPV2C_IE_PRESENCE_MANDATORY, gtpv2c_ebi_ie_get, &ind_p->linked_ebi);
  DevAssert(NW_OK == rc);

  /** Add the PTI to inform to UEs. */
  rc = nwGtpv2cMsgParserAddIe(
      pMsgParser, NW_GTPV2C_IE_BEARER_CONTEXT, NW_GTPV2C_IE_INSTANCE_ZERO,
      NW_GTPV2C_IE_PRESENCE_CONDITIONAL, gtpv2c_pti_ie_get, &ind_p->pti);
  DevAssert(NW_OK == rc);
  ...
  if (rc != NW_OK) {
    MSC_LOG_RX_DISCARDED_MESSAGE(
        MSC_S11_MME, MSC_SGW, NULL, 0,
        "0 BEARER_RESOURCE_FAILURE_INDICATION local S11 teid " TEID_FMT " ",
        ind_p->teid);
    /*
     * TODO: handle this case
     */
...
//------------------------------------------------------------------------------
int s11_mme_handle_create_bearer_request(nw_gtpv2c_stack_handle_t *stack_p,
                                         nw_gtpv2c_ulp_api_t *pUlpApi) {
  nw_rc_t rc = NW_OK;
  uint8_t offendingIeType, offendingIeInstance;
  uint16_t offendingIeLength;
  ...
  if (message_p) {
    ...
    if (rc != NW_OK) {
      MSC_LOG_RX_DISCARDED_MESSAGE(
          MSC_S11_MME, MSC_SGW, NULL, 0,
          "0 CREATE_BEARER_REQUEST local S11 teid " TEID_FMT " ", req_p->teid);
      /*
       * TODO: handle this case
       */
      itti_free(ITTI_MSG_ORIGIN_ID(message_p), message_p);
...
int s11_mme_create_bearer_response(
  ...
  for (int i = 0; i < response_p->bearer_contexts.num_bearer_context; i++) {
    rc = gtpv2c_bearer_context_within_create_bearer_response_ie_set(
        &(ulp_req.hMsg), &response_p->bearer_contexts.bearer_context[i]);
    DevAssert(NW_OK == rc);
  }
...
//------------------------------------------------------------------------------
int s11_mme_handle_update_bearer_request(nw_gtpv2c_stack_handle_t *stack_p,
                                         nw_gtpv2c_ulp_api_t *pUlpApi) {
  nw_rc_t rc = NW_OK;
  uint8_t offendingIeType, offendingIeInstance;
  uint16_t offendingIeLength;
  ...
  if (message_p) {
     ...
     * Create a new message parser
     */
    rc = nwGtpv2cMsgParserNew(*stack_p, NW_GTP_UPDATE_BEARER_REQ,
                              s11_ie_indication_generic, NULL, &pMsgParser);
    DevAssert(NW_OK == rc);
    ...
    rc = nwGtpv2cMsgParserAddIe(
        pMsgParser, NW_GTPV2C_IE_PCO, NW_GTPV2C_IE_INSTANCE_ZERO,
        NW_GTPV2C_IE_PRESENCE_CONDITIONAL, gtpv2c_pco_ie_get, &req_p->pco);
    DevAssert(NW_OK == rc);
    ...
    rc = nwGtpv2cMsgParserAddIe(
        ...
        NW_GTPV2C_IE_PRESENCE_MANDATORY, gtpv2c_ambr_ie_get, &req_p->apn_ambr);
    DevAssert(NW_OK == rc);
    ...
    if (rc != NW_OK) {
      MSC_LOG_RX_DISCARDED_MESSAGE(
          MSC_S11_MME, MSC_SGW, NULL, 0,
          "0 UPDATE_BEARER_REQUEST local S11 teid " TEID_FMT " ", req_p->teid);
      /*
       * TODO: handle this case
       */
      itti_free(ITTI_MSG_ORIGIN_ID(message_p), message_p);
    ...
    MSC_LOG_RX_MESSAGE(MSC_S11_MME, MSC_SGW, NULL, 0,
                       "0 UPDATE_BEARER_REQUEST local S11 teid " TEID_FMT
                       " lebi %u",
                       req_p->teid, req_p->linked_eps_bearer_id);
    rc = nwGtpv2cMsgParserDelete(*stack_p, pMsgParser);
    DevAssert(NW_OK == rc);
...
//------------------------------------------------------------------------------
int s11_mme_update_bearer_response(
    nw_gtpv2c_stack_handle_t *stack_p,
  ...
  ulp_req.apiType = NW_GTPV2C_ULP_API_TRIGGERED_RSP;
  ulp_req.u_api_info.triggeredRspInfo.hTrxn = trxn;
  rc = nwGtpv2cMsgNew(*stack_p, true, NW_GTP_UPDATE_BEARER_RSP,
                      response_p->teid, 0, &(ulp_req.hMsg));
  DevAssert(NW_OK == rc);
  /*
   * Set the remote TEID
  ...
  for (int i = 0; i < response_p->bearer_contexts.num_bearer_context; i++) {
    rc = gtpv2c_bearer_context_within_update_bearer_response_ie_set(
        &(ulp_req.hMsg), &response_p->bearer_contexts.bearer_context[i]);
    DevAssert(NW_OK == rc);
  }
  ...
  MSC_LOG_TX_MESSAGE(
      MSC_S11_MME, MSC_SGW, NULL, 0,
      "0 UPDATE_BEARER_RESPONSE S11 teid " TEID_FMT " num bearers ctx %u",
      response_p->teid, response_p->bearer_contexts.num_bearer_context);
...
int s11_mme_handle_delete_bearer_request(nw_gtpv2c_stack_handle_t *stack_p,
  ...
  if (message_p) {
     ...
     * Create a new message parser
     */
    rc = nwGtpv2cMsgParserNew(*stack_p, NW_GTP_DELETE_BEARER_REQ,
                              s11_ie_indication_generic, NULL, &pMsgParser);
    DevAssert(NW_OK == rc);

    rc = nwGtpv2cMsgParserAddIe(
        pMsgParser, NW_GTPV2C_IE_EBI, NW_GTPV2C_IE_INSTANCE_ZERO,
        NW_GTPV2C_IE_PRESENCE_CONDITIONAL, gtpv2c_ebi_ie_get,
        &req_p->linked_eps_bearer_id);
    DevAssert(NW_OK == rc);
    ...
    rc = nwGtpv2cMsgParserAddIe(pMsgParser, NW_GTPV2C_IE_EBI,
                                NW_GTPV2C_IE_INSTANCE_ONE,
                                NW_GTPV2C_IE_PRESENCE_CONDITIONAL,
                                gtpv2c_ebi_ie_get_list, &req_p->ebi_list);
    DevAssert(NW_OK == rc);

    //    rc = nwGtpv2cMsgParserAddIe (pMsgParser, NW_GTPV2C_IE_BEARER_CONTEXT,
    //    NW_GTPV2C_IE_INSTANCE_ZERO, NW_GTPV2C_IE_PRESENCE_OPTIONAL,
    ...
    rc = nwGtpv2cMsgParserAddIe(
        pMsgParser, NW_GTPV2C_IE_BEARER_CONTEXT, NW_GTPV2C_IE_INSTANCE_ZERO,
        NW_GTPV2C_IE_PRESENCE_CONDITIONAL, gtpv2c_pti_ie_get, &req_p->pti);
    DevAssert(NW_OK == rc);
    ...
    if (rc != NW_OK) {
      MSC_LOG_RX_DISCARDED_MESSAGE(
          MSC_S11_MME, MSC_SGW, NULL, 0,
          "0 DELETE_BEARER_REQUEST local S11 teid " TEID_FMT " ", req_p->teid);
      /*
       * TODO: handle this case
       */
      itti_free(ITTI_MSG_ORIGIN_ID(message_p), message_p);
...
//------------------------------------------------------------------------------
int s11_mme_delete_bearer_response(
    nw_gtpv2c_stack_handle_t *stack_p,
  ...
  ulp_req.apiType = NW_GTPV2C_ULP_API_TRIGGERED_RSP;
  ulp_req.u_api_info.triggeredRspInfo.hTrxn = trxn;
  rc = nwGtpv2cMsgNew(*stack_p, true, NW_GTP_DELETE_BEARER_RSP,
                      response_p->teid, 0, &(ulp_req.hMsg));
  DevAssert(NW_OK == rc);
  /*
   * Set the remote TEID
  ...
  if (response_p->linked_eps_bearer_id)
    gtpv2c_ebi_ie_set(&(ulp_req.hMsg),
                      (unsigned)response_p->linked_eps_bearer_id,
                      NW_GTPV2C_IE_INSTANCE_ZERO);

  for (int i = 0; i < response_p->bearer_contexts.num_bearer_context; i++) {
    rc = gtpv2c_bearer_context_within_delete_bearer_response_ie_set(
        &(ulp_req.hMsg), &response_p->bearer_contexts.bearer_context[i]);
    DevAssert(NW_OK == rc);
  }
  ...
  MSC_LOG_TX_MESSAGE(
      MSC_S11_MME, MSC_SGW, NULL, 0,
      "0 DELETE_BEARER_RESPONSE S11 teid " TEID_FMT " num bearers ctx %u",
      response_p->teid, response_p->bearer_contexts.num_bearer_context);
...
decoding, it is the local TEID, used to find the MME_APP ue_context. */
  ...
  MSC_LOG_RX_MESSAGE(MSC_S10_MME, MSC_SGW, NULL, 0,
                     "DOWNLINK DATA NOTIFICATION to local S10 teid " TEID_FMT,
                     notif_p->teid);
