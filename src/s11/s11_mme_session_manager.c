...
//------------------------------------------------------------------------------
int s11_mme_create_session_request(nw_gtpv2c_stack_handle_t *stack_p,
  ...
  //                              &req_p->pgw_address_for_cp.ipv4_address : 0,
  ...
  for (int i = 0; i < req_p->bearer_contexts_to_be_created->num_bearer_context;
    ...
    gtpv2c_bearer_context_to_be_created_ie_set(
        &(ulp_req.hMsg),
        &req_p->bearer_contexts_to_be_created->bearer_context[i]);
  }
...
//------------------------------------------------------------------------------
int s11_mme_handle_create_session_response(nw_gtpv2c_stack_handle_t *stack_p,
  ...
  if (resp_p->cause.cause_value == LATE_OVERLAPPING_REQUEST) {
    ...
    OAILOG_WARNING(LOG_S11,
                   "Received a late overlapping request. Not forwarding "
...
//------------------------------------------------------------------------------
int s11_mme_delete_session_request(nw_gtpv2c_stack_handle_t *stack_p,
...
//------------------------------------------------------------------------------
int s11_mme_handle_delete_session_response(nw_gtpv2c_stack_handle_t *stack_p,
                                           nw_gtpv2c_ulp_api_t *pUlpApi) {
  nw_rc_t rc = NW_OK;
  uint8_t offendingIeType, offendingIeInstance;
  uint16_t offendingIeLength;
  ...
  if (pUlpApi->u_api_info.triggeredRspIndInfo.noDelete) {
    ...
    if (HASH_TABLE_OK != hash_rc) {
      OAILOG_ERROR(LOG_S11,
                   "Could not get GTPv2-C hTunnel for local teid %X (skipping "
                   "deletion of tunnel)\n",
                   resp_p->teid);
...
//------------------------------------------------------------------------------
int s11_mme_handle_ulp_error_indicatior(nw_gtpv2c_stack_handle_t *stack_p,
                                        nw_gtpv2c_ulp_api_t *pUlpApi) {
  ...
  switch (msgType) {
    ...
    case NW_GTP_RELEASE_ACCESS_BEARERS_REQ: {
      ...
      rsp_p->cause.cause_value =
          REQUEST_ACCEPTED; /**< Would mean that this message either did not
                             ...
                             */
      OAILOG_ERROR(LOG_S11,
                   "RELEASE_ACCESS_BEARERS_RESPONSE could not be received for "
                   "for local teid " TEID_FMT
                   ". Sending ACCEPTED back (ignoring the network failure). \n",
                   rsp_p->teid);
    } break;
      /** Failed commands. */
    case NW_GTP_DELETE_BEARER_CMD: {
      ...
      message_p = itti_alloc_new_message(TASK_S11,
    ...
    case NW_GTP_BEARER_RESOURCE_CMD: {
      ...
      message_p = itti_alloc_new_message(
          TASK_S11, S11_BEARER_RESOURCE_FAILURE_INDICATION);
