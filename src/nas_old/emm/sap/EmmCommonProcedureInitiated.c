  ...
  case _EMMREG_PROC_ABORT:
    ...
    if (emm_ctx) {
      ...
      if (emm_ctx->common_proc) { // assume cleaned if terminated
        /*
         * Not all common procedures should cause the UE context to be removed and to go back to DEREGISTERED state and to abort also any running specific procedure.
         * For example:
         *  SMC, AUTH: abort common, abort any running specific procedures, remove contexts and bearers.
         *
         */
        rc = emm_proc_common_abort (&emm_ctx->common_proc);
        /** We cannot set the state to DEREGISTERED always. **/
