...
*/
//------------------------------------------------------------------------------
int sctp_set_init_opt(const int sd, const sctp_stream_id_t instreams,
...
//------------------------------------------------------------------------------
int sctp_get_sockinfo(int sock, sctp_stream_id_t *instream,
                      sctp_stream_id_t *outstream, sctp_assoc_id_t *assoc_id) {
  socklen_t i = 0;
...
int sctp_get_peeraddresses(int sock, struct sockaddr **remote_addr,
  ...
  for (j = 0; j < nb; j++) {
    if (temp_addr_p[j].sa_family == AF_INET) {
      ...
      //      if (inet_ntop (AF_INET6, &addr->sin6_addr.s6_addr, address, sizeof
      //      (address)) != NULL) {
      //        OAILOG_DEBUG (LOG_SCTP, "    - [%s]\n", address);
      //      }
    }
