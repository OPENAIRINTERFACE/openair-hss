...
typedef int (*sctp_recv_callback)(const sctp_assoc_id_t assoc_id,
                                  ...
                                  uint8_t* buffer, const size_t length);
...
int sctp_connect_to_remote_host(char* local_ip_addr[], int nb_local_addr,
                                char* remote_ip_addr, const uint16_t port,
 ...
 * @return < 0 in case of failure
 */
int sctp_send_msg(sctp_data_t* sctp_data_p, const uint16_t ppid,
                  const sctp_stream_id_t stream, const uint8_t* const buffer,
                  const size_t length);
