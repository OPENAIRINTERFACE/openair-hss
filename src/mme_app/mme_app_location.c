...
int mme_app_handle_s6a_update_location_ans(s6a_update_location_ans_t *ula_pP) {
  ...
  if ((emm_context = emm_data_context_get_by_imsi(&_emm_data, imsi64)) ==
      NULL) {
    OAILOG_ERROR(LOG_MME_APP,
                 "That's embarrassing as we don't know this IMSI " IMSI_64_FMT
                 " for the EMM Data Context. \n",
                 imsi64);
    OAILOG_FUNC_RETURN(LOG_MME_APP, RETURNerror);
  }
  ...
  if (ula_pP->subscription_data) {
    mme_insert_subscription_profile(&mme_app_desc.mme_ue_contexts, imsi64,
                                    subscription_data);
  } else {
    goto err;
  }
  ...
  OAILOG_INFO(LOG_MME_APP,
              "Updated the subscription profile for IMSI " IMSI_64_FMT
              " in the cache. \n",
              imsi64);
  ...
  mme_app_update_ue_subscription(ue_context->privates.mme_ue_s1ap_id,
                                 subscription_data);
  ...
  memcpy(&message_p->ittiMsg.nas_pdn_config_rsp.imsi, &emm_context->_imsi,
         sizeof(imsi_t));
  memcpy(&message_p->ittiMsg.nas_pdn_config_rsp.target_tai,
         &emm_context->originating_tai, sizeof(tai_t));

  /** Check if an attach procedure is running, if so send it to the ESM layer,
   * if not, we assume it is triggered by a TAU request, send it to the EMM. */
  /** For error codes, use nas_pdn_cfg_fail. */
  MSC_LOG_TX_MESSAGE(MSC_MMEAPP_MME, MSC_NAS_ESM, NULL, 0,
                     "0 NAS_PDN_CONFIG_RESP IMSI " IMSI_64_FMT, imsi64);
...
err:
  OAILOG_ERROR(
      LOG_MME_APP,
      "Error processing ULA for ueId " MME_UE_S1AP_ID_FMT
      ". Sending NAS_PDN_CONFIG_FAIL to NAS. (ULA.result.choice.base=%d)\n",
  ...
  itti_nas_pdn_config_fail_t *nas_pdn_config_fail =
      &message_p->ittiMsg.nas_pdn_config_fail;
  ...
  MSC_LOG_TX_MESSAGE(MSC_MMEAPP_MME, MSC_NAS_MME, NULL, 0,
                     "0 NAS_PDN_CONFIG_FAIL IMSI " IMSI_64_FMT, imsi64);
  rc = itti_send_msg_to_task(TASK_NAS_ESM, INSTANCE_DEFAULT, message_p);
...
int mme_app_handle_s6a_cancel_location_req(
  ...
  OAILOG_DEBUG(LOG_MME_APP,
               "%s Handling CANCEL_LOCATION_REQUEST for imsi " IMSI_64_FMT "\n",
               __FUNCTION__, imsi);
  ...
  if ((ue_context = mme_ue_context_exists_imsi(&mme_app_desc.mme_ue_contexts,
    ...
    OAILOG_ERROR(LOG_MME_APP,
                 "That's embarrassing as we don't know this IMSI\n");
    MSC_LOG_EVENT(MSC_MMEAPP_MME,
                  "0 S6A_CANCEL_LOCATION unknown imsi " IMSI_64_FMT " ", imsi);
    OAILOG_FUNC_RETURN(LOG_MME_APP, RETURNerror);
  }
  ...
  if (clr_pP->cancellation_type == MME_UPDATE_PROCEDURE) {
     ...
     * Check the MME mobility timer.. if the timer has not expired yet, just set
     * a flag (not waiting in this thread and resending the signal not to block
     * resources --> any other way to send a signal after a given time --> timer
     * with signal sending!!!). Else remove the UE implicitly.. (not sending
     * Purge_UE for this one).
     */
    OAILOG_INFO(
        LOG_MME_APP,
        "Handling CLR for MME_UPDATE_PROCEDURE for UE with imsi " IMSI_64_FMT
        " "
    ...
    if (s10_handover_proc) {
      if (s10_handover_proc->proc.timer.id != MME_APP_TIMER_INACTIVE_ID) {
        OAILOG_INFO(LOG_MME_APP,
                    "S10 Handover Proc timer %ul, is still running. Marking "
                    "CLR but not removing the UE yet with imsi " IMSI_64_FMT
                    ". \n",
                    s10_handover_proc->proc.timer.id, imsi);
        s10_handover_proc->pending_clear_location_request =
            true;  // todo: not checking the pending flag..
        OAILOG_FUNC_RETURN(LOG_MME_APP, rc);
      } else {
        OAILOG_INFO(LOG_MME_APP,
                    "S10 Handover Proc timer is not running for UE. Implicit "
                    "removal for UE with imsi " IMSI_64_FMT
                    " due handover/tau. \n",
                    imsi);
        /** We also will remove the UE context, if the UE handovered back and
         * CLR received afterwards. */
      }
    } else {
      OAILOG_INFO(LOG_MME_APP,
                  "No S10 Handover Proc found for UE. Implicit removal for UE "
                  "with imsi " IMSI_64_FMT " due handover/tau. \n",
                  imsi);
    }
  } else {
    ...
    OAILOG_INFO(
        LOG_MME_APP,
        "Received cancellation type %d (not due handover). Not checking the "
        "MME_MOBILITY timer. Performing directly implicit detach on "
        "UE with imsi " IMSI_64_FMT
        " received unhandled cancellation type %d. \n",
        clr_pP->cancellation_type, imsi);
    // todo: not implicit removal but proper detach in this case..
  }

  /** Perform an implicit detach via NAS layer.. We purge context ourself or
   * purge the MME_APP context. NAS has to purge the EMM context and the MME_APP
   * context. */
