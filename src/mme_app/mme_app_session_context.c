...
ue_session_pool_t *get_new_session_pool(mme_ue_s1ap_id_t ue_id) {
   ...
   * the end. */
  STAILQ_REMOVE_HEAD(&mme_app_desc.mme_ue_session_pools_list,
  ...
  STAILQ_INSERT_TAIL(&mme_app_desc.mme_ue_session_pools_list, ue_session_pool,
                     entries);
  DevAssert(mme_insert_ue_session_pool(&mme_app_desc.mme_ue_session_pools,
                                       ue_session_pool) == 0);
  // todo: unlock!
...
void release_session_pool(ue_session_pool_t **ue_session_pool) {
   ...
   * be at the very end. */
  STAILQ_REMOVE(&mme_app_desc.mme_ue_session_pools_list, (*ue_session_pool),
                ue_session_pool_s, entries);
  clear_session_pool(*ue_session_pool);
  /** Put it into the head. */
  STAILQ_INSERT_HEAD(&mme_app_desc.mme_ue_session_pools_list,
                     (*ue_session_pool), entries);
...
void mme_app_esm_detach(mme_ue_s1ap_id_t ue_id) {
  ...
  OAILOG_INFO(LOG_MME_APP,
              "Removed all ESM procedures of UE: " MME_UE_S1AP_ID_FMT
              " for detach. \n",
              ue_id);
  ...
  mme_remove_ue_session_pool(&mme_app_desc.mme_ue_session_pools,
                             ue_session_pool);
  OAILOG_INFO(LOG_MME_APP,
...
int mme_app_pdn_process_session_creation(
  ...
  if (saegw_s11_fteid->ipv4) {
    ...
    ((struct sockaddr_in6 *)&pdn_context->s_gw_addr_s11_s4)->sin6_family =
        AF_INET6;
    memcpy(&((struct sockaddr_in6 *)&pdn_context->s_gw_addr_s11_s4)->sin6_addr,
           &saegw_s11_fteid->ipv6_address,
           sizeof(saegw_s11_fteid->ipv6_address));
  }
  ...
  if (ambr->br_dl && ambr->br_ul) { /**< New APN-AMBR received. */
    ...
    if (total_apn_ambr.br_dl > subscribed_ue_ambr.br_dl ||
      ...
      OAILOG_ERROR(LOG_MME_APP,
                   ...
                   subscribed_ue_ambr.br_dl, subscribed_ue_ambr.br_ul);
      ...
      DevAssert(pdn_context->subscribed_apn_ambr.br_dl &&
  ...
  for (int i = 0; i < bcs_created->num_bearer_context; i++) {
     ...
     */
    DevCheck((bearer_id < BEARERS_PER_UE + 5) && (bearer_id >= 5), bearer_id,
             BEARERS_PER_UE, 0);
    DevAssert(bcs_created->bearer_context[i].s1u_sgw_fteid.interface_type ==
              S1_U_SGW_GTP_U);
    ...
    if (bearer_context) {
      ...
      memcpy(&bearer_context->s_gw_fteid_s1u,
             &bcs_created->bearer_context[i].s1u_sgw_fteid,
             sizeof(fteid_t)); /**< Also copying the IPv4/V6 address. */
      memcpy(&bearer_context->p_gw_fteid_s5_s8_up,
             &bcs_created->bearer_context[i].s5_s8_u_pgw_fteid,
             sizeof(fteid_t));
...
void mme_app_ue_session_pool_s1_release_enb_informations(
  ...
  RB_FOREACH(registered_pdn_ctx, PdnContexts, &ue_session_pool->pdn_contexts) {
    ...
    STAILQ_FOREACH(bearer_context_to_set_idle,
                   &registered_pdn_ctx->session_bearers, entries) {
...
ue_session_pool_t *mme_ue_session_pool_exists_mme_ue_s1ap_id(
  ...
  hashtable_ts_get(mme_ue_session_pool_p->mme_ue_s1ap_id_ue_session_pool_htbl,
                   (const hash_key_t)mme_ue_s1ap_id, (void **)&ue_session_pool);
  if (ue_session_pool) {
...
struct ue_session_pool_s *mme_ue_session_pool_exists_s11_teid(
  ...
  h_rc =
      hashtable_uint64_ts_get(mme_ue_session_pool_p->tun11_ue_session_pool_htbl,
...
static void clear_session_pool(ue_session_pool_t *ue_session_pool) {
  ...
  OAILOG_INFO(LOG_MME_APP,
              "Clearing UE session pool of UE " MME_UE_S1AP_ID_FMT ". \n",
              ue_id);
  ...
  for (int num_bearer = 0; num_bearer < MAX_NUM_BEARERS_UE; num_bearer++) {
    ...
    clear_bearer_context(ue_session_pool,
                         &ue_session_pool->privates.bcs_ue[num_bearer]);
  }
  ...
  for (int num_pdn = 0; num_pdn < MAX_APN_PER_UE; num_pdn++) {
    ...
    STAILQ_INSERT_TAIL(&ue_session_pool->free_pdn_contexts,
  ...
  memset(&ue_session_pool->privates.fields, 0,
