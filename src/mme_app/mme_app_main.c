  ...
  while (1) {
    ...
    switch (ITTI_MSG_ID(received_message_p)) {
      ...
      case NAS_ACTIVATE_EPS_BEARER_CTX_CNF: {
        mme_app_handle_activate_eps_bearer_ctx_cnf(
            &NAS_ACTIVATE_EPS_BEARER_CTX_CNF(received_message_p));
      } break;

      case NAS_ACTIVATE_EPS_BEARER_CTX_REJ: {
        mme_app_handle_activate_eps_bearer_ctx_rej(
            &NAS_ACTIVATE_EPS_BEARER_CTX_REJ(received_message_p));
      } break;

      case NAS_MODIFY_EPS_BEARER_CTX_CNF: {
        mme_app_handle_modify_eps_bearer_ctx_cnf(
            &NAS_MODIFY_EPS_BEARER_CTX_CNF(received_message_p));
      } break;

      case NAS_MODIFY_EPS_BEARER_CTX_REJ: {
        mme_app_handle_modify_eps_bearer_ctx_rej(
            &NAS_MODIFY_EPS_BEARER_CTX_REJ(received_message_p));
      } break;

      case NAS_DEACTIVATE_EPS_BEARER_CTX_CNF: {
        mme_app_handle_deactivate_eps_bearer_ctx_cnf(
            &NAS_DEACTIVATE_EPS_BEARER_CTX_CNF(received_message_p));
      } break;
      ...
      case S11_DOWNLINK_DATA_NOTIFICATION: {
        mme_app_handle_downlink_data_notification(
            &received_message_p->ittiMsg.s11_downlink_data_notification);
      } break;

      case NAS_RETRY_BEARER_CTX_PROC_IND: {
        mme_app_handle_bearer_ctx_retry(
            NAS_RETRY_BEARER_CTX_PROC_IND(received_message_p).ue_id);
      } break;

      case NAS_PAGING_DUE_SIGNALING_IND: {
        mme_app_trigger_paging_due_signaling(
            NAS_PAGING_DUE_SIGNALING_IND(received_message_p).ue_id);
      } break;

      case NAS_ERAB_SETUP_REQ: {
        mme_app_handle_nas_erab_setup_req(
            &NAS_ERAB_SETUP_REQ(received_message_p));
      } break;

      case NAS_ERAB_MODIFY_REQ: {
        mme_app_handle_nas_erab_modify_req(
            &NAS_ERAB_MODIFY_REQ(received_message_p));
      } break;

      case NAS_ERAB_RELEASE_REQ: {
        mme_app_handle_nas_erab_release_req(
            NAS_ERAB_RELEASE_REQ(received_message_p).ue_id,
            NAS_ERAB_RELEASE_REQ(received_message_p).ebi,
            NAS_ERAB_RELEASE_REQ(received_message_p).retry,
            NAS_ERAB_RELEASE_REQ(received_message_p).retx_count,
            NAS_ERAB_RELEASE_REQ(received_message_p).nas_msg);
      } break;

      case NAS_PDN_DISCONNECT_REQ: {
        mme_app_handle_nas_pdn_disconnect_req(
            &received_message_p->ittiMsg.nas_pdn_disconnect_req);
      } break;
      ...
      case S11_UPDATE_BEARER_REQUEST:
        mme_app_handle_s11_update_bearer_req(
            &received_message_p->ittiMsg.s11_update_bearer_request);
        break;

      case S11_DELETE_BEARER_REQUEST:
        mme_app_handle_s11_delete_bearer_req(
            &received_message_p->ittiMsg.s11_delete_bearer_request);
        break;

      case S11_DELETE_BEARER_FAILURE_INDICATION: {
        mme_app_delete_bearer_failure_indication(
            &received_message_p->ittiMsg.s11_delete_bearer_failure_indication);
      } break;
      ...
      case S1AP_E_RAB_MODIFY_RSP: {
        mme_app_handle_e_rab_modify_rsp(
            &S1AP_E_RAB_MODIFY_RSP(received_message_p));
      } break;

      case S1AP_E_RAB_RELEASE_IND: {
        mme_app_handle_e_rab_release_ind(
            &S1AP_E_RAB_RELEASE_IND(received_message_p));
      } break;
      ...
      case S1AP_ENB_DEREGISTERED_IND: {
        mme_app_handle_s1ap_enb_deregistered_ind(
      ...
      /** Forward Relocation Complete Notification (After Handover_Notify : end
       * of handover). */
      ...
      case NAS_CONTEXT_REQ: {
        ...
        /** Context Acknowledgment will be handled via State Change Callback
         * Handler. */
      ...
      case TIMER_HAS_EXPIRED: {
        ...
        if (received_message_p->ittiMsg.timer_has_expired.timer_id ==
          ...
          mme_ue_s1ap_id_t mme_ue_s1ap_id = ((mme_ue_s1ap_id_t)(
          ...
          OAILOG_WARNING(LOG_MME_APP,
                         "TIMER_HAS_EXPIRED with ID %u and FOR UE id %d \n",
          ...
          if (received_message_p->ittiMsg.timer_has_expired.timer_id ==
            ...
            mme_app_handle_mobile_reachability_timer_expiry(ue_context);
                     ...
                     ue_context->privates.implicit_detach_timer.id) {
          ...
          else if (s10_handover_proc &&
                   received_message_p->ittiMsg.timer_has_expired.timer_id ==
                       s10_handover_proc->proc.timer.id) {
            // MME Mobility Completion Timer expiry handler (we need this in
            // addition to the one in the S1AP for CLR handling after TAU at
            // source MME. */
...
int mme_app_init(const mme_config_t *mme_config_p) {
  ...
  // todo: (from develop)   pthread_rwlock_init (&mme_app_desc.rw_lock, NULL);
  // && where to unlock it?
  ...
  AssertFatal(sizeof(uintptr_t) >= sizeof(uint64_t),
              "Problem with tun11_ue_context_htbl in MME_APP");
  btrunc(b, 0);
  bassigncstr(b, "mme_app_tun10_ue_context_htbl");
  mme_app_desc.mme_ue_contexts.tun10_ue_context_htbl =
      hashtable_uint64_ts_create(mme_config.max_ues, NULL, b);
  AssertFatal(sizeof(uintptr_t) >= sizeof(uint64_t),
              "Problem with mme_app_tun10_ue_context_htbl in MME_APP");
  btrunc(b, 0);
  ...
  mme_app_desc.mme_ue_contexts.guti_ue_context_htbl =
      obj_hashtable_uint64_ts_create(mme_config.max_ues, NULL, hash_free_func,
                                     b);
  btrunc(b, 0);
  bassigncstr(b, "imsi_apn_configuration_htbl");
  mme_app_desc.mme_ue_contexts.imsi_subscription_profile_htbl =
  ...
  mme_app_desc.mme_ue_session_pools.tun11_ue_session_pool_htbl =
      hashtable_uint64_ts_create(mme_config.max_ues, NULL, b);
  AssertFatal(sizeof(uintptr_t) >= sizeof(uint64_t),
              "Problem with tun11_ue_session_pool_htbl in MME_APP");
  btrunc(b, 0);
  ...
  for (int num_sp = 0; num_sp < CHANGEABLE_VALUE; num_sp++) {
    mme_app_desc.ue_contexts[num_sp].privates.mme_ue_s1ap_id =
        INVALID_MME_UE_S1AP_ID;
    mme_app_desc.ue_contexts[num_sp].privates.enb_s1ap_id_key =
  ...
  for (int num_sp = 0; num_sp < CHANGEABLE_VALUE; num_sp++) {
    mme_app_desc.ue_session_pools[num_sp].privates.mme_ue_s1ap_id =
        INVALID_MME_UE_S1AP_ID;
    /** Create a new mutex for each and put them into the list. */
    ...
    if (rc) {
      OAILOG_ERROR(
          LOG_MME_APP,
          "Cannot create UE session pool, failed to init mutex attribute: %s\n",
          strerror(rc));
      OAILOG_FUNC_RETURN(LOG_MME_APP, RETURNerror);
    }
    rc = pthread_mutexattr_settype(&mutexattr, PTHREAD_MUTEX_RECURSIVE);
    if (rc) {
      OAILOG_ERROR(LOG_MME_APP,
                   "Cannot create UE session pool, failed to set mutex "
                   "attribute type: %s\n",
                   strerror(rc));
      OAILOG_FUNC_RETURN(LOG_MME_APP, RETURNerror);
    }
    ...
    if (rc) {
      OAILOG_ERROR(LOG_MME_APP,
                   "Cannot create UE session pool, failed to init mutex: %s\n",
                   strerror(rc));
      OAILOG_FUNC_RETURN(LOG_MME_APP, RETURNerror);
    }
    //  rc = lock_ue_contexts(new_p);
    if (rc) {
      OAILOG_ERROR(LOG_MME_APP,
                   "Cannot create UE session pool, failed to lock mutex: %s\n",
                   strerror(rc));
      OAILOG_FUNC_RETURN(LOG_MME_APP, NULL);
    }
  ...
  mme_app_desc.mme_mobility_management_timer_period =
      mme_config_p->mme_mobility_completion_timer;

  /*
   * Request for periodic timer
   */
  if (timer_setup(mme_config_p->mme_statistic_timer, 0, TASK_MME_APP,
                  INSTANCE_DEFAULT, TIMER_PERIODIC, NULL,
  ...
  OAILOG_DEBUG(LOG_MME_APP,
               "Initializing MME applicative layer: DONE -- ASSERTING\n");
