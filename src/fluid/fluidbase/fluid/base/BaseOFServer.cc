...
private:
    ...
    static void conn_accept_cb(struct evconnlistener *listener,
                               ...
                               int socklen,
                               void *arg);
    static void conn_accept_error_cb(struct evconnlistener *listener,
                                     void* arg);
};

BaseOFServer::BaseOFServer(const char* address_, const int port, const int nthreads, bool secure) {
...
bool BaseOFServer::listen(EventLoop* evloop) {
    ...
    m_implementation->listener = evconnlistener_new(base,
        m_implementation->conn_accept_cb,
        this,
    ...
    evconnlistener_set_error_cb(m_implementation->listener, m_implementation->conn_accept_error_cb);
...
void BaseOFServer::base_connection_callback(BaseOFConnection* conn, BaseOFConnection::Event event_type) {
    if (event_type == BaseOFConnection::EVENT_CLOSED)
        delete conn;
}

void BaseOFServer::free_data(void* data) {
...
void BaseOFServer::LibEventBaseOFServer::conn_accept_error_cb(struct evconnlistener *listener,
        ...
        fprintf(stderr, "BaseOFServer error (%d :%s).",
