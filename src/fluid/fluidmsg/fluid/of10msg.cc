...
Hello::Hello()
    : OFMsg(of10::OFP_VERSION, of10::OFPT_HELLO) {
}

Hello::Hello(uint32_t xid)
    : OFMsg(of10::OFP_VERSION, of10::OFPT_HELLO, xid) {
}
...
Error::Error()
    : ErrorCommon(of10::OFP_VERSION, of10::OFPT_ERROR) {
...
FeaturesReply::FeaturesReply(uint32_t xid, uint64_t datapath_id,
    uint32_t n_buffers, uint8_t n_tables, uint32_t capabilities,
    uint32_t actions, std::vector<of10::Port> ports)
    : FeaturesReplyCommon(of10::OFP_VERSION, of10::OFPT_FEATURES_REPLY, xid,
        datapath_id, n_buffers, n_tables, capabilities) {
    this->actions_ = actions;
    this->ports_ = ports;
...
uint8_t* FeaturesReply::pack() {
    ...
    for (std::vector<of10::Port>::iterator it = this->ports_.begin(), end =
        this->ports_.end(); it != end; ++it) {
        it->pack(p);
...
FlowMod::FlowMod(uint32_t xid, uint64_t cookie, uint16_t command,
    uint16_t idle_timeout, uint16_t hard_timeout, uint16_t priority,
    uint32_t buffer_id, uint16_t out_port, uint16_t flags, of10::Match match)
    : FlowModCommon(of10::OFP_VERSION, of10::OFPT_FLOW_MOD, xid, cookie,
          idle_timeout, hard_timeout, priority, buffer_id, flags),
      command_(command),
      out_port_(out_port),
...
bool FlowMod::operator==(const FlowMod &other) const {
    return ((OFMsg::operator==(other)) && (this->command_ == other.command_) &&
        (this->out_port_ == other.out_port_) && (this->match_ == other.match_) 
...
PacketOut::PacketOut()
    : PacketOutCommon(of10::OFP_VERSION, of10::OFPT_PACKET_OUT),
      in_port_(0) {
...
PacketIn::PacketIn()
    : PacketInCommon(of10::OFP_VERSION, of10::OFPT_PACKET_IN),
      in_port_(0) {
...
PortMod::PortMod()
    : PortModCommon(of10::OFP_VERSION, of10::OFPT_PORT_MOD),
      port_no_(0) {
...
StatsRequestDesc::StatsRequestDesc()
    : StatsRequest(OFPST_DESC) {
}
...
StatsReplyDesc::StatsReplyDesc()
    : StatsReply(OFPST_DESC) {
...
StatsRequestFlow::StatsRequestFlow()
    : StatsRequest(OFPST_FLOW) {
...
StatsRequestFlow::StatsRequestFlow(uint32_t xid, uint16_t flags,
    of10::Match match, uint8_t table_id, uint16_t out_port)
    : StatsRequest(xid, of10::OFPST_FLOW, flags),
      table_id_(table_id),
      out_port_(out_port),
      match_(match) {
...
StatsReplyFlow::StatsReplyFlow()
    : StatsReply(OFPST_FLOW) {
}
...
StatsRequestAggregate::StatsRequestAggregate()
    : StatsRequest(OFPST_AGGREGATE) {
...
of_error StatsRequestAggregate::unpack(uint8_t *buffer) {
    ...
    if (this->length_
        < sizeof(ofp_stats_request)
            + sizeof(of10::ofp_aggregate_stats_request)) {
        return openflow_error(OFPET_BAD_REQUEST, OFPBRC_BAD_LEN);
    }
    this->table_id_ = fs->table_id;
    this->out_port_ = hton16(fs->out_port);
...
StatsReplyAggregate::StatsReplyAggregate()
    : StatsReply(OFPST_AGGREGATE) {
...
StatsRequestTable::StatsRequestTable()
    : StatsRequest(OFPST_TABLE) {
}
...
StatsReplyTable::StatsReplyTable()
    : StatsReply(OFPST_TABLE) {
}
...
StatsReplyPort::StatsReplyPort()
    : StatsReply(OFPST_PORT) {
}
...
StatsRequestQueue::StatsRequestQueue()
    : StatsRequest(OFPST_QUEUE) {
...
StatsReplyQueue::StatsReplyQueue()
    : StatsReply(OFPST_QUEUE) {
}
...
uint8_t* StatsReplyQueue::pack() {
    uint8_t* buffer = StatsReply::pack();
    uint8_t *p = buffer + sizeof(struct ofp_stats_reply);
    for (std::vector<of10::QueueStats>::iterator it =
...
StatsRequestVendor::StatsRequestVendor()
    : StatsRequest(OFPST_VENDOR) {
    this->length_ += 4;
}

StatsRequestVendor::StatsRequestVendor(uint32_t xid, uint16_t flags,
    uint32_t vendor)
    : StatsRequest(xid, of10::OFPST_VENDOR, flags),
      vendor_(vendor) {
    this->length_ += 4;
}
...
StatsReplyVendor::StatsReplyVendor()
    : StatsReply(OFPST_VENDOR) {
    this->length_ += 4;
}

StatsReplyVendor::StatsReplyVendor(uint32_t xid, uint16_t flags,
    uint32_t vendor)
    : StatsReply(xid, of10::OFPST_VENDOR, flags),
      vendor_(vendor) {
    this->length_ += 4;
}
