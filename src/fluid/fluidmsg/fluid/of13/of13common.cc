...
Port::Port(uint32_t port_no, EthAddress hw_addr, std::string name,
    ...
    uint32_t supported, uint32_t peer, uint32_t curr_speed, uint32_t max_speed)
...
FlowStats::FlowStats()
    : FlowStatsCommon(),
      flags_(0) {
    this->length_ = sizeof(struct of13::ofp_flow_stats)
...
void GroupStats::bucket_stats(std::vector<BucketStats> bucket_stats) {
    ...
    this->length_ += bucket_stats.size()
        * sizeof(struct of13::ofp_bucket_counter);
}
...
TableFeatureProp::TableFeatureProp(uint16_t type) {
    ...
    this->padding_ = ROUND_UP(
        sizeof(struct of13::ofp_table_feature_prop_header), 8)
...
TableFeatures::TableFeatures(uint8_t table_id, std::string name,
    uint64_t metadata_match, uint64_t metadata_write, uint32_t config,
...
bool TableFeatures::operator==(const TableFeatures &other) const {
    return ((this->length_ == other.length_)
        && (this->table_id_ == other.table_id_) && (this->name_ == other.name_)
