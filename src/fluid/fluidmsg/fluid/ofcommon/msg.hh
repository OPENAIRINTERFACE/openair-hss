...
public:
    ...
    void err_type(uint16_t err_type) {
        this->err_type_ = err_type;
    }
    void code(uint16_t code) {
        this->code_ = code;
    }
    void data(void* data, size_t data_len);
};
...
public:
    ...
    PortStatusCommon(uint8_t version, uint8_t type, uint32_t xid,
        uint8_t reason);
...
public:
    ...
    PortModCommon(uint8_t version, uint8_t type, uint32_t xid,
