...
class GTPApplication : public Application {
 public:
  GTPApplication(const std::string& uplink_mac,
 ...
 private:
  ...
  void delete_ue_paging_flow(const AddGTPTunnelEvent& ev,
                             const OpenflowMessenger& messenger,
                             const int pool_id);
  ...
  void add_pdn_loop(const AddGTPTunnelEvent& ev,
                    const OpenflowMessenger& messenger,
                    const uint16_t flow_priority, const uint32_t goto_table);

  void delete_pdn_loop(const DeleteGTPTunnelEvent& ev,
                       const OpenflowMessenger& messenger,
                       const uint16_t flow_priority, const uint32_t goto_table);
