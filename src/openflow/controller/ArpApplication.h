...
class ArpApplication : public PacketInApplication {
 public:
  ArpApplication(PacketInSwitchApplication& pin_sw_app, const int in_port,
                 const std::string l2, const struct in_addr l3,
                 const sgi_arp_boot_cache_t* const sgi_arp_boot_cache);

 private:
  void packet_in_callback(const PacketInEvent& pin_ev, of13::PacketIn& ofpi,
                          const OpenflowMessenger& messenger);

  void send_arp_reply(of13::PacketIn& pi, fluid_base::OFConnection* ofconn,
                      uint32_t in_port, struct in_addr& spa);

  void flow_mod_arp_reply(const PacketInEvent& pi, of13::PacketIn& ofpi,
                          const OpenflowMessenger& messenger,
                          struct in_addr& spa);
   ...
   */
  void handle_arp_request_message(fluid_base::OFConnection* ofconn,
                                  uint8_t* data,
                                  const OpenflowMessenger& messenger);
  ...
  void add_default_sgi_out_flow(fluid_base::OFConnection* ofconn,
                                const OpenflowMessenger& messenger);

  void add_sgi_arp_cache_out_flow(fluid_base::OFConnection* ofconn,
                                  const OpenflowMessenger& messenger);
  ...
  void learn_neighbour_from_arp_request(const PacketInEvent& pin_ev,
                                        const OpenflowMessenger& messenger,
                                        const ether_arp_t* const ether_arp);

  void update_neighbours(const PacketInEvent& pin_ev,
                         const OpenflowMessenger& messenger, in_addr_t l3,
                         std::string l2);
