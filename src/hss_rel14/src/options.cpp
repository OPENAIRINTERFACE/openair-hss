...
bool Options::validateOptions() {
  return (
      (options & diameterconfiguration) && (options & cassserver) &&
      ...
      (options & numworkers) && (options & concurrent) && (options & ossport) &&
      (options & ossfile));
}
...
bool Options::parseInputOptions(int argc, char** argv) {
  ...
  while (1) {
    c = getopt_long(
        argc, argv, "hj:f:s:u:p:d:ro:iqwx:y:", long_options, &option_index);
