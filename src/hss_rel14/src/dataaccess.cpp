...
void convert_ascii_to_binary(unsigned char* dest, unsigned char* src,
                             int length) {
  int i;
  for (i = 0; i < length; i++)
...
DataAccess::~DataAccess() { disconnect(); }
...
void DataAccess::disconnect() { m_db.disconnect(); }
...
bool DataAccess::getEvent(const char* scef_id, uint32_t scef_ref_id,
                          DAEvent& event) {
  std::stringstream ss;
...
bool DataAccess::getEvents(const char* scef_id,
                           std::list<uint32_t> scef_ref_ids,
                           DAEventList& events, CassFutureCallback cb,
                           void* data) {
  std::stringstream ss;
...
bool DataAccess::getExtIdsFromImsiData(SCassFuture& future,
                                       DAExtIdList& extids) {
  if (future.errorCode() != CASS_OK) {
    Logger::system().error(
...
bool DataAccess::getEventIdsFromMsisdnData(SCassFuture& future,
                                           DAEventIdList& eil) {
  if (future.errorCode() != CASS_OK) {
    Logger::system().error(
...
bool DataAccess::getEventIdsFromMsisdn(int64_t msisdn, DAEventIdList& eil,
                                       CassFutureCallback cb, void* data) {
  std::stringstream ss;
...
bool DataAccess::getEventIdsFromExtIds(const char* extids, DAEventIdList& el,
                                       CassFutureCallback cb, void* data) {
  std::stringstream ss;
...
bool DataAccess::getEventIdsFromExtIdsData(SCassFuture& future,
                                           DAEventIdList& el) {
  if (future.errorCode() != CASS_OK) {
    Logger::system().error(
  ...
  try {
    ...
    Logger::system().error("DataAccess::%s - EXCEPTION - %s", __func__,
                           ex.what());
    return false;
  }
...
bool DataAccess::checkOpcKeys(const uint8_t opP[16]) {
  ...
  while (more_pages) {
    ...
    while (rows.nextRow()) {
      ...
      Logger::system().info("COUNT: %d IMSI: %s KEY: %s OPC: %s NEW OPC: %s",
                            ++cnt, imsi, key, opc, newopc);
...
bool DataAccess::getMmeIdentityFromImsi(std::string& imsi,
                                        DAMmeIdentity& mmeid) {
  std::stringstream ss;
...
bool DataAccess::getLatestIdentity(const char* table_name, int64_t& id,
                                   CassFutureCallback cb, void* data) {
  std::stringstream ss;
...
bool DataAccess::updateLatestIdentity(const char* table_name,
                                      CassFutureCallback cb, void* data) {
  std::stringstream ss;
...
bool DataAccess::addMmeIdentity(std::string& host, std::string& realm,
                                int32_t mmeid) {
...
bool DataAccess::addMmeIdentity1(std::string& host, std::string& realm,
                                 int32_t mmeid, CassFutureCallback cb,
                                 void* data) {
  std::stringstream ss;
...
bool DataAccess::addMmeIdentity2(std::string& host, std::string& realm,
                                 int32_t mmeid, CassFutureCallback cb,
                                 void* data) {
  std::stringstream ss;
...
bool DataAccess::updateLocation(DAImsiInfo& location, uint32_t present_flags,
                                int32_t idmmeidentity, CassFutureCallback cb,
                                void* data) {
  std::stringstream ss;
...
bool DataAccess::getImsiSecData(SCassFuture& future, DAImsiSec& imsisec) {
  ...
  if (row.valid()) {
    ...
    convert_ascii_to_binary(imsisec.rand, (uint8_t*)rand_str.c_str(),
                            RAND_LENGTH);
...
bool DataAccess::updateRandSqn(const std::string& imsi, uint8_t* rand_p,
                               uint8_t* sqn, bool inc_sqn,
                               CassFutureCallback cb, void* data) {
  SqnU64Union eu;
...
void DataAccess::UpdateValidityTime(const char* imsi,
...
void DataAccess::UpdateNIRDestination(const char* imsi, std::string& host,
                                      std::string& realm) {
  std::stringstream ss;
