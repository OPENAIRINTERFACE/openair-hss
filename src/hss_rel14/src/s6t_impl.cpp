...
DataAccess& Application::getDbObj() { return m_dbobj; }
...
int processHssDb(s6t::ConfigurationInformationRequestExtractor& cir,
                 int64_t msisdn, EvenStatusMap* evt_map, Application& m_app) {
  ///////////////////////
  // COMON BLOCK
  ///////////////////////
...
int processSimpleImsi(FDMessageRequest* req, std::string& imsi, int64_t msisdn,
  ...
  int ret = fdHss.sendINSDRreq(cir.monitoring_event_configuration, imsi, req,
                               hss_db_rst, NULL);
...
int processMultiImsi(FDMessageRequest* req, DAImsiList& list_imsi,
                     s6t::ConfigurationInformationRequestExtractor& cir,
                     Application& m_app) {
  ...
  for (std::list<std::string>::iterator it_imsi = list_imsi.begin();
    ...
    fdHss.sendINSDRreq(cir.monitoring_event_configuration, *it_imsi, req,
                       hss_db_rst, rir_builder);
  }
...
int COIRcmd::process(FDMessageRequest* req) {
  ...
  if (experimental) {
    StatsHss::singleton().registerStatResult(stat_hss_cir, VENDOR_3GPP,
                                             result_code);
  } else {
...
bool checkAPNSubscribed(const char* imsi, std::string& apn,
                        Application& m_app) {
...
int NIIRcmd::process(FDMessageRequest* req) {
      ...
      for (std::list<std::string>::iterator it = imsilst.begin();
        ...
        if (nir.nidd_authorization_request.requested_validity_time.get(
          ...
          if (nir.origin_host.get(origHost) &&
            ...
            m_app.getDbObj().UpdateNIRDestination(imsichar, origHost,
                                                  origRealm);
          }
        }
      }
  ...
  if (experimental) {
    StatsHss::singleton().registerStatResult(stat_hss_nir, VENDOR_3GPP,
                                             result_code);
  } else {
