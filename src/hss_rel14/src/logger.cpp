...
void Logger::_init(const char* app) {
  ...
  m_statsinks.push_back(std::make_shared<spdlog::sinks::rotating_file_sink_mt>(
      Options::statlogFilename(), Options::statlogMaxSize() * 1024 * 1024,
      Options::statlogNumberFiles()));
  m_statsinks[0]->set_level(spdlog::level::info);

  m_auditsinks.push_back(std::make_shared<spdlog::sinks::rotating_file_sink_mt>(
      Options::auditlogFilename(), Options::auditlogMaxSize() * 1024 * 1024,
      Options::statlogNumberFiles()));
  ...
  m_system =
      new SLogger("system", m_sinks, ss.str().c_str(), Options::logQueueSize());
  m_s6as6d =
      new SLogger("s6as6d", m_sinks, ss.str().c_str(), Options::logQueueSize());
...
void Logger::_cleanup() {
  ...
  if (m_s6as6d) delete m_s6as6d;
  if (m_s6c) delete m_s6c;
  if (m_s6t) delete m_s6t;
}

void Logger::_flush() {
  if (m_system) m_system->flush();
  if (m_s6as6d) m_s6as6d->flush();
  if (m_s6c) m_s6c->flush();
  if (m_s6t) m_s6t->flush();
  if (m_stat) m_stat->flush();
}
...
std::string Logger::_serialize() {
  ...
  systemlog.AddMember(
      "name", RAPIDJSON_NAMESPACE::StringRef(m_system->get_name().c_str()),
      allocator);
