...
void HSSWorkerQueue::finishProcessor() { finishMessage(); }

////////////////////////////////////////////////////////////////////////////////

FDHss::FDHss() : m_s6tapp(NULL), m_s6aapp(NULL), m_s6capp(NULL) {}
...
int FDHss::s6a_peer_validate(struct peer_info* info, int* auth,
...
bool FDHss::init(hss_config_t* hss_config_p) {
  ...
  try {
    Pistache::Address addr(Pistache::Ipv4::any(),
                           Pistache::Port(Options::getrestport()));
    ...
    Pistache::Address addrOss(Pistache::Ipv4::any(),
                              Pistache::Port(Options::getossport()));
    ...
    Logger::system().startup("Started REST server on port [%i]",
                             Options::getrestport());
    Logger::system().startup("Started OSS  server on port [%i]",
                             Options::getossport());
...
void FDHss::updateOpcKeys(const uint8_t opP[16]) { m_dbobj.checkOpcKeys(opP); }
...
int FDHss::sendINSDRreq(
  ...
  return m_s6aapp->sendINSDRreq(cir_monevtcfg, imsi, cir_req, evt_map,
                                rir_builder);
}

void FDHss::buildCfgStatusAvp(FDAvp& mon_evt_cfg_status,
                              MonitoringConfEventStatus& status) {
  mon_evt_cfg_status.add(m_s6tapp->getDict().avpScefId(), status.scef_id);
  mon_evt_cfg_status.add(m_s6tapp->getDict().avpScefReferenceId(),
                         status.scef_ref_id);
...
bool FDHss::synchFix() {
  ...
  if (sqn != NULL) {
    ...
    result = m_dbobj.updateRandSqn(Options::getsynchimsi(), rand, sqn, false,
                                   NULL, NULL);
...
void RIRBuilder::sendRIR() {
  ...
  for (EventImsiStatus::iterator iter_imsist = imsi_status_map.begin();
    ...
    group_monitoring_event_report.add(fdHss.gets6tApp()->getDict().avpScefId(),
                                      iter_imsist->first.first);
    ...
    for (std::vector<ImsiStatus>::iterator iter_imsi =
...

      {
        ...
        serv_result.add(fdHss.gets6tApp()->getDict().avpServiceResultCode(),
                        iter_imsi->m_status.result);

        if (iter_imsi->m_status.result != DIAMETER_SUCCESS) {
          serv_result.add(fdHss.gets6tApp()->getDict().avpVendorId(),
                          VENDOR_3GPP);
        }
...
void RIRBuilder::dispatch(SEventThreadMessage& msg) {
  if (msg.getId() == GUARD_TIMEOUT) {
    ...
    for (EvenStatusMap::iterator it_hss_result = m_hss_insert_status->begin();
      ...
      ////////////////////////////
      ////////////////////////////
      ImsiStatus imsi_status(mme_response_msg.m_imsi, mixed_cfg,
                             mme_response_msg.m_imsi_reachable,
...
HandleMmeResponseEvtMsg::HandleMmeResponseEvtMsg(EvenStatusMap* mme_response,
                                                 std::string& imsi,
                                                 int imsi_reachable,
                                                 std::string& msisdn)
