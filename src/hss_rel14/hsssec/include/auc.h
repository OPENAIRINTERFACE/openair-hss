...
void f1(uint8_t const kP[16], uint8_t const k[16], uint8_t const rand[16],
        uint8_t const sqn[6], uint8_t const amf[2], uint8_t mac_a[8]);
void f1star(uint8_t const kP[16], uint8_t const k[16], uint8_t const rand[16],
            uint8_t const sqn[6], uint8_t const amf[2], uint8_t mac_s[8]);
void f2345(uint8_t const kP[16], uint8_t const k[16], uint8_t const rand[16],
           uint8_t res[8], uint8_t ck[16], uint8_t ik[16], uint8_t ak[6]);
void f5star(uint8_t const kP[16], uint8_t const k[16], uint8_t const rand[16],
            uint8_t ak[6]);

void generate_autn(uint8_t const sqn[6], uint8_t const ak[6],
                   uint8_t const amf[2], uint8_t const mac_a[8],
                   uint8_t autn[16]);
int generate_vector(uint8_t const opc[16], uint64_t imsi, uint8_t key[16],
                    uint8_t plmn[3], uint8_t sqn[6], auc_vector_t* vector);

void kdf(uint8_t* key, uint16_t key_len, uint8_t* s, uint16_t s_len,
         uint8_t* out, uint16_t out_len);

void derive_kasme(uint8_t ck[16], uint8_t ik[16], uint8_t plmn[3],
                  uint8_t sqn[6], uint8_t ak[6], uint8_t kasme[32]);

uint8_t* sqn_ms_derive(uint8_t const opc[16], uint8_t* key, uint8_t* auts,
                       uint8_t* rand);

static inline void print_buffer(const char* prefix, uint8_t* buffer,
                                int length) {
#ifdef NODEBUG
  return;
#else
