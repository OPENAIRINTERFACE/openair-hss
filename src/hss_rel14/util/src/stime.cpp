...
size_t format_time_into_string(char* st, size_t max_limit,
                               const char* type_format, struct tm* time_f,
...
char* format_time_into_specs(const char* time_format, const struct tm* t,
                             char* dest, const char* max_limit,
      ...
      switch (temp) {
        case FULL_DAY_NAME:
          if ((t->tm_wday < 0 || t->tm_wday > 6))
            ...
            dest = add_timeformat_to_string(g_day_names[t->tm_wday], dest,
                                            max_limit);
        ...
        case NAME_DAY_WEEK_ABB:
          if ((t->tm_wday < 0 || t->tm_wday > 6))
            ...
            dest = add_timeformat_to_string(g_day_names_short[t->tm_wday], dest,
                                            max_limit);
        ...
        case FULL_MON_NAME:
          if ((t->tm_mon < 0 || t->tm_mon > 11))
            ...
            dest = add_timeformat_to_string(g_mnth_names[t->tm_mon], dest,
                                            max_limit);
        ...
        case ABBRE_MON_NAME_2:
          ...
          if (t->tm_mon < 0 || t->tm_mon > 11)
            ...
            dest = add_timeformat_to_string(g_mnth_names_short[t->tm_mon], dest,
                                            max_limit);
        ...
        case DATE_TIME:
          ...
          dest = format_time_into_specs("%a %b %e %H:%M:%S %Y", t, dest,
                                        max_limit, p_timeval);
          i++;
          continue;
        ...
        case RAND_VALUE_1:
          dest = format_time_into_specs("%Y-%m-%dT%H:%M:%S.%0", t, dest,
                                        max_limit, p_timeval);
          i++;
          continue;
        ...
        case HOUR_AS_12_CLK:
          ...
          if ((t->tm_hour % 12) == 0)
            ...
            dest = convert_date_time_format((t->tm_hour % 12), "%02d", dest,
                                            max_limit);
        ...
        case TIME_AM_PM:
          dest = format_time_into_specs("%I:%M:%S %p", t, dest, max_limit,
                                        p_timeval);
        ...
        case HOUR_MIN_OFFSET: {
          ...
          dest = convert_date_time_format(temp_var / 3600, "%02d", dest,
                                          max_limit);
          dest = convert_date_time_format((temp_var % 3600) / 60, "%02d", dest,
                                          max_limit);
        ...
        case DATE_TIME_TZ:
          dest = format_time_into_specs("%a, %d %b %Y %H:%M:%S %z", t, dest,
                                        max_limit, p_timeval);
          i++;
          continue;
...
static bool create_date(parse_date* pd, struct tm* time_ptr, int day, int month,
                        int year) {
...
static bool initiate_and_create_date(parse_date* pd, struct tm* time_ptr,
                                     uint32_t date, uint32_t date_order,
  ...
  if (2 == pd->field_count) {
    ...
    return initiate_and_create_date(pd, time_ptr, date, date_order, day, month,
                                    year);
  }
...
static bool determine_date(parse_date* pd, uint32_t date, uint32_t seek,
                           struct tm* timer_ptr) {
  ...
  return initiate_and_create_date(pd, timer_ptr, date, date_order, day, month,
                                  year);
  return false;
}
  ...
  switch (val) {
    case 0x1: /* TT TTDD TTDDD */
      if (dp.field_count > 3 && ((dp.field_flags[2] & (AM_SEP | PM_SEP)) ||
        ...
        return false;  // hRet = DISP_E_TYPEMISMATCH;
      else if (dp.field_count != 2 && dp.field_count != 4 &&
               dp.field_count != 5)
        return false;  // hRet = DISP_E_TYPEMISMATCH;
    ...
    case 0x3: /* TTT TTTDD TTTDDD */
      if (dp.field_count > 4 && ((dp.field_flags[3] & (AM_SEP | PM_SEP)) ||
        ...
        return false;  // hRet = DISP_E_TYPEMISMATCH;
      else if (dp.field_count != 3 && dp.field_count != 5 &&
               dp.field_count != 6)
        return false;  // hRet = DISP_E_TYPEMISMATCH;
      t.tm_hour = dp.field_values[0];
      t.tm_min = dp.field_values[1];
    ...
    case 0x0: /* T DD DDD TDDD TDDD */
      if (dp.field_count == 1 && (dp.field_parse_flags & (AM_SEP | PM_SEP))) {
        ...
        break;
                 ...
                 (dp.field_count < 3 &&
                  dp.field_parse_flags & (AM_SEP | PM_SEP))) {
        return false;  // hRet = DISP_E_TYPEMISMATCH;
...
STime::STime(int32_t year, int32_t mon, int32_t day, int32_t hour, int32_t min,
