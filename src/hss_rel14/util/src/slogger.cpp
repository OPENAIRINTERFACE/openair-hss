...
SLogger::SLogger(const char* category, std::vector<spdlog::sink_ptr>& sinks,
                 const char* pattern, size_t queue_size)
    : m_log(category, sinks.begin(), sinks.end(), queue_size,
...
void SLogger::set_level(spdlog::level::level_enum lvl) { m_log.set_level(lvl); }

spdlog::level::level_enum SLogger::get_level() { return m_log.level(); }

const std::string& SLogger::get_name() { return m_log.name(); }
