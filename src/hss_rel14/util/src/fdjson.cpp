...
class AvpDictionaryEntry {
 public:
  ...
  void init(const char* avp_name) {
    ...
    if (ret != 0)
      throw runtimeInfo(
          string_format("%s:%d - INFO - Unable to retrieve the dictionary data "
                        "for the [%s] dictionary entry",
                        __FILE__, __LINE__, avp_name));
    ...
    if (ret != 0)
      throw runtimeInfo(
          string_format("%s:%d - INFO - Unable to retrieve the dictionary for "
                        "the [%s] dictionary entry",
                        __FILE__, __LINE__, avp_name));

    /* get the dictionary entry associated with the derived type */
    ret = fd_dict_search(dict, DICT_TYPE, TYPE_OF_AVP, m_baseentry,
                         &derivedtype, EINVAL);
  ...
  void _init(const char* avp_name) {
    ...
    if (it == dictEntries.end()) {
      ...
      if (it == dictEntries.end()) {
        ...
        ade->init(avp_name);
        std::pair<std::string, AvpDictionaryEntry*> data(ade->getAvpName(),
                                                         ade);
            ...
            : value.GetType() == RAPIDJSON_NAMESPACE::kFalseType                                                          \
                  ...
                  : value.GetType() == RAPIDJSON_NAMESPACE::kTrueType                                                     \
                        ...
                        : value.GetType() == RAPIDJSON_NAMESPACE::kObjectType                                             \
                              ? "kObjectType"                                                                             \
                              : value.GetType() ==                                                                        \
                                        RAPIDJSON_NAMESPACE::kArrayType                                                   \
                                    ? "kArrayType"                                                                        \
                                    : value.GetType() ==                                                                  \
                                              RAPIDJSON_NAMESPACE::kStringType                                            \
                                          ? "kStringType"                                                                 \
                                          : value.GetType() ==                                                            \
                                                    RAPIDJSON_NAMESPACE::                                                 \
                                                        kNumberType                                                       \
...
static void fdJsonAddAvps(AVP& avp, const RAPIDJSON_NAMESPACE::Value& element,
                          void (*errfunc)(const char*));
static void fdJsonAddAvps(msg_or_avp* reference,
                          const RAPIDJSON_NAMESPACE::Value& element,
                          void (*errfunc)(const char*));

static void fdJsonAddAvp(msg_or_avp* reference, const char* name,
                         const RAPIDJSON_NAMESPACE::Value& value,
                         void (*errfunc)(const char*)) {
  AVP avp(name);
  ...
  switch (value.GetType()) {
    ...
    case RAPIDJSON_NAMESPACE::kTrueType: {
      throw runtimeInfo(
          string_format("%s:%d - INFO - Invalid format (true/false) for [%s] "
                        "in JSON block, ignoring",
                        __FILE__, __LINE__, name));
      break;
    }
    ...
    case RAPIDJSON_NAMESPACE::kNumberType: {
      switch (avp.getBaseType()) {
        ...
        default: { THROW_DATATYPE_MISMATCH(); }
      }
    ...
    case RAPIDJSON_NAMESPACE::kStringType: {
      ...
      if (avp.getType() == ADTAddress) {
        ...
        } else if (inet_pton(AF_INET6, value.GetString(),
                             &((sSA6*)&ss)->sin6_addr) == 1) {
...
int fdJsonAddAvps(const char* json, msg_or_avp* msg,
                  void (*errfunc)(const char*)) {
  int ret = FDJSON_SUCCESS;
  RAPIDJSON_NAMESPACE::Document doc;

  if (!json) {
    errfunc(string_format("%s:%d - ERROR - Error parsing JSON string", __FILE__,
                          __LINE__)
                .c_str());
...
static std::string fdJsonTimeToStr(struct dict_avp_data* dictData,
...
static std::string fdJsonAddressToStr(struct dict_avp_data* dictData,
                                      const unsigned char* buffer, size_t len) {
  sSS ss;
  ...
  switch (addressType) {
    case 1:  // IPV4
    {
      if (len != 6)
        throw runtimeInfo(
            string_format("%s:%d - INFO - The length for an IPV4 address is "
                          "not 6 in AVP [%s]",
                          __FILE__, __LINE__, dictData->avp_name));
...
static void fdJsonAddMembers(
  ...
  if (fd_msg_browse_internal(ref, MSG_BRW_FIRST_CHILD, (msg_or_avp**)&a,
                             NULL) != 0)
    return;
  ...
  do {
    ...
    if (dictData.avp_basetype == AVP_TYPE_OCTETSTRING) {
      ...
      if (ret != 0)
        throw runtimeInfo(
            string_format("%s:%d - INFO - Unable to retrieve the dictionary "
                          "for the [%s] dictionary entry",
                          __FILE__, __LINE__, dictData.avp_name));

      /* get the dictionary entry associated with the derived type */
      ret = fd_dict_search(dict, DICT_TYPE, TYPE_OF_AVP, dictEntry,
                           &derivedtype, EINVAL);
      if (ret == 0) /* if found, then derived */
        ...
        case AVP_TYPE_OCTETSTRING: {
          if (isDerived) {
            if (!strcmp(derivedData.avp_name, "Address")) {
              ...
              std::string str((const char*)hdr->avp_value->os.data,
            ...
            std::string hex = fdJsonBinaryToHex(hdr->avp_value->os.data,
                                                hdr->avp_value->os.len);
            RAPIDJSON_NAMESPACE::Value v;
...
static std::string fdJsonGetName(msg_or_avp* ref) {
  ...
  bool isAvp = fd_msg_browse_internal(ref, MSG_BRW_PARENT,
                                      (msg_or_avp**)&parent, NULL) == 0;
...
void fdJsonGetJSON(msg_or_avp* ref, std::string& json,
                   void (*errfunc)(const char*)) {
  RAPIDJSON_NAMESPACE::Document doc;
...
bool fdJsonGetValueOfMember(std::string json, std::string member,
                            std::string& value) {
  RAPIDJSON_NAMESPACE::Document doc;
