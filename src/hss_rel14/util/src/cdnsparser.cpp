...
void Parser::parse() {
  ...
  for (int i = 0; i < m_qdcount; i++) m_query->addQuestion(parseQuestion());

  // parse answer section
  for (int i = 0; i < m_ancount; i++) m_query->addAnswer(parseResourceRecord());
...
ResourceRecord* Parser::parseResourceRecord() {
  ...
  if (m_class == ns_c_in && m_type == ns_t_a)
    ...
    rr = parseRR();
    // SError::throwRuntimeException( "Parser::parseResourceRecord() -
    // unrecognized record type %d", m_type );
  }
...
ResourceRecord* Parser::parseA() {
  ...
  if (m_rdlength != sizeof(address))
    SError::throwRuntimeException(
        "Parser::parseA() - invalid RDLENGTH [%d] expected [%d]", m_rdlength,
        sizeof(address));
...
ResourceRecord* Parser::parseAAAA() {
  ...
  if (m_rdlength != sizeof(address))
    SError::throwRuntimeException(
        "Parser::parseAAAA() - invalid RDLENGTH [%d] expected [%d]", m_rdlength,
        sizeof(address));
...
ResourceRecord* Parser::parseNAPTR() {
  ...
  return new RRecordNAPTR(
      m_name, m_ttl, order, preference, flags, service, regexp, replacement);
}
...
void Parser::parseDomainName(std::string& dn) {
  ...
  while (*ptr) {
    if ((val = LABEL_LENGTH(ptr)) != -1) {
      ...
      if (!offsetActive) compressedLength += val + 1;
      currOfs += val + 1;
      ...
      SError::throwRuntimeException(
          "Unexpected label/offset field %x at offset %d", *ptr,
          m_data.calculateOffset(ptr));
    }
  }
