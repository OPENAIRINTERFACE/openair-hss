...
CassValueType SCassValue::type() { return cass_value_type(m_value); }

bool SCassValue::isNull() { return cass_value_is_null(m_value); }

////////////////////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////////////////////

SCassRow::SCassRow() : m_row(NULL) {}

SCassRow::SCassRow(const CassRow* row) : m_row(row) {}
...
SCassIterator::SCassIterator() : m_iterator(NULL) {}

SCassIterator::SCassIterator(CassIterator* it) : m_iterator(it) {}

SCassIterator::~SCassIterator() { release(); }
...
bool SCassIterator::nextRow() { return cass_iterator_next(m_iterator); }
...
SCassResult::SCassResult() : m_result(NULL) {}

SCassResult::SCassResult(const CassResult* result) : m_result(result) {}

SCassResult::~SCassResult() { release(); }
...
bool SCassResult::morePages() { return cass_result_has_more_pages(m_result); }
...
SCassFuture::SCassFuture(CassFuture* future, bool incb)
    : m_error((CassError)-1), m_future(future), m_incb(incb) {}

SCassFuture::~SCassFuture() { release(); }
...
CassError SCassFuture::errorCode() {
  if (m_error == (CassError)-1) m_error = cass_future_error_code(m_future);
  return m_error;
}

void SCassFuture::release() {
  if (m_future) {
    if (!m_incb) cass_future_free(m_future);
    m_future = NULL;
  }

  if (m_error != (CassError)-1) m_error = (CassError)-1;
}
...
SCassStatement::SCassStatement() : m_statement(NULL) {}

SCassStatement::SCassStatement(const char* qry) : m_statement(NULL) {
  query(qry);
}

SCassStatement::SCassStatement(const std::string& qry) : m_statement(NULL) {
  query(qry);
}

SCassStatement::~SCassStatement() { release(); }
...
SCassFuture SCassandra::connect() {
  ...
  return SCassFuture(
      cass_session_connect_keyspace(m_session, m_cluster, m_keyspace.c_str()));
}

void SCassandra::disconnect() { release(); }
