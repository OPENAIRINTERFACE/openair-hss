...
AppServiceEnum Utility::getAppService(const std::string& s) {
  return s == "x-3gpp-pgw" ? x_3gpp_pgw :
                             s == "x-3gpp-sgw" ?
                             x_3gpp_sgw :
                             s == "x-3gpp-ggsn" ?
                             x_3gpp_ggsn :
...
AppProtocolEnum Utility::getAppProtocol(const std::string& p) {
  return p == "x-gn" ?
             ...
             p == "x-s8-gtp" ?
             x_s8_gtp :
             p == "x-s8-pmip" ? x_s8_pmip : p == "x-sv" ? x_sv : x_unknown;
}

const char* Utility::getAppService(AppServiceEnum s) {
  return s == x_3gpp_pgw ? "x-3gpp-pgw" :
                           s == x_3gpp_sgw ?
                           "x-3gpp-sgw" :
                           s == x_3gpp_ggsn ?
                           "x-3gpp-ggsn" :
...
AppProtocolEnum Utility::getAppProtocol(PGWAppProtocolEnum proto) {
  return proto == pgw_x_gn ?
             x_gn :
             proto == pgw_x_gp ? x_gp :
                                 proto == pgw_x_s2a_gtp ?
                                 x_s2a_gtp :
                                 proto == pgw_x_s2a_mipv4 ?
                                 x_s2a_mipv4 :
                                 proto == pgw_x_s2a_pmip ?
                                 x_s2a_pmip :
                                 proto == pgw_x_s2b_gtp ?
                                 x_s2b_gtp :
                                 proto == pgw_x_s2b_pmip ?
                                 x_s2b_pmip :
                                 proto == pgw_x_s2c_dsmip ?
                                 x_s2c_dsmip :
                                 proto == pgw_x_s5_gtp ?
                                 x_s5_gtp :
                                 proto == pgw_x_s5_pmip ?
...
AppProtocolEnum Utility::getAppProtocol(SGWAppProtocolEnum proto) {
  return proto == sgw_x_s11 ?
             x_s11 :
             proto == sgw_x_s12 ?
             x_s12 :
             proto == sgw_x_s1_u ?
             x_s1_u :
             proto == sgw_x_s2a_pmip ?
             x_s2a_pmip :
             proto == sgw_x_s2b_pmip ?
             x_s2b_pmip :
             proto == sgw_x_s4 ? x_s4 :
                                 proto == sgw_x_s5_gtp ?
                                 x_s5_gtp :
...
AppProtocolEnum Utility::getAppProtocol(GGSNAppProtocolEnum proto) {
  return proto == ggsn_x_gn ? x_gn : proto == ggsn_x_gp ? x_gp : x_unknown;
}

AppProtocolEnum Utility::getAppProtocol(SGSNAppProtocolEnum proto) {
  return proto == sgsn_x_gn ?
             x_gn :
             proto == sgsn_x_gp ?
             x_gp :
             proto == sgsn_x_nqprime ?
             x_nqprime :
             proto == sgsn_x_s16 ?
             x_s16 :
             proto == sgsn_x_s3 ?
             x_s3 :
             proto == sgsn_x_s4 ? x_s4 : proto == sgsn_x_sv ? x_sv : x_unknown;
}

AppProtocolEnum Utility::getAppProtocol(MMEAppProtocolEnum proto) {
  return proto == mme_x_gn ?
             x_gn :
             proto == mme_x_gp ?
             x_gp :
             proto == mme_x_nq ?
             ...
             proto == mme_x_s3 ?
             x_s3 :
             proto == mme_x_s6a ? x_s6a : proto == mme_x_sv ? x_sv : x_unknown;
}

AppProtocolEnum Utility::getAppProtocol(MSCAppProtocolEnum proto) {
  return proto == msc_x_sv ? x_sv : x_unknown;
}
...
std::string Utility::diameter_fqdn(const char* mnc, const char* mcc) {
  ...
  s.append("diameter.epc.").APPEND_MNC(mnc).APPEND_MCC(mcc).APPEND_3GPPNETWORK;
...
std::string Utility::getDiameterService(
    DiameterApplicationEnum app, DiameterProtocolEnum protocol) {
  std::ostringstream buffer;
  buffer << "aaa+ap" << getDiameterApplication(app) << ":"
         << getDiameterProtocol(protocol);
  return buffer.str();
}
...
uint32_t Utility::getDiameterApplication(DiameterApplicationEnum app) {
  return
      ...
      */
      app == dia_app_nasreq ?
          1 :
          app == dia_app_mobile_ipv4 ?
          2 :
          app == dia_app_base_accounting ?
          3 :
          app == dia_app_credit_control ?
          4 :
          app == dia_app_eap ?
          5 :
          app == dia_app_sip6 ?
          6 :
          app == dia_app_mobile_ipv6_ike ?
          7 :
          app == dia_app_mobile_ipv6_auth ?
          8 :
          app == dia_app_qos ?
          9 :
          app == dia_app_relay ?
          ...
          +----------------+----------------------+
          */
                                              app == dia_app_3gpp_sta ?
          16777250 :
          app == dia_app_3gpp_s6a ?
          16777251 :
          app == dia_app_3gpp_swm ?
          16777264 :
          app == dia_app_3gpp_s9 ?
          16777267 :
          /*
          ...
          +----------------+--------------------------------------------------+
          */
                                                              app == dia_app_wimax_wnaaada ?
          16777281 :
          app == dia_app_wimax_wnada ?
          16777282 :
          app == dia_app_wimax_wm4da ?
          16777283 :
          app == dia_app_wimax_wm6da ?
          16777284 :
          app == dia_app_wimax_wdda ?
          16777285 :
          app == dia_app_wimax_wlaada ?
          16777286 :
          app == dia_app_wimax_pcc_r3_p ?
          16777287 :
          app == dia_app_wimax_pcc_r3_ofc ?
          16777288 :
          app == dia_app_wimax_pcc_r3_ofc_prime ?
          16777289 :
          app == dia_app_wimax_pcc_r3_oc ? 16777290 : 0;
}

const char* Utility::getDiameterProtocol(DiameterProtocolEnum protocol) {
  return protocol == dia_protocol_tcp ?
             "diameter.tcp" :
             protocol == dia_protocol_sctp ?
             "diameter.sctp" :
...
CanonicalNodeName::CanonicalNodeName() : m_topon(false) {}

CanonicalNodeName::CanonicalNodeName(const std::string& n) : m_topon(false) {
  setName(n);
}
...
void CanonicalNodeName::setName(const std::string& n) {
  ...
  if (m_topon) {
    ...
    while (std::getline(ss, arg, '.')) push_back(arg);
...
NodeSelectorResultList& NodeSelector::process() {
  ...
  bool cacheHit = false;
  m_query =
      CachedDNS::Cache::getInstance().query(ns_t_naptr, m_domain, cacheHit);

  // evaluate each answer to see if it matches the service/protocol requirements
  for (std::list<CachedDNS::ResourceRecord*>::const_iterator rrit =
           m_query->getAnswers().begin();
       rrit != m_query->getAnswers().end(); ++rrit) {
    ...
    if (service.getService() == m_desiredService) {
      ...
      for (AppProtocolList::const_iterator dpit = m_desiredProtocols.begin();
           dpit != m_desiredProtocols.end(); ++dpit) {
        ...
        if (sp) {
          ...
          if (sp->getUsageTypes().empty()) {
            // the naptr app protocol is not ue specific which means that it
            // supports all usage types
            ...
            for (UsageTypeList::const_iterator utit =
                     m_desiredUsageTypes.begin();
                 utit != m_desiredUsageTypes.end(); ++utit) {
              if (sp->findUsageType(*utit)) nsrap->addUsageType(*utit);
            }
      ...
      if (!nsr->getSupportedProtocols().empty()) {
        // iterate through the dns query additonal records adding the ip
        // addresses for the host to the result
        for (CachedDNS::ResourceRecordList::const_iterator it =
                 m_query->getAdditional().begin();
             it != m_query->getAdditional().end(); ++it) {
          if (nsr->getHostname() == (*it)->getName()) {
            switch ((*it)->getType()) {
              case ns_t_a: {
                nsr->addIPv4Host(
                    ((CachedDNS::RRecordA*) *it)->getAddressString());
                break;
              }
              case ns_t_aaaa: {
                nsr->addIPv6Host(
                    ((CachedDNS::RRecordAAAA*) *it)->getAddressString());
                break;
              }
              default: {
                break;
...
bool NodeSelectorResultList::sort_compare(
    NodeSelectorResult*& first, NodeSelectorResult*& second) {
  if (first->getOrder() < second->getOrder()) return true;

  if (first->getOrder() > second->getOrder()) return false;
...
ColocatedCandidate::ColocatedCandidate(
    NodeSelectorResult& candidate1, NodeSelectorResult& candidate2)
    : m_candidate1(candidate1), m_candidate2(candidate2) {
  m_cnn1.setName(m_candidate1.getHostname());
  m_cnn2.setName(m_candidate2.getHostname());

  m_pairtype = m_cnn1.getName() == m_cnn2.getName() ?
                   ptColocated :
                   m_cnn1.getTopon() && m_cnn2.getTopon() ?
                   ptTopologicalDistance :
                   ptDNSPriority;

  m_topologicalMatches = m_pairtype == ptTopologicalDistance ?
                             m_cnn1.topologicalCompare(m_cnn2) :
                             0;
}

ColocatedCandidateList::ColocatedCandidateList(
    NodeSelectorResultList& nodelist1, NodeSelectorResultList& nodelist2)
    : m_nodelist1(nodelist1), m_nodelist2(nodelist2) {
...
bool ColocatedCandidateList::sort_compare(
    ColocatedCandidate*& first, ColocatedCandidate*& second) {
  // sort by pairtype ascending
  if ((int) first->getPairType() < (int) second->getPairType()) return true;
  if ((int) first->getPairType() > (int) second->getPairType()) return false;
  ...
  return first->getCandidate1().getPreference() <
         second->getCandidate1().getPreference();
}
...
DiameterNaptrList& DiameterSelector::process() {
  ...
  if (m_application == dia_app_unknown) return m_results;

  // validate m_protocol
  if (m_protocol == dia_protocol_unknown) return m_results;

  // validate realm
  if (m_realm.empty()) return m_results;
  ...
  bool cacheHit = false;
  m_query =
      CachedDNS::Cache::getInstance().query(ns_t_naptr, m_realm, cacheHit);
  ...
  for (std::list<CachedDNS::ResourceRecord*>::const_iterator rrit =
    ...
    // does the naptr service field match the app/protocol that we are looking
    // for
    if (naptr->getService() == service) {
      ...
      if (n->getType() == dnt_hostname) {
        ...
        for (CachedDNS::ResourceRecordList::const_iterator rr =
                 m_query->getAdditional().begin();
             rr != m_query->getAdditional().end(); ++rr) {
          if ((*rr)->getName() == a->getHost().getName()) {
            switch ((*rr)->getType()) {
              case ns_t_a: {
                a->getHost().addIPv4Address(
                    ((CachedDNS::RRecordA*) *rr)->getAddressString());
                break;
              }
              case ns_t_aaaa: {
                a->getHost().addIPv6Address(
                    ((CachedDNS::RRecordAAAA*) *rr)->getAddressString());
                break;
              }
              default: {
                break;
        ...
        for (CachedDNS::ResourceRecordList::const_iterator rr =
                 m_query->getAdditional().begin();
             rr != m_query->getAdditional().end(); ++rr) {
          if ((*rr)->getType() == ns_t_srv &&
              (*rr)->getName() == s->getReplacement()) {
            ...
            // loop through the additional records adding the hostname entries
            // that match the SRV hostname
            for (CachedDNS::ResourceRecordList::const_iterator rr2 =
                     m_query->getAdditional().begin();
                 rr2 != m_query->getAdditional().end(); ++rr2) {
              if ((*rr2)->getName() == rrs->getTarget()) {
                ds->getHost().setName((*rr2)->getName());
                switch ((*rr2)->getType()) {
                  case ns_t_a: {
                    ds->getHost().addIPv4Address(
                        ((CachedDNS::RRecordA*) *rr2)->getAddressString());
                    break;
                  }
                  case ns_t_aaaa: {
                    ds->getHost().addIPv6Address(
                        ((CachedDNS::RRecordAAAA*) *rr2)->getAddressString());
                    break;
                  }
                  default: {
                    break;
...
void DiameterSrvVector::sort_vector() {
  ...
  for (uint32_t i = 0; i <= size(); i++) {
    if ((i == size() || at(i)->getPriority() != priority) && priority != -1) {
      if (priority != -1) {
        // the sorted vector contains indices in the specified range in sorted
        // order
        ...
        for (int j = first; j <= last; j++) (*this)[j] = newsorted[j - first];
      }
