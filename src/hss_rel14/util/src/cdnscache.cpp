...
class QueryProcessor {
 public:
  QueryProcessor(Query* q) : m_query(q) {
    m_status = ARES_SUCCESS;
    m_channel = NULL;
    m_query = q;
  }

  ~QueryProcessor() { ares_destroy(m_channel); }
  ...
  void execute() {
    if ((m_status = ares_init(&m_channel)) == ARES_SUCCESS) {
      ...
      opt.ednspsz = 8192;
      ares_init_options(&m_channel, &opt,
                        ARES_OPT_TIMEOUTMS | ARES_OPT_NDOTS | ARES_OPT_EDNSPSZ |
                            ARES_OPT_FLAGS);

      ares_query(m_channel, m_query->getDomain().c_str(), ns_c_in,
                 m_query->getType(), ares_callback, this);
  ...
  void wait_for_completion() {
    ...
    while (true) {
      ...
      for (int i = 0; i < ARES_GETSOCK_MAXNUM; i++) {
        ...
        fds[fdcnt].events |=
            ARES_GETSOCK_READABLE(rwbits, i) ? (POLLIN | POLLRDNORM) : 0;
        fds[fdcnt].events |=
            ARES_GETSOCK_WRITABLE(rwbits, i) ? (POLLOUT | POLLWRNORM) : 0;
        if (fds[fdcnt].events != 0) fdcnt++;
      }
      ...
      if (poll(fds, fdcnt, timeout) != 0) {
        for (int i = 0; i < fdcnt; i++) {
          if (fds[i].revents != 0) {
            ares_process_fd(
                m_channel,
                fds[i].revents & (POLLIN | POLLRDNORM) ? fds[i].fd
                                                       : ARES_SOCKET_BAD,
                fds[i].revents & (POLLOUT | POLLWRNORM) ? fds[i].fd
                                                        : ARES_SOCKET_BAD);
          }
        }
      } else {
...
class QueryThread : public SThread {
 public:
  QueryThread(ns_type rtype, const std::string& domain,
              CachedDNSQueryCallback cb, void* data = NULL)
      : SThread(true),
        m_type(rtype),
        m_domain(domain),
        m_cb(cb),
...
Cache::Cache() {
  ...
  if (status != ARES_SUCCESS) {
    std::string msg(string_format(
        "Cache::Cache() - ares_library_init() failed status = %d", status));
...
void Cache::query(ns_type rtype, const std::string& domain,
                  CachedDNSQueryCallback cb, void* data) {
  Query* q = lookupQuery(rtype, domain);
...
void Cache::ares_callback(void* arg, int status, int timeouts,
                          unsigned char* abuf, int alen) {}
