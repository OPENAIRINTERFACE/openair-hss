...
SThread::SThread(bool selfDestruct) : mSuspended(0, 1000), mSuspendSem(0, 1) {
...
void SThread::init(void* arg, bool suspended) {
  {
    ...
    if (suspended) mSuspendCnt++;

    resume();
  }

  while (mSuspendCnt == 0 && isWaitingToRun()) yield();
}

bool SThread::isInitialized() { return mInitialized; }
...
void SThread::suspend() {
  ...
  if (suspendCnt == 1) mSuspended.decrement();
}
...
void SThread::yield() { sched_yield(); }
...
void* SThread::_threadProc(void* arg) {
  ...
  // set to running state
  {
    ...
    if (!ths->getUpdateStateManually()) ths->mState = SThread::rsRunning;
  }
  ...
  {
    SMutexLock l(ths->mMutex);
    if (!ths->getUpdateStateManually()) ths->mState = SThread::rsDoneRunning;
  }
...
SEventThread::SEventThread(bool selfDestruct) : SThread(selfDestruct) {}
...
void SEventThread::postMessage(SEventThreadMessage* msg) { m_events.push(msg); }

void SEventThread::quit() { postMessage(ETM_QUIT); }
...
void SEventThread::initTimer(SEventThread::Timer& t) { t.init(this); }

void SEventThread::onInit() {}

void SEventThread::onQuit() {}

void SEventThread::onSuspend() { _suspend(); }
...
SEventThread::Timer::~Timer() { destroy(); }
...
void SEventThread::Timer::start() {
  ...
  // std::cout << "SEventThread::Timer::start() 2 - value.tv_sec=" <<
  // its.it_value.tv_sec << " value.tv_nsec=" << its.it_value.tv_nsec << "
  // interval.tv_sec=" << its.it_interval.tv_sec << " interval.tv_nsec=" <<
  // its.it_interval.tv_nsec << std::endl;
...
void SEventThread::Timer::_timerHandler(int signo, siginfo_t* pinfo,
                                        void* pcontext) {
