...
int SUtility::indexOf(const char* path, char search, int start) {
  if (start > (int)strlen(path)) return -1;
...
std::vector<std::string> SUtility::split(const std::string& s,
                                         const char delim) {
...
void SUtility::deletefile(const char* fn) { remove(fn); }

void SUtility::_string_format(std::string& dest, const char* format,
                              va_list& args) {
  char buf[2048];
