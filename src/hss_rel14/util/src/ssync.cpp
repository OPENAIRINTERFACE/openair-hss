...
SMutex::SMutex(bool bInit) : mInitialized(false) {
  if (bInit) init(NULL);
}

SMutex::~SMutex() { destroy(); }
...
void SMutex::init(const char* pName) {
  if (!mInitialized) {
    ...
    else if ((res = pthread_mutexattr_setpshared(&attr,
                                                 PTHREAD_PROCESS_SHARED)) != 0)
      SError::throwRuntimeExceptionWithErrno("Unable to initialize mutex");
    else if ((res = pthread_mutexattr_settype(&attr,
...
bool SMutex::enter(bool wait) {
  ...
  if (res != 0 && wait)
    SError::throwRuntimeExceptionWithErrno(
        "SMutex::enter() - Unable to lock mutex", res);
...
void SMutex::leave() {
  ...
  if (res != 0)
    SError::throwRuntimeExceptionWithErrno(
        "SMutex::enter() - Unable to unlock mutex", res);
}

////////////////////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////////////////////

SSemaphore::SSemaphore() { mInitialized = false; }

SSemaphore::SSemaphore(unsigned int lInitialCount, unsigned int lMaxCount,
                       const char* pszName, bool bInit) {
  mInitialized = false;

  if (bInit) init(lInitialCount, lMaxCount, pszName);
}

SSemaphore::~SSemaphore() { destroy(); }

void SSemaphore::init(unsigned int lInitialCount, unsigned int lMaxCount,
                      const char* pszName) {
  if (mInitialized)
    SError::throwRuntimeException("Semaphore already initialized");

  if (pszName != NULL) mName = pszName;
  ...
  for (unsigned int i = lMaxCount; i > lInitialCount; i--) sem_wait(&mSem);
...
bool SSemaphore::decrement(bool wait) {
  if (!wait) {
    ...
    if (sem_getvalue(&mSem, &val) != 0)
      SError::throwRuntimeExceptionWithErrno(
          "Error retrieving semaphore value");
...
SEvent::~SEvent() { closepipe(); }

void SEvent::set() { write(m_pipefd[1], &m_buf, 1); }
