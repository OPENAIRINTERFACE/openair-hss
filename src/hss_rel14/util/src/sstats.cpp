...
uint32_t StatCollector::getNbCodesTracked() { return m_trackedCodes.size(); }

///////////////////////

SStats::SStats(bool logElapsed, StatSerializationMode serializ_mode,
               StatSerializationEngine engine)
    : m_interval(0),
...
void SStats::addGenerationTimeStamp(
    std::map<std::string, std::string>& keyValues) {
...
void SStats::appendStatObject(
    RAPIDJSON_NAMESPACE::Value& arrayObjects,
  ...
  statsObject.AddMember(
      "type", RAPIDJSON_NAMESPACE::StringRef(collector.getName().c_str()),
      allocator);
  ...
  std::vector<std::pair<uint32_t, uint32_t>> trackedcode =
      collector.getTrackedCodes();
...
void SStatsSerializerJson::serialize(
    std::map<std::string, std::string>& keyValues, std::string& output) {
  ...
  for (std::map<std::string, std::string>::const_iterator it =
           keyValues.begin();
       it != keyValues.end(); ++it) {
    document.AddMember(fdrapidjson::StringRef(it->first.c_str()),
                       fdrapidjson::StringRef(it->second.c_str()), allocator);
  }
...
void SStatsSerializerCSV::serialize(
    std::map<std::string, std::string>& keyValues, std::string& output) {
  std::stringstream ss;
