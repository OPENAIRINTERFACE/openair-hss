...
static void copy(struct avp* from, msg_or_avp* to) {
  ...
  if (avp_data.avp_basetype == AVP_TYPE_GROUPED) {
    ...
    if (ret != 0)
      throw FDException(
          SUtility::string_format("%s:%d - ERROR - fd_msg_avp_hdr returned %d "
                                  "retrieving header for [%s]",
    ...
    if (ret != 0)
      throw FDException(
          SUtility::string_format("%s:%d - ERROR - fd_msg_avp_setvalue "
                                  "returned %d setting the AVP value for [%s]",
                                  __FILE__, __LINE__, ret, avp_data.avp_name));
  }
...
static void copyall(msg_or_avp* from, msg_or_avp* to) {
  ...
  ret = fd_msg_browse_internal(from, MSG_BRW_FIRST_CHILD,
                               (msg_or_avp**)&loopavp, NULL);
  if (ret != 0)
    throw FDException(SUtility::string_format(
  ...
  while (loopavp) {
    ...
    // get the next AVP
    ret = fd_msg_browse_internal(loopavp, MSG_BRW_NEXT, (msg_or_avp**)&loopavp,
                                 NULL);
    if (ret != 0)
      throw FDException(SUtility::string_format(
...
FDEngine::FDEngine(const std::string& cfgfile) { setConfigFile(cfgfile); }
...
void FDEngine::waitForShutdown() { fd_core_wait_shutdown_complete(); }

void FDEngine::advertiseSupport(FDDictionaryEntryApplication& app, int auth,
                                int acct) {
  int ret;
...
void FDEngine::advertiseSupport(FDDictionaryEntryApplication& app,
                                FDDictionaryEntryVendor& vendor, int auth,
                                int acct) {
  int ret;
...
void FDDictionaryEntry::init(const void* what, enum dict_object_type type,
                             int criteria, struct dictionary* dict) {
...
FDDictionaryEntryVendor::FDDictionaryEntryVendor(
    ...
    : FDDictionaryEntry(app.getEntry(), DICT_VENDOR, VENDOR_OF_APPLICATION,
                        dict) {
  if (!isValid())
    throw FDException(SUtility::string_format(
...
FDDictionaryEntryAVP::FDDictionaryEntryAVP(const char* name, bool allVendors,
                                           struct dictionary* dict)
    : FDDictionaryEntry(name, DICT_AVP,
                        allVendors ? AVP_BY_NAME_ALL_VENDORS : AVP_BY_NAME,
                        dict),
      m_name(name),
      m_vendorid(0),
      m_derivedtype(NULL),
...
FDDictionaryEntryAVP::FDDictionaryEntryAVP(const char* name,
  ...
  if (isValid())
    ...
    throw FDException(
        SUtility::string_format("%s:%d - INFO - Unable to find AVP dictionary "
                                "entry for [%s] vendor [%u]",
...
void FDDictionaryEntryAVP::getTypeInfo() {
  ...
  if (ret == 0) {
    ...
    if (ret == 0) {
      ret = fd_dict_search(dict, DICT_TYPE, TYPE_OF_AVP, getEntry(),
                           &m_derivedtype, EINVAL);
      if (ret == 0) {
...
FDDictionaryEntryCommand::FDDictionaryEntryCommand(command_code_t cmdid,
                                                   struct dictionary* dict)
    : FDDictionaryEntry((const void*)&cmdid, DICT_COMMAND, CMD_BY_CODE_R,
                        dict) {
  if (!isValid())
    throw FDException(
        SUtility::string_format("%s:%d - INFO - Unable to find command "
                                "dictionary entry for command code [%u]",
...
FDDictionaryEntryCommand::FDDictionaryEntryCommand(
  ...
  if (!isValid())
    throw FDException(
        SUtility::string_format("%s:%d - INFO - Unable to find answer command "
                                "dictionary entry for request command [%s]",
  ...
  if (ret != 0)
    throw FDException(
        SUtility::string_format("%s:%d - INFO - Unable to find answer command "
                                "dictionary entry for answer command [%s]",
                                __FILE__, __LINE__, req.m_data.cmd_name));
}
...
bool FDAvp::get(std::string& v) {
  ...
  switch (m_de->getDataType()) {
    ...
    case DDTDiameterURI: {
      v.assign((char*)m_avphdr->avp_value->os.data,
               m_avphdr->avp_value->os.len);
      break;
    }
    default: {
      v.clear();
...
bool FDAvp::get(sSS& ss) {
  ...
  switch (addressType) {
    case 1:  // IPV4
    {
      ...
      memcpy(&((sSA4*)&ss)->sin_addr.s_addr, &m_avphdr->avp_value->os.data[2],
             4);
    ...
    case 2:  // IPV6
    {
      ...
      memcpy(&((sSA6*)&ss)->sin6_addr.s6_addr, &m_avphdr->avp_value->os.data[2],
             16);
...
FDAvp FDAvp::getChild(bool& found) {
  ...
  if (m_de->getDataType() == DDTGrouped) {
    found = fd_msg_browse_internal(m_avp, MSG_BRW_LAST_CHILD, (msg_or_avp**)&a,
                                   NULL) == 0;
...
FDMessage::FDMessage(bool req2ans, FDDictionaryEntryCommand* de,
                     struct msg* pmsg, bool dedel, bool msgdel)
...
FDAvp FDMessage::getFirstAVP(bool& found) {
  ...
  found = fd_msg_browse_internal(m_msg, MSG_BRW_FIRST_CHILD,
                                 (msg_or_avp**)&child, NULL) == 0;
...
FDMessage& FDMessage::sendRequest(void (*anscb)(void*, struct msg**),
...
FDMessageAnswer::FDMessageAnswer(FDDictionaryEntryCommand* cmd,
                                 struct msg* pmsg)
    : FDMessage(cmd, pmsg) {}
...
int FDApplication::commandcb(struct msg** m, struct avp* avp,
...
FDPeerState FDPeer::getState() {
  ...
  if (!m_peer) {
    ret = fd_peer_getbyid((DiamId_t)m_diamid.c_str(), m_diamid.size(), 0,
                          &m_peer);
    if (ret != 0)
      throw FDException(
...
void FDPeer::add() {
  if (m_diamid.empty())
    throw FDException(SUtility::string_format("%s:%d - Diameter ID not set",
                                              __FILE__, __LINE__));
  ...
  if (!m_destip.empty()) {
    ...
    ret = fd_ep_add_merge(&pi.pi_endpoints, ai->ai_addr, ai->ai_addrlen,
...
void FDExtractor::add(FDExtractorBase& base) {
  FDExtractorKey k(base.getDictionaryEntry()->getVendorId(),
...
void FDExtractor::resolve() {
  ...
  if (!m_reference) {
    if (m_parent) {
      if (!getDictionaryEntry())
        throw FDException(
            SUtility::string_format("%s:%d - ERROR - FDExtractor dictionary "
                                    "entry not defined for grouped AVP",
                                    __FILE__, __LINE__));
  ...
  if (m_reference && needToResolve) {
    ...
    // get the first child AVP
    int ret = fd_msg_browse_internal(m_reference, MSG_BRW_FIRST_CHILD,
                                     (msg_or_avp**)&loopavp, NULL);
    if (ret != 0)
      throw FDException(
          SUtility::string_format("%s:%d - ERROR - FDExtractor browse returned "
                                  "%d direction MSG_BRW_FIRST_CHILD",
                                  __FILE__, __LINE__, ret));
    ...
    while (loopavp) {
      ...
      // get the next AVP
      ret = fd_msg_browse_internal(loopavp, MSG_BRW_NEXT,
                                   (msg_or_avp**)&loopavp, NULL);
      if (ret != 0)
        throw FDException(
            SUtility::string_format("%s:%d - ERROR - FDExtractor browse "
                                    "returned %d direction MSG_BRW_NEXT",
                                    __FILE__, __LINE__, ret));
    }
  }
...
void FDExtractorList::addExtractor(FDExtractor* e) { m_list.push_back(e); }

bool FDExtractorList::exists() { return !getList().empty(); }
...
FDExtractorAvp::FDExtractorAvp(FDExtractor& extractor, FDDictionaryEntryAVP& de,
                               bool dedel)
...
bool FDExtractorAvpList::exists() { return !getList().empty(); }
...
void FDUtility::splitDiameterFQDN(std::string& fqdn, std::string& host,
                                  std::string& realm) {
...
size_t FDUtility::str2tbcd(const char* src, size_t srclen, uint8_t* dst,
                           size_t dstlen) {
  size_t len, idx;
...
size_t FDUtility::str2tbcd(const std::string& src, uint8_t* dst,
                           size_t dstlen) {
...
size_t FDUtility::tbcd2str(uint8_t* src, size_t srclen, char* dst,
                           size_t dstlen) {
