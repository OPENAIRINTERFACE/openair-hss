...
SDirectory::SDirectory() { m_handle = NULL; }

SDirectory::~SDirectory() { closeHandle(); }
...
const char* SDirectory::getFirstEntry(const char* directory,
                                      const char* filemask) {
...
const char* SDirectory::getFirstEntry() {
  ...
  if (!m_handle)
    SError::throwRuntimeExceptionWithErrno(
        "Error getting the first directory entry");
  ...
  while (true) {
    ...
    if (result)
      SError::throwRuntimeException(
          "Error getting the next directory entry - %d", result);
...
#define MATCH(a, b, ignoreCase) \
  (ignoreCase ? m_table[(int)a] == m_table[(int)b] : a == b)
...
bool SDirectory::match(const char* str, const char* mask, bool ignoreCase) {
  if (!m_table) buildTable();

  const char* mp = NULL;
  const char* sp = NULL;

  while (*str && *mask != '*') {
    if (*mask != '?' && !MATCH(*str, *mask, ignoreCase)) return false;
    str++;
    mask++;
  }
...
SDirectoryListing SDirectory::getListing(const char* directory,
                                         const char* filemask) {
  setDirectory(directory);
  setFilemask(filemask);
