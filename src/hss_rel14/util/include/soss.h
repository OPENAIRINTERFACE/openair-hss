...
class OssRestHandler {
 public:
  OssRestHandler(
      SStats* stats, SLogger* auditlogger, T* logger_manager,
  ...
  void updateLogger(
    ...
    if (doc.HasParseError()) {
      response.send(
          Pistache::Http::Code::Bad_Request, "{\"result\": \"ERROR\"}");
      return;
    }
...
class OssEndpoint {
 public:
  OssEndpoint(
      Pistache::Address addr, SStats* stats, SLogger* auditlogger,
  ...
  void setupRoutes() {
    Pistache::Rest::Routes::Get(
        ...
        Pistache::Rest::Routes::bind(
            &OssRestHandler<T>::getLoggers, &m_handler));
    Pistache::Rest::Routes::Post(
        m_router, "/logger",
        Pistache::Rest::Routes::bind(
            &OssRestHandler<T>::updateLogger, &m_handler));
    Pistache::Rest::Routes::Get(
        m_router, "/statfreq",
        Pistache::Rest::Routes::bind(
            &OssRestHandler<T>::getStatFrequency, &m_handler));
    Pistache::Rest::Routes::Post(
        m_router, "/statfreq",
        Pistache::Rest::Routes::bind(
            &OssRestHandler<T>::updateStatFrequency, &m_handler));
    Pistache::Rest::Routes::Get(
        m_router, "/statlive",
        Pistache::Rest::Routes::bind(
            &OssRestHandler<T>::getStatLive, &m_handler));
    Pistache::Rest::Routes::Get(
        m_router, "/ossoptions",
        Pistache::Rest::Routes::bind(
            &OssRestHandler<T>::getOssOptions, &m_handler));
  }
