...
class OssOptionReader {
 public:
  OssOptionReader(const std::string& file) : m_file(file) {}
...
class OssRestHandler {
 public:
  OssRestHandler(SStats* stats, SLogger* auditlogger, T* logger_manager,
                 const std::string& ossoptionfile)
      : m_stats(stats),
        m_auditlogger(auditlogger),
        m_logger_manager(logger_manager),
        m_ossoption_reader(ossoptionfile) {}
  void getLoggers(const Pistache::Http::Request& request,
                  Pistache::Http::ResponseWriter response) {
    logAuditLog(request);
    response.send(Pistache::Http::Code::Ok, m_logger_manager->serialize());
  }
  void updateLogger(const Pistache::Http::Request& request,
                    Pistache::Http::ResponseWriter response) {
    logAuditLog(request);
    RAPIDJSON_NAMESPACE::Document doc;
  ...
  void getStatFrequency(const Pistache::Http::Request& request,
                        Pistache::Http::ResponseWriter response) {
    logAuditLog(request);
    std::string res =
        "{\"statfreq\": " + std::to_string(m_stats->getInterval()) + "}";
    response.send(Pistache::Http::Code::Ok, res);
  }
  void getStatLive(const Pistache::Http::Request& request,
                   Pistache::Http::ResponseWriter response) {
    logAuditLog(request);
  ...
  void updateStatFrequency(const Pistache::Http::Request& request,
                           Pistache::Http::ResponseWriter response) {
    logAuditLog(request);
    RAPIDJSON_NAMESPACE::Document doc;
    doc.Parse(request.body().c_str());
  ...
  void getOssOptions(const Pistache::Http::Request& request,
                     Pistache::Http::ResponseWriter response) {
    std::string res;
...
class OssEndpoint {
 public:
  OssEndpoint(Pistache::Address addr, SStats* stats, SLogger* auditlogger,
              T* loggermanager, const std::string& ossoptionfile)
 ...
 private:
  void setupRoutes() {
    Pistache::Rest::Routes::Get(
        m_router, "/logger",
        Pistache::Rest::Routes::bind(&OssRestHandler<T>::getLoggers,
                                     &m_handler));
    Pistache::Rest::Routes::Post(
        m_router, "/logger",
        Pistache::Rest::Routes::bind(&OssRestHandler<T>::updateLogger,
                                     &m_handler));
    Pistache::Rest::Routes::Get(
        m_router, "/statfreq",
        Pistache::Rest::Routes::bind(&OssRestHandler<T>::getStatFrequency,
                                     &m_handler));
    Pistache::Rest::Routes::Post(
        m_router, "/statfreq",
        Pistache::Rest::Routes::bind(&OssRestHandler<T>::updateStatFrequency,
                                     &m_handler));
    Pistache::Rest::Routes::Get(
        m_router, "/statlive",
        Pistache::Rest::Routes::bind(&OssRestHandler<T>::getStatLive,
                                     &m_handler));
    Pistache::Rest::Routes::Get(
        m_router, "/ossoptions",
        Pistache::Rest::Routes::bind(&OssRestHandler<T>::getOssOptions,
                                     &m_handler));
  }
