...
#define stringcmp_format(string1, string2, count) \
  __builtin_strncasecmp(string1, string2, count)
...
size_t format_time_into_string(char* st, size_t max_limit,
                               const char* type_format, const struct tm* time_f,
                               const struct timeval* tval);

char* format_time_into_specs(const char* format, const struct tm* t, char* pt,
                             const char* ptlim, const struct timeval* tv);
char* convert_date_time_format(const int32_t n, const char* format, char* pt,
                               const char* ptlim);
...
class STime {
 public:
  ...
  STime(int64_t ms) { set(ms); }

  STime(int32_t year, int32_t mon, int32_t day, int32_t hour, int32_t min,
  ...
  bool operator==(const STime& a) {
    return ((m_time.tv_sec == a.m_time.tv_sec) &&
            (m_time.tv_usec == a.m_time.tv_usec));
  }
  ...
  bool operator<(const STime& a) {
    if (m_time.tv_sec < a.m_time.tv_sec) return true;
    if (m_time.tv_sec == a.m_time.tv_sec) {
      if (m_time.tv_usec < a.m_time.tv_usec) return true;
    }
  ...
  bool operator>=(const STime& a) { return !(*this < a); }

  bool operator<=(const STime& a) { return !(*this > a); }

  STime operator+(const STime& a) { return add(a.m_time); }

  STime operator+(const timeval& t) { return add(t); }

  STime operator-(const STime& a) { return subtract(a.m_time); }

  STime operator-(const timeval& t) { return subtract(t); }

  STime add(int32_t days, int32_t hrs, int32_t mins, int32_t secs,
            int32_t usecs) {
    timeval t;
    t.tv_sec = (days * 86400) + (hrs * 3600) + (mins * 60) + secs;
    t.tv_usec = usecs;
  ...
  STime subtract(int32_t days, int32_t hrs, int32_t mins, int32_t secs,
                 int32_t usecs) {
    timeval t;
    t.tv_sec = (days * 86400) + (hrs * 3600) + (mins * 60) + secs;
    t.tv_usec = usecs;
  ...
  const timeval& getTimeVal() { return m_time; }
  ...
  //    {
  //        return ((m_time.tv_sec - a.m_time.tv_sec) * 1000000) +
  //        (m_time.tv_usec - a.m_time.tv_usec);
  //    }
