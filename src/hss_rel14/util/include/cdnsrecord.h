...
class Question {
 public:
  Question(const std::string& qname, ns_type qtype, ns_class qclass)
      : m_qname(qname), m_qtype(qtype), m_qclass(qclass) {}
  ...
  void dump() {
    std::cout << "Question:"
              << " qtype=" << m_qtype << " qclass=" << m_qclass
              << " qname=" << m_qname << std::endl;
  }
...
class ResourceRecord {
 public:
  ResourceRecord(const std::string& name, ns_type rtype, ns_class rclass,
                 int32_t ttl)
      : m_name(name),
        m_type(rtype),
        m_class(rclass),
  ...
  virtual void dump() {
    std::cout << "ResourceRecord:"
              << " type=" << getType() << " class=" << getClass()
              << " ttl=" << getTTL() << " expires=" << getExpires()
 ...
 public:
  ...
  virtual void dump() {
    std::cout << "RRecordA:"
              << " type=" << getType() << " class=" << getClass()
              << " ttl=" << getTTL() << " expires=" << getExpires()
              << " address=" << getAddressString() << " name=" << getName()
              << std::endl;
  }
...
class RRecordNS : public ResourceRecord {
 public:
  RRecordNS(const std::string& name, int32_t ttl, const std::string& ns)
      : ResourceRecord(name, ns_t_cname, ns_c_in, ttl), m_namedserver(ns) {}
  ...
  virtual void dump() {
    std::cout << "RRecordNS:"
              << " type=" << getType() << " class=" << getClass()
              << " ttl=" << getTTL() << " expires=" << getExpires()
              << " ns=" << getNamedServer() << " name=" << getName()
              << std::endl;
  }
...
class RRecordAAAA : public ResourceRecord {
 public:
  RRecordAAAA(const std::string& name, int32_t ttl,
...
class RRecordCNAME : public ResourceRecord {
 public:
  RRecordCNAME(const std::string& name, int32_t ttl, const std::string& alias)
      : ResourceRecord(name, ns_t_cname, ns_c_in, ttl), m_alias(alias) {}
  ...
  virtual void dump() {
    std::cout << "RRecordCNAME:"
              << " type=" << getType() << " class=" << getClass()
              << " ttl=" << getTTL() << " expires=" << getExpires()
              << " alias=" << getAlias() << " name=" << getName() << std::endl;
  }
...
class RRecordSRV : public ResourceRecord {
 public:
  RRecordSRV(const std::string& name, int32_t ttl, uint16_t priority,
             uint16_t weight, uint16_t port, const std::string& target)
  ...
  virtual void dump() {
    std::cout << "RRecordSRV:"
              << " type=" << getType() << " class=" << getClass()
              << " ttl=" << getTTL() << " expires=" << getExpires()
              << " priority=" << getPriority() << " weight=" << getWeight()
              << " port=" << getPort() << " target=" << getTarget()
              << " name=" << getName() << std::endl;
  }
...
class RRecordNAPTR : public ResourceRecord {
 public:
  RRecordNAPTR(const std::string& name, int32_t ttl, uint16_t order,
               uint16_t preference, const std::string& flags,
               const std::string& service, const std::string& regexp,
  ...
  virtual void dump() {
    std::cout << "RRecordNAPTR:"
              << " type=" << getType() << " class=" << getClass()
              << " ttl=" << getTTL() << " expires=" << getExpires()
              << " order=" << getOrder() << " preference=" << getPreference()
              << " flags=" << getFlags() << " service=" << getService()
              << " regexp=" << getRegexp()
              << " replacement=" << getReplacement() << " name=" << getName()
              << std::endl;
  }
