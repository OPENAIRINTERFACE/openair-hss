...
class ResourceRecord {
 public:
  ResourceRecord(
      const std::string& name, ns_type rtype, ns_class rclass, int32_t ttl)
      : m_name(name),
        m_type(rtype),
        m_class(rclass),
        m_ttl(ttl),
...
class RRecordAAAA : public ResourceRecord {
 public:
  RRecordAAAA(
      const std::string& name, int32_t ttl, const struct in6_addr& address)
...
class RRecordSRV : public ResourceRecord {
 public:
  RRecordSRV(
      const std::string& name, int32_t ttl, uint16_t priority, uint16_t weight,
...
class RRecordNAPTR : public ResourceRecord {
 public:
  RRecordNAPTR(
      const std::string& name, int32_t ttl, uint16_t order, uint16_t preference,
