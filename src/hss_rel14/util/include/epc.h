...
class CanonicalNodeName : public std::list<std::string> {
 public:
  ...
  void dump(const char* prefix) {
    ...
    std::cout << prefix << "topon - " << (m_topon ? "true" : "false")
              << std::endl;
...
class UsageTypeList : public std::list<UsageType> {
 public:
  UsageTypeList() {}
  void dump(const char* prefix) {
    for (UsageTypeList::const_iterator it = begin(); it != end(); ++it) {
      std::cout << prefix << *it << std::endl;
    }
  }
};
...
class StringVector : public std::vector<std::string> {
 public:
  ...
  void shuffle() { std::random_shuffle(begin(), end()); }
};
...
class AppProtocol {
 public:
  ...
  bool findUsageType(UsageType ut) {
    return std::find(m_usageTypes.begin(), m_usageTypes.end(), ut) !=
           m_usageTypes.end();
  }

  void dump(const char* prefix = "") {
    std::cout << prefix << "Protocol [" << Utility::getAppProtocol(m_protocol);
    if (!m_rawProtocol.empty()) std::cout << "/" << m_rawProtocol;
    std::cout << "] - ";
    bool first = true;
    for (UsageTypeList::const_iterator it = m_usageTypes.begin();
         it != m_usageTypes.end(); ++it) {
      std::cout << (first ? "" : ",") << *it;
      first = false;
    }
    std::cout << std::endl;
...
class AppProtocolList : public std::list<AppProtocol*> {
 public:
  ...
  void dump(const char* prefix) {
    for (AppProtocolList::const_iterator it = begin(); it != end(); ++it) {
      (*it)->dump(prefix);
    }
  }
};
...
class AppService {
 public:
  ...
  AppProtocol* findProtocol(AppProtocolEnum protocol) {
    for (AppProtocolList::const_iterator it = m_protocols.begin();
         it != m_protocols.end(); ++it) {
      if ((*it)->getProtocol() == protocol) return *it;
    }
  ...
  void dump() {
    ...
    std::cout << "   Service  [" << Utility::getAppService(m_service) << "]"
              << std::endl;
    for (AppProtocolList::const_iterator it = m_protocols.begin();
         it != m_protocols.end(); ++it) {
      (*it)->dump("   ");
...
class NodeSelectorResult {
 public:
  ...
  const std::string& setHostname(const std::string& hostname) {
    m_hostname = hostname;
    return m_hostname;
  }
  void addSupportedProtocol(AppProtocol* ap) {
    m_supported_protocols.push_back(ap);
  }
...
class NodeSelectorResultList : public std::list<NodeSelectorResult*> {
 public:
  ...
  void dump(const char* prefix) {
    for (NodeSelectorResultList::const_iterator it = begin(); it != end();
         ++it) {
      (*it)->dump(prefix);
    }
  }

  static bool sort_compare(NodeSelectorResult*& first,
...
class NodeSelector {
 public:
  ...
  UsageType addDesiredUsageType(UsageType ut) {
    m_desiredUsageTypes.push_back(ut);
    return ut;
  }
  ...
  void dump() {
    ...
    std::cout << "  desired service - "
              << Utility::getAppService(m_desiredService) << std::endl;
 ...
 protected:
  ...
  AppServiceEnum setAppService(AppServiceEnum s) {
    return m_desiredService = s;
  }
  const std::string& setDomainName(const std::string& d) {
    m_domain = d;
    return m_domain;
  }
 ...
 private:
  AppServiceEnum parseService(const std::string& service,
                              std::list<AppProtocolEnum>& protocols) const;
  static bool naptr_compare(CachedDNS::RRecordNAPTR*& first,
                            CachedDNS::RRecordNAPTR*& second);
...
class ColocatedCandidate {
 public:
  ...
  ColocatedCandidate(NodeSelectorResult& candidate1,
  ...
  void dump(const char* prefix) {
    ...
    std::cout << prefix << "canonical name 1 - " << m_cnn1.getName()
              << std::endl;
    m_candidate1.dump(pfx.c_str());
    std::cout << prefix << "canonical name 2 - " << m_cnn2.getName()
              << std::endl;
    m_candidate2.dump(pfx.c_str());
    std::cout << prefix << "pair type - "
              << (m_pairtype == ptColocated
                      ? "colocated"
                      : m_pairtype == ptTopologicalDistance
                            ? "topological distance"
                            : m_pairtype == ptDNSPriority ? "DNS priority"
                                                          : "unknown")
              << std::endl;
    std::cout << prefix << "topological matches - " << m_topologicalMatches
              << std::endl;
  }
...
class ColocatedCandidateList : public std::list<ColocatedCandidate*> {
 public:
  ColocatedCandidateList(NodeSelectorResultList& nodelist1,
  ...
  void dump(const char* prefix = "") {
    ...
    for (ColocatedCandidateList::const_iterator it = begin(); it != end();
         ++it) {
 ...
 private:
  ...
  static bool sort_compare(ColocatedCandidate*& first,
...
class MMENodeSelector : public NodeSelector {
 public:
  MMENodeSelector(const char* mmec, const char* mmegi, const char* mnc,
                  const char* mcc) {
    setAppService(x_3gpp_mme);
    setDomainName(Utility::mme_fqdn(mmec, mmegi, mnc, mcc));
  }

  MMENodeSelector(const char* mmec, const char* mmegi,
                  const unsigned char* plmnid) {
    setAppService(x_3gpp_mme);
...
class SGWNodeSelector : public NodeSelector {
 public:
  SGWNodeSelector(const unsigned char lb, const unsigned char hb,
                  const char* mnc, const char* mcc) {
    char lbbuf[3];
    char hbbuf[3];
  ...
  SGWNodeSelector(unsigned char lb, unsigned char hb,
                  const unsigned char* plmnid) {
    char lbbuf[3];
    char hbbuf[3];
  ...
  SGWNodeSelector(const char* lb, const char* hb, const char* mnc,
                  const char* mcc) {
    setAppService(x_3gpp_sgw);
...
class DiameterNaptr {
 public:
  ...
  const std::string& setService(const char* service) {
    return m_service = service;
  }
  const std::string& setService(const std::string& service) {
    return setService(service.c_str());
  }

  const std::string& getReplacement() { return m_replacement; }
  const std::string& setReplacement(const char* replacement) {
    return m_replacement = replacement;
  }
  const std::string& setReplacement(const std::string& replacement) {
    return setReplacement(replacement.c_str());
  }
  ...
  void dump(const char* prefix) {
    std::cout << prefix << "Flags: "
              << (m_type == dnt_hostname
                      ? "hostname"
                      : m_type == dnt_service ? "service" : "unknown")
              << std::endl;
    std::cout << prefix << "Order: " << m_order << std::endl;
    std::cout << prefix << "Preference: " << m_preference << std::endl;
...
class DiameterSrvVector : public std::vector<DiameterSrv*> {
 public:
  ...
  void dump(const char* prefix) {
    ...
    int cnt = 0;
    for (DiameterSrvVector::const_iterator it = begin(); it != end(); ++it) {
...
class DiameterNaptrList : public std::list<DiameterNaptr*> {
 public:
  ...
  void dump(const char* prefix = "") {
    for (DiameterNaptrList::const_iterator it = begin(); it != end(); ++it) {
      if ((*it)->getType() == dnt_hostname)
        ((DiameterNaptrA*)*it)->dump(prefix);
      if ((*it)->getType() == dnt_service) ((DiameterNaptrS*)*it)->dump(prefix);
    }
  }
};
...
class DiameterSelector {
 public:
  ...
  const std::string& setRealm(const char* mnc, const char* mcc) {
    return m_realm = Utility::diameter_fqdn(mnc, mcc);
  }
  const std::string& setRealm(const unsigned char* plmnid) {
    return m_realm = Utility::diameter_fqdn(plmnid);
  }
