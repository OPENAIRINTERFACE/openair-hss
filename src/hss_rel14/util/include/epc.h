...
class NodeSelector {
 ...
 private:
  AppServiceEnum parseService(
      const std::string& service, std::list<AppProtocolEnum>& protocols) const;
  static bool naptr_compare(
      CachedDNS::RRecordNAPTR*& first, CachedDNS::RRecordNAPTR*& second);
  ...
  void dump(const char* prefix) {
    ...
    std::cout << prefix << "pair type - "
              << (m_pairtype == ptColocated ?
                      ...
                      m_pairtype == ptDNSPriority ? "DNS priority" : "unknown")
              << std::endl;
...
class SGWNodeSelector : public NodeSelector {
 public:
  SGWNodeSelector(
      const unsigned char lb, const unsigned char hb, const char* mnc,
      const char* mcc) {
    char lbbuf[3];
    char hbbuf[3];
  ...
  SGWNodeSelector(
      unsigned char lb, unsigned char hb, const unsigned char* plmnid) {
    char lbbuf[3];
    char hbbuf[3];
