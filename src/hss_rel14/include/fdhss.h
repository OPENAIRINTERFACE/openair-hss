...
class HandleMmeResponseEvtMsg : public SEventThreadMessage {
 public:
  HandleMmeResponseEvtMsg(EvenStatusMap* mme_response, std::string& imsi,
                          int imsi_reachable, std::string& msisdn);
...
class ImsiStatus {
 public:
  ImsiStatus(std::string& imsi, MonitoringConfEventStatus& status,
             int reachability, std::string& msisdn) {
    m_imsi = imsi;
    m_status = status;
...
class FDHss {
 public:
  ...
  static int s6a_peer_validate(struct peer_info* info, int* auth,
  ...
  void buildCfgStatusAvp(FDAvp& mon_evt_cfg_status,
