...
int nas_message_decrypt(const unsigned char *const inbuf,
                        ...
                        void *security, uint8_t *ul_nas_count,
                        nas_message_decode_status_t *status) {
  OAILOG_FUNC_IN(LOG_NAS);
  emm_security_context_t *emm_security_context =
      (emm_security_context_t *)security;
  ...
  *ul_nas_count = header->sequence_number;
  ...
  if (size < 0) {
     ...
     */
    uint32_t mac =
        _nas_message_get_mac(inbuf + offset, length - offset,
...
int nas_message_decode(const unsigned char *const buffer, nas_message_t *msg,
                       size_t length, void *security, uint8_t *ul_seq_no,
                       nas_message_decode_status_t *status) {
  OAILOG_FUNC_IN(LOG_NAS);
  emm_security_context_t *emm_security_context =
      (emm_security_context_t *)security;
  ...
  if (is_sr) {
     ...
     * context
     */
    mac = _nas_message_get_mac(buffer, SR_MAC_SIZE_BYTES, SECU_DIRECTION_UPLINK,
                               emm_security_context);
  ...
  if (size > 1) {
    ...
    if (emm_security_context) {
       ...
       * context
       */
      mac = _nas_message_get_mac(buffer + offset, length - offset,
      ...
      if (mac == msg->header.message_authentication_code) {
        ...
        OAILOG_DEBUG(LOG_NAS,
                     "msg->header.message_authentication_code = %04X != "
                     "computed = %04X\n",
...
int nas_message_encode(unsigned char *buffer, const nas_message_t *const msg,
  ...
  if (size < 0) {
     ...
     */
    bytes =
        _nas_message_protected_encode(buffer + size, &msg->security_protected,
...
static int _nas_message_decrypt(
  ...
  switch (security_header_type) {
    ...
    case SECURITY_HEADER_TYPE_INTEGRITY_PROTECTED:
    case SECURITY_HEADER_TYPE_INTEGRITY_PROTECTED_NEW:
      // todo: currently also in this case trying to get the security context
      // from the source MME
      OAILOG_DEBUG(LOG_NAS,
                   "No decryption of message length %lu according to security "
                   "header type 0x%02x\n",
      ...
      if (emm_security_context) {
        ...
        switch (emm_security_context->selected_algorithms.encryption) {
          ...
          case NAS_SECURITY_ALGORITHMS_EEA0:
            OAILOG_DEBUG(LOG_NAS,
                         "NAS_SECURITY_ALGORITHMS_EEA0 dir %d ul_count.seq_num "
                         "%d dl_count.seq_num %d\n",
...
static uint32_t _nas_message_get_mac(
  ...
  switch (emm_security_context->selected_algorithms.integrity) {
    case NAS_SECURITY_ALGORITHMS_EIA1: {
      ...
      OAILOG_DEBUG(LOG_NAS,
                   "NAS_SECURITY_ALGORITHMS_EIA1 returned MAC %x.%x.%x.%x(%u) "
                   "for length %lu direction %d, count %d\n",
    ...
    case NAS_SECURITY_ALGORITHMS_EIA2: {
      ...
      OAILOG_DEBUG(LOG_NAS,
                   "NAS_SECURITY_ALGORITHMS_EIA2 returned MAC %x.%x.%x.%x(%u) "
                   "for length %lu direction %d, count %d\n",
                   mac[0], mac[1], mac[2], mac[3], *((uint32_t *)&mac), length,
                   direction, count);
      mac32 = (uint32_t *)&mac;
