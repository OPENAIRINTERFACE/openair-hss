...
int main(int argc, const char *argv[]) {
  ...
  printf("INFO\t: %s - Closing UE's connection endpoint %d\n", __FUNCTION__,
         socket_get_fd(_as_simulator_ue_sid));
  socket_close(_as_simulator_ue_sid);
  printf("INFO\t: %s - Closing MME's connection endpoint %d\n", __FUNCTION__,
         socket_get_fd(_as_simulator_mme_sid));
  socket_close(_as_simulator_mme_sid);
...
static void *_as_simulator_ue_mngr(void *args) {
  ...
  while (_as_simulator_ue_is_running) {
    ...
    if (_as_simulator_ue_size > 0) {
       ...
       */
      sbytes = socket_send(_as_simulator_ue_sid, _as_simulator_ue_buffer,
    ...
    if (_as_simulator_mme_size > 0) {
       ...
       */
      sbytes = socket_send(_as_simulator_mme_sid, _as_simulator_mme_buffer,
...
static int _as_simulator_ue_process(int msg_id, as_message_t *msg) {
  ...
  switch (msg_id) {
    case AS_CELL_INFO_REQ:
      cnf.msgID = process_cell_info_req(msg_id, &msg->msg.cell_info_req,
    ...
    case AS_NAS_ESTABLISH_REQ:
      ind.msgID = process_nas_establish_req(msg_id, &msg->msg.nas_establish_req,
    ...
    case AS_UL_INFO_TRANSFER_REQ:
      ind.msgID = process_ul_info_transfer_req(
          msg_id, &msg->msg.ul_info_transfer_req, &ind.msg.ul_info_transfer_ind,
...
static int _as_simulator_mme_process(int msg_id, as_message_t *msg) {
  ...
  switch (msg_id) {
    case AS_NAS_ESTABLISH_RSP:
      cnf.msgID = process_nas_establish_rsp(msg_id, &msg->msg.nas_establish_rsp,
                                            &cnf.msg.nas_establish_cnf);
    ...
    case AS_DL_INFO_TRANSFER_REQ:
      ind.msgID = process_dl_info_transfer_req(
          msg_id, &msg->msg.dl_info_transfer_req, &ind.msg.dl_info_transfer_ind,
    ...
    case AS_NAS_RELEASE_REQ:
      ind.msgID = process_nas_release_ind(msg_id, &msg->msg.nas_release_req,
  ...
  if (_as_simulator_mme_transfer & AS_SIMULATOR_TRANSFER_TO_UE) {
    if (ind.msgID > 0) {
       ...
       * * * * at the UE side
       */
      _as_simulator_ue_size = as_message_encode(_as_simulator_ue_buffer, &ind,
                                                AS_SIMULATOR_UE_BUFFER_SIZE);
    } else if (cnf.msgID > 0) {
      /*
  ...
  if (_as_simulator_mme_transfer & AS_SIMULATOR_TRANSFER_TO_MME) {
     ...
     * * * * at the MME side
     */
    _as_simulator_mme_size = as_message_encode(_as_simulator_mme_buffer, &cnf,
                                               AS_SIMULATOR_MME_BUFFER_SIZE);
