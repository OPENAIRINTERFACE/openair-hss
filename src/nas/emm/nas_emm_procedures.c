...
static nas_emm_cn_proc_t *get_nas_cn_procedure(
    const struct emm_data_context_s *const ctxt, cn_proc_type_t proc_type);
...
static void nas_delete_context_req_procedure(
    struct emm_data_context_s *emm_context, nas_ctx_req_proc_t **ctx_req_proc);
static void nas_delete_child_procedures(
    struct emm_data_context_s *const emm_context,
    nas_emm_base_proc_t *const parent_proc);
static void nas_delete_cn_procedures(struct emm_data_context_s *emm_context);
static void nas_delete_common_procedures(
    struct emm_data_context_s *emm_context);
static nas_emm_proc_t *nas_emm_find_procedure_by_puid(
    struct emm_data_context_s *const emm_context, uint64_t puid);
...
//-----------------------------------------------------------------------------
inline bool is_nas_tau_complete_received(
    const nas_emm_tau_proc_t *const tau_proc) {
...
static void nas_delete_child_procedures(
  ...
  if (emm_context->emm_procedures) {
    ...
    if (emm_context->emm_procedures->emm_con_mngt_proc) {
      if (((nas_emm_base_proc_t *)(emm_context->emm_procedures
                                       ->emm_con_mngt_proc))
              ->parent == parent_proc) {
...
//-----------------------------------------------------------------------------
void nas_delete_common_procedure(struct emm_data_context_s *emm_context,
    ...
    switch ((*proc)->type) {
      ...
      case EMM_COMM_PROC_AUTH: {
        ...
        OAILOG_TRACE(LOG_NAS_EMM,
                     "UE " MME_UE_S1AP_ID_FMT " Delete AUTH procedure\n",
...
void nas_delete_attach_procedure(struct emm_data_context_s *emm_context) {
  ...
  if (proc) {
    ...
    OAILOG_TRACE(LOG_NAS_EMM,
                 "UE " MME_UE_S1AP_ID_FMT " Delete ATTACH procedure\n", ue_id);
    void *unused = NULL;
    ...
    if (ue_ref) {
      OAILOG_DEBUG(LOG_NAS_EMM,
                   "EMM-PROC (NASx)  -  * * * * * (2) ueREF %p has "
                   "mmeId " MME_UE_S1AP_ID_FMT ", enbId " ENB_UE_S1AP_ID_FMT
                   " state %d and eNB_ref %p. \n",
                   ue_ref, ue_ref->mme_ue_s1ap_id, ue_ref->enb_ue_s1ap_id,
                   ue_ref->s1_ue_state, ue_ref->enb);
    }
    unused = NULL;
    nas_stop_T_retry_specific_procedure(ue_id, &proc->emm_spec_proc.retry_timer,
                                        unused);

    OAILOG_TRACE(LOG_NAS_EMM,
                 "UE " MME_UE_S1AP_ID_FMT
                 " stopped the retry timer for attach procedure\n",
                 ue_id);
    if (ue_ref) {
      OAILOG_DEBUG(LOG_NAS_EMM,
                   "EMM-PROC (NASx)  -  * * * * * (2.5) ueREF %p has "
                   "mmeId " MME_UE_S1AP_ID_FMT ", enbId " ENB_UE_S1AP_ID_FMT
                   " state %d and eNB_ref %p \n",
                   ue_ref, ue_ref->mme_ue_s1ap_id, ue_ref->enb_ue_s1ap_id,
                   ue_ref->s1_ue_state, ue_ref->enb);
...
void nas_delete_tau_procedure(struct emm_data_context_s *emm_context) {
  ...
  if (proc) {
    ...
    mme_ue_s1ap_id_t ue_id = emm_context->ue_id;
    OAILOG_TRACE(LOG_NAS_EMM,
                 "UE " MME_UE_S1AP_ID_FMT " Delete TAU procedure\n", ue_id);
    void *unused = NULL;
    nas_stop_T3450(ue_id, &proc->T3450, unused);
    if (proc->ies) {
    ...
    nas_stop_T_retry_specific_procedure(
        emm_context->ue_id, &proc->emm_spec_proc.retry_timer, unused);
    OAILOG_TRACE(LOG_NAS_EMM,
                 "UE " MME_UE_S1AP_ID_FMT
                 " stopped the retry timer for TAU procedure\n",
                 ue_id);
...
void nas_delete_detach_procedure(struct emm_data_context_s *emm_context) {
  ...
  if (proc) {
    OAILOG_TRACE(LOG_NAS_EMM,
                 "UE " MME_UE_S1AP_ID_FMT " Delete DETACH procedure\n",
                 emm_context->ue_id);
    // free content
    if (proc->ies) {
...
static void nas_delete_auth_info_procedure(
  ...
  if (*auth_info_proc) {
    OAILOG_TRACE(LOG_NAS_EMM,
                 "UE " MME_UE_S1AP_ID_FMT " Delete AUTH INFO procedure\n",
                 emm_context->ue_id);
    if ((*auth_info_proc)->cn_proc.base_proc.parent) {
      (*auth_info_proc)->cn_proc.base_proc.parent->child = NULL;
    }
    void *unused = NULL;
    nas_stop_Ts6a_auth_info(emm_context->ue_id, &(*auth_info_proc)->timer_s6a,
                            unused);
...
static void nas_delete_context_req_procedure(
  ...
  if (*ctx_req_proc) {
    OAILOG_TRACE(LOG_NAS_EMM,
                 "UE " MME_UE_S1AP_ID_FMT " Delete Context Request procedure\n",
                 emm_context->ue_id);
    ...
    OAILOG_DEBUG(LOG_NAS_EMM,
                 "UE " MME_UE_S1AP_ID_FMT
                 " Delete Context Request procedure timer with timer id %u \n",
                 emm_context->ue_id, (*ctx_req_proc)->timer_s10.id);

    nas_stop_Ts10_ctx_res(emm_context->ue_id, &(*ctx_req_proc)->timer_s10,
                          unused);
      ...
      if (p1->proc == cn_proc) {
        switch (cn_proc->type) {
          ...
          case CN_PROC_CTX_REQ:
            nas_delete_context_req_procedure(emm_context,
                                             (nas_ctx_req_proc_t **)&cn_proc);
            break;
          case CN_PROC_NONE:
        ...
        OAILOG_TRACE(LOG_NAS_EMM,
                     "UE " MME_UE_S1AP_ID_FMT " Delete CN procedure %p\n",
...
nas_emm_attach_proc_t *nas_new_attach_procedure(
  ...
  if (!(emm_context->emm_procedures)) {
    ...
    OAILOG_ERROR(LOG_NAS_EMM,
                 "UE " MME_UE_S1AP_ID_FMT
                 " Attach procedure creation requested but another specific "
                 "procedure found\n",
                 emm_context->ue_id);
    return NULL;
  }
  ...
  /** Timer. */
  emm_context->emm_procedures->emm_specific_proc->retry_timer.sec =
      TIMER_SPECIFIC_RETRY_DEFAULT_VALUE;
  emm_context->emm_procedures->emm_specific_proc->retry_timer.usec =
      TIMER_SPECIFIC_RETRY_DEFAULT_VALUE_USEC;
  emm_context->emm_procedures->emm_specific_proc->retry_timer.id =
      NAS_TIMER_INACTIVE_ID;
  emm_context->emm_procedures->emm_specific_proc->type =
      EMM_SPEC_PROC_TYPE_ATTACH;
  /** Set the success notifications, entered when the UE goes from
   * EMM_DEREGISTERED to EMM_REGISTERED (former MME_APP callbacks). */
  //  emm_context->emm_procedures->emm_specific_proc->emm_proc.base_proc.success_notif
  //  = _emm_registration_complete;
...
nas_emm_tau_proc_t *nas_new_tau_procedure(
  ...
  if (!(emm_context->emm_procedures)) {
    ...
    OAILOG_ERROR(LOG_NAS_EMM,
                 "UE " MME_UE_S1AP_ID_FMT
                 " TAU procedure creation requested but another specific "
                 "procedure found\n",
                 emm_context->ue_id);
    return NULL;
  }
...
nas_emm_detach_proc_t *nas_new_detach_procedure(
  ...
  if (!(emm_context->emm_procedures)) {
    ...
    OAILOG_ERROR(LOG_NAS_EMM,
                 "UE " MME_UE_S1AP_ID_FMT
                 " Detach procedure creation requested but another specific "
                 "procedure found\n",
                 emm_context->ue_id);
    return NULL;
  }
...
nas_sr_proc_t *nas_new_service_request_procedure(
  ...
  if (!(emm_context->emm_procedures)) {
    ...
    OAILOG_ERROR(LOG_NAS_EMM,
                 "UE " MME_UE_S1AP_ID_FMT
                 " SR procedure creation requested but another connection "
                 "management procedure found\n",
                 emm_context->ue_id);
    return NULL;
  }
  emm_context->emm_procedures->emm_con_mngt_proc =
...
void nas_emm_procedure_register_emm_message(mme_ue_s1ap_id_t ue_id,
  ...
  if (nas_msg) {
    nas_emm_proc_t *emm_proc =
        nas_emm_find_procedure_by_puid(emm_data_context, puid);

    if (emm_proc) {
      int index =
          emm_data_context->emm_procedures->nas_proc_mess_sign_next_location;
      emm_data_context->emm_procedures->nas_proc_mess_sign[index]
      ...
      nas_digest_msg(
          ...
          (char *const)emm_data_context->emm_procedures
