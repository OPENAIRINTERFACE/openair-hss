...
static void _emm_proc_create_procedure_detach_request(
    emm_data_context_t *const emm_context, emm_detach_request_ies_t *const ies);
...
void _clear_emm_ctxt(mme_ue_s1ap_id_t ue_id) {
  ...
  if (!emm_context) {
    OAILOG_DEBUG(
        LOG_NAS_EMM,
        "No EMM context was found to clear for ue_id " MME_UE_S1AP_ID_FMT
        ". \n",
        ue_id);
  ...
  if (!emm_context->is_dynamic) {
    OAILOG_DEBUG(
        LOG_NAS_EMM,
        "Cannot clear not-dynamic EMM context for ue_id " MME_UE_S1AP_ID_FMT
        ". \n",
        ue_id);
    OAILOG_FUNC_OUT(LOG_NAS_EMM);
  }
  //  if(s1ap_is_ue_mme_id_in_list(emm_context->ue_id)ue_ref){
  //    OAILOG_DEBUG(LOG_NAS_EMM, "EMM-PROC  -  * * * * * (0) ueREF %p has mmeId
  //    " MME_UE_S1AP_ID_FMT ", enbId " ENB_UE_S1AP_ID_FMT " state %d and
  //    eNB_ref %p. \n",
  ...
  //  if(ue_ref){
  //    OAILOG_DEBUG(LOG_NAS_EMM, "EMM-PROC  -  * * * * * (1) ueREF %p has mmeId
  //    " MME_UE_S1AP_ID_FMT ", enbId " ENB_UE_S1AP_ID_FMT " state %d and
  //    eNB_ref %p. \n",
  //            ue_ref, ue_ref->mme_ue_s1ap_id, ue_ref->enb_ue_s1ap_id,
  //            ue_ref->s1_ue_state, ue_ref->enb);
...
int emm_proc_detach(mme_ue_s1ap_id_t ue_id, emm_proc_detach_type_t detach_type,
  ...
  OAILOG_INFO(
      LOG_NAS_EMM,
      "EMM-PROC  - Initiate detach type = %s (%d) for ueId " MME_UE_S1AP_ID_FMT
      ". \n",
  ...
  if (!ue_context) {
    if (emm_context) {
      OAILOG_ERROR(LOG_NAS_EMM,
                   "EMM-PROC  -  * * * * * NO UE CONTEXT FOUND FOR IMPLICIT "
                   "DETACH OF UE WITH ueID " MME_UE_S1AP_ID_FMT
                   " and IMSI " IMSI_64_FMT ". \n",
                   emm_context->ue_id, emm_context->_imsi64);
    } else {
      OAILOG_ERROR(LOG_NAS_EMM,
                   "EMM-PROC  -  * * * * * NO EMM CONTEXT FOUND FOR IMPLICIT "
                   "DETACH OF UE WITH ueID " MME_UE_S1AP_ID_FMT
                   " and IMSI " IMSI_64_FMT ". \n",
                   ue_id, NULL);
    }
   ...
   */
  if (emm_fsm_set_state(ue_id, emm_context, EMM_DEREGISTERED_INITIATED) ==
      RETURNerror) {
    OAILOG_WARNING(LOG_NAS_EMM,
                   "An implicit detach procedure is already ongoing for EMM "
                   "context of the UE (ue_id=" MME_UE_S1AP_ID_FMT
                   "). Aborting new implicit detach.",
                   ue_id);
    OAILOG_FUNC_RETURN(LOG_NAS_EMM, RETURNok);
  }

  /**
   * Although MME_APP and EMM contexts are separated, doing it like this causes
   * the recursion problem.
   *
   * TS 23.401: 5.4.4.1 -> (Although dedicated bearers, works for me).
   *
   * If all the bearers belonging to a UE are released, the MME shall change the
   * MM state of the UE to EMM- DEREGISTERED and the MME sends the S1 Release
   * Command to the eNodeB, which initiates the release of the RRC connection
   * for the given UE if it is not released yet, and returns an S1 Release
   * Complete message to the MME.
   *
   * Don't do it recursively over MME_APP, do it over the EMM/ESM, that's what
   * they're for. So just triggering the ESM (before purging the context, to
   * remove/purge all the PDN connections.
   *
   */

  /** Check if there are any active sessions, if so terminate them all. */
  OAILOG_INFO(LOG_NAS_EMM,
              "ue_id=" MME_UE_S1AP_ID_FMT " EMM-PROC  - Detach UE \n",
              emm_context->ue_id);
  /*
  ...
  if (detach_type) {
    OAILOG_INFO(LOG_NAS_EMM,
                ...
                " EMM-PROC  - Detach type is set for UE. We will send detach "
                "request. \n",
                emm_context->ue_id);
    ...
    MSC_LOG_TX_MESSAGE(
        ...
        "0 EMM_AS_NAS_INFO_DETACH_REQUEST ue id " MME_UE_S1AP_ID_FMT " ",
        ue_id);
    /*
     ...
     */
    emm_as_set_security_data(&emm_as->sctx_data, &emm_context->_security, false,
                             true);
    /*
    ...
    OAILOG_INFO(LOG_NAS_EMM,
                "ue_id=" MME_UE_S1AP_ID_FMT
                " EMM-PROC  - No detach type is set for UE. We will not send "
                "detach request. \n",
                emm_context->ue_id);
    if (ue_context)
  ...
  if (rc != RETURNerror) {
    ...
    MSC_LOG_TX_MESSAGE(MSC_NAS_EMM_MME, MSC_NAS_EMM_MME, NULL, 0,
                       "0 EMMREG_DETACH_CNF ue id " MME_UE_S1AP_ID_FMT " ",
                       emm_context->ue_id);
    OAILOG_INFO(LOG_NAS_EMM,
                "ue_id=" MME_UE_S1AP_ID_FMT
                " EMM-PROC  - Sending EMMREG_DETACH_CNF. \n",
                emm_context->ue_id);
...
int emm_proc_detach_request(mme_ue_s1ap_id_t ue_id,
  ...
  OAILOG_INFO(
      LOG_NAS_EMM,
      "EMM-PROC  - Detach type = %s (%d) requested (ue_id=" MME_UE_S1AP_ID_FMT
      ")\n",
  ...
  if (switch_off) {
    MSC_LOG_EVENT(MSC_NAS_EMM_MME,
                  "0 Removing UE context ue id " MME_UE_S1AP_ID_FMT " ", ue_id);
    rc = RETURNok;
  } else {
    /*
    ...
    MSC_LOG_TX_MESSAGE(MSC_NAS_EMM_MME, MSC_NAS_EMM_MME, NULL, 0,
                       "0 EMM_AS_NAS_INFO_DETACH ue id " MME_UE_S1AP_ID_FMT " ",
                       ue_id);
    /*
     * Setup NAS information message to transfer
     */
     ...
     */
    if (emm_context) {
      emm_as_set_security_data(&emm_as->sctx_data, &emm_context->_security,
                               false, true);
    } else {
      OAILOG_INFO(
          LOG_NAS_EMM,
          "ue_id=" MME_UE_S1AP_ID_FMT
          " EMM-PROC  - Not setting security context for detach accept. \n",
          ue_id);
    }
    /*
     * Notify EMM-AS SAP that Detach Accept message has to
  ...
  if (emm_context) {
    /*
     * Validate, just check if a specific procedure exist, if so disregard the
     * detach request. Assuming that DSR failed, with the timeout, we should
     * send one without a security header.
     */
    nas_emm_specific_proc_t *specific_proc =
        get_nas_specific_procedure(emm_context);
    if (specific_proc) {
      /** A specific procedure exists, abort the detach request, wait the
       * specific procedure to complete. */

      if (specific_proc->emm_proc.type == EMM_SPEC_PROC_TYPE_ATTACH) {
        OAILOG_INFO(
            LOG_NAS_EMM,
            " EMM-PROC  - An attach procedure for ueId " MME_UE_S1AP_ID_FMT
            " exists. "
            "Aborting the attach procedure and continuing with the detach "
            "request. \n",
        ...
        OAILOG_INFO(LOG_NAS_EMM,
                    " EMM-PROC  - A TAU procedure for ueId " MME_UE_S1AP_ID_FMT
                    " exists. "
                    "Aborting the attach procedure and continuing with the "
                    "detach request. \n",
                    emm_context->ue_id, specific_proc->type);
        // todo
        DevAssert(0);
        ...
        OAILOG_ERROR(
            LOG_NAS_EMM,
            " EMM-PROC  - A detach procedure for ueId " MME_UE_S1AP_ID_FMT
            " exists. "
...
static void _emm_proc_create_procedure_detach_request(
    ...
    emm_detach_request_ies_t *const ies) {
  nas_emm_detach_proc_t *detach_proc = nas_new_detach_procedure(emm_context);
  AssertFatal(detach_proc, "TODO Handle this");
  if ((detach_proc)) {
    detach_proc->ies = ies;
    /** No success notification needed. Everything will be handled in the states
     * with EMMREG messages. */
  }
}
