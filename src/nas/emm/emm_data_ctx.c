...
static hashtable_rc_t _emm_data_context_hashtable_insert(
    emm_data_t *emm_data, struct emm_data_context_s *elm) {
  hashtable_rc_t h_rc = HASH_TABLE_OK;

  if (IS_EMM_CTXT_PRESENT_IMSI(elm)) {
    h_rc = hashtable_ts_insert(emm_data->ctx_coll_imsi, elm->_imsi64,
                               (void *)(&elm->ue_id));
  } else {
    // This should not happen. Possible UE bug?
    OAILOG_WARNING(
        LOG_NAS_EMM,
        "EMM-CTX doesn't contain valid imsi UE id " MME_UE_S1AP_ID_FMT "\n",
        elm->ue_id);
  }
  return h_rc;
}
...
//------------------------------------------------------------------------------
inline void emm_ctx_set_attribute_present(emm_data_context_t *const ctxt,
                                          const int attribute_bit_pos) {
  ctxt->member_present_mask |= attribute_bit_pos;
}

inline void emm_ctx_clear_attribute_present(emm_data_context_t *const ctxt,
                                            const int attribute_bit_pos) {
  ctxt->member_present_mask &= ~attribute_bit_pos;
  ctxt->member_valid_mask &= ~attribute_bit_pos;
}

inline void emm_ctx_set_attribute_valid(emm_data_context_t *const ctxt,
                                        const int attribute_bit_pos) {
  ctxt->member_present_mask |= attribute_bit_pos;
  ctxt->member_valid_mask |= attribute_bit_pos;
}

inline void emm_ctx_clear_attribute_valid(emm_data_context_t *const ctxt,
                                          const int attribute_bit_pos) {
  ctxt->member_valid_mask &= ~attribute_bit_pos;
}
...
inline void emm_ctx_set_valid_guti(emm_data_context_t *const ctxt,
                                   guti_t *guti) {
  ctxt->_guti = *guti;
...
/* Set GUTI, mark it as valid */
inline void emm_ctx_set_valid_old_guti(emm_data_context_t *const ctxt,
                                       guti_t *guti) {
  ctxt->_old_guti = *guti;
...
/* Set IMSI */
inline void emm_ctx_set_imsi(emm_data_context_t *const ctxt, imsi_t *imsi,
...
inline void emm_ctx_set_valid_imsi(emm_data_context_t *const ctxt, imsi_t *imsi,
                                   const imsi64_t imsi64) {
  ctxt->_imsi = *imsi;
  ctxt->_imsi64 = imsi64;
  ...
  OAILOG_DEBUG(LOG_NAS_EMM,
               ...
               ctxt->ue_id, imsi_str);
#endif
  /** Notifying the MME_APP without an ITTI request of the validated IMSI, to
   * update the registration tables. */
...
inline void emm_ctx_set_valid_imei(emm_data_context_t *const ctxt,
                                   imei_t *imei) {
  ctxt->_imei = *imei;
...
/* Set IMEI_SV */
inline void emm_ctx_set_imeisv(emm_data_context_t *const ctxt,
                               imeisv_t *imeisv) {
  ctxt->_imeisv = *imeisv;
...
inline void emm_ctx_set_valid_imeisv(emm_data_context_t *const ctxt,
                                     imeisv_t *imeisv) {
  ctxt->_imeisv = *imeisv;
...
inline void emm_ctx_set_lvr_tai(emm_data_context_t *const ctxt,
                                tai_t *lvr_tai) {
  ...
  // TAI_ARG(&ctxt->_lvr_tai));
}

/* Set last_visited_registered_tai, mark it as valid */
inline void emm_ctx_set_valid_lvr_tai(emm_data_context_t *const ctxt,
                                      tai_t *lvr_tai) {
...
//------------------------------------------------------------------------------
/* Clear AUTH vector  */
inline void emm_ctx_clear_auth_vector(emm_data_context_t *const ctxt,
                                      ksi_t eksi) {
...
//------------------------------------------------------------------------------
inline void emm_ctx_set_security_type(emm_data_context_t *const ctxt,
...
//------------------------------------------------------------------------------
inline void emm_ctx_set_security_eksi(emm_data_context_t *const ctxt,
                                      ksi_t eksi) {
...
//------------------------------------------------------------------------------
inline void emm_ctx_clear_security_vector_index(
...
//------------------------------------------------------------------------------
inline void emm_ctx_set_security_vector_index(emm_data_context_t *const ctxt,
                                              int vector_index) {
...
inline void emm_ctx_set_drx_parameter(emm_data_context_t *const ctxt,
                                      drx_parameter_t *drx) {
...
inline void emm_ctx_set_valid_drx_parameter(emm_data_context_t *const ctxt,
                                            drx_parameter_t *drx) {
...
inline void emm_ctx_clear_eps_bearer_context_status(
    emm_data_context_t *const ctxt) {
  //  memset (&ctxt->_eps_bearer_context_status, 0, sizeof
  //  (ctxt->_eps_bearer_context_status)); emm_ctx_clear_attribute_present(ctxt,
...
inline void emm_ctx_set_eps_bearer_context_status(
    emm_data_context_t *const ctxt, eps_bearer_context_status_t *status) {
...
inline void emm_ctx_set_valid_eps_bearer_context_status(
    emm_data_context_t *const ctxt, eps_bearer_context_status_t *status) {
  //  ctxt->_eps_bearer_context_status = *status;
  //  emm_ctx_set_attribute_valid(ctxt,
  //  EMM_CTXT_MEMBER_EPS_BEARER_CONTEXT_STATUS);
  OAILOG_DEBUG(LOG_NAS_EMM,
               "ue_id=" MME_UE_S1AP_ID_FMT
...
inline void emm_ctx_update_from_mm_eps_context(
    emm_data_context_t *const emm_ctx_p, void *const _mm_eps_ctxt) {
  int rc = RETURNerror;
  ...
  /** Update the EMM context and the Security context from a received MM
   * context. */
  ...
  /*
   * todo: fill the authentication vector based on auth_quadruplet and
   * auth_quintuplet
   */
  ...
  //    auth_info_proc->vector[i]->kasme, AUTH_KASME_SIZE); memcpy
  //    (emm_ctx->_vector[destination_index].autn,
  //    auth_info_proc->vector[i]->autn, AUTH_AUTN_SIZE); memcpy
  //    (emm_ctx->_vector[destination_index].rand,
  //    auth_info_proc->vector[i]->rand, AUTH_RAND_SIZE); memcpy
  ...
  //    auth_info_proc->vector[i]->xres.size; OAILOG_INFO (LOG_NAS_EMM,
  //    "EMM-PROC  - Received Vector %u:\n", i); OAILOG_INFO (LOG_NAS_EMM,
  //    "EMM-PROC  - Received XRES ..: " XRES_FORMAT "\n", XRES_DISPLAY
  //    (emm_ctx->_vector[destination_index].xres)); OAILOG_INFO (LOG_NAS_EMM,
  //    "EMM-PROC  - Received RAND ..: " RAND_FORMAT "\n", RAND_DISPLAY
  //    (emm_ctx->_vector[destination_index].rand)); OAILOG_INFO (LOG_NAS_EMM,
  //    "EMM-PROC  - Received AUTN ..: " AUTN_FORMAT "\n", AUTN_DISPLAY
  //    (emm_ctx->_vector[destination_index].autn)); OAILOG_INFO (LOG_NAS_EMM,
  ...
  //    emm_ctx_set_attribute_valid(emm_ctx,
  //    EMM_CTXT_MEMBER_AUTH_VECTOR0+destination_index);
  //  }
  //  if (auth_info_proc->nb_vectors > 0) {
  //      emm_ctx_set_attribute_present(emm_ctx_p,
  //      EMM_CTXT_MEMBER_AUTH_VECTORS);
  //
  //      for (; eksi < MAX_EPS_AUTH_VECTORS; eksi++) {
  //        if (IS_EMM_CTXT_VALID_AUTH_VECTOR(emm_ctx, (eksi %
  ...
  //      AssertFatal(IS_EMM_CTXT_VALID_AUTH_VECTOR(emm_ctx, eksi_mod), "TODO No
  //      valid vector, should not happen");
  ...
  OAILOG_INFO(LOG_NAS_EMM,
              "EMM-PROC  - Received Vector %u from S10 Context Response from "
              "Source MME:\n",
              0);
  OAILOG_INFO(LOG_NAS_EMM,
              "EMM-PROC  - Received KASME .: " KASME_FORMAT " " KASME_FORMAT
              "\n",
              KASME_DISPLAY_1(emm_ctx_p->_vector[0].kasme),
              KASME_DISPLAY_2(emm_ctx_p->_vector[0].kasme));
  emm_ctx_set_attribute_present(
      emm_ctx_p,
      EMM_CTXT_MEMBER_AUTH_VECTOR0); /**< Still setting AUTH_VECTOR present
                                        todo: only set them via quanXXX from MM
                                        context. */
  emm_ctx_set_attribute_present(emm_ctx_p, EMM_CTXT_MEMBER_AUTH_VECTORS);

  ksi_t eksi = 0;
  //  if (emm_ctx_p->_security.eksi !=  KSI_NO_KEY_AVAILABLE) {
  //    AssertFatal(0 !=  0, "emm_ctx->_security.eksi %d",
  //    emm_ctx_p->_security.eksi); eksi = (emm_ctx_p->_security.eksi + 1) %
  //    (EKSI_MAX_VALUE + 1);
  //  }
   ...
   * Validate the rest of the parameters which were received with Authentication
   * Response from UE in normal attach.
   */
  emm_ctx_p->auth_sync_fail_count = 0;

  emm_ctx_set_security_eksi(emm_ctx_p, mm_eps_ctxt->ksi);
  OAILOG_DEBUG(LOG_NAS_EMM,
               "EMM-PROC  - Setting the UE authentication parameters from S10 "
               "Context Response as successfull for UE "
               "with IMSI " IMSI_64_FMT ". \n",
               emm_ctx_p->_imsi64);

  /**
   * Continue with the parameters set and validated in the SECURITY_MODE_CONTROL
   * messages.
   */
  /*
   * NAS counts.
  ...
  if (emm_ctx_p->_security.ul_count.seq_num == 0) {
    OAILOG_DEBUG(LOG_NAS_EMM,
                 "EMM-PROC  - Setting the UE NAS UL-Count to the one received "
                 "from S10 %d for UE " MME_UE_S1AP_ID_FMT ".\n",
    ...
    OAILOG_DEBUG(LOG_NAS_EMM,
                 "EMM-PROC  - Final UE NAS UL-Count to the one received from "
                 "S10 %d (overflow %d) for UE " MME_UE_S1AP_ID_FMT ".\n",
                 mm_eps_ctxt->nas_ul_count.seq_num,
                 mm_eps_ctxt->nas_ul_count.overflow, emm_ctx_p->ue_id);
  } else {
    OAILOG_DEBUG(LOG_NAS_EMM,
                 "EMM-PROC  - Leaving the UE NAS UL-Count to the one received "
                 "from NAS message %d but updating the overflow from S10 %d "
                 "for UE " MME_UE_S1AP_ID_FMT ".\n",
                 emm_ctx_p->_security.ul_count.seq_num,
                 emm_ctx_p->_security.ul_count.overflow, emm_ctx_p->ue_id);
  ...
  OAILOG_DEBUG(LOG_NAS_EMM,
               "EMM-PROC  - Final UE NAS DL-Count to the one received from S10 "
               "%d (overflow %d) for UE " MME_UE_S1AP_ID_FMT ".\n",
               emm_ctx_p->_security.dl_count.seq_num,
               emm_ctx_p->_security.dl_count.overflow, emm_ctx_p->ue_id);
  ...
  if (IS_EMM_CTXT_PRESENT_UE_NETWORK_CAPABILITY(emm_ctx_p)) {
    OAILOG_DEBUG(LOG_NAS_EMM,
                 "EMM-PROC  - UE network capabilities already present for "
                 "IMSI " IMSI_64_FMT
                 ". Not setting from S10 Context Response. \n",
                 emm_ctx_p->_imsi64);
  } else {
  ...
  if (IS_EMM_CTXT_PRESENT_MS_NETWORK_CAPABILITY(emm_ctx_p)) {
    OAILOG_DEBUG(LOG_NAS_EMM,
                 "EMM-PROC  - MS network capabilities already present for "
                 "IMSI " IMSI_64_FMT
                 ". Not setting from S10 Context Response. \n",
                 emm_ctx_p->_imsi64);
  } else {
    emm_ctx_set_valid_ms_nw_cap(emm_ctx_p,
                                (ms_network_capability_t *)&mm_eps_ctxt->ms_nc);
    // todo: review all of this
    //    emm_ctx_p->gea = (mm_eps_ctxt->ms_nc.gea1 << 6)|
    //    mm_eps_ctxt->ms_nc.egea; emm_ctx_p->gprs_present = true; /**< Todo:
  ...
  //    emm_ctx->_security.capability.umts_present    =
  //    mm_eps_ctxt->ue_nc.umts_present;
  ...
  /** Set the selected encryption and integrity algorithms from the S10 Context
   * Request. */
  emm_ctx_p->_security.selected_algorithms.encryption =
      mm_eps_ctxt->nas_cipher_alg;
  ...
  AssertFatal(
      EMM_SECURITY_VECTOR_INDEX_INVALID != emm_ctx_p->_security.vector_index,
      "Vector index not initialized");
  AssertFatal(MAX_EPS_AUTH_VECTORS > emm_ctx_p->_security.vector_index,
              "Vector index outbound value %d/%d",
              emm_ctx_p->_security.vector_index, MAX_EPS_AUTH_VECTORS);
  derive_key_nas(NAS_INT_ALG,
                 emm_ctx_p->_security.selected_algorithms.integrity,
                 emm_ctx_p->_vector[emm_ctx_p->_security.vector_index].kasme,
                 emm_ctx_p->_security.knas_int);
  derive_key_nas(NAS_ENC_ALG,
                 emm_ctx_p->_security.selected_algorithms.encryption,
                 emm_ctx_p->_vector[emm_ctx_p->_security.vector_index].kasme,
                 emm_ctx_p->_security.knas_enc);

  memcpy(emm_ctx_p->_vector[emm_ctx_p->_security.vector_index].kasme,
         mm_eps_ctxt->k_asme, 32);
  ...
  /**
   * All fields set and validated like SECURITY_MODE_COMPLETE was received and
   * processed from the UE. Set the rest parameters received in the
   * MM_UE_EPS_CONTEXT.
   */

  /* No need to abort any common procedures. Directly continue with the PDN
   * Configuration Request procedure in both cases (ULR). */
  ...
  memcpy(emm_ctx_p->_vector[emm_ctx_p->_security.vector_index].nh_conj,
         mm_eps_ctxt->nh, 32);
 ...
 * procedure. */
//   todo: _clear_emm_ctxt(emm_ctx_p->ue_id);
inline void temp_sec_ctx_from_mm_eps_context(
  ...
  derive_key_nas(NAS_INT_ALG, emm_sec_ctx_p->selected_algorithms.integrity,
                 k_asme_temp, emm_sec_ctx_p->knas_int);
  derive_key_nas(NAS_ENC_ALG, emm_sec_ctx_p->selected_algorithms.encryption,
                 k_asme_temp, emm_sec_ctx_p->knas_enc);
...
struct emm_data_context_s *emm_data_context_get(emm_data_t *emm_data,
  ...
  if (INVALID_MME_UE_S1AP_ID != ue_id) {
    hashtable_ts_get(emm_data->ctx_coll_ue_id, (const hash_key_t)(ue_id),
                     (void **)&emm_data_context_p);
    OAILOG_INFO(LOG_NAS_EMM,
...
struct emm_data_context_s *emm_data_context_get_by_imsi(emm_data_t *emm_data,
  ...
  h_rc = hashtable_ts_get(emm_data->ctx_coll_imsi, (const hash_key_t)imsi64,
                          /* sizeof(imsi64_t), */ (void **)&emm_ue_id_p);

  if (HASH_TABLE_OK == h_rc) {
    struct emm_data_context_s *tmp =
        emm_data_context_get(emm_data, (const hash_key_t)*emm_ue_id_p);
#if DEBUG_IS_ON
    if ((tmp)) {
      OAILOG_DEBUG(LOG_NAS_EMM,
...
struct emm_data_context_s *emm_data_context_get_by_guti(emm_data_t *emm_data,
  ...
  if (guti) {
    h_rc =
        obj_hashtable_uint64_ts_get(emm_data->ctx_coll_guti, (const void *)guti,
                                    sizeof(*guti), (void **)&emm_ue_id_p);

    if (HASH_TABLE_OK == h_rc) {
      struct emm_data_context_s *tmp =
          emm_data_context_get(emm_data, *emm_ue_id_p);
#if DEBUG_IS_ON
      if ((tmp)) {
        OAILOG_DEBUG(LOG_NAS_EMM,
                     "EMM-CTX - get UE id " MME_UE_S1AP_ID_FMT
...
int emm_context_unlock(struct emm_data_context_s *emm_context_p) {
  if (emm_context_p) {
    //    return unlock_ue_contexts(PARENT_STRUCT(emm_context_p, struct
    //    ue_context_s, emm_context));
  }
  return RETURNerror;
}
...
struct emm_data_context_s *emm_data_context_remove(
  ...
  if (IS_EMM_CTXT_PRESENT_GUTI(elm)) {
    obj_hashtable_ts_remove(emm_data->ctx_coll_guti, (const void *)&elm->_guti,
                            sizeof(elm->_guti), (void **)&emm_ue_id);
    if (emm_ue_id) {
      // The GUTI is only inserted as part of attach complete, so it might be
      // NULL.
      OAILOG_DEBUG(LOG_NAS_EMM,
                   "EMM-CTX - Remove in ctx_coll_guti context %p UE "
                   "id " MME_UE_S1AP_ID_FMT
                   " guti "
                   " " GUTI_FMT "\n",
                   elm, (mme_ue_s1ap_id_t)(*emm_ue_id), GUTI_ARG(&elm->_guti));
    }
    if (clear_fields) emm_ctx_clear_guti(elm);
  }

  if (IS_EMM_CTXT_PRESENT_IMSI(elm)) {
    imsi64_t imsi64 = imsi_to_imsi64(&elm->_imsi);
    hashtable_ts_remove(emm_data->ctx_coll_imsi, (const hash_key_t)imsi64,
                        (void **)&emm_ue_id);

    OAILOG_DEBUG(
        LOG_NAS_EMM,
        "EMM-CTX - Remove in ctx_coll_imsi context %p UE id " MME_UE_S1AP_ID_FMT
        " imsi " IMSI_64_FMT "\n",
        elm, (mme_ue_s1ap_id_t)((uintptr_t)emm_ue_id), imsi64);
    if (clear_fields) emm_ctx_clear_imsi(elm);
  }

  hashtable_ts_remove(emm_data->ctx_coll_ue_id, (const hash_key_t)(elm->ue_id),
                      (void **)&emm_data_context_p);
  return emm_data_context_p;
}
...
//
//  h_rc = hashtable_uint64_ts_remove (emm_data->ctx_coll_imsi, (const
//  hash_key_t)elm->_imsi64); if (INVALID_MME_UE_S1AP_ID != ue_id) {
//    h_rc = hashtable_uint64_ts_insert (emm_data->ctx_coll_imsi, (const
//    hash_key_t)(const hash_key_t)elm->_imsi64, ue_id);
//  } else {
//    h_rc = HASH_TABLE_KEY_NOT_EXISTS;
//  }
//  if (HASH_TABLE_OK != h_rc) {
//    OAILOG_TRACE (LOG_MME_APP,
//        "Error could not update this ue context mme_ue_s1ap_id "
//        MME_UE_S1AP_ID_FMT " imsi " IMSI_64_FMT ": %s\n", ue_id, elm->_imsi64,
...
//    OAILOG_DEBUG (LOG_NAS_EMM, "EMM-CTX - Upsert in context UE id "
//    MME_UE_S1AP_ID_FMT " with IMSI "IMSI_64_FMT"\n",
//                  elm->ue_id, elm->_imsi64);
//    return RETURNok;
//  } else {
//    OAILOG_ERROR (LOG_NAS_EMM, "EMM-CTX - Upsert in context UE id "
//    MME_UE_S1AP_ID_FMT " with IMSI "IMSI_64_FMT" "
//        "Failed %s\n", elm->ue_id, elm->_imsi64, hashtable_rc_code2string
//        (h_rc));
//    return RETURNerror;
//  }
//}
...
int emm_data_context_update_security_parameters(
  ...
  if (!emm_ctx) {
    OAILOG_ERROR(
        LOG_NAS_EMM,
        "EMM-CTX - no EMM context existing for UE id " MME_UE_S1AP_ID_FMT
        ". Cannot update the AS security. \n",
        ue_context->privates.mme_ue_s1ap_id);
    OAILOG_FUNC_RETURN(LOG_NAS_EMM, RETURNerror);
  }
  if (!IS_EMM_CTXT_VALID_SECURITY(emm_ctx)) {
    OAILOG_ERROR(LOG_NAS_EMM,
                 "EMM-CTX - no valid security context exist EMM context for UE "
                 "id " MME_UE_S1AP_ID_FMT " and IMSI " IMSI_64_FMT
                 ". Cannot update the AS security. \n",
                 ue_context->privates.mme_ue_s1ap_id, emm_ctx->_imsi64);
    OAILOG_FUNC_RETURN(LOG_NAS_EMM, RETURNerror);
  }

  AssertFatal((0 <= emm_ctx->_security.vector_index) &&
  ...
  *integrity_algorithm_capabilities =
  ...
  /* The NCC is a 3-bit key index (values from 0 to 7) for the NH and is sent to
   * the UE in the handover command signaling. */
  emm_ctx->_security.ncc = emm_ctx->_security.ncc % 8;
  /** If a wrap up occurs, skip the first one. */
  // todo: NCC CASE!!
  //  if(!emm_ctx->_security.ncc){
  //    /** Set the pending deactivation flag of the ncc. Will start from 0
  //    then. */ ue_context->pending_bearer_deactivation = true;
  //    OAILOG_ERROR(LOG_NAS_EMM, "EMM-CTX - NCC reached 7. Activating the
  //    pending bearer deactivation for UE context with ueId "
  //    MME_UE_S1AP_ID_FMT ". \n", ue_context->privates.mme_ue_s1ap_id);
  //  }
  //  if(!emm_ctx->_security.ncc){
  //
  //    OAILOG_INFO(LOG_NAS_EMM, "EMM-CTX - NCC wrap around for UE id "
  //    MME_UE_S1AP_ID_FMT " and IMSI " IMSI_64_FMT ". UL Count is %d.
  //    Sec_vector %d. \n",
  //        emm_ctx->ue_id, emm_ctx->_imsi64,
  //        emm_ctx->_security.ul_count.seq_num,
  //        emm_ctx->_security.vector_index);
  //
  //    memset(emm_ctx->_vector[emm_ctx->_security.vector_index].nh_conj, 0,
  //    32); OAILOG_STREAM_HEX(OAILOG_LEVEL_DEBUG, LOG_NAS, "Resetted NH_CONJ:
  //    ", emm_ctx->_vector[emm_ctx->_security.vector_index].nh_conj, 32);
  //
  //    /** NCC wrap around, recalculate KeNB. */
  //    derive_keNB (emm_ctx->_vector[emm_ctx->_security.vector_index].kasme,
  //        emm_ctx->_security.ul_count.seq_num,
  //        emm_ctx->_vector[emm_ctx->_security.vector_index].nh_conj);
  //    OAILOG_STREAM_HEX(OAILOG_LEVEL_DEBUG, LOG_NAS, "New KeNB/NH_CONJ: ",
  //    emm_ctx->_vector[emm_ctx->_security.vector_index].nh_conj, 32);
  //
  //    /** Increase the NCC. */
  //    emm_ctx->_security.ncc++;
  ...
  //    derive_nh(emm_ctx->_vector[emm_ctx->_security.vector_index].kasme,
  //    emm_ctx->_vector[emm_ctx->_security.vector_index].nh_conj);
  //    OAILOG_STREAM_HEX(OAILOG_LEVEL_DEBUG, LOG_NAS_EMM, "New NH_CONJ for
  //    ncc1: ", emm_ctx->_vector[emm_ctx->_security.vector_index].nh_conj, 32);
  //  }
  //  UNLOCK_EMM_CONTEXT(emm_ctx);

  OAILOG_INFO(LOG_NAS_EMM,
              "EMM-CTX - Updated AS security parameters for EMM context with "
              "UE id " MME_UE_S1AP_ID_FMT " and IMSI " IMSI_64_FMT ". \n",
              ue_context->privates.mme_ue_s1ap_id, emm_ctx->_imsi64);
...
void mm_ue_eps_context_update_security_parameters(
    mme_ue_s1ap_id_t ue_id, mm_context_eps_t *mm_eps_ue_context,
  ...
  *encryption_algorithm_capabilities =
      ((uint16_t)mm_eps_ue_context->ue_nc.eea & ~(1 << 7)) << 1;
  *integrity_algorithm_capabilities =
      ((uint16_t)mm_eps_ue_context->ue_nc.eia & ~(1 << 7)) << 1;
  ...
  //  }
  OAILOG_INFO(LOG_NAS_EMM,
              "EMM-CTX - Updated MM EPS UE context security context parameters "
              "for UE context with ueId " MME_UE_S1AP_ID_FMT ". \n",
              ue_context->privates.mme_ue_s1ap_id);
  OAILOG_FUNC_OUT(LOG_NAS_EMM);
}
...
int _start_context_request_procedure(struct emm_data_context_s *emm_context,
   ...
   * Context request.
   */
  ctx_req_proc->cn_proc.base_proc.parent =
      (nas_emm_base_proc_t *)&spec_proc->emm_proc.base_proc;
  /*
   * Not configuring an AS common procedure as parent procedure of the S10
   * procedure.
   */
  ctx_req_proc->last_tai = last_tai;
  ctx_req_proc->cn_proc.base_proc.parent->child =
      &ctx_req_proc->cn_proc.base_proc;
  ctx_req_proc->success_notif =
      _context_res_proc_success; /**< Continue with a PDN Configuration (ULR,
                                    CSR). */
  ctx_req_proc->failure_notif =
      _context_res_proc_fail; /**< Continue with the identification procedure
                                 (IdReq, AuthReq, SMC). */
  ctx_req_proc->cn_proc.base_proc.time_out =
      s10_context_req_timer_expiry_handler;
  ctx_req_proc->ue_id = ue_id;

  nas_start_Ts10_ctx_req(ctx_req_proc->ue_id, &ctx_req_proc->timer_s10,
                         ctx_req_proc->cn_proc.base_proc.time_out,
                         (void *)ue_id);
...
void nas_start_T3450(const mme_ue_s1ap_id_t ue_id,
  ...
  if ((T3450) && (T3450->id == NAS_TIMER_INACTIVE_ID)) {
    T3450->id =
        nas_emm_timer_start(T3450->sec, 0, time_out_cb, timer_callback_args);
    if (NAS_TIMER_INACTIVE_ID != T3450->id) {
      MSC_LOG_EVENT(MSC_NAS_EMM_MME,
                    "0 T3450 started UE " MME_UE_S1AP_ID_FMT " ", ue_id);
...
void nas_start_T3460(const mme_ue_s1ap_id_t ue_id,
  ...
  if ((T3460) && (T3460->id == NAS_TIMER_INACTIVE_ID)) {
    T3460->id =
        nas_emm_timer_start(T3460->sec, 0, time_out_cb, timer_callback_args);
    if (NAS_TIMER_INACTIVE_ID != T3460->id) {
      MSC_LOG_EVENT(MSC_NAS_EMM_MME,
                    "0 T3460 started UE " MME_UE_S1AP_ID_FMT " ", ue_id);
...
void nas_start_T3470(const mme_ue_s1ap_id_t ue_id,
  ...
  if ((T3470) && (T3470->id == NAS_TIMER_INACTIVE_ID)) {
    T3470->id =
        nas_emm_timer_start(T3470->sec, 0, time_out_cb, timer_callback_args);
    if (NAS_TIMER_INACTIVE_ID != T3470->id) {
      MSC_LOG_EVENT(MSC_NAS_EMM_MME,
                    "0 T3470 started UE " MME_UE_S1AP_ID_FMT " ", ue_id);
...
void nas_start_Ts6a_auth_info(const mme_ue_s1ap_id_t ue_id,
  ...
  if ((Ts6a_auth_info) && (Ts6a_auth_info->id == NAS_TIMER_INACTIVE_ID)) {
    Ts6a_auth_info->id = nas_emm_timer_start(Ts6a_auth_info->sec, 0,
                                             time_out_cb, timer_callback_args);
...
void nas_start_Ts10_ctx_req(const mme_ue_s1ap_id_t ue_id,
  ...
  if ((Ts10_ctx_res) && (Ts10_ctx_res->id == NAS_TIMER_INACTIVE_ID)) {
    Ts10_ctx_res->id = nas_emm_timer_start(Ts10_ctx_res->sec, 0, time_out_cb,
                                           timer_callback_args);
    if (NAS_TIMER_INACTIVE_ID != Ts10_ctx_res->id) {
      MSC_LOG_EVENT(MSC_NAS_EMM_MME,
                    "0 Ts10_ctx_res started UE " MME_UE_S1AP_ID_FMT " ", ue_id);
      OAILOG_DEBUG(LOG_NAS_EMM,
                   "Ts10_ctx_res started UE " MME_UE_S1AP_ID_FMT
                   " with timer id %u \n",
                   ue_id, Ts10_ctx_res->id);
    } else {
      OAILOG_ERROR(LOG_NAS_EMM,
                   "Could not start Ts10_ctx_res UE " MME_UE_S1AP_ID_FMT " ",
                   ue_id);
    }
  }
}
...
void nas_start_T_retry_specific_procedure(const mme_ue_s1ap_id_t ue_id,
  ...
  if ((T_retry) && (T_retry->id == NAS_TIMER_INACTIVE_ID)) {
    T_retry->id = nas_emm_timer_start(T_retry->sec, T_retry->usec, time_out_cb,
                                      timer_callback_args);
    if (NAS_TIMER_INACTIVE_ID != T_retry->id) {
      MSC_LOG_EVENT(MSC_NAS_EMM_MME,
                    "0 T_retry started UE " MME_UE_S1AP_ID_FMT " ", ue_id);
      OAILOG_DEBUG(LOG_NAS_EMM, "T_retry started UE " MME_UE_S1AP_ID_FMT "\n",
                   ue_id);
    } else {
      OAILOG_ERROR(LOG_NAS_EMM,
                   "Could not start T_retry UE " MME_UE_S1AP_ID_FMT " ", ue_id);
    }
  }
}
...
void nas_stop_T3450(const mme_ue_s1ap_id_t ue_id,
  ...
  if ((T3450) && (T3450->id != NAS_TIMER_INACTIVE_ID)) {
    ...
    MSC_LOG_EVENT(MSC_NAS_EMM_MME, "0 T3450 stopped UE " MME_UE_S1AP_ID_FMT " ",
                  ue_id);
...
void nas_stop_T3460(const mme_ue_s1ap_id_t ue_id,
  ...
  if ((T3460) && (T3460->id != NAS_TIMER_INACTIVE_ID)) {
    ...
    MSC_LOG_EVENT(MSC_NAS_EMM_MME, "0 T3460 stopped UE " MME_UE_S1AP_ID_FMT " ",
                  ue_id);
...
void nas_stop_T3470(const mme_ue_s1ap_id_t ue_id,
  ...
  if ((T3470) && (T3470->id != NAS_TIMER_INACTIVE_ID)) {
    ...
    MSC_LOG_EVENT(MSC_NAS_EMM_MME, "0 T3470 stopped UE " MME_UE_S1AP_ID_FMT " ",
                  ue_id);
...
void nas_stop_Ts6a_auth_info(const mme_ue_s1ap_id_t ue_id,
  ...
  if ((Ts6a_auth_info) && (Ts6a_auth_info->id != NAS_TIMER_INACTIVE_ID)) {
    ...
    MSC_LOG_EVENT(MSC_NAS_EMM_MME,
                  "0 Ts6a_auth_info stopped UE " MME_UE_S1AP_ID_FMT " ", ue_id);
    OAILOG_DEBUG(LOG_NAS_EMM,
                 "Ts6a_auth_info stopped UE " MME_UE_S1AP_ID_FMT "\n", ue_id);
  }
}
...
void nas_stop_Ts10_ctx_res(const mme_ue_s1ap_id_t ue_id,
  ...
  if ((Ts10_ctx_res) && (Ts10_ctx_res->id != NAS_TIMER_INACTIVE_ID)) {
    ...
    MSC_LOG_EVENT(MSC_NAS_EMM_MME,
                  "0 Ts10_ctx_res stopped UE " MME_UE_S1AP_ID_FMT " ", ue_id);
    OAILOG_DEBUG(LOG_NAS_EMM,
                 "Ts10_ctx_res stopped UE " MME_UE_S1AP_ID_FMT
                 " with timer id %u \n",
                 ue_id, Ts10_ctx_res->id);
  }
}
...
void nas_stop_T_retry_specific_procedure(const mme_ue_s1ap_id_t ue_id,
  ...
  if ((T_retry) && (T_retry->id != NAS_TIMER_INACTIVE_ID)) {
    ...
    MSC_LOG_EVENT(MSC_NAS_EMM_MME,
                  "0 T_retry stopped UE " MME_UE_S1AP_ID_FMT " ", ue_id);
...
//------------------------------------------------------------------------------
void emm_context_dump(const struct emm_data_context_s *const emm_context,
  ...
  if (emm_context) {
    ...
    bformata(bstr_dump,
             "%*s     - is_dynamic:       %u      (Dynamically allocated "
             "context indicator)\n",
             indent_spaces, " ", emm_context->is_dynamic);
    //    bformata (bstr_dump, "%*s     - is_attached:      %u      (Attachment
    //    indicator)\n", indent_spaces, " ", emm_context->is_attached);
    bformata(
        bstr_dump,
        "%*s     - is_has_been_attached:      %u      (Attachment indicator)\n",
    ...
    if (IS_EMM_CTXT_PRESENT_IMSI(emm_context)) {
      ...
      bformata(bstr_dump,
               "%*s     - imsi:             %s      (The IMSI provided by the "
               "UE or the MME)\n",
    ...
    for (int vector_index = 0; vector_index < MAX_EPS_AUTH_VECTORS;
         vector_index++) {
      bformata(bstr_dump,
               "%*s     - auth_vector[%d]:              (EPS authentication "
               "vector)\n",
                 ...
                 SECURITY_CTX_TYPE_PARTIAL_NATIVE)
                    ...
                    : (emm_context->_security.sc_type ==
                       SECURITY_CTX_TYPE_FULL_NATIVE)
                          ...
                          : "MAPPED");
      bformata(bstr_dump,
               "%*s         - eksi:  %u              (NAS key set identifier "
               "for E-UTRAN)\n",
               indent_spaces, " ", emm_context->_security.eksi);
    ...
    //      +-----+-------------------------------------------------+\n",
    //      indent_spaces, " "); bformata (bstr_dump, "%*s     |     |  0  1  2
    ...
    /// octet_index++) { /        if ((octet_index % 16) == 0) { /          if
    ///(octet_index != 0) { /            bformata (bstr_dump, " |\n"); / } /
    ...
    ////         */
    ////        bformata (bstr_dump, " %02x",
    /// emm_context->esm_msg->data[octet_index]); /      }
    //      /*
...
int emm_data_context_add(emm_data_t *emm_data, struct emm_data_context_s *elm) {
  ...
  h_rc = hashtable_ts_insert(emm_data->ctx_coll_ue_id,
                             (const hash_key_t)(elm->ue_id), elm);

  if (HASH_TABLE_OK == h_rc) {
    OAILOG_DEBUG(LOG_NAS_EMM,
                 "EMM-CTX - Add in context %p UE id " MME_UE_S1AP_ID_FMT "\n",
                 elm, elm->ue_id);

    if (IS_EMM_CTXT_PRESENT_GUTI(elm)) {
      h_rc = obj_hashtable_ts_insert(emm_data->ctx_coll_guti,
                                     (const void *const)(&elm->_guti),
                                     sizeof(elm->_guti), &elm->ue_id);

      if (HASH_TABLE_OK == h_rc) {
        OAILOG_DEBUG(LOG_NAS_EMM,
                     "EMM-CTX - Add in context UE id " MME_UE_S1AP_ID_FMT
                     " with GUTI " GUTI_FMT "\n",
                     elm->ue_id, GUTI_ARG(&elm->_guti));
      } else {
        OAILOG_ERROR(LOG_NAS_EMM,
                     "EMM-CTX - Add in context UE id " MME_UE_S1AP_ID_FMT
                     " with GUTI " GUTI_FMT " Failed %s\n",
                     elm->ue_id, GUTI_ARG(&elm->_guti),
                     hashtable_rc_code2string(h_rc));
        return RETURNerror;
      }
    ...
    if (IS_EMM_CTXT_PRESENT_IMSI(elm)) {
      imsi64_t imsi64 = imsi_to_imsi64(&elm->_imsi);
      h_rc = hashtable_ts_insert(emm_data->ctx_coll_imsi, imsi64,
                                 (void *)&elm->ue_id);
      ...
      if (HASH_TABLE_OK == h_rc) {
        ...
        OAILOG_ERROR(LOG_NAS_EMM,
                     "EMM-CTX - Add in context UE id " MME_UE_S1AP_ID_FMT
                     " with IMSI " IMSI_64_FMT " Failed %s\n",
...
int emm_data_context_add_old_guti(emm_data_t *emm_data,
  ...
  if (IS_EMM_CTXT_PRESENT_OLD_GUTI(elm)) {
    h_rc = obj_hashtable_ts_insert(emm_data->ctx_coll_guti,
                                   (const void *const)(&elm->_old_guti),
                                   sizeof(elm->_old_guti), &elm->ue_id);

    if (HASH_TABLE_OK == h_rc) {
      OAILOG_DEBUG(LOG_NAS_EMM,
                   "EMM-CTX - Add in context UE id " MME_UE_S1AP_ID_FMT
                   " with old GUTI " GUTI_FMT "\n",
                   elm->ue_id, GUTI_ARG(&elm->_old_guti));
    } else {
      OAILOG_ERROR(LOG_NAS_EMM,
                   "EMM-CTX - Add in context UE id " MME_UE_S1AP_ID_FMT
                   " with old GUTI " GUTI_FMT " Failed %s\n",
                   elm->ue_id, GUTI_ARG(&elm->_old_guti),
                   hashtable_rc_code2string(h_rc));
      return RETURNerror;
    }
...
int emm_data_context_upsert_imsi(emm_data_t *emm_data,
  ...
  if (HASH_TABLE_OK == h_rc || HASH_TABLE_INSERT_OVERWRITTEN_DATA == h_rc) {
    OAILOG_DEBUG(LOG_NAS_EMM,
                 "EMM-CTX - Upsert in context UE id " MME_UE_S1AP_ID_FMT
                 " with IMSI " IMSI_64_FMT "\n",
                 elm->ue_id, elm->_imsi64);
    return RETURNok;
  } else {
