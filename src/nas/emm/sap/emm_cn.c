...
static int _emm_cn_implicit_detach_ue(const uint32_t ue_id,
                                      const emm_proc_detach_type_t detach_type,
                                      const int emm_cause, const bool clr) {
  int rc = RETURNok;

  OAILOG_FUNC_IN(LOG_NAS_EMM);
  OAILOG_DEBUG(LOG_NAS_EMM,
               "EMM-PROC Implicit Detach ue_id " MME_UE_S1AP_ID_FMT "\n",
               ue_id);
  ...
  /** No detach procedure for implicit detach. Set the UE state into
   * EMM-DEREGISTER-INITIATED state. */
...
static int _emm_cn_context_res(const emm_cn_context_res_t *msg) {
   ...
   * If we received already security parameters like UE network capability,
   * ignoring the parameters received from the source MME and using the UE
   * parameters.
   */
  OAILOG_FUNC_IN(LOG_NAS_EMM);
  emm_context = emm_data_context_get(&_emm_data, msg->ue_id);
  if (emm_context == NULL) { /**< We assume that an MME_APP UE context also
  ...
  if (!ctx_req_proc) {
    OAILOG_ERROR(LOG_NAS_EMM,
                 "EMM-PROC  - "
                 "Failed to find context request procedure associated to UE "
                 "id " MME_UE_S1AP_ID_FMT "...\n",
                 msg->ue_id);
...
static int _emm_cn_context_fail(const emm_cn_context_fail_t *msg) {
  ...
  /**
   * An UE could or could not exist. We need to check. If it exists, it needs to
   * be purged. Since no UE context is established yet, we don't have
   * security/no bearers.0 If the message is received after the timeout, the
   * MME_APP context also should not exist. If the MME_APP context existed at
   * that point in time, it will later be removed. Just discard the message
   * then.
   */
  ...
  if (emm_ctx == NULL) {
    OAILOG_ERROR(LOG_NAS_EMM,
                 ...
                 msg->ue_id);
    /**
     * In this case, don't wait for the timer to remove the rest! Assume no
     * timers exist. Purge the rest of the UE context (MME_APP etc.).
     */
    OAILOG_FUNC_RETURN(LOG_NAS_EMM, rc);
  }
 ...
 *
 * Description: Checks if an attach or TAU procedure is ongoing and proceeds
 * with the accept.
 *
...
int _emm_wrapper_esm_accept(mme_ue_s1ap_id_t ue_id, bstring *esm_rsp,
  ...
  if (!emm_context) {
    OAILOG_WARNING(
        LOG_NAS_EMM,
        "EMM-PROC  - No EMM context was found for ue_id " MME_UE_S1AP_ID_FMT
        ". Ignoring ESM accept. \n",
        ue_id);
  ...
  if (is_nas_specific_procedure_attach_running(emm_context)) {
    ...
    if (esm_rsp) bdestroy_wrapper(esm_rsp);
    rc = emm_wrapper_tracking_area_update_accept(ue_id, ebr_status);
    OAILOG_FUNC_RETURN(LOG_NAS_EMM, RETURNok);
  }
  OAILOG_WARNING(LOG_NAS_EMM,
                 "EMM-PROC  - Neither attach nor TAU procedure is ongoing for "
                 "ue_id " MME_UE_S1AP_ID_FMT
                 ". Ignoring the received ESM reject. \n",
                 ue_id);
  bdestroy_wrapper(esm_rsp);
  OAILOG_FUNC_RETURN(LOG_NAS_EMM, RETURNerror);
}
  ...
  switch (primitive) {
    ...
    case EMMCN_IMPLICIT_DETACH_UE:
      rc = _emm_cn_implicit_detach_ue(msg->u.emm_cn_implicit_detach.ue_id,
                                      msg->u.emm_cn_implicit_detach.detach_type,
                                      msg->u.emm_cn_implicit_detach.emm_cause,
