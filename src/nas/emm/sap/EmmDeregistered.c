...
int EmmDeregistered(emm_reg_t *const evt) {
  ...
  if (!emm_ctx) {
    OAILOG_ERROR(LOG_NAS_EMM,
                 "UE " MME_UE_S1AP_ID_FMT " has not emm context. \n",
                 evt->ue_id);
    OAILOG_FUNC_RETURN(LOG_NAS_EMM, RETURNerror);
  }
  if (emm_fsm_get_state(emm_ctx) != EMM_DEREGISTERED) {
    OAILOG_ERROR(LOG_NAS_EMM,
                 "EMM-FSM state of UE " MME_UE_S1AP_ID_FMT
                 " is not EMM_DEREGISTERED - instead (%d). \n",
  ...
  switch (evt->primitive) {
    ...
    case _EMMREG_TAU_REQ:
      OAILOG_ERROR(LOG_NAS_EMM,
                   "EMM-FSM state EMM_DEREGISTERED - Primitive _EMMREG_TAU_REQ "
                   "is not valid\n");
      MSC_LOG_RX_DISCARDED_MESSAGE(
          MSC_NAS_EMM_MME, MSC_NAS_EMM_MME, NULL, 0,
    ...
    case _EMMREG_TAU_CNF:
      OAILOG_INFO(LOG_NAS_EMM,
                  "EMM-FSM state EMM_DEREGISTERED - Setting UE to registered "
                  "state via _EMMREG_TAU_CNF. \n");
