...
*/
static int _emm_start_tracking_area_update_proc_authentication(
    emm_data_context_t *emm_context, nas_emm_tau_proc_t *tau_proc);
static int _emm_start_tracking_area_update_proc_security(
    emm_data_context_t *emm_context, nas_emm_tau_proc_t *tau_proc);

/* TODO Commented some function declarations below since these were called from
 * the code that got removed from TAU request handling function. Reason this
 * code was removed: This portion of code was incomplete and was related to
 * handling of some optional IEs /scenarios that were not relevant for the TAU
 * periodic update handling and might have resulted in unexpected
 * behaviour/instability. At present support for TAU is limited to handling of
 * periodic TAU request only  mandatory IEs . Other aspects of TAU are TODOs for
 * future.
 */

static int _emm_tracking_area_update_reject(const mme_ue_s1ap_id_t ue_id,
                                            const int emm_cause);
...
static int _emm_tracking_area_update_success_identification_cb(
    emm_data_context_t *emm_context);
static int _emm_tracking_area_update_failure_identification_cb(
    emm_data_context_t *emm_context);
static int _emm_tracking_area_update_success_authentication_cb(
    emm_data_context_t *emm_context);
static int _emm_tracking_area_update_failure_authentication_cb(
    emm_data_context_t *emm_context);
static int _emm_tracking_area_update_success_security_cb(
    emm_data_context_t *emm_context);
static int _emm_tracking_area_update_failure_security_cb(
    emm_data_context_t *emm_context);
...
static int _emm_tracking_area_update_run_procedure(
    emm_data_context_t *emm_context);
static void _emm_proc_create_procedure_tracking_area_update_request(
    emm_data_context_t *const ue_context, emm_tau_request_ies_t *const ies,
    retry_cb_t retry_cb);
static bool _emm_tracking_area_update_ies_have_changed(
    mme_ue_s1ap_id_t ue_id, emm_tau_request_ies_t *const tau_ies1,
    emm_tau_request_ies_t *const tau_ies2);
...
static int _emm_tracking_area_update_accept_retx(
    emm_data_context_t *emm_context);

static int emm_proc_tracking_area_update_request_validity(
    emm_data_context_t *emm_context, mme_ue_s1ap_id_t new_ue_id,
    emm_tau_request_ies_t *const ies);

static void _emm_tracking_area_update_registration_complete(
    emm_data_context_t *emm_context);
...
int emm_proc_tracking_area_update_request(
    const mme_ue_s1ap_id_t ue_id, emm_tau_request_ies_t *ies, const int gea,
    const bool gprs_present, int *emm_cause,
  ...
  *emm_cause = EMM_CAUSE_SUCCESS;
  ...
  if (INVALID_MME_UE_S1AP_ID == ue_id) {
    ...
    OAILOG_INFO(LOG_NAS_EMM,
                "EMM-PROC  TAU - Received an invalid ue_id. Not continuing "
                "with the tracking area update request. \n");
  ...
  if (!ue_context) {
    OAILOG_INFO(LOG_NAS_EMM,
                "EMM-PROC  TAU - For ueId " MME_UE_S1AP_ID_FMT
  ...
  OAILOG_DEBUG(
      LOG_NAS_EMM,
      "EMM-PROC-  Tracking Area Update request for new UeId " MME_UE_S1AP_ID_FMT
      ". TAU_Type=%d, active_flag=%d)\n",
      ue_id, ies->eps_update_type.eps_update_type_value,
  ...
  if (!(*duplicate_emm_ue_ctx_pP)) {
     ...
     * GUTI will always be there. Checking for validity of the GUTI via the
     * validity of the TMSI. */
    if ((INVALID_M_TMSI != ies->old_guti.m_tmsi)) {
      if (((*duplicate_emm_ue_ctx_pP) =
               emm_data_context_get_by_guti(&_emm_data, &ies->old_guti)) !=
          NULL) { /**< May be set if S-TMSI is set. */
        OAILOG_DEBUG(LOG_NAS_EMM,
                     "EMM-PROC-  Found a valid UE with correct GUTI " GUTI_FMT
                     " and (old) ue_id " MME_UE_S1AP_ID_FMT
                     ". "
    ...
    if (!(*duplicate_emm_ue_ctx_pP)) {
      /** Check if there is an ongoing handover procedure and if the IMSI
       * already exists. */
      if (!ies->is_initial) {
        ...
        if (s10_handover_proc) {
          ...
          OAILOG_INFO(
              LOG_NAS_EMM,
              "EMM-PROC  - We have receive the TAU as part of an S10 procedure "
          ...
          if (((*duplicate_emm_ue_ctx_pP) = emm_data_context_get_by_imsi(
                   &_emm_data, s10_handover_proc->nas_s10_context.imsi)) !=
              NULL) { /**< May be set if S-TMSI is set. */
            OAILOG_DEBUG(LOG_NAS_EMM,
                         "EMM-PROC-  Found a valid UE with IMSI " IMSI_64_FMT
                         " and (old) ue_id " MME_UE_S1AP_ID_FMT
                         ". "
                         "Continuing with the Tracking Area Update Request. \n",
                         s10_handover_proc->nas_s10_context.imsi,
                         (*duplicate_emm_ue_ctx_pP)->ue_id);
          }
        }
      }
    ...
    OAILOG_DEBUG(
        LOG_NAS_EMM,
        "EMM-PROC-  Found a valid UE with new mmeUeS1apId " MME_UE_S1AP_ID_FMT
        " and GUTI " GUTI_FMT
        ". "
        "Continuing with the Tracking Area Update Request. \n",
        ue_id, GUTI_ARG(&(*duplicate_emm_ue_ctx_pP)->_guti));
  }
  /** No IMSI as IE @ TAU-Req. Check the validity of the EMM context and the
   * request. */
  if ((*duplicate_emm_ue_ctx_pP)) { /**< We found a matching EMM context via
    ...
    rc = emm_proc_tracking_area_update_request_validity(
        (*duplicate_emm_ue_ctx_pP), ue_id, ies);
    ...
    if (rc != RETURNok) {
      ...
      OAILOG_WARNING(
          LOG_NAS_EMM,
          "EMM-PROC  - Not continuing with the Tracking-Area-Update-Request. "
          "\n"); /**< All rejects should be sent and everything should be
                    handled by now. */
      OAILOG_FUNC_RETURN(LOG_NAS_EMM, rc);
    }
  }
  /** After validation. Continuing with an existing or new to be created EMM
   * context. */
  OAILOG_INFO(LOG_NAS_EMM,
              "EMM-PROC  - Continuing to handle the new "
  ...
  if (!(*duplicate_emm_ue_ctx_pP)) {
    OAILOG_INFO(
        ...
        ue_id);
    /*
     * Create UE's EMM context todo: THIS IS NOT OK --> WE MIGHT NOT BE ABLE TO
     * CREATE NEW CONTEXT!
     */
    new_emm_ue_context =
    ...
    emm_init_context(
        new_emm_ue_context); /**< Initialize the context, we might do it again
                                if the security was not verified. No IMSI, GUTI
                                set so adding should work. */
    ...
    OAILOG_INFO(LOG_NAS_EMM,
                "EMM-PROC  - No valid old EMM context was found for "
    ...
    if (ue_id == (*duplicate_emm_ue_ctx_pP)->ue_id) {
      OAILOG_ERROR(LOG_NAS_EMM,
                   ...
                   "ueId " MME_UE_S1AP_ID_FMT
                   ". Rejecting TAU (should also implicitly detach). \n",
                   ue_id);
      rc = _emm_tracking_area_update_reject(ue_id,
    ...
    //    emm_context->additional_update_type = ies->additional_update_type;
    /**
     * BY NO MEANS WE CAN ADD HERE AN EMM CONTEXT WITH SAME UE_ID!!!!!
     * OVERWRITING INEVITABLE AND CAUSES LEAKS!!!
     */
    new_emm_ue_context->emm_cause = EMM_CAUSE_SUCCESS;
    /**
     * Initialize the context, we might do it again if the security was not
     * verified. IMSI and GUTI are cleared, so we should have no problems with
     * adding the EMM context. The old duplicate context should still be there
     * and intact. Will be removed outside this function and new emm_context
     * will continue @ retry.
     */
    ...
    if ((*duplicate_emm_ue_ctx_pP)->ue_id != ue_id) {
      ...
      memcpy(new_emm_ue_context, (*duplicate_emm_ue_ctx_pP),
      ...
      subscription_data_t *subscription_data =
          mme_api_remove_subscription_data(new_emm_ue_context->_imsi64);
      ...
      void *unused = NULL;
      _emm_proc_create_procedure_tracking_area_update_request(
          new_emm_ue_context, ies, _emm_tau_retry_procedure);
      nas_emm_tau_proc_t *tau_proc =
          get_nas_specific_procedure_tau(new_emm_ue_context);
      nas_stop_T_retry_specific_procedure(new_emm_ue_context->ue_id,
                                          &tau_proc->emm_spec_proc.retry_timer,
                                          unused);
      nas_start_T_retry_specific_procedure(
          new_emm_ue_context->ue_id, &tau_proc->emm_spec_proc.retry_timer,
          tau_proc->emm_spec_proc.retry_cb, (void *)new_emm_ue_context->ue_id);
      ...
      *duplicate_emm_ue_ctx_pP = NULL;
       ...
       * detection. Not continuing with the new TAU procedure until the old one
       * has been successfully deallocated.
       */
      ...
      emm_init_context(new_emm_ue_context);
      DevAssert(RETURNok ==
                emm_data_context_add(&_emm_data, new_emm_ue_context));
  ...
  _emm_proc_create_procedure_tracking_area_update_request(
      new_emm_ue_context, ies,
      _emm_tau_retry_procedure); /**< Created TAU procedure might be floating
                                    (not registered to any list yet). */
  if ((*duplicate_emm_ue_ctx_pP)) {
    /** Set the new tau procedure into pending mode and continue with it after
     * the completion of the duplicate removal. */
    void *unused = NULL;
    nas_emm_tau_proc_t *tau_proc =
        get_nas_specific_procedure_tau(new_emm_ue_context);
    nas_stop_T_retry_specific_procedure(new_emm_ue_context->ue_id,
     ...
     * Not continuing with the new TAU procedure until the old one has been
    ...
    if (rc != RETURNok) {
      ...
      rc = _emm_tracking_area_update_reject(new_emm_ue_context->ue_id,
                                            new_emm_ue_context->emm_cause);
 ...
 **                                                                        **
 ***************************************************************************/
int emm_wrapper_tracking_area_update_reject(const mme_ue_s1ap_id_t ue_id,
...
int emm_proc_tracking_area_update_complete(mme_ue_s1ap_id_t ue_id) {
  ...
  if (emm_context) {
    if (is_nas_specific_procedure_tau_running(emm_context)) {
      ...
      /*
       * Upon receiving an TRACKING AREA UPDATE COMPLETE message, the MME shall
       * enter state EMM-REGISTERED and consider the GUTI sent in the TRACKING
       * AREA UPDATE ACCEPT message as valid.
       */
      ...
      /*
       * Upon receiving an ATTACH COMPLETE message, the MME shall stop timer
       * T3450
       */
      ...
      MSC_LOG_EVENT(MSC_NAS_EMM_MME, "T3450 stopped UE " MME_UE_S1AP_ID_FMT " ",
                    ue_id);
      /*
      ...
      OAILOG_INFO(LOG_NAS_EMM,
                  ...
                  " TRACKING AREA UPDATE COMPLETE discarded (EMM procedure not "
                  "found)\n",
                  ue_id);
    }
  } else {
    ...
    OAILOG_INFO(
        LOG_NAS_EMM,
        "UE " MME_UE_S1AP_ID_FMT
        " TRACKING AREA UPDATE COMPLETE discarded (context not found)\n",
        ue_id);
    OAILOG_FUNC_RETURN(LOG_NAS_EMM, RETURNerror);
  }
  ...
  if (emm_context && emm_context->_emm_fsm_state != EMM_REGISTERED) {
    OAILOG_WARNING(
        ...
        " is still not in EMM_REGISTERED state although TAU_CNF has arrived. "
...
static int emm_proc_tracking_area_update_request_validity(
  ...
  if (emm_context->ue_id != new_ue_id) {
    OAILOG_WARNING(LOG_NAS_EMM,
                   ...
                   " it not equal to new ueId" MME_UE_S1AP_ID_FMT ". \n",
  ...
  if (is_nas_common_procedure_smc_running(emm_context)) {
    ...
    MSC_LOG_TX_MESSAGE(MSC_NAS_EMM_MME, MSC_NAS_EMM_MME, NULL, 0,
                       "0 EMMREG_PROC_ABORT (SMC) ueId" MME_UE_S1AP_ID_FMT " ",
                       emm_context->mme_ue_s1ap_id);
    rc = emm_sap_send(&emm_sap);
    /** Continue with the specific procedure. May or may not implicitly detach
     * the EMM context. */
    OAILOG_FUNC_RETURN(LOG_NAS_EMM, rc);
    /** We don't need to check the EMM context further. It is deallocated, but
     * continue with processing the TAU Request., return. */ // todo: check that the emm_ctx is nulled. (else, need to do it here).
  }
  /*
   ...
   * an tau procedure (no specific tau procedure is running - assuming all
  ...
  //  /*
  //   * UE may or may not be in COMMON-PROCEDURE-INITIATED state (common
  //   procedure active or implicit GUTI reallocation), DEREGISTERED or already
  //   REGISTERED.
  ...
  //    OAILOG_TRACE (LOG_NAS_EMM, "EMM-PROC  - the new ATTACH REQUEST is
  //    progressed and the current registered EMM context, "
  //        "together with the states, common procedures and bearers will be
  //        destroyed. \n");
  ...
  //     destroy them, too.
  //     * We don't need to check if a TAU procedure is running, too. It will be
  //     destroyed in the implicit detach.
  //     * It will destroy any EMM context. EMM_CTX wil be nulled (not
  //     EMM_DEREGISTERED).
  //     */
  //    // Trigger clean up
  //    emm_sap_t                               emm_sap = {0};
  //    emm_sap.primitive = EMMCN_IMPLICIT_DETACH_UE; /**< UE context will be
  //    purged. */ emm_sap.u.emm_cn.u.emm_cn_implicit_detach.ue_id =
  //    emm_context->ue_id;
  //    /** Don't send the detach type, such that no NAS Detach Request is sent
  //    to the UE. */ rc = emm_sap_send (&emm_sap);
  //    /** Additionally, after performing a NAS implicit detach, reset all the
  //    EMM context (the UE is in DEREGISTERED state, we reset it to invalid
  //    state. */
  //    // todo: cleaning up!             clear_emm_ctxt = true;
  //    OAILOG_FUNC_RETURN(LOG_NAS_EMM, rc); /**< Return with the deallocated
  //    (NULL) EMM context and continue to process the Attach Request. */
  //  }
  /*
   ...
   */
  if (attach_procedure) {
    if (is_nas_attach_accept_sent(
            attach_procedure)) {  // && (!emm_ctx->is_attach_complete_received):
                                  // implicit
      REQUIREMENT_3GPP_24_301(R10_5_5_1_2_7_g);
      OAILOG_WARNING(
          LOG_NAS_EMM,
          "EMM-PROC  - TAU request received for IMSI " IMSI_64_FMT
          " with mme_ue_s1ap_id " MME_UE_S1AP_ID_FMT
          " while an Attach Procedure is running (Attach Complete pending). \n",
          emm_context->_imsi64, emm_context->ue_id);
      /** GUTI must be validated. */
      emm_ctx_set_attribute_valid(emm_context, EMM_CTXT_MEMBER_GUTI);
      /*
       * Abort the specific attach procedure and perform an implicit NAS detach,
       * without sending a Detach Request to the UE (stops the T3450 timer).
       * Aborting an attach procedure currently performs an IMPLICIT Detach (as
       * defined in the specification), but also removes the EMM context. todo:
       * Making optional such that EMM context is only DEREGISTERED with
       * validated GUTI?
       *
       * Currently, in the IMPLICIT DETACH procedure, the EMM context will be
       * removed completely, so validating GUTI for the next attach is
       * meaningless here.
       */
      emm_sap_t emm_sap = {0};
      ...
      rc = emm_wrapper_tracking_area_update_reject(
          new_ue_id,
          EMM_CAUSE_IMPLICITLY_DETACHED); /**< Will remove the contexts for the
                                             TAU-Req. */
      free_emm_tau_request_ies(&ies);
      OAILOG_FUNC_RETURN(LOG_NAS_EMM, RETURNerror);
    } else {
      OAILOG_WARNING(LOG_NAS_EMM,
                     "EMM-PROC  - TAU request received for IMSI " IMSI_64_FMT
                     " with mme_ue_s1ap_id " MME_UE_S1AP_ID_FMT
                     " while an Attach Procedure is running (Attach "
                     "Accept/Reject not sent yet). Discarding TAU Request. \n",
                     emm_context->_imsi64, emm_context->ue_id);
      /** TAU before ATTACH_ACCEPT sent should be rejected directly. */
      //      rc =
      //      emm_wrapper_tracking_area_update_reject(attach_procedure->ue_id,
      //      EMM_CAUSE_MSC_NOT_REACHABLE); /**< Will remove the contexts for
      //      the TAU-Req. */
  ...
  // */
  if (tau_procedure) {
    if (is_nas_tau_accept_sent(
            tau_procedure) /* || is_nas_tau_reject_sent(tau_procedure) */) {  // && (!){
      if (_emm_tracking_area_update_ies_have_changed(emm_context->ue_id,
                                                     tau_procedure->ies, ies)) {
        OAILOG_WARNING(LOG_NAS_EMM,
                       "EMM-PROC  - TAU Request parameters have changed\n");
         ...
         * previous TRACKING AREA UPDATE REQUEST message, the previously
         * initiated tracking area updating procedure shall be aborted if the
         * TRACKING AREA UPDATE COMPLETE message has not been received and the
         * new tracking area updating procedure shall be progressed;
         */
        emm_sap_t emm_sap = {0};
        ...
        rc = emm_sap_send(&emm_sap);
        // trigger clean up
        /** Set the EMM cause to invalid. We process the new request with new
         * parameters --> create a new EMM context. */
        emm_context->emm_cause = EMM_CAUSE_ILLEGAL_UE;
         ...
         * restarted if an ATTACH COMPLETE message is expected. In that case,
         * the retransmission counter related to T3450 is not incremented.
         */
        _emm_tracking_area_update_accept_retx(
            emm_context); /**< Resend the TAU_ACCEPT with the old EMM context
                             (with old UE IDs). */
        ...
        OAILOG_WARNING(LOG_NAS_EMM,
                       "EMM-PROC  - Received duplicated TAU Request\n");
        OAILOG_FUNC_RETURN(LOG_NAS_EMM, RETURNerror);
      }
    } else if (!is_nas_tau_accept_sent(tau_procedure) &&
               (!is_nas_tau_reject_sent(
                   tau_procedure))) { /**< TAU accept or reject not sent yet. */
      //       REQUIREMENT_3GPP_24_301(R10_5_5_3_2_7_e__1);
      if (_emm_tracking_area_update_ies_have_changed(emm_context->ue_id,
                                                     tau_procedure->ies, ies)) {
        OAILOG_WARNING(LOG_NAS_EMM,
                       "EMM-PROC  - TAU parameters have changed in an ongoing "
                       "TAU Request handling. \n");
        /**
         * If one or more of the information elements in the TAU REQUEST message
         * differs from the ones received within the previous TAU REQUEST
         * message, the previously initiated attach procedure shall be aborted
         * and the new tau procedure shall be executed; The TAU accept/reject
         * has not been sent yet. Check if this UE has been attached to the
         * current MME or is trying to do an inter-MME TAU.
         */
        /** The UE has not been attached to the current MME. Abort the process
         * and purge the current context. */
        emm_sap_t emm_sap = {0};
        emm_sap.primitive = EMMREG_TAU_ABORT;
        emm_sap.u.emm_reg.ue_id = tau_procedure->ue_id;
         ...
         * continue with the previous tau procedure and shall ignore the second
         * TAU REQUEST message.
         */
  ...
  /** If the TAU request was received in EMM_DEREGISTERED or
   * EMM_COMMON_PROCEDURE_INITIATED state, we should remove the context
   * implicitly. */
  if (!(EMM_REGISTERED == fsm_state ||
        EMM_COMMON_PROCEDURE_INITIATED == fsm_state)) {
    OAILOG_WARNING(
        LOG_NAS_EMM,
        "EMM-PROC  - Received TAU Request while state is not in "
        "EMM_REGISTERED/EMM_COMMON_PROCEDURE_INITIATED, instead %d. \n",
        fsm_state);
    /** Set the EMM cause to invalid. */
    emm_context->emm_cause = EMM_CAUSE_ILLEGAL_UE;
  ...
  /** Check that the UE has a valid security context and that the message could
  ...
  if (!(IS_EMM_CTXT_PRESENT_SECURITY(emm_context) &&
    ...
    int decoder_rc = 0;
    decoder_rc =
        nas_message_decode(ies->complete_tau_request->data, &nas_msg,
                           blength(ies->complete_tau_request),
                           &emm_context->_security, NULL, &ies->decode_status);

    if (decoder_rc < 0) {
      OAILOG_WARNING(LOG_NAS_EMM,
                     "EMM-PROC  - Received TAU Request while UE context does "
                     "not have a valid security context (could not decode, "
                     "rc=%d). Performing an implicit detach. \n",
                     decoder_rc);
      /** Set the EMM cause to invalid. */
      emm_context->emm_cause = EMM_CAUSE_ILLEGAL_UE;
      /** Continue to handle the TAU Request inside a new EMM context. */
    ...
    if (!ies->decode_status.mac_matched) {
      OAILOG_WARNING(LOG_NAS_EMM,
                     "EMM-PROC  - Received TAU Request while UE context does "
                     "not have a valid security context (mac not matched). "
                     "Performing an implicit detach. \n");
      /** Set the EMM cause to invalid. */
      emm_context->emm_cause = EMM_CAUSE_ILLEGAL_UE;
      /** Continue to handle the TAU Request inside a new EMM context. */
      OAILOG_FUNC_RETURN(LOG_NAS_EMM, RETURNok);
  ...
  /** Return Ok: Can process with the TAU request (and EMM context is
   * validated). */
  OAILOG_FUNC_RETURN(LOG_NAS_EMM, RETURNok);
}

static void _emm_proc_create_procedure_tracking_area_update_request(
    emm_data_context_t *const emm_context, emm_tau_request_ies_t *const ies,
    retry_cb_t retry_cb) {
  ...
  if (tau_proc) {
    ...
    ((nas_emm_base_proc_t *)tau_proc)->time_out =
        _emm_tracking_area_update_t3450_handler;
    /** Set the MME_APP registration complete procedure for callback. */
    ((nas_emm_base_proc_t *)tau_proc)->success_notif =
        _emm_tracking_area_update_registration_complete;
 ...
 * expect TAU Complete from UE as we dont support implicit GUTI re-allocation
 * during TAU procedure.
 */
#if 0
...
static void _emm_tracking_area_update_t3450_handler(void *args) {
  ...
  if (!(emm_context)) {
    OAILOG_ERROR(LOG_NAS_EMM,
                 "T3450 timer expired No EMM context for UE " MME_UE_S1AP_ID_FMT
                 ". \n",
                 ue_id);
...
int emm_wrapper_tracking_area_update_accept(
  ...
  if (!emm_context) {
    OAILOG_ERROR(LOG_NAS_EMM,
                 ...
                 " exists. Cannot proceed with TAU.",
                 ue_id);
    OAILOG_FUNC_RETURN(LOG_NAS_EMM, RETURNerror);
  }
  ...
  if (!tau_proc) {
    OAILOG_ERROR(LOG_NAS_EMM,
                 "EMM-PROC  - No TAU procedure for ue_id " MME_UE_S1AP_ID_FMT
                 " exists. Cannot proceed with TAU. Ignoring the message.",
                 ue_id);
    OAILOG_FUNC_RETURN(LOG_NAS_EMM, RETURNerror);
  }
...
/** \fn void _emm_tracking_area_update_accept (emm_data_context_t *
...
//------------------------------------------------------------------------------
static int _emm_send_tracking_area_update_accept(
    emm_data_context_t *const emm_context, nas_emm_tau_proc_t *const tau_proc) {
  OAILOG_FUNC_IN(LOG_NAS_EMM);
  int rc = RETURNok;
  emm_sap_t emm_sap = {0};
  int i = 0;
  ...
  /* If active flag is not set to true in TAU request then just send TAU accept.
   * After sending TAU accept initiate S1 context release procedure for the UE
   * if new GUTI is not sent in TAU accept message. Note - At present implicit
   * GUTI reallocation is not supported and hence GUTI is not sent in TAU accept
   * message.
   */
  if (tau_proc->ies->is_initial) {
    /**
     * Check the active flag. If false, set a notification to release the
     * bearers after TAU_ACCEPT/COMPLETE (depending on the EMM state).
     */
    if (!tau_proc->ies->eps_update_type.active_flag) {
      if (!tau_proc->pending_qos) {
        emm_sap.primitive = EMMAS_DATA_REQ;
        emm_sap.u.emm_as.u.data.pending_deac =
            true; /**< Only set for emm_as.data. */
      } else {
        OAILOG_WARNING(LOG_NAS_EMM,
                       "EMM-PROC  - TAU procedure for ue_id " MME_UE_S1AP_ID_FMT
                       " has pending QoS. Overwriting the active flag.",
      ...
      /**
       * Notify EMM-AS SAP that Tracking Area Update Accept message together
       * with an Activate Default EPS Bearer Context Request message has to be
       * sent to the UE.
       *
       ...
       * MME releases the signaling connection with UE, according to
       * clause 5.3.5.
       */
      emm_sap.primitive = EMMAS_ESTABLISH_CNF;
    }
  } else {
    emm_sap.primitive = EMMAS_DATA_REQ; /**< We also check the current ECM state
                                           to handle active flag. */
  }

  /** Set the rest as data. */
  emm_sap.u.emm_as.u.data.ue_id =
      emm_context->ue_id; /**< These should also set for data. */
  ...
  if (tau_proc->ies->ms_network_capability) {
    emm_ctx_set_valid_ms_nw_cap(emm_context,
                                tau_proc->ies->ms_network_capability);
  ...
  if (tau_proc->ies->ue_network_capability) {
    emm_ctx_set_valid_ue_nw_cap(emm_context,
                                tau_proc->ies->ue_network_capability);
  } else {
    // optional IE
    /** Not clearing them, we might have received it from S10. */
  }
  ...
  if (!IS_EMM_CTXT_PRESENT_GUTI(emm_context)) {
    ...
    rc = mme_api_new_guti(
        &emm_context->_imsi, &old_guti, &guti, &emm_context->originating_tai,
        &emm_context
             ->_tai_list); /**< This will increment/update the TAI_LIST. */
    if (RETURNok != rc) {
      OAILOG_WARNING(LOG_NAS_EMM,
                     "EMM-PROC- Error allocating GUTI @ TAU_ACCEPT for "
                     "ue_id=" MME_UE_S1AP_ID_FMT ", \n",
                     emm_context->ue_id);
      OAILOG_FUNC_RETURN(LOG_NAS_EMM, rc);
    }
    emm_ctx_set_guti(emm_context,
                     &guti); /**< Set the GUTI as present and continue. */
    ...
    if (_emm_data.conf.force_tau) {
      ...
      OAILOG_INFO(LOG_NAS,
                  "UE " MME_UE_S1AP_ID_FMT
                  " has an intra-MME TAU. Only updating the TAI-List to new "
                  "TAC " TAC_FMT ".\n",
                  ue_context->privates.mme_ue_s1ap_id,
                  emm_context->originating_tai.tac);
      emm_context->_tai_list.numberoflists = 1;
      emm_context->_tai_list.partial_tai_list[0].numberofelements =
          0; /**< + 1. */
      emm_context->_tai_list.partial_tai_list[0].typeoflist =
          TRACKING_AREA_IDENTITY_LIST_ONE_PLMN_CONSECUTIVE_TACS;
      emm_context->_tai_list.partial_tai_list[0]
  ...
  memcpy(&emm_sap.u.emm_as.u.data.tai_list, &emm_context->_tai_list,
         sizeof(tai_list_t)); /**< Updated in the EMM context with new GUTI
                                 reallocation. */
  ...
  if (!IS_EMM_CTXT_VALID_GUTI(emm_context) &&
    ...
    OAILOG_INFO(LOG_NAS_EMM,
                ...
                "assigned GUTI in the Tracking Area Update Accept message\n",
                emm_context->ue_id);
    ...
    OAILOG_INFO(
        LOG_NAS_EMM,
        "ue_id=" MME_UE_S1AP_ID_FMT " EMM-PROC  - UE with IMSI " IMSI_64_FMT
        " has already a valid GUTI " GUTI_FMT
        ". "
  ...
  emm_sap.u.emm_as.u.data.eps_update_result =
      (tau_proc->ies->eps_update_type.eps_update_type_value == 1 ||
       tau_proc->ies->eps_update_type.eps_update_type_value == 2)
          ? 1
          : 0; /**< Set the UPDATE_RESULT irrelevant of data/establish. */
  if (tau_proc->ies->eps_bearer_context_status)
    emm_sap.u.emm_as.u.data.eps_bearer_context_status =
        *(tau_proc->ies->eps_bearer_context_status);
  ...
  rc = emm_sap_send(
      &emm_sap); /**< This may be a DL_DATA_REQ or an ESTABLISH_CNF, initiating
                    an S1AP connection. */
  if (rc != RETURNerror) {
    ...
    if (emm_context1) tau_proc1 = get_nas_specific_procedure_tau(emm_context1);
    if (emm_context1 && tau_proc1) {
      if (emm_sap.u.emm_as.u.establish.new_guti !=
          NULL) { /**< A new GUTI was included. Start the timer to wait for
                     TAU_COMPLETE. */
                  /*
        ...
        nas_start_T3450(ue_id, &tau_proc1->T3450,
                        tau_proc1->emm_spec_proc.emm_proc.base_proc.time_out,
                        (void *)ue_id);
        //      unlock_ue_contexts(ue_context);
        OAILOG_FUNC_RETURN(LOG_NAS_EMM, rc);

        //        OAILOG_INFO (LOG_NAS_EMM, "EMM-PROC  - Timer T3450 (%d)
        //        expires in %ld seconds (TAU)", emm_ctx->T3450.id,
        //        emm_ctx->T3450.sec);
        //      }
      }
    }
...
static int _emm_tracking_area_update_accept(
  ...
  if ((tau_proc) && (tau_proc->ies)) {
     ...
     */
    if (tau_proc->ies->eps_update_type.eps_update_type_value ==
        EPS_UPDATE_TYPE_PERIODIC_UPDATING) {
      /**
       * No common procedure needs to be triggered, and no TAU_ACCEPT_DATA needs
       * to be stored. UE will be in the same EMM_REGISTERED state.
       */
      if (emm_context->_emm_fsm_state != EMM_REGISTERED) {
        OAILOG_ERROR(
            LOG_NAS_EMM,
            "EMM-PROC  - IMSI " IMSI_64_FMT
            " is not in EMM_REGISTERED mode but instead %d for "
            "PERIODIC_TAU_ACCEPT. "
            "Sending TAU REJECT back and implicitly removing contexts. \n",
            emm_context->_imsi64, emm_context->_emm_fsm_state);
        rc = _emm_tracking_area_update_reject(emm_context->ue_id,
                                              SYSTEM_FAILURE);
      } else {
        /** Send a periodic TAU_ACCEPT back without GUTI, COMMON_PROCEDURE
         * initiation or expecting a TAU_COMPLETE. */
         ...
         * Since we area in EMM_REGISTERED state, check the pending session
         * releasion flag and release the state. Sending 2 consecutive SAP
         * messages are anyway possible.
         */
    ...
    /**
     * If no valid GUTI is present, a new GUTI will be allocated in the
     * following tau_accept method. It will determine if COMMON procedure is
     * activated or the timer T3450 will be started. If a GUTI exists, the UE
     * must be in EMM-REGISTERED state, else we send a reject and implicitly
     * detach the UE context.
     */
    if (IS_EMM_CTXT_VALID_GUTI(
            emm_context)) { /**< If its invalid --> directly enter
                               EMM-REGISTERED state from EMM-DEREGISTERED state,
                               no T3450, no COMMON_PROCEDURE. */
      /** Assert that the UE is in REGISTERED state. */
      //      if(emm_context->_emm_fsm_state != EMM_REGISTERED){
      //        OAILOG_ERROR(LOG_NAS_EMM, "EMM-PROC  - IMSI " IMSI_64_FMT " has
      //        already a valid GUTI "GUTI_FMT " but is not in EMM-REGISTERED
      //        state, instead %d. \n",
      //            emm_context->_imsi64, GUTI_ARG(&emm_context->_guti),
      //            emm_context->_emm_fsm_state);
      //        rc = _emm_tracking_area_update_reject (emm_context->ue_id,
      //        SYSTEM_FAILURE); OAILOG_FUNC_RETURN (LOG_NAS_EMM, RETURNerror);
      //      }
      OAILOG_WARNING(LOG_NAS_EMM,
                     "EMM-PROC  - IMSI " IMSI_64_FMT
                     " has already a valid GUTI " GUTI_FMT
                     ". A new GUTI will not be allocated and send. UE staying "
                     "in EMM_REGISTERED state. \n",
                     emm_context->_imsi64, GUTI_ARG(&emm_context->_guti));
      /**
       * Not allocating GUTI. No parameters should be validated here.
       * All the parameters should already be validated with the
       * TAU_COMPLETE/ATTACH_COMPLETE before! todo: check that there are no new
       * parameters (DRX, UE/MS NC,).
       */
       ...
       * Might release bearers depending on the ECM state and the active flag
       * without waiting for TAU-Complete.
       */
      ...
      if (emm_context->_emm_fsm_state != EMM_REGISTERED) {
        OAILOG_INFO(LOG_NAS_EMM,
                    "EMM-PROC  - IMSI " IMSI_64_FMT
                    " for UE_ID " MME_UE_S1AP_ID_FMT
                    " not in EMM-REGISTERED state but in state %d. \n",
                    emm_context->_imsi64, emm_context->ue_id,
                    emm_context->_emm_fsm_state);
        emm_sap.primitive = EMMREG_TAU_CNF;
        emm_sap.u.emm_reg.ue_id = emm_context->ue_id;
      ...
      //      DevAssert(emm_context->_emm_fsm_status == EMM_REGISTERED);
      /**
       * The NAS_INFO (EMM_AS_NAS_DATA_TAU) --> will trigger a bearer removal if
       * the UE is in EMM_REGISTERED state, later in (_emm_as_data_req),
       * depending on the pending bearer removal flag.
       */

      // todo: if later a new GUTI is still send, although a VALID GUTI exists,
      // invalidate the existing valid GUTI of the emm_ctx before sending
      // tau_accept with new_guti.
      OAILOG_FUNC_RETURN(LOG_NAS_EMM, rc);

    } else if (IS_EMM_CTXT_PRESENT_GUTI(emm_context)) {
      /** This case should be handled in the old_context verification when the
       * TAU_REQUEST is arrived. */
      OAILOG_ERROR(LOG_NAS_EMM,
                   "EMM-PROC  - IMSI " IMSI_64_FMT
                   " has a PRESENT but INVALID GUTI " GUTI_FMT
                   ". This indicated a TAU/ATTACH Request "
                   "message is received before the complete message for the "
                   "request before is received by the MME. UE_State %d. \n",
                   emm_context->_imsi64, GUTI_ARG(&emm_context->_guti),
                   emm_context->_emm_fsm_state);
      rc = _emm_tracking_area_update_reject(emm_context->ue_id, SYSTEM_FAILURE);
      OAILOG_FUNC_RETURN(LOG_NAS_EMM, RETURNerror);
    } else if (!IS_EMM_CTXT_PRESENT_OLD_GUTI(
                   emm_context)) { /**< Cleared with TAU/ATTACH Complete. Should
                                      be received with TAU_REQUEST. */
      /** If we received a TAU-Request where no OLD-GUTI is present and also no
       * VALID GUTI, its an error. */
      OAILOG_ERROR(
          LOG_NAS_EMM,
          "EMM-PROC  - IMSI " IMSI_64_FMT
          " has not valid GUTI and OLD-GUTI is not present. UE_State %d. \n",
          emm_context->_imsi64, emm_context->_emm_fsm_state);
      rc = _emm_tracking_area_update_reject(emm_context->ue_id, SYSTEM_FAILURE);
      OAILOG_FUNC_RETURN(LOG_NAS_EMM, RETURNerror);
    }
    /**
     * Neither valid nor present GUTI at the MME. Allocating a new GUTI,
     * entering the COMMON_PROCEDURE_INITIATEd state and starting the T3450
     * timer. No common process needs to be allocated. No common procedure needs
     * to be initialized. It will be done outside of this in emm_cn.
     */
    /**
     * Set the active_flag indicator and t
     */
    //    tau_accept_data_p->active_flag = epsupdatetype->activeflag;
    //    tau_accept_data_p->eps_update_type =
    //    epsupdatetype->epsupdatetypevalue;
    /**
     * No need to free the tau_data, since in this case, we always send new GUTI
     * and expect TAU_COMPLETE! Inside the TAU_ACCEPT procedure, it will be
     * checked if a new GUTI is/will be allocated (currently depends on
     * active_flag. If so, the T3450 timer will be started in the TAU_ACCEPT
     * method already.
     *
     * The pending bearer removal flag may or may not be set. If the state is
     * COMMON, the bearer will be idled when TAU_COMPLETE received and UE enters
     * EMM_REGISTERED state (in the callback function). If TAU_COMPLETE does not
     * arrive, the T3450 timer will implicitly detach the UE anyway.
     *
    ...
    if (IS_EMM_CTXT_PRESENT_OLD_GUTI(emm_context) &&
      ...
      OAILOG_ERROR(
          LOG_NAS_EMM,
          "EMM-PROC  - No new GUTI could be allocated for IMSI " IMSI_64_FMT
          ". \n",
          emm_context->_imsi64);
      rc = _emm_tracking_area_update_reject(emm_context->ue_id, SYSTEM_FAILURE);
    }
    ...
    OAILOG_ERROR(LOG_NAS_EMM,
                 "EMM-PROC  - No attach process running for IMSI " IMSI_64_FMT
                 " with ueId " MME_UE_S1AP_ID_FMT ". \n",
                 emm_context->_imsi64, emm_context->ue_id);
    rc = _emm_tracking_area_update_reject(emm_context->ue_id, SYSTEM_FAILURE);
  }
  OAILOG_FUNC_RETURN(LOG_NAS_EMM, rc);
...
static int _emm_tracking_area_update_abort(
  ...
  if (tau_proc) {
    ...
    //     * Currently, for _emm_attach_abort() and for
    //     _emm_tracking_area_update_abort() we implicitly detach the UE.
    //     * todo: should this not be  done with DEREGISTER? (leaving the
    //     EMM/Security context)
    //     * todo: how to define in NAS_IMPLICIT_DETACH if EMM/Security context
    //     should stay or not?
    //     */
    //    emm_sap_t                               emm_sap = {0};
    //
...
static int _emm_tracking_area_update_accept_retx(
  ...
  if (tau_proc) {
    /*
     * Notify EMM-AS SAP that Tracking Area Update Accept message has to be sent
     * to the UE. Retx of Tracking Area Update Accept needs to be done via DL
     * NAS Transport S1AP message
     */
    emm_sap.primitive = EMMAS_DATA_REQ;
    emm_sap.u.emm_as.u.data.ue_id = ue_id;
    emm_sap.u.emm_as.u.data.nas_info = EMM_AS_NAS_DATA_TAU;
    emm_sap.u.emm_as.u.data.tai_list.numberoflists =
        emm_context->_tai_list.numberoflists;
    for (i = 0; i < emm_context->_tai_list.numberoflists; i++) {
      memcpy(&emm_sap.u.emm_as.u.data.tai_list.partial_tai_list[i],
             &emm_context->_tai_list.partial_tai_list[i], sizeof(tai_t));
    }

    emm_sap.u.emm_as.u.data.eps_id.guti = &emm_context->_guti;
    OAILOG_DEBUG(LOG_NAS_EMM,
                 "ue_id=" MME_UE_S1AP_ID_FMT
                 " EMM-PROC  - Include the same GUTI in the TAU Retx message\n",
                 ue_id);
    emm_sap.u.emm_as.u.data.new_guti = &emm_context->_guti;
    emm_sap.u.emm_as.u.data.eps_network_feature_support =
        &_emm_data.conf.eps_network_feature_support;

    //    /** If the GUTI is not validated, add the GUTI, else don't add the
    //    GUTI, not to enter COMMON state. */
    //      if(!IS_EMM_CTXT_VALID_GUTI(emm_ctx)){
    //        emm_sap.u.emm_as.u.data.eps_id.guti = &emm_ctx->_guti;
    //        OAILOG_DEBUG (LOG_NAS_EMM, "ue_id=" MME_UE_S1AP_ID_FMT " EMM-PROC
    //        - Include the same GUTI " GUTI_FMT " in the Tracking Area Update
    //        Accept Retx message. \n", emm_ctx->ue_id,
    //        GUTI_ARG(&emm_ctx->_guti)); emm_sap.u.emm_as.u.data.new_guti    =
    //        &emm_ctx->_guti;
    //      }else{
    //        OAILOG_DEBUG (LOG_NAS_EMM, "ue_id=" MME_UE_S1AP_ID_FMT " EMM-PROC
    //        - Not including the validated GUTI " GUTI_FMT " in the Tracking
    //        Area Update Accept Retx message. \n", emm_ctx->ue_id,
    //        GUTI_ARG(&emm_ctx->_guti));
    //      }
    /*
     * Setup EPS NAS security data
     */
    ...
    if (RETURNerror != rc) {
      ...
      if (emm_context1) {
        ...
        if (tau_proc) {
          OAILOG_INFO(LOG_NAS_EMM,
                      "ue_id=" MME_UE_S1AP_ID_FMT
                      " EMM-PROC  -Sent Retx TAU Accept message\n",
                      ue_id);
          /*
           * Re-start T3450 timer
           */
          ...
          nas_start_T3450(ue_id, &tau_proc->T3450,
                          tau_proc->emm_spec_proc.emm_proc.base_proc.time_out,
                          (void *)tau_proc->ue_id);
        } else {
          OAILOG_WARNING(LOG_NAS_EMM,
                         "ue_id=" MME_UE_S1AP_ID_FMT
                         " EMM-PROC  - Send failed- Retx TAU Accept message\n",
                         ue_id);
        }
      }
    }
...
/*
 * Description: Check whether the given tracking area update parameters differs
 * from those previously stored when the tracking area update procedure has been
 * initiated.
 *
 * Outputs:     None
...
//-----------------------------------------------------------------------------
static bool _emm_tracking_area_update_ies_have_changed(
    mme_ue_s1ap_id_t ue_id, emm_tau_request_ies_t *const ies1,
    emm_tau_request_ies_t *const ies2) {
  OAILOG_FUNC_IN(LOG_NAS_EMM);

  //  if (ies1->type != ies2->type) {
  //    OAILOG_INFO (LOG_NAS_EMM, "UE " MME_UE_S1AP_ID_FMT" TAU IEs changed:
  //    type EMM_ATTACH_TYPE\n", ue_id); OAILOG_FUNC_RETURN (LOG_NAS_EMM, true);
  //  }
  //  if (ies1->is_native_sc != ies2->is_native_sc) {
  //    OAILOG_INFO (LOG_NAS_EMM, "UE " MME_UE_S1AP_ID_FMT" TAU IEs changed: Is
  //    native security context\n", ue_id); OAILOG_FUNC_RETURN (LOG_NAS_EMM,
  //    true);
  //  }
  ...
  if (ies1->ksi != ies2->ksi) {
    OAILOG_INFO(LOG_NAS_EMM,
                "UE " MME_UE_S1AP_ID_FMT " TAU IEs changed: KSI %d -> %d \n",
  ...
  if ((ies1->old_guti_type) && (ies2->old_guti_type)) {
    if (*ies1->old_guti_type != *ies2->old_guti_type) {
      OAILOG_DEBUG(LOG_NAS_EMM,
                   "UE " MME_UE_S1AP_ID_FMT
                   " TAU IEs changed: Native GUTI %d -> %d \n",
  ...
  if (memcmp(&ies1->old_guti, &ies2->old_guti, sizeof(ies1->old_guti))) {
    OAILOG_INFO(LOG_NAS_EMM,
                "UE " MME_UE_S1AP_ID_FMT
                " GUTI IEs changed:  guti/tmsi " GUTI_FMT " -> " GUTI_FMT "\n",
                ue_id, GUTI_ARG(&ies1->old_guti), GUTI_ARG(&ies2->old_guti));
    OAILOG_FUNC_RETURN(LOG_NAS_EMM, true);

    // todo: verify, else
    //      * If the EMM context has a GUTI, the received OLD GUTI should be
    //      equal to the stored GUTI.
    //      */
    //     if ((old_guti) && (IS_EMM_CTXT_PRESENT_GUTI(ctx))) {
    //       if (old_guti->m_tmsi != ctx->_guti.m_tmsi) {
    //         OAILOG_INFO (LOG_NAS_EMM, "UE " MME_UE_S1AP_ID_FMT "  tau
    //         changed:  old_guti/tmsi " GUTI_FMT " -> " GUTI_FMT "\n",
    ...
    //           (old_guti->gummei.plmn.mcc_digit1 !=
    //           ctx->_guti.gummei.plmn.mcc_digit1) ||
    //           (old_guti->gummei.plmn.mcc_digit2 !=
    //           ctx->_guti.gummei.plmn.mcc_digit2) ||
    //           (old_guti->gummei.plmn.mcc_digit3 !=
    //           ctx->_guti.gummei.plmn.mcc_digit3) ||
    //           (old_guti->gummei.plmn.mnc_digit1 !=
    //           ctx->_guti.gummei.plmn.mnc_digit1) ||
    //           (old_guti->gummei.plmn.mnc_digit2 !=
    //           ctx->_guti.gummei.plmn.mnc_digit2) ||
    //           (old_guti->gummei.plmn.mnc_digit3 !=
    //           ctx->_guti.gummei.plmn.mnc_digit3)) {
    //         OAILOG_INFO (LOG_NAS_EMM, "UE " MME_UE_S1AP_ID_FMT "  tau
    //         changed:  old_guti/tmsi " GUTI_FMT " -> " GUTI_FMT "\n",
    //         ctx->ue_id, GUTI_ARG(&ctx->_guti), GUTI_ARG(old_guti));
    //         OAILOG_FUNC_RETURN (LOG_NAS_EMM, true);
  ...
  if ((ies1->last_visited_registered_tai) &&
    ...
    OAILOG_INFO(LOG_NAS_EMM,
                "UE " MME_UE_S1AP_ID_FMT " TAU IEs changed: LVR TAI " TAI_FMT
                "/NULL\n",
  ...
  if ((!ies1->last_visited_registered_tai) &&
    ...
    OAILOG_INFO(LOG_NAS_EMM,
                "UE " MME_UE_S1AP_ID_FMT
                " TAU IEs changed: LVR TAI NULL/" TAI_FMT "\n",
  ...
  //    OAILOG_INFO (LOG_NAS_EMM, "UE " MME_UE_S1AP_ID_FMT" TAU IEs changed:
  //    orig TAI " TAI_FMT "/NULL\n", ue_id, TAI_ARG(ies1->originating_tai));
  //    OAILOG_FUNC_RETURN (LOG_NAS_EMM, true);
  //  }
  //
  //  if ((!ies1->originating_tai) && (ies2->originating_tai)) {
  //    OAILOG_INFO (LOG_NAS_EMM, "UE " MME_UE_S1AP_ID_FMT" TAU IEs changed:
  //    orig TAI NULL/" TAI_FMT "\n", ue_id, TAI_ARG(ies2->originating_tai));
  //    OAILOG_FUNC_RETURN (LOG_NAS_EMM, true);
  //  }
  //
  //  if ((ies1->originating_tai) && (ies2->originating_tai)) {
  //    if (memcmp (ies1->originating_tai, ies2->originating_tai, sizeof
  //    (*ies2->originating_tai)) != 0) {
  //      OAILOG_INFO (LOG_NAS_EMM, "UE " MME_UE_S1AP_ID_FMT" TAU IEs changed:
  //      orig TAI " TAI_FMT "/" TAI_FMT "\n", ue_id,
  ...
  //
  //  if ((ies1->originating_ecgi) && (ies2->originating_ecgi)) {
  //    if (memcmp (ies1->originating_ecgi, ies2->originating_ecgi, sizeof
  //    (*ies2->originating_ecgi)) != 0) {
  //      OAILOG_INFO (LOG_NAS_EMM, "UE " MME_UE_S1AP_ID_FMT" TAU IEs changed:
  //      orig ECGI\n", ue_id); OAILOG_FUNC_RETURN (LOG_NAS_EMM, true);
  //    }
  //  }
  ...
  if (memcmp(ies1->ue_network_capability, ies2->ue_network_capability,
    ...
    OAILOG_INFO(LOG_NAS_EMM,
                "UE " MME_UE_S1AP_ID_FMT
                " TAU IEs changed: UE network capability\n",
                ue_id);
    OAILOG_FUNC_RETURN(LOG_NAS_EMM, true);
  }
  ...
  if ((ies1->ms_network_capability) && (!ies2->ms_network_capability)) {
    OAILOG_INFO(LOG_NAS_EMM,
                "UE " MME_UE_S1AP_ID_FMT
                " TAU IEs changed: MS network capability\n",
                ue_id);
    OAILOG_FUNC_RETURN(LOG_NAS_EMM, true);
  }
...
static int _emm_tau_retry_procedure(mme_ue_s1ap_id_t ue_id) {
  ...
  if (!emm_context) {
    OAILOG_ERROR(
        LOG_NAS_EMM,
        "EMM-PROC  - TAU retry: no EMM context for UE " MME_UE_S1AP_ID_FMT
  ...
  nas_emm_tau_proc_t *tau_proc =
      (nas_emm_tau_proc_t *)get_nas_specific_procedure(emm_context);
  emm_data_context_t *duplicate_emm_context =
      emm_data_context_get(&_emm_data, tau_proc->emm_spec_proc.old_ue_id);
  ue_context_t *duplicate_ue_context = mme_ue_context_exists_mme_ue_s1ap_id(
      &mme_app_desc.mme_ue_contexts, tau_proc->emm_spec_proc.old_ue_id);
  ...
  if (duplicate_emm_context) {
    ...
    OAILOG_WARNING(
        ...
        " still existing. Aborting the TAU procedure. \n",
        tau_proc->emm_spec_proc.old_ue_id);
    rc =
        _emm_tracking_area_update_reject(tau_proc->ue_id, EMM_CAUSE_ILLEGAL_UE);
  ...
  OAILOG_WARNING(
      ...
      ". Continuing with TAU procedure for new ueId " MME_UE_S1AP_ID_FMT ". \n",
...
static int _emm_tracking_area_update_run_procedure(
  ...
  if (!ue_context) {
    OAILOG_INFO(LOG_NAS_EMM,
                "EMM-PROC  TAU - For ueId " MME_UE_S1AP_ID_FMT
                " no ue context exists (while running TAU procedure). Removing "
                "the EMM context without sending attach reject.. \n",
                emm_context->ue_id);
    // Release EMM context
    if (emm_context->is_dynamic) {
      _clear_emm_ctxt(emm_context->ue_id);
  ...
  if (tau_proc) {
     ...
     */
    if (tau_proc->ies->ue_network_capability) {
      emm_ctx_set_ue_nw_cap((emm_data_context_t *const)emm_context,
                            tau_proc->ies->ue_network_capability);
    } else {
    ...
    if (tau_proc->ies->ms_network_capability) {
      ...
      //      emm_context->gea = (msg->msnetworkcapability.gea1 << 6)|
      //      msg->msnetworkcapability.egea; emm_context->gprs_present = true;
    ...
    //    OAILOG_DEBUG(LOG_NAS_EMM, "EMM-PROC-  Num TAU_REQUESTs for UE with
    //    GUTI " GUTI_FMT " and ue_id " MME_UE_S1AP_ID_FMT " is: %d. \n",
    //    GUTI_ARG(&emm_context->_guti), emm_context->ue_id,
    //    tau_proc->->num_tau_request);

    /** Check for TAU Reject after validating the old context or creating a new
     * one! */
    /*
    ...
    if (tau_proc->ies->eps_bearer_context_status) {
      ...
      //    emm_ctx_set_eps_bearer_context_status(emm_ctx,
      //    &msg->epsbearercontextstatus); /**< This will indicate bearers in
      //    active mode. */
      //    //#pragma message  "TODO Requirement MME24.301R10_5.5.3.2.4_5a: TAU
      //    Request: Deactivate EPS bearers if necessary (S11 Modify Bearer
      //    Request)"
      //    // todo:
      OAILOG_WARNING(LOG_NAS_EMM,
                     "EMM-PROC- Tracking Area Update Request: Bearer context "
    ...
    //    if ( RETURNok == rc) {
    //      OAILOG_INFO(LOG_NAS_EMM, "TrackingAreaUpdate - Successfully updated
    //      TAI list of EMM context!\n");
    //    } else {
    //      OAILOG_ERROR(LOG_NAS_EMM, "TrackingAreaUpdate - Error updating TAI
    //      list of EMM context!\n"); rc =
    //      emm_wrapper_tracking_area_update_reject (ue_id,
    //      EMM_CAUSE_TA_NOT_ALLOWED); OAILOG_FUNC_RETURN (LOG_NAS_EMM, rc);
    //    }

    /** Set the Old-GUTI (event is context there) as present. */
    emm_ctx_set_old_guti((emm_data_context_t *const)emm_context,
                         &tau_proc->ies->old_guti);
    ...
    if (IS_EMM_CTXT_PRESENT_SECURITY(emm_context)) {
      /** Consider the ESM security context of the UE as valid and continue to
       * handle the ESM message with making a ULR at the HSS. */
      OAILOG_INFO(LOG_NAS_EMM,
                  "EMM-PROC  - EMM context for the ue_id=" MME_UE_S1AP_ID_FMT
                  " has a valid and active EPS security context. "
                  "Continuing with the tracking area update request. \n",
                  emm_context->ue_id);
      /*
       ...
       */
      /** Check if any S6a Subscription information exist, if not pull them from
       * the HSS. */
      OAILOG_INFO(LOG_NAS_EMM,
                  "EMM-PROC- THE UE with ue_id=" MME_UE_S1AP_ID_FMT
                  ", has already a security context. Checking for a "
                  "subscription profile. \n",
                  emm_context->ue_id);

      /** Check for subscription data. */
      subscription_data_t *subscription_data =
          mme_ue_subscription_data_exists_imsi(&mme_app_desc.mme_ue_contexts,
                                               emm_context->_imsi64);
      if (!subscription_data) { /**< Means, that the MM UE context is received
                                   from the sourc MME already due HO (and only
                                   due HO). */
        if (tau_proc->ies->subscription_data) {
          subscription_data = tau_proc->ies->subscription_data;
          mme_insert_subscription_profile(&mme_app_desc.mme_ue_contexts,
                                          emm_context->_imsi64,
                                          subscription_data);
          mme_app_update_ue_subscription(emm_context->ue_id,
                                         tau_proc->ies->subscription_data);
          tau_proc->ies->subscription_data = NULL;
          /** Send the S6a message. */
          MessageDef *message_p =
              itti_alloc_new_message(TASK_MME_APP, NAS_PDN_CONFIG_RSP);
          message_p->ittiMsg.nas_pdn_config_rsp.ue_id =
              ue_context->privates.mme_ue_s1ap_id;
          message_p->ittiMsg.nas_pdn_config_rsp.imsi64 = emm_context->_imsi64;
          imsi64_t imsi64_2 = imsi_to_imsi64(&emm_context->_imsi);
          memcpy(&message_p->ittiMsg.nas_pdn_config_rsp.imsi,
                 &emm_context->_imsi, sizeof(imsi_t));
          memcpy(&message_p->ittiMsg.nas_pdn_config_rsp.target_tai,
                 &emm_context->originating_tai, sizeof(tai_t));
          /** Check if an attach procedure is running, if so send it to the ESM
           * layer, if not, we assume it is triggered by a TAU request, send it
           * to the EMM. */
          /** For error codes, use nas_pdn_cfg_fail. */
          MSC_LOG_TX_MESSAGE(MSC_MMEAPP_MME, MSC_NAS_ESM, NULL, 0,
                             "0 NAS_PDN_CONFIG_RESP IMSI " IMSI_64_FMT,
                             emm_context->_imsi64);
          ...
          OAILOG_WARNING(
              LOG_NAS_EMM,
              "EMM-PROC- THE UE with ue_id=" MME_UE_S1AP_ID_FMT
              ", does not have a subscription profile set for IMSI " IMSI_64_FMT
              ". "
          ...
          rc = nas_itti_pdn_config_req(emm_context->ue_id, &emm_context->_imsi,
                                       ...
                                       &emm_context->originating_tai.plmn);
          OAILOG_FUNC_RETURN(LOG_NAS_EMM, rc);
        }
      } else {
        OAILOG_DEBUG(
            LOG_NAS_EMM,
            "EMM-PROC- Sending Tracking Area Update Accept for UE with valid "
            "subscription ue_id=" MME_UE_S1AP_ID_FMT ", active flag=%d)\n",
            emm_context->ue_id, tau_proc->ies->eps_update_type.active_flag);
        /* Check the state of the EMM context. If it is REGISTERED, send an
         * TAU_ACCEPT back and remove the tau procedure. */
        if (emm_context->_emm_fsm_state == EMM_REGISTERED) {
          rc = _emm_tracking_area_update_accept(
              tau_proc); /**< Will remove the TAU procedure. */
          nas_delete_tau_procedure(emm_context);
        } else {
          OAILOG_INFO(
              LOG_NAS_EMM,
              "EMM-PROC  - EMM context for the ue_id=" MME_UE_S1AP_ID_FMT
              " has a valid and active EPS security context and subscription "
              "but is not in EMM_REGISTERED state. Instead %d. "
              "Continuing with the tracking area update reject. \n",
              emm_context->ue_id);
          rc = emm_wrapper_tracking_area_update_reject(
              emm_context, EMM_CAUSE_NETWORK_FAILURE);
          OAILOG_FUNC_RETURN(LOG_NAS_EMM, RETURNerror);
        }
      }
      ...
      if (mme_app_check_ta_local(
              &tau_proc->ies->last_visited_registered_tai->plmn,
              tau_proc->ies->last_visited_registered_tai->tac)) {
        OAILOG_WARNING(LOG_NAS_EMM,
                       "EMM-PROC  - For UE " MME_UE_S1AP_ID_FMT
                       " the last visited TAI " TAI_FMT
                       " is attached to current MME (assuming RF issues). "
                       "Proceeding with identification procedure. \n",
                       TAI_ARG(tau_proc->ies->last_visited_registered_tai),
                       emm_context->ue_id);
        rc = emm_proc_identification(
            emm_context, (nas_emm_proc_t *)tau_proc, IDENTITY_TYPE_2_IMSI,
            _emm_tracking_area_update_success_identification_cb,
            _emm_tracking_area_update_failure_identification_cb);
        OAILOG_FUNC_RETURN(LOG_NAS_EMM, rc);
      }

      /** Check if the origin TAI is a neighboring MME where we can request the
       * UE MM context. */
      ...
      mme_app_select_service(tau_proc->ies->last_visited_registered_tai,
                             &edns_neigh_mme_ipv4_addr, S10_MME_GTP_C);
      if (!edns_neigh_mme_ipv4_addr) {
        OAILOG_WARNING(LOG_NAS_EMM,
                       "EMM-PROC  - For UE " MME_UE_S1AP_ID_FMT
                       " the last visited TAI " TAI_FMT
                       ", no neighboring object for MME S10 could be found. \n",
                       emm_context->ue_id,
                       TAI_ARG(tau_proc->ies->last_visited_registered_tai));
        rc = emm_proc_identification(
            emm_context, (nas_emm_proc_t *)tau_proc, IDENTITY_TYPE_2_IMSI,
      ...
      if (edns_neigh_mme_ipv4_addr->sa_family == 0) {
        OAILOG_WARNING(LOG_NAS_EMM,
                       "EMM-PROC  - For UE " MME_UE_S1AP_ID_FMT
                       " the last visited TAI " TAI_FMT
                       " is not configured as a MME S10 neighbor. "
                       "Proceeding with identification procedure. \n",
                       emm_context->ue_id,
                       TAI_ARG(tau_proc->ies->last_visited_registered_tai));
        rc = emm_proc_identification(
        ...
        OAILOG_INFO(LOG_NAS_EMM,
                    "EMM-PROC  - Originating TAI " TAI_FMT
                    " is configured as a MME S10 neighbor. Will request UE "
                    "context from source MME for ue_id = " MME_UE_S1AP_ID_FMT
                    ". "
         ...
         * (We may have received the signal as NAS uplink data request, without
         * any intermission from MME_APP layer).
         */
        mme_app_s10_proc_mme_handover_t *s10_handover_proc =
            mme_app_get_s10_procedure_mme_handover(ue_context);
        if (s10_handover_proc) {
          DevAssert(s10_handover_proc->nas_s10_context.mm_eps_ctx);
          OAILOG_INFO(LOG_NAS_EMM,
                      "EMM-PROC  - We have receive the TAU as part of an S10 "
                      "procedure for ue_id = " MME_UE_S1AP_ID_FMT
                      ". "
                      "Continuing with the pending MM EPS Context. \n",
                      emm_context->ue_id);
          /*
           * No need to start a new EMM_CN_CONTEXT_REQ procedure.
           * Will update the current EMM and ESM context with the pending values
           * and continue with the registration in the HSS (ULR).
           */
          ...
          if (!tau_proc->ies->is_initial) {
            OAILOG_DEBUG(
                LOG_NAS_EMM,
                "EMM-PROC- No handover procedure exists for non-initial TAU "
                "request for UE with ue_id=" MME_UE_S1AP_ID_FMT ". \n",
                emm_context->ue_id);
            rc = emm_wrapper_tracking_area_update_reject(
                emm_context, EMM_CAUSE_NETWORK_FAILURE);
            OAILOG_FUNC_RETURN(LOG_NAS_EMM, rc);
          ...
          /**
           * Send an S10 context request. Send the PLMN together (will be
           * serving network IE). New TAI not need to be sent. Set the new TAI
           * and the new PLMN to the UE context. In case of a TAC-Accept, will
           * be sent and registered.
           */
          OAILOG_DEBUG(LOG_NAS_EMM,
                       "EMM-PROC- Security context for EMM_DATA_CONTEXT is "
                       "missing & no S10 procedu. Sending a S10 context "
                       "request for UE with ue_id=" MME_UE_S1AP_ID_FMT
                       " to old MME. \n",
                       emm_context->ue_id);
          /**
           * Directly inform the MME_APP of the new context.
           * Depending on if there is a pending MM_EPS_CONTEXT or not, it should
           * send S10 message.. MME_APP needs to register the MME_APP context
           * with the IMSI before sending S6a ULR.
           *
           * A S10 Context Request process may already have been started.
           */
          rc = _start_context_request_procedure(
              emm_context, tau_proc, tau_proc->ies->last_visited_registered_tai,
    ...
    OAILOG_WARNING(LOG_NAS_EMM,
                   "EMM-PROC  - For UE " MME_UE_S1AP_ID_FMT
                   " no TAU specific procedure is running. Not proceeding with "
                   "TAU Request. ",
                   emm_context->ue_id);
    rc = emm_wrapper_tracking_area_update_reject(emm_context,
...
static int _context_req_proc_success_cb(emm_data_context_t *emm_context) {
  ...
  nas_emm_tau_proc_t *tau_proc = get_nas_specific_procedure_tau(emm_context);
  // todo: unify later the two procedures (all CN procedures (MME_APP, NAS,...
  // into one).
  ...
  nas_ctx_req_proc_t *nas_ctx_req_proc =
  ...
  if (!nas_ctx_req_proc) {
    ...
    mme_app_s10_proc_mme_handover_t *s10_handover_proc =
        mme_app_get_s10_procedure_mme_handover(ue_context);
    DevAssert(s10_handover_proc);  // todo: optimize this as well, either s10
                                   // handover procedure or nas context request
   ...
   * Assuming IMSI is always returned with S10 Context Response and the IMSI
   * hastable registration method validates the received IMSI.
   */
  ...
  emm_data_context_upsert_imsi(
      &_emm_data, emm_context); /**< Register the IMSI in the hash table. */

  /*
   * Update the security context & security vectors of the UE independent of
   * TAU/Attach here (set fields valid/present). Then inform the MME_APP that
   * the context was successfully authenticated. Trigger a CSR.
   */
  ...
  OAILOG_INFO(
      LOG_NAS_EMM,
      "EMM-PROC  - "
      "Successfully updated the EMM context with ueId " MME_UE_S1AP_ID_FMT
      " from the received MM_EPS_Context from the MME for UE with "
      "imsi: " IMSI_64_FMT ". \n",
      emm_context->ue_id, emm_context->_imsi64);
  /** No need to touch the ESM context. No ESM procedure is running. */

  /** Delete the CN procedure, if exists. */
  if (nas_ctx_req_proc) nas_delete_cn_procedure(emm_context, nas_ctx_req_proc);
...
static int _context_req_proc_failure_cb(emm_data_context_t *emm_context) {
  ...
  if (tau_proc) {
    // todo: requirement to continue with identification, auth and SMC if
    // context res fails!
    rc = emm_proc_identification(
        emm_context, (nas_emm_proc_t *)tau_proc, IDENTITY_TYPE_2_IMSI,
        _emm_tracking_area_update_success_identification_cb,
...
static int _emm_tracking_area_update_success_identification_cb(
  ...
  if (tau_proc) {
    REQUIREMENT_3GPP_24_301(R10_5_5_3_2_3__2);
    rc = _emm_start_tracking_area_update_proc_authentication(
        emm_context,
        tau_proc);  //, IDENTITY_TYPE_2_IMSI, _emm_attach_authentified,
                    //_emm_attach_release);
  }
  OAILOG_FUNC_RETURN(LOG_NAS_EMM, rc);
}
...
static int _emm_start_tracking_area_update_proc_authentication(
  ...
  if ((emm_context) && (tau_proc)) {
    rc = emm_proc_authentication(
        emm_context, &tau_proc->emm_spec_proc,
        _emm_tracking_area_update_success_authentication_cb,
        _emm_tracking_area_update_failure_authentication_cb);
  }
  OAILOG_FUNC_RETURN(LOG_NAS_EMM, rc);
}
...
static int _emm_start_tracking_area_update_proc_security(
  ...
  if ((emm_context) && (tau_proc)) {
    ...
    rc = emm_proc_security_mode_control(
        emm_context, &tau_proc->emm_spec_proc, tau_proc->ksi,
        _emm_tracking_area_update_success_security_cb,
        _emm_tracking_area_update_failure_security_cb);
    if (rc != RETURNok) {
      /*
      ...
      rc = _emm_tracking_area_update_reject(tau_proc->ue_id,
                                            EMM_CAUSE_ILLEGAL_UE);
      OAILOG_FUNC_RETURN(LOG_NAS_EMM, rc);
    }
  }
...
static void _emm_tracking_area_update_registration_complete(
  ...
  mme_app_s10_proc_mme_handover_t *s10_proc_handover =
      mme_app_get_s10_procedure_mme_handover(ue_context);

  if (!s10_proc_handover) {
    if (!active_flag) {
      /**
       * No MBR should be send because no InitialContextSetupResponse is
       * received. Also no CBR is received and waiting in pending state.
       */
      OAILOG_WARNING(LOG_NAS_EMM,
                     "EMM-PROC  - EMM Context for ueId " MME_UE_S1AP_ID_FMT
                     " has done an IDLE-TAU without active flag. Triggering UE "
                     "context release. \n",
                     emm_context->ue_id);
      mme_app_itti_ue_context_release(
          ue_context->privates.mme_ue_s1ap_id,
          ue_context->privates.fields.enb_ue_s1ap_id, S1AP_NAS_NORMAL_RELEASE,
      ...
      OAILOG_INFO(
          LOG_NAS_EMM,
          "EMM-PROC  - EMM Context for ueId " MME_UE_S1AP_ID_FMT
          " has done an IDLE-TAU without active flag but bearer QoS "
     ...
     * We should check in the MME_APP layer, if the default bearer has been
     * already activated. Else, if we sent a CBResp before the MBR procedure
     * completes, we might trigger an initial detach in the PGW.
     */
