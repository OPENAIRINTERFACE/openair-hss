...
static int _identification_ll_failure(struct emm_data_context_s *emm_context,
...
static int _identification_abort(struct emm_data_context_s *emm_context,
...
int emm_proc_identification(struct emm_data_context_s *const emm_context,
  ...
  if ((emm_context) && ((EMM_DEREGISTERED == emm_context->_emm_fsm_state) ||
    ...
    nas_emm_ident_proc_t *ident_proc =
        get_nas_common_procedure_identification(emm_context);
    if (!ident_proc) {
    ...
    ((nas_emm_base_proc_t *)ident_proc)->parent =
        (nas_emm_base_proc_t *)emm_proc;
  ...
  if (emm_ctx) {
    ...
    if (ident_proc) {
      ...
      if (imsi) {
        ...
        emm_data_context_t *imsi_emm_ctx_duplicate =
            emm_data_context_get_by_imsi(&_emm_data, imsi64);
        if (imsi_emm_ctx_duplicate) { /**< We have the UE with this IMSI
                                         (different GUTI). */
          if (imsi_emm_ctx_duplicate->ue_id != emm_ctx->ue_id) {
            OAILOG_INFO(
                LOG_NAS_EMM,
                "EMM-PROC  - We already have EMM context with "
                "ueId " MME_UE_S1AP_ID_FMT " and IMSI " IMSI_64_FMT
                ". Setting new EMM context with ueId " MME_UE_S1AP_ID_FMT
                " into pending mode "
            ...
            nas_start_T_retry_specific_procedure(
                ...
                &((nas_emm_specific_proc_t
                       *)(((nas_emm_base_proc_t *)ident_proc)->parent))
                     ->retry_timer,
                ((nas_emm_specific_proc_t *)(((nas_emm_base_proc_t *)ident_proc)
                                                 ->parent))
                    ->retry_cb,
                (void *)emm_ctx->ue_id);
            ...
            ((nas_emm_specific_proc_t *)(((nas_emm_base_proc_t *)ident_proc)
                                             ->parent))
                ->old_ue_id = imsi_emm_ctx_duplicate->ue_id;
            /*
            ...
            emm_sap.u.emm_cn.u.emm_cn_implicit_detach.emm_cause =
                imsi_emm_ctx_duplicate
                    ->emm_cause; /**< Not sending detach type. */
            ...
            emm_sap.u.emm_cn.u.emm_cn_implicit_detach.ue_id =
                imsi_emm_ctx_duplicate->ue_id;
            /*
             * Don't send the detach type, such that no NAS Detach Request is
             * sent to the UE. Depending on the cause, the MME_APP will check
             * and inform the NAS layer to continue with the procedure, before
             * the timer expires.
             */
            emm_sap_send(&emm_sap);

            /*
             * Notify EMM that the identification procedure successfully
             * completed. Free the procedure, but don't continue.
             */
            memset(&emm_sap, 0, sizeof(emm_sap_t));
            MSC_LOG_TX_MESSAGE(
                MSC_NAS_EMM_MME, MSC_NAS_EMM_MME, NULL, 0,
        ...
        ue_context_t *ue_context_duplicate_imsi =
            mme_ue_context_exists_imsi(&mme_app_desc.mme_ue_contexts, imsi64);
        if (ue_context_duplicate_imsi &&
            (ue_context_duplicate_imsi->privates.mme_ue_s1ap_id !=
             emm_ctx->ue_id)) {
          OAILOG_ERROR(LOG_NAS_EMM,
                       "EMM-PROC  - We already have MME_APP UE context with "
                       "ueId " MME_UE_S1AP_ID_FMT " and IMSI " IMSI_64_FMT
                       ". "
                       "Setting new EMM context with ueId " MME_UE_S1AP_ID_FMT
                       " into pending mode "
                       ...
                       ue_context_duplicate_imsi->privates.mme_ue_s1ap_id,
                       imsi64, emm_ctx->ue_id);

          nas_itti_esm_detach_ind(
              ue_context_duplicate_imsi->privates.mme_ue_s1ap_id, false);
          ...
          ((nas_emm_specific_proc_t *)(((nas_emm_base_proc_t *)ident_proc)
                                           ->parent))
              ->old_ue_id = ue_context_duplicate_imsi->privates.mme_ue_s1ap_id;
        ...
        emm_data_context_t *imsi_emm_ctx_test =
            emm_data_context_get_by_imsi(&_emm_data, imsi64);
         ...
         */
        AssertFatal(false,
                    "TODO, should not happen because this type of identity is "
                    "not requested by MME");
      }
...
static void _identification_t3470_handler(void *args) {
  ...
  if (!(emm_ctx)) {
    OAILOG_ERROR(LOG_NAS_EMM,
                 "T3470 timer expired No EMM context for UE " MME_UE_S1AP_ID_FMT
                 ". \n",
                 ue_id);
  ...
  if (ident_proc) {
    ...
    if (ident_proc->retransmission_count < IDENTIFICATION_COUNTER_MAX) {
       ...
       * A non delivery indicator, might have triggered another identity
       * request, and in the meantime, the MME_APP context might have been
       * removed, due to a Context Release Complete.
       */
      if ((emm_ctx = emm_data_context_get(&_emm_data, ue_id))) {
        OAILOG_WARNING(LOG_NAS_EMM,
                       "EMM-PROC  - EMM Context for ueId " MME_UE_S1AP_ID_FMT
                       " is still existing after aborted identification. "
...
static int _identification_abort(struct emm_data_context_s *emm_context,
  ...
  if ((emm_context) && (emm_base_proc)) {
    ...
    AssertFatal(
        (NAS_EMM_PROC_TYPE_COMMON == ((nas_emm_proc_t *)emm_base_proc)->type) &&
            (EMM_COMM_PROC_IDENT ==
             ((nas_emm_common_proc_t *)emm_base_proc)->type),
