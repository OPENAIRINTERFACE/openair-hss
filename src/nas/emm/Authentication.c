...
static int _authentication_ll_failure(struct emm_data_context_s *emm_context,
                                      struct nas_emm_proc_s *emm_proc);
static int _authentication_non_delivered_ho(
    struct emm_data_context_s *emm_context, struct nas_emm_proc_s *emm_proc);
static int _authentication_abort(struct emm_data_context_s *emm_context,
...
static int _start_authentication_information_procedure(
    ...
    nas_emm_auth_proc_t *const auth_proc, const_bstring auts);
...
static int _authentication_reject(struct emm_data_context_s *emm_context,
                                  struct nas_emm_base_proc_s *emm_base_proc);
...
int emm_proc_authentication_ksi(
  ...
  if ((emm_context) && ((EMM_DEREGISTERED == emm_context->_emm_fsm_state) ||
    ...
    if (auth_proc) {
      ...
      ((nas_emm_base_proc_t *)auth_proc)->parent =
          (nas_emm_base_proc_t *)emm_specific_proc;
...
int emm_proc_authentication(struct emm_data_context_s *emm_context,
  ...
  if (!auth_proc) {
    ...
    // todo: don't we need to decouple the common procedure from the specific
    // procedure abort it first? todo: in addition, when creating a specific
    // procedure, all commons should be binded to the specific procedure to
    // terminate them in case of an error in specific procedure collision.
  }
  if (auth_proc) {
    ...
    if (run_auth_info_proc) {
      rc = _start_authentication_information_procedure(emm_context, auth_proc,
                                                       NULL);
    }
  }
  OAILOG_FUNC_RETURN(LOG_NAS_EMM, rc);
...
static int _auth_info_proc_failure_cb(struct emm_data_context_s *emm_ctx) {
  ...
  if (auth_info_proc) {
    ...
    nas_delete_cn_procedure(emm_ctx, &auth_info_proc->cn_proc);
    /** Done handling the CN task, starting to handle the relevant common or
     * specific task, in this case, the common task. */
    if (auth_proc) {
      ...
      case EMM_CAUSE_SYNCH_FAILURE:
        ...
        if (EMM_AUTHENTICATION_SYNC_FAILURE_MAX > auth_proc->sync_fail_count) {
          OAILOG_DEBUG(LOG_NAS_EMM,
                       "EMM-PROC  - USIM has detected a mismatch in SQN Ask "
                       "for new vector(s)\n");
          ...
          bstring rand_bstr =
              blk2bstr(emm_ctx->_vector[0].rand, RAND_LENGTH_OCTETS);
          bconcat(rand_bstr, auts);
          // TODO: Double check this case as there is no identity request being
          // sent.
          _start_authentication_information_procedure_synch(emm_ctx, auth_proc,
                                                            rand_bstr);
...
int emm_proc_authentication_complete(mme_ue_s1ap_id_t ue_id, int emm_cause,
  ...
  if (auth_proc) {
    ...
    nas_stop_T3460(ue_id, &auth_proc->T3460,
                   unused); /**< Stop the timer, leave the procedure?. */
    ...
    OAILOG_DEBUG(LOG_NAS_EMM,
                 "EMM-PROC  - Success to authentify the UE  RESP XRES == XRES "
                 "UE CONTEXT\n");
    /*
...
static void _authentication_t3460_handler(void *args) {
  ...
  if (!(emm_ctx)) {
    OAILOG_ERROR(
        LOG_NAS_EMM,
        "T3460 timer expired No EMM context. Triggering an MME_APP detach.. \n",
        ue_id);
  ...
  if (auth_proc) {
    ...
    if (auth_proc->retransmission_count < AUTHENTICATION_COUNTER_MAX) {
       ...
       * A non delivery indicator would just retrigger the message, not a
       * guarantee for removal.
       */
      OAILOG_INFO(LOG_NAS_EMM,
                  "EMM-PROC  - T3460 timer expired: aborted authentication for "
                  "ueId " MME_UE_S1AP_ID_FMT ". Not continuing further. \n",
                  ue_id);
      OAILOG_FUNC_OUT(LOG_NAS_EMM);
    }
  } else {
    OAILOG_WARNING(LOG_NAS_EMM,
                   "EMM-PROC  - T3460 timer expired, but no authentication "
                   "procedure exists for ueId " MME_UE_S1AP_ID_FMT
                   ". Ignoring. \n",
                   ue_id);
  }
  OAILOG_FUNC_OUT(LOG_NAS_EMM);
    ...
    if (IS_EMM_CTXT_VALID_IMSI(
      ...
      if (memcmp(auth_proc->unchecked_imsi, &emm_context->_imsi,
        ...
        nas_emm_specific_proc_t *emm_specific_proc =
            (nas_emm_specific_proc_t *)((nas_emm_base_proc_t *)auth_proc)
                ->parent;
  ...
  if (auth_proc) {
    ...
    if (!emm_ctx_p) {
      OAILOG_ERROR(LOG_NAS_EMM,
                   "No EMM context could be found for ueId " MME_UE_S1AP_ID_FMT
                   ". "
    ...
    emm_as_set_security_data(&emm_sap.u.emm_as.u.security.sctx,
                             &emm_ctx_p->_security, false, true);
    ...
    if (rc != RETURNerror) {
      if (emm_ctx_p) {
        if (auth_proc->T3460.id !=
            NAS_TIMER_INACTIVE_ID) { /**< NAS timer will be restarted after
                                        sending S6a. */
          void *unused = NULL;
 ...
 **                                                                        **
 ***************************************************************************/
static int _authentication_reject(emm_data_context_t *emm_context,
                                  struct nas_emm_base_proc_s *emm_base_proc) {
  OAILOG_FUNC_IN(LOG_NAS_EMM);
  emm_sap_t emm_sap = {0};
  int rc = RETURNerror;
 ...
 **                                                                        **
 ***************************************************************************/
static int _authentication_abort(emm_data_context_t *emm_ctx,
                                 struct nas_emm_base_proc_s *emm_base_proc) {
  OAILOG_FUNC_IN(LOG_NAS_EMM);
  int rc = RETURNerror;
