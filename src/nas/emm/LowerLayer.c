...
int lowerlayer_success(mme_ue_s1ap_id_t ue_id, bstring *nas_msg) {
  ...
  if (emm_data_context) {
    ...
    MSC_LOG_TX_MESSAGE(
        MSC_NAS_EMM_MME, MSC_NAS_MME, NULL, 0,
        "EMMREG_LOWERLAYER_SUCCESS ue id " MME_UE_S1AP_ID_FMT " ", ue_id);
...
int lowerlayer_failure(mme_ue_s1ap_id_t ue_id, STOLEN_REF bstring *nas_msg) {
  ...
  if (emm_data_context) {
    ...
    MSC_LOG_TX_MESSAGE(
        MSC_NAS_EMM_MME, MSC_NAS_MME, NULL, 0,
        "EMMREG_LOWERLAYER_FAILURE ue id " MME_UE_S1AP_ID_FMT " ", ue_id);
    rc = emm_sap_send(&emm_sap);
    //    unlock_ue_contexts(ue_context);
    OAILOG_FUNC_RETURN(LOG_NAS_EMM, rc);
  } else {
...
int lowerlayer_non_delivery_indication(mme_ue_s1ap_id_t ue_id,
  ...
  if (emm_data_context) {
    ...
    MSC_LOG_TX_MESSAGE(
        MSC_NAS_EMM_MME, MSC_NAS_MME, NULL, 0,
        "EMMREG_LOWERLAYER_NON_DELIVERY ue id " MME_UE_S1AP_ID_FMT " ", ue_id);
    rc = emm_sap_send(&emm_sap);
    //    unlock_ue_contexts(ue_context);
    OAILOG_FUNC_RETURN(LOG_NAS_EMM, rc);
  } else {
...
int lowerlayer_release(mme_ue_s1ap_id_t ue_id, int cause) {
  ...
  MSC_LOG_TX_MESSAGE(MSC_NAS_EMM_MME, MSC_NAS_MME, NULL, 0,
...
//
//  emm_data_context_t                     *emm_data_context =
//  emm_data_context_get(&_emm_data, ue_id); esm_sap.primitive =
//  ESM_UNITDATA_IND; esm_sap.ue_id = ue_id; esm_sap.ctx = emm_data_context;
//  esm_sap.recv = data;
//  data = NULL;
...
int lowerlayer_data_req(mme_ue_s1ap_id_t ue_id, bstring data) {
   ...
   */
  emm_as_set_security_data(&emm_sap.u.emm_as.u.data.sctx_data, sctx, false,
                           true);
  MSC_LOG_TX_MESSAGE(MSC_NAS_EMM_MME, MSC_NAS_MME, NULL, 0,
...
int lowerlayer_activate_bearer_req(const mme_ue_s1ap_id_t ue_id,
  ...
  if (!emm_data_context) {
    ...
    OAILOG_ERROR(LOG_NAS_EMM,
                 "EMM context not existing UE " MME_UE_S1AP_ID_FMT
                 ". "
                 "Aborting the dedicated bearer activation. \n",
                 ue_id);
    OAILOG_FUNC_RETURN(LOG_NAS_EMM, RETURNerror);
  } else if (emm_data_context->_emm_fsm_state != EMM_REGISTERED) {
    /** Check if a TAU procedure is ongoing. Set the pending flag for pending
     * QoS. */
    OAILOG_ERROR(
        LOG_NAS_EMM,
        "EMM context not in EMM_REGISTERED state for UE " MME_UE_S1AP_ID_FMT
        ". "
        "Cannot activate bearers. \n",
        ue_id);
    /** Check for an EMM (idle TAU) or S10 handover procedure. */
    nas_emm_tau_proc_t *nas_proc_tau =
        get_nas_specific_procedure_tau(emm_data_context);
    if (nas_proc_tau) {
      OAILOG_INFO(LOG_NAS_EMM,
                  "A TAU procedure is running for UE " MME_UE_S1AP_ID_FMT
                  ". Triggering pending qos. \n",
                  ue_id);
      ...
      if (emm_data_context->_emm_fsm_state != EMM_COMMON_PROCEDURE_INITIATED) {
        ...
        /** This may be an idle-TAU, so set the active flag (todo: here check
         * glitches). */
        if (!nas_proc_tau->ies->eps_update_type.active_flag) {
          OAILOG_INFO(LOG_NAS_EMM,
                      "Setting the active flag for TAU procedure for "
        ...
        OAILOG_INFO(LOG_NAS_EMM,
                    "UE " MME_UE_S1AP_ID_FMT
                    " is not in DEREGISTERED state, to late to set qos/active "
                    "flag.. will do via paging. \n",
                    ue_id);
      }
    } else {
      OAILOG_INFO(LOG_NAS_EMM,
                  "No S10-TAU procedure is running for UE " MME_UE_S1AP_ID_FMT
                  ", which is not in registered state. "
...
int lowerlayer_modify_bearer_req(const mme_ue_s1ap_id_t ue_id, const ebi_t ebi,
  ...
  if (data) {
     ...
     * Setup EPS NAS security data
     */
    emm_as_set_security_data(&emm_sap.u.emm_as.u.modify_bearer_context_req.sctx,
                             sctx, false, true);
    MSC_LOG_TX_MESSAGE(
        MSC_NAS_EMM_MME, MSC_NAS_MME, NULL, 0,
        "EMMAS_ERAB_MODIFY_REQ  (STATUS) ue id " MME_UE_S1AP_ID_FMT
        " ebi %u gbr_dl %" PRIu64 " gbr_ul %" PRIu64 " ",
        ue_id, ebi, emm_sap.u.emm_as.u.modify_bearer_context_req.gbr_dl,
        emm_sap.u.emm_as.u.modify_bearer_context_req.gbr_ul);
    rc = emm_sap_send(&emm_sap);
    //  unlock_ue_contexts(ue_context);
    OAILOG_FUNC_RETURN(LOG_NAS_EMM, rc);
  }
  OAILOG_WARNING(LOG_NAS_EMM,
                 "No data exists ERAB-Modify for UE " MME_UE_S1AP_ID_FMT
                 ". Returning. \n",
                 ue_id);
  OAILOG_FUNC_RETURN(LOG_NAS_EMM, RETURNok);
}
...
int lowerlayer_deactivate_bearer_req(const mme_ue_s1ap_id_t ue_id,
  ...
  if (data) {
    ...
    MSC_LOG_TX_MESSAGE(
        MSC_NAS_EMM_MME, MSC_NAS_MME, NULL, 0,
        "EMMAS_ERAB_RELEASE_REQ  (STATUS) ue id " MME_UE_S1AP_ID_FMT " ebi %u ",
        ue_id, ebi);
    rc = emm_sap_send(&emm_sap);
    //  unlock_ue_contexts(ue_context);
  }
