...
static int decode_imsi_eps_mobile_identity(imsi_eps_mobile_identity_t *imsi,
  ...
  if (decoded < ie_len) {
    ...
    if (decoded < ie_len) {
      ...
      decoded++;
      if ((decoded == ie_len) && (EPS_MOBILE_IDENTITY_EVEN == imsi->oddeven) &&
          (imsi->identity_digit5 != 0x0f)) {
        OAILOG_FUNC_RETURN(LOG_NAS_EMM, TLV_VALUE_DOESNT_MATCH);
      } else {
        imsi->num_digits++;
      }
      if (decoded < ie_len) {
        ...
        decoded++;
        if ((decoded == ie_len) &&
            (EPS_MOBILE_IDENTITY_EVEN == imsi->oddeven) &&
            (imsi->identity_digit7 != 0x0f)) {
          OAILOG_FUNC_RETURN(LOG_NAS_EMM, TLV_VALUE_DOESNT_MATCH);
        } else {
          imsi->num_digits++;
        }
          ...
          if (decoded < ie_len) {
            ...
            if (decoded < ie_len) {
              ...
              if (decoded < ie_len) {
                ...
                if ((EPS_MOBILE_IDENTITY_EVEN == imsi->oddeven) &&
