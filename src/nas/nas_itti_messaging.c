...
//------------------------------------------------------------------------------
int nas_itti_esm_data_ind(const mme_ue_s1ap_id_t ue_id, bstring esm_msg_p,
                          imsi_t *imsi, tai_t *visited_tai) {
  MessageDef *message_p =
      itti_alloc_new_message(TASK_NAS_EMM, NAS_ESM_DATA_IND);
  NAS_ESM_DATA_IND(message_p).ue_id = ue_id;
  NAS_ESM_DATA_IND(message_p).req = esm_msg_p;
  if (imsi) memcpy(&NAS_ESM_DATA_IND(message_p).imsi, imsi, sizeof(imsi_t));
  if (visited_tai)
    memcpy(&NAS_ESM_DATA_IND(message_p).visited_tai, visited_tai,
           sizeof(tai_t));

  MSC_LOG_TX_MESSAGE(MSC_NAS_MME, MSC_S1AP_MME, NULL, 0,
                     "0 NAS_ESM_DATA_IND ue id " MME_UE_S1AP_ID_FMT " len %u",
                     ue_id, blength(esm_msg_p));
  // make a long way by MME_APP instead of S1AP to retrieve the
  // sctp_association_id key.
  return itti_send_msg_to_task(TASK_NAS_ESM, INSTANCE_DEFAULT, message_p);
}

//------------------------------------------------------------------------------
int nas_itti_esm_detach_ind(const mme_ue_s1ap_id_t ue_id, const bool clr) {
  MessageDef *message_p =
      itti_alloc_new_message(TASK_NAS_EMM, NAS_ESM_DETACH_IND);
  NAS_ESM_DETACH_IND(message_p).ue_id = ue_id;
  NAS_ESM_DETACH_IND(message_p).clr = clr;

  MSC_LOG_TX_MESSAGE(MSC_NAS_MME, MSC_S1AP_MME, NULL, 0,
                     "0 NAS_ESM_DETACH_IND ue id " MME_UE_S1AP_ID_FMT, ue_id);
  // make a long way by MME_APP instead of S1AP to retrieve the
  // sctp_association_id key.
  return itti_send_msg_to_task(TASK_NAS_ESM, INSTANCE_DEFAULT, message_p);
}
...
//------------------------------------------------------------------------------
int nas_itti_s11_retry_ind(const mme_ue_s1ap_id_t ue_id) {
  MessageDef *message_p =
      itti_alloc_new_message(TASK_NAS_EMM, NAS_RETRY_BEARER_CTX_PROC_IND);
  NAS_RETRY_BEARER_CTX_PROC_IND(message_p).ue_id = ue_id;

  MSC_LOG_TX_MESSAGE(
      MSC_NAS_MME, MSC_S1AP_MME, NULL, 0,
      "0 NAS_RETRY_BEARER_CTX_PROC_IND ue id " MME_UE_S1AP_ID_FMT, ue_id);
  // make a long way by MME_APP instead of S1AP to retrieve the
  // sctp_association_id key.
...
//------------------------------------------------------------------------------
int nas_itti_dl_data_req(const mme_ue_s1ap_id_t ue_id, bstring nas_msg,
                         nas_error_code_t transaction_status) {
  MessageDef *message_p =
      itti_alloc_new_message(TASK_NAS_EMM, NAS_DOWNLINK_DATA_REQ);
  ...
  MSC_LOG_TX_MESSAGE(MSC_NAS_MME, MSC_S1AP_MME, NULL, 0,
                     "0 NAS_DOWNLINK_DATA_REQ ue id " MME_UE_S1AP_ID_FMT
                     " len %u",
                     ue_id, blength(nas_msg));
  // make a long way by MME_APP instead of S1AP to retrieve the
  // sctp_association_id key.
  return itti_send_msg_to_task(TASK_MME_APP, INSTANCE_DEFAULT, message_p);
}
...
//------------------------------------------------------------------------------
int nas_itti_erab_setup_req(const mme_ue_s1ap_id_t ue_id, const ebi_t ebi,
                            ...
                            bstring nas_msg) {
  MessageDef *message_p =
      itti_alloc_new_message(TASK_NAS_ESM, NAS_ERAB_SETUP_REQ);
  ...
  nas_msg = NULL;
  MSC_LOG_TX_MESSAGE(
      MSC_NAS_MME, MSC_MMEAPP_MME, NULL, 0,
      "0 NAS_ERAB_SETUP_REQ ue id " MME_UE_S1AP_ID_FMT " ebi %u len %u", ue_id,
      ebi, blength(NAS_ERAB_SETUP_REQ(message_p).nas_msg));
  // make a long way by MME_APP instead of S1AP to retrieve the
  // sctp_association_id key.
  return itti_send_msg_to_task(TASK_MME_APP, INSTANCE_DEFAULT, message_p);
}
...
//------------------------------------------------------------------------------
int nas_itti_erab_modify_req(const mme_ue_s1ap_id_t ue_id, const ebi_t ebi,
                             const bool retry, const int retx_count,
                             const bitrate_t mbr_dl, const bitrate_t mbr_ul,
                             const bitrate_t gbr_dl, const bitrate_t gbr_ul,
                             bstring nas_msg) {
  MessageDef *message_p =
      itti_alloc_new_message(TASK_NAS_ESM, NAS_ERAB_MODIFY_REQ);
  ...
  MSC_LOG_TX_MESSAGE(
      MSC_NAS_MME, MSC_MMEAPP_MME, NULL, 0,
      "0 NAS_ERAB_MODIFY_REQ ue id " MME_UE_S1AP_ID_FMT " ebi %u len %u", ue_id,
      ebi, blength(NAS_ERAB_MODIFY_REQ(message_p).nas_msg));
  // make a long way by MME_APP instead of S1AP to retrieve the
  // sctp_association_id key.
  return itti_send_msg_to_task(TASK_MME_APP, INSTANCE_DEFAULT, message_p);
}
...
//------------------------------------------------------------------------------
int nas_itti_erab_release_req(const mme_ue_s1ap_id_t ue_id, const ebi_t ebi,
                              const bool retry, const int retx_count,
                              bstring nas_msg) {
  MessageDef *message_p =
      itti_alloc_new_message(TASK_NAS_ESM, NAS_ERAB_RELEASE_REQ);
  ...
  MSC_LOG_TX_MESSAGE(
      MSC_NAS_MME, MSC_MMEAPP_MME, NULL, 0,
      "0 NAS_ERAB_RELEASE_REQ ue id " MME_UE_S1AP_ID_FMT " ebi %u len %u",
      ue_id, ebi, blength(NAS_ERAB_SETUP_REQ(message_p).nas_msg));
  // make a long way by MME_APP instead of S1AP to retrieve the
  // sctp_association_id key.
  return itti_send_msg_to_task(TASK_MME_APP, INSTANCE_DEFAULT, message_p);
}
...
//------------------------------------------------------------------------------
void nas_itti_activate_eps_bearer_ctx_cnf(const mme_ue_s1ap_id_t ue_idP,
                                          const ebi_t ebi,
                                          const teid_t saegw_s1u_teid) {
  OAILOG_FUNC_IN(LOG_NAS);
  MessageDef *message_p =
      itti_alloc_new_message(TASK_NAS_ESM, NAS_ACTIVATE_EPS_BEARER_CTX_CNF);
  ...
  MSC_LOG_TX_MESSAGE(
      MSC_NAS_MME, MSC_MMEAPP_MME, NULL, 0,
      "0 NAS_ACTIVATE_EPS_BEARER_CTX_CNF ue id " MME_UE_S1AP_ID_FMT, ue_idP);
  itti_send_msg_to_task(TASK_MME_APP, INSTANCE_DEFAULT, message_p);
  OAILOG_FUNC_OUT(LOG_NAS);
}

//------------------------------------------------------------------------------
void nas_itti_activate_eps_bearer_ctx_rej(const mme_ue_s1ap_id_t ue_idP,
                                          const teid_t saegw_s1u_teid,
                                          const esm_cause_t esm_cause) {
  OAILOG_FUNC_IN(LOG_NAS);
  MessageDef *message_p =
      itti_alloc_new_message(TASK_NAS_ESM, NAS_ACTIVATE_EPS_BEARER_CTX_REJ);
  ...
  switch (esm_cause) {
    case ESM_CAUSE_SEMANTIC_ERROR_IN_THE_TFT_OPERATION:
      NAS_ACTIVATE_EPS_BEARER_CTX_REJ(message_p).cause_value =
  ...
  MSC_LOG_TX_MESSAGE(
      MSC_NAS_MME, MSC_MMEAPP_MME, NULL, 0,
      "0 NAS_ACTIVATE_EPS_BEARER_CTX_REJ ue id " MME_UE_S1AP_ID_FMT, ue_idP);
  itti_send_msg_to_task(TASK_MME_APP, INSTANCE_DEFAULT, message_p);
  OAILOG_FUNC_OUT(LOG_NAS);
}

//------------------------------------------------------------------------------
void nas_itti_modify_eps_bearer_ctx_cnf(const mme_ue_s1ap_id_t ue_idP,
                                        const ebi_t ebi) {
  OAILOG_FUNC_IN(LOG_NAS);
  MessageDef *message_p =
      itti_alloc_new_message(TASK_NAS_ESM, NAS_MODIFY_EPS_BEARER_CTX_CNF);
  NAS_MODIFY_EPS_BEARER_CTX_CNF(message_p).ue_id = ue_idP;
  NAS_MODIFY_EPS_BEARER_CTX_CNF(message_p).ebi = ebi;
  MSC_LOG_TX_MESSAGE(
      MSC_NAS_MME, MSC_MMEAPP_MME, NULL, 0,
      "0 NAS_MODIFY_EPS_BEARER_CTX_CNF ue id " MME_UE_S1AP_ID_FMT, ue_idP);
  itti_send_msg_to_task(TASK_MME_APP, INSTANCE_DEFAULT, message_p);
  OAILOG_FUNC_OUT(LOG_NAS);
}

//------------------------------------------------------------------------------
void nas_itti_modify_eps_bearer_ctx_rej(const mme_ue_s1ap_id_t ue_idP,
                                        const ebi_t ebi,
                                        const esm_cause_t esm_cause) {
  OAILOG_FUNC_IN(LOG_NAS);
  MessageDef *message_p =
      itti_alloc_new_message(TASK_NAS_ESM, NAS_MODIFY_EPS_BEARER_CTX_REJ);
  NAS_MODIFY_EPS_BEARER_CTX_REJ(message_p).ue_id = ue_idP;
  NAS_MODIFY_EPS_BEARER_CTX_REJ(message_p).ebi = ebi;
  switch (esm_cause) {
    case ESM_CAUSE_SEMANTIC_ERROR_IN_THE_TFT_OPERATION:
      NAS_MODIFY_EPS_BEARER_CTX_REJ(message_p).cause_value =
          SEMANTIC_ERROR_IN_TFT;
      break;
    case ESM_CAUSE_SYNTACTICAL_ERROR_IN_THE_TFT_OPERATION:
      NAS_MODIFY_EPS_BEARER_CTX_REJ(message_p).cause_value =
  ...
  MSC_LOG_TX_MESSAGE(
      MSC_NAS_MME, MSC_MMEAPP_MME, NULL, 0,
      "0 NAS_MODIFY_EPS_BEARER_CTX_REJ ue id " MME_UE_S1AP_ID_FMT, ue_idP);
  itti_send_msg_to_task(TASK_MME_APP, INSTANCE_DEFAULT, message_p);
  OAILOG_FUNC_OUT(LOG_NAS);
}
...
void nas_itti_dedicated_eps_bearer_deactivation_complete(
    const mme_ue_s1ap_id_t ue_idP, const ebi_t ded_ebi,
    const esm_cause_t esm_cause) {
  OAILOG_FUNC_IN(LOG_NAS);
  MessageDef *message_p =
      itti_alloc_new_message(TASK_NAS_ESM, NAS_DEACTIVATE_EPS_BEARER_CTX_CNF);
  ...
  switch (esm_cause) {
    case ESM_CAUSE_SUCCESS:
      NAS_DEACTIVATE_EPS_BEARER_CTX_CNF(message_p).cause_value =
          REQUEST_ACCEPTED;
      break;
    case ESM_CAUSE_SERVICE_OPTION_TEMPORARILY_OUT_OF_ORDER:
      NAS_DEACTIVATE_EPS_BEARER_CTX_CNF(message_p).cause_value =
  ...
  MSC_LOG_TX_MESSAGE(
      MSC_NAS_MME, MSC_MMEAPP_MME, NULL, 0,
      "0 NAS_DEACTIVATE_EPS_BEARER_CTX_CNF ue id " MME_UE_S1AP_ID_FMT " ebi %u",
      ue_idP, ded_ebi);
  itti_send_msg_to_task(TASK_MME_APP, INSTANCE_DEFAULT, message_p);
  OAILOG_FUNC_OUT(LOG_NAS);
}
...
//------------------------------------------------------------------------------
void nas_itti_pdn_config_req(unsigned int ue_idP, const imsi_t *const imsi_pP,
  ...
  s6a_update_location_req_t *s6a_ulr_p =
      &message_p->ittiMsg.s6a_update_location_req;
  ...
  MSC_LOG_TX_MESSAGE(MSC_NAS_ESM, MSC_S6A_MME, NULL, 0,
                     "0 S6A_UPDATE_LOCATION_REQ imsi %s", s6a_ulr_p->imsi);
...
void nas_itti_pdn_disconnect_req(
    mme_ue_s1ap_id_t ue_idP, ebi_t default_ebi, pti_t pti, bool deleteTunnel,
    bool handover,
    struct sockaddr *saegw_s11_addr, /**< Put them into the UE context ? */
    teid_t saegw_s11_teid, pdn_cid_t pdn_cid) {
  OAILOG_FUNC_IN(LOG_NAS);
  MessageDef *message_p = NULL;
  ...
  NAS_PDN_DISCONNECT_REQ(message_p).noDelete =
      ((pti != PROCEDURE_TRANSACTION_IDENTITY_UNASSIGNED) || !deleteTunnel);
  memcpy((void *)&NAS_PDN_DISCONNECT_REQ(message_p).saegw_s11_ip_addr,
         saegw_s11_addr,
  ...
  if (saegw_s11_addr->sa_family == 0) {
    OAILOG_DEBUG(LOG_NAS_EMM,
                 "EMM-PROC  - No SAE-GW IP for UE " MME_UE_S1AP_ID_FMT
                 " to send PDN-Disc Req. \n",
                 ue_idP);
  }
  MSC_LOG_TX_MESSAGE(MSC_NAS_MME, MSC_MMEAPP_MME, NULL, 0,
...
//------------------------------------------------------------------------------
void nas_itti_ctx_req(const uint32_t ue_idP, const guti_t *const guti_p,
                      tai_t *const new_taiP, tai_t *const last_visited_taiP,
                      bstring request_msg) {
  OAILOG_FUNC_IN(LOG_NAS);
  MessageDef *message_p = NULL;
  ...
  MSC_LOG_TX_MESSAGE(MSC_NAS_MME, MSC_S10_MME, NULL, 0,
                     "0 S10_CTX_REQ GUTI " GUTI_FMT " originating TAI " TAI_FMT,
                     GUTI_ARG(&nas_context_req_p->old_guti),
                     TAI_FMT(last_visited_taiP));
  itti_send_msg_to_task(
      TASK_MME_APP, INSTANCE_DEFAULT,
      message_p); /**< Send it to MME_APP since we already may have one. */

  /** No NAS S10 context response timer needs to be started. It will be started
   * in the GTPv2c stack. */
  OAILOG_FUNC_OUT(LOG_NAS);
}

//------------------------------------------------------------------------------
void nas_itti_auth_info_req(const mme_ue_s1ap_id_t ue_idP,
  ...
  AssertFatal(
      (15 >= auth_info_req->imsi_length) && (0 < auth_info_req->imsi_length),
  ...
  MSC_LOG_TX_MESSAGE(MSC_NAS_MME, MSC_S6A_MME, NULL, 0,
                     "0 S6A_AUTH_INFO_REQ IMSI " IMSI_64_FMT
                     " visited_plmn " PLMN_FMT " re_sync %u",
                     auth_info_req->imsi, PLMN_ARG(visited_plmnP),
...
//------------------------------------------------------------------------------
void nas_itti_s11_bearer_resource_cmd(const pti_t pti, const ebi_t linked_ebi,
  ...
  if (saegw_s11_fteid->ipv4) {
    ...
    bearer_resource_cmd->edns_peer_ip.addr_v4.sin_addr =
        saegw_s11_fteid->ipv4_address;
  } else if (saegw_s11_fteid->ipv6) {
    bearer_resource_cmd->edns_peer_ip.addr_v6.sin6_family = AF_INET6;
    memcpy((void *)&bearer_resource_cmd->edns_peer_ip.addr_v6.sin6_addr,
           &saegw_s11_fteid->ipv6_address, sizeof(struct in6_addr));
  } else {
    AssertFatal(
        0, "Received neither IPv4 not IPv6 address for BRC sending via S11.");
  }

  MSC_LOG_TX_MESSAGE(MSC_NAS_MME, MSC_S6A_MME, NULL, 0,
                     "0 S11_BEARER_RESOURCE_CMD LOCAL_TEID " TEID_FMT
                     " PEER_TEID " TEID_FMT " ",
                     local_teid, peer_teid);
...
//------------------------------------------------------------------------------
void nas_itti_paging_due_signaling(const mme_ue_s1ap_id_t ue_id) {
  OAILOG_FUNC_IN(LOG_NAS);
  MessageDef *message_p = NULL;
  message_p =
      itti_alloc_new_message(TASK_NAS_ESM, NAS_PAGING_DUE_SIGNALING_IND);
...
//------------------------------------------------------------------------------
void nas_itti_establish_cnf(const mme_ue_s1ap_id_t ue_idP,
                            const nas_error_code_t error_codeP, bstring msgP,
  ...
  if (!emm_ctx) {
    OAILOG_ERROR(LOG_NAS_EMM,
                 "EMM-PROC  - No EMM context could be established for "
  ...
  message_p =
      itti_alloc_new_message(TASK_NAS_EMM, NAS_CONNECTION_ESTABLISHMENT_CNF);
  ...
  OAILOG_DEBUG(LOG_NAS_EMM,
               "EMM-PROC  - KeNB with UL Count %d for UE " MME_UE_S1AP_ID_FMT
               ". \n",
  ...
  OAILOG_STREAM_HEX(OAILOG_LEVEL_DEBUG, LOG_NAS,
                    "KENB: ", NAS_CONNECTION_ESTABLISHMENT_CNF(message_p).kenb,
                    32)

  /** Derive the next hop. */
  /** Calculate and set the next hop value into the context. Use the above
   * derived kenb as first nh value. */
  if (memcmp(emm_ctx->_vector[emm_ctx->_security.vector_index].nh_conj, zero,
             32) == 0) {
    OAILOG_DEBUG(
        LOG_NAS_EMM,
        "EMM-PROC  - NH value is 0 as expected. Setting kEnb as NH0  \n");
    memcpy(emm_ctx->_vector[emm_ctx->_security.vector_index].nh_conj,
           NAS_CONNECTION_ESTABLISHMENT_CNF(message_p).kenb, 32);
  } else {
    OAILOG_WARNING(LOG_NAS_EMM,
                   "EMM-PROC  - NH value is NOT 0 @ initial S1AP context "
                   "establishment  \n");
  }

  OAILOG_STREAM_HEX(OAILOG_LEVEL_DEBUG, LOG_NAS, "New NH_CONJ of emmCtx: ",
                    emm_ctx->_vector[emm_ctx->_security.vector_index].nh_conj,
                    32);

  /** Reset the next chaining hop counter.
   * TS. 33.401:
   * Upon receipt of the NAS Service Request message, the MME shall derive key
   KeNB as specified in Annex A.3 using the NAS COUNT [9] corresponding to the
   NAS Service Request and initialize the value of the Next hop Chaining Counter
    (NCC) to one. The MME shall further derive a next hop parameter NH as
   specified in Annex A.4 using the newly derived KeNB as basis for the
   derivation. This fresh {NH, NCC=1} pair shall be stored in the MME and shall
   be used for the next forward security key derivation. The MME shall
   communicate the {KeNB, NCC=0} pair and KSIASME to the serving eNB in the
   S1-AP procedure INITIAL CONTEXT SETUP.
   */
  ...
  MSC_LOG_TX_MESSAGE(
      ...
      " len %u sea %x sia %x ",
      ue_idP, blength(msgP), selected_encryption_algorithmP,
...
void nas_itti_detach_req(const mme_ue_s1ap_id_t ue_id) {
  ...
  OAILOG_INFO(
      LOG_NAS_EMM,
      "EMM-PROC  - Sending NAS_ITTI_DETACH_REQ for UE " MME_UE_S1AP_ID_FMT
      ". \n",
      ue_id);
...
void s6a_auth_info_rsp_timer_expiry_handler(void *args) {
  ...
  if (emm_ctx) {
    ...
    void *unused = NULL;
    nas_stop_Ts6a_auth_info(auth_info_proc->ue_id, &auth_info_proc->timer_s6a,
                            unused);

    auth_info_proc->timer_s6a.id = NAS_TIMER_INACTIVE_ID;
    if (auth_info_proc->resync) {
      OAILOG_ERROR(LOG_NAS_EMM,
                   "EMM-PROC  - Timer timer_s6_auth_info_rsp expired. Resync "
                   "auth procedure was in progress. Aborting attach procedure. "
                   "UE id " MME_UE_S1AP_ID_FMT "\n",
                   auth_info_proc->ue_id);
    } else {
      OAILOG_ERROR(LOG_NAS_EMM,
                   "EMM-PROC  - Timer timer_s6_auth_info_rsp expired. Initial "
                   "auth procedure was in progress. Aborting attach procedure. "
                   "UE id " MME_UE_S1AP_ID_FMT "\n",
                   auth_info_proc->ue_id);
    }
    ...
    OAILOG_ERROR(LOG_NAS_EMM,
                 "EMM-PROC  - Timer timer_s6_auth_info_rsp expired. Null EMM "
                 "Context for UE " MME_UE_S1AP_ID_FMT ". \n",
                 ue_id);
...
void s10_context_req_timer_expiry_handler(void *args) {
  ...
  if (emm_ctx) {
    ...
    //    nas_stop_Ts10_ctx_res(ctx_req_proc->ue_id, &ctx_req_proc->timer_s10,
    //    timer_callback_args);
    ...
    nas_itti_esm_detach_ind(ue_id, false);
    OAILOG_ERROR(LOG_NAS_EMM,
                 "EMM-PROC  - Timer timer_s10_context_req expired. Null EMM "
                 "Context for UE " MME_UE_S1AP_ID_FMT ". \n",
                 ue_id);
  }
