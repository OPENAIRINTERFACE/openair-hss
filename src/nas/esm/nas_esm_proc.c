...
/*****************************************************************************
  ...
  Description NAS ESM procedure call manager. This will be the main service
access point.
...
void _nas_proc_esm_timeout_handler(void *args) {
  ...
  if (rsp) {
    /**
     * Will get and lock the EMM context to set the security header if there is
     * a valid EMM context. No changes in the state of the EMM context.
     */
    if (esm_sap.data.eps_bearer_context_ll_cb_arg.ll_handler) {
      OAILOG_INFO(LOG_NAS_EMM,
                  "LL-Handler has been overwritten for timeout of "
                  "UE " MME_UE_S1AP_ID_FMT "\n",
                  esm_sap.data.eps_bearer_context_ll_cb_arg.ue_id);
      esm_sap.data.eps_bearer_context_ll_cb_arg.ll_handler(
          esm_sap.data.eps_bearer_context_ll_cb_arg.ue_id,
          esm_sap.data.eps_bearer_context_ll_cb_arg.eps_bearer_id, false, 0,
...
int nas_esm_proc_data_ind(itti_nas_esm_data_ind_t *esm_data_ind) {
  ...
  _esm_sap_recv(esm_data_ind->ue_id, &esm_data_ind->imsi,
                &esm_data_ind->visited_tai, esm_data_ind->req, &rsp);
  /** We don't check for the error.. If a response message is there, we directly
   * transmit it over the lower layers.. */
  if (rsp) {
    /**
     * Will get and lock the EMM context to set the security header if there is
     * a valid EMM context. No changes in the state of the EMM context.
...
int nas_esm_proc_esm_detach(itti_nas_esm_detach_ind_t *esm_detach) {
  ...
  MSC_LOG_TX_MESSAGE(MSC_NAS_MME, MSC_NAS_ESM_MME, NULL, 0,
                     "0 ESM_DETACH_IND " MME_UE_S1AP_ID_FMT " ",
                     esm_detach->ue_id);
...
//------------------------------------------------------------------------------
int nas_esm_proc_pdn_config_res(esm_cn_pdn_config_res_t *pdn_config_res) {
  OAILOG_FUNC_IN(LOG_NAS_ESM);
  ...
  MSC_LOG_TX_MESSAGE(MSC_NAS_MME, MSC_NAS_ESM_MME, NULL, 0,
                     "0 ESM_PDN_CONFIG_RES ue_id " MME_UE_S1AP_ID_FMT " ",
                     pdn_config_res->ue_id);
  esm_sap_signal(&esm_sap, &rsp);
  if (rsp) {
    /**
     * Will get and lock the EMM context to set the security header if there is
    ...
    if (esm_sap.is_attach_tau && esm_sap.active_ebrs) {
      ...
      OAILOG_WARNING(LOG_MME_APP,
                     "No ESM data received and no attach/tau signaled for "
                     "UE " MME_UE_S1AP_ID_FMT ".\n",
                     esm_sap.ue_id);
      rc = RETURNok;
    }
...
//------------------------------------------------------------------------------
int nas_esm_proc_pdn_config_fail(esm_cn_pdn_config_fail_t *pdn_config_fail) {
  OAILOG_FUNC_IN(LOG_NAS_ESM);
  ...
  MSC_LOG_TX_MESSAGE(MSC_NAS_MME, MSC_NAS_ESM_MME, NULL, 0,
                     "0 ESM_PDN_CONFIG_FAIL ue_id " MME_UE_S1AP_ID_FMT " ",
                     pdn_config_fail->ue_id);
  ...
  if (rsp) {
    if (esm_sap.is_attach_tau) {
      ...
      /**
       * Will get and lock the EMM context to set the security header if there
       * is a valid EMM context. No changes in the state of the EMM context.
       */
      rc = lowerlayer_data_req(esm_sap.ue_id, rsp);
    }
    bdestroy_wrapper(&rsp);
...
int nas_esm_proc_pdn_connectivity_res(
  ...
  esm_sap.primitive = (pdn_conn_res->esm_cause == ESM_CAUSE_SUCCESS)
  ...
  MSC_LOG_TX_MESSAGE(MSC_NAS_MME, MSC_NAS_ESM_MME, NULL, 0,
                     "0 ESM_PDN_CONNECTIVITY_RES ue_id " MME_UE_S1AP_ID_FMT " ",
                     pdn_conn_res->ue_id);
  esm_sap_signal(&esm_sap, &rsp);  // todo: esm_cause
  if (rsp) {
    if (pdn_conn_res->esm_cause == ESM_CAUSE_SUCCESS &&
        esm_sap.esm_cause == ESM_CAUSE_SUCCESS) {
      if (esm_sap.is_attach_tau)
        rc = _emm_wrapper_esm_accept(pdn_conn_res->ue_id, &rsp,
                                     esm_sap.active_ebrs);
      else {
        bearer_qos_t bearer_level_qos;
        memset(&bearer_level_qos, 0, sizeof(bearer_level_qos));
        rc = lowerlayer_activate_bearer_req(
            pdn_conn_res->ue_id, esm_sap.data.pdn_connectivity_res->linked_ebi,
...
//------------------------------------------------------------------------------
int nas_esm_proc_pdn_disconnect_res(
  ...
  MSC_LOG_TX_MESSAGE(MSC_NAS_MME, MSC_NAS_ESM_MME, NULL, 0,
                     "0 ESM_PDN_DISCONNECT_RES ue_id " MME_UE_S1AP_ID_FMT " ",
                     pdn_disconn_res->ue_id);
  esm_sap_signal(&esm_sap, &rsp);
  if (rsp) {
    if (esm_sap.esm_cause == ESM_CAUSE_SUCCESS) {
      rc = lowerlayer_deactivate_bearer_req(
          pdn_disconn_res->ue_id, esm_sap.data.pdn_disconnect_res->default_ebi,
          false, 0, NULL, rsp);
      bdestroy_wrapper(&rsp);
      int num_ded_ebi = 0;
      while (num_ded_ebi < esm_sap.data.pdn_disconnect_res->ded_ebis.num_ebi &&
             rc == RETURNok) {
        rc = lowerlayer_deactivate_bearer_req(
            pdn_disconn_res->ue_id,
            esm_sap.data.pdn_disconnect_res->ded_ebis.ebis[num_ded_ebi], false,
            0, NULL, NULL);
        num_ded_ebi++;
      }
    } else {
...
int nas_esm_proc_activate_eps_bearer_ctx(
  ...
  esm_sap.data.eps_bearer_context_activate.linked_ebi =
      esm_cn_activate->linked_ebi;
  /* Process each bearer context to be created separately. */
  bearer_contexts_to_be_created_t *bcs_tbc =
      (bearer_contexts_to_be_created_t *)esm_cn_activate->bcs_to_be_created_ptr;
  int num_bcs = bcs_tbc->num_bearer_context;
  for (int num_bc = 0; num_bc < num_bcs; num_bc++) {
    esm_sap.data.eps_bearer_context_activate.bc_tbc =
        &bcs_tbc->bearer_context[num_bc];
    esm_sap_signal(&esm_sap, &rsp);
    if (rsp) {
      rc = lowerlayer_activate_bearer_req(
          esm_cn_activate->ue_id,
          esm_sap.data.eps_bearer_context_activate.bc_tbc->eps_bearer_id,
          esm_sap.data.eps_bearer_context_activate.retry,
          esm_sap.data.eps_bearer_context_activate.retx_count,
...
int nas_esm_proc_modify_eps_bearer_ctx(
  ...
  esm_sap.data.eps_bearer_context_modify.apn_ambr.br_dl =
      esm_cn_modify->apn_ambr.br_dl;
  esm_sap.data.eps_bearer_context_modify.apn_ambr.br_ul =
      esm_cn_modify->apn_ambr.br_ul;
  /* Handle each bearer context separately. */
  bearer_contexts_to_be_updated_t *bcs_tbu =
      (bearer_contexts_to_be_updated_t *)esm_cn_modify->bcs_to_be_updated_ptr;
  int num_bcs = bcs_tbu->num_bearer_context;
  for (int num_bc = 0; num_bc < num_bcs; num_bc++) {
    esm_sap.data.eps_bearer_context_modify.bc_tbu =
        &bcs_tbu->bearer_context[num_bc];
    ...
    if (esm_sap.data.eps_bearer_context_modify.bc_tbu->bearer_level_qos) {
      OAILOG_DEBUG(LOG_MME_APP,
                   "Triggering bearer modification for changed qos (ebi=%d) "
                   "for UE " MME_UE_S1AP_ID_FMT ".\n",
                   esm_sap.data.eps_bearer_context_modify.bc_tbu->eps_bearer_id,
                   esm_sap.ue_id);
      rc = lowerlayer_modify_bearer_req(
          esm_cn_modify->ue_id,
          esm_sap.data.eps_bearer_context_modify.bc_tbu->eps_bearer_id,
          esm_sap.data.eps_bearer_context_modify.retry,
          esm_sap.data.eps_bearer_context_modify.retx_count,
          &esm_sap.data.eps_bearer_context_modify.bc_tbu->bearer_level_qos,
          rsp);
    } else {
      OAILOG_INFO(LOG_MME_APP,
                  "Not triggering bearer modification since qos is not changed "
                  "(ebi=%d) for UE " MME_UE_S1AP_ID_FMT ".\n",
                  esm_sap.data.eps_bearer_context_modify.bc_tbu->eps_bearer_id,
                  esm_sap.ue_id);
      rc = lowerlayer_data_req(esm_cn_modify->ue_id, rsp);
    }
    if (rsp) bdestroy_wrapper(&rsp);
...
int nas_esm_proc_deactivate_eps_bearer_ctx(
  ...
  MSC_LOG_TX_MESSAGE(
      MSC_NAS_MME, MSC_NAS_ESM_MME, NULL, 0,
      "0 ESM_EPS_BEARER_CONTEXT_DEACTIVATE_REQ " MME_UE_S1AP_ID_FMT " ",
      esm_cn_deactivate->ue_id);
  /** Handle each bearer context separately. */
  /** Get the bearer contexts to be updated. */

  int num_bcs = esm_cn_deactivate->ebis.num_ebi;
  for (int num_ebi = 0; num_ebi < num_bcs; num_ebi++) {
    esm_sap.data.eps_bearer_context_deactivate.ded_ebi =
        esm_cn_deactivate->ebis.ebis[num_ebi];
    ...
    rc = lowerlayer_deactivate_bearer_req(
        ...
        esm_sap.data.eps_bearer_context_deactivate.retx_count, NULL, rsp);
    if (rsp) {
      bdestroy_wrapper(&rsp);
    }
  }
...
//------------------------------------------------------------------------------
int nas_esm_proc_bearer_resource_failure_indication(
    itti_s11_bearer_resource_failure_indication_t
        *s11_bearer_resource_failure_ind) {
  OAILOG_FUNC_IN(LOG_NAS_ESM);
  ...
  ue_session_pool_t *ue_session_pool = mme_ue_session_pool_exists_s11_teid(
  ...
  MSC_LOG_TX_MESSAGE(MSC_NAS_MME, MSC_NAS_ESM_MME, NULL, 0,
                     "0 ESM_EPS_BEARER_RESOURCE_FAILURE_IND " MME_UE_S1AP_ID_FMT
                     " ",
                     esm_sap.ue_id);
  /** Get the bearer contexts to be updated. */
  esm_sap_signal(&esm_sap, &rsp);
