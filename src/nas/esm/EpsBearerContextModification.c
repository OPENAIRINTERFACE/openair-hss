...
modification procedure is initiated by the network, but it may also be initiated
as part of the UE requested bearer resource allocation procedure or the UE
...
update the APN-AMBR of the UE, for instance after an inter-system handover. See
3GPP TS 23.401 [10] annex E.
...
/*
   Timer handlers
*/
static esm_cause_t _modify_eps_bearer_context_t3486_handler(
    nas_esm_proc_t *esm_base_proc, ESM_msg *esm_resp_msg,
...
void esm_send_modify_eps_bearer_context_request(
   ...
   */
  esm_msg->modify_eps_bearer_context_request.protocoldiscriminator =
      EPS_SESSION_MANAGEMENT_MESSAGE;
  esm_msg->modify_eps_bearer_context_request.epsbeareridentity = ebi;
  esm_msg->modify_eps_bearer_context_request.messagetype =
  ...
  if (qos) {
    ...
    esm_msg->modify_eps_bearer_context_request.presencemask |=
   ...
   */
  if (tft) {
    memcpy(&esm_msg->modify_eps_bearer_context_request.tft, tft,
  ...
  if (ambr) {
    if (ambr->br_dl && ambr->br_ul) {
      ...
      ambr_kbps_calc(&esm_msg->modify_eps_bearer_context_request.apnambr,
                     (ambr->br_dl / 1000), (ambr->br_ul / 1000));
    }
  } else {
    OAILOG_WARNING(LOG_NAS_ESM,
                   "ESM-SAP   - no APN AMBR is present for activating default "
                   "eps bearer. \n");
  }

  if (pco) {
    memcpy(&esm_msg->modify_eps_bearer_context_request
                .protocolconfigurationoptions,
           pco, sizeof(protocol_configuration_options_t));
    esm_msg->modify_eps_bearer_context_request.presencemask |=
        MODIFY_EPS_BEARER_CONTEXT_REQUEST_PROTOCOL_CONFIGURATION_OPTIONS_PRESENT;
  }
  OAILOG_INFO(
      LOG_NAS_ESM,
      "ESM-SAP   - Send Modify EPS Bearer Context "
      "Request message (pti=%d, ebi=%d). \n",
      esm_msg->modify_eps_bearer_context_request.proceduretransactionidentity,
      esm_msg->modify_eps_bearer_context_request.epsbeareridentity);
...
esm_cause_t esm_proc_modify_eps_bearer_context(
  ...
  OAILOG_INFO(LOG_NAS_ESM,
              "ESM-PROC  - EPS bearer context modification "
  ...
  if (esm_proc_pdn_connectivity) {
    if (esm_proc_pdn_connectivity->default_ebi == linked_ebi) {
      OAILOG_ERROR(
          LOG_NAS_EMM,
          "EMMCN-SAP  - "
          "A PDN procedure for default ebi %d exists for UE " MME_UE_S1AP_ID_FMT
          " (cid=%d). Rejecting the establishment of the dedicated bearer.\n",
      ...
      OAILOG_WARNING(
          ...
          "EMMCN-SAP  - "
          "A PDN procedure for default ebi %d exists for UE " MME_UE_S1AP_ID_FMT
          " (cid=%d). Continuing with establishment of dedicated bearers.\n",
          esm_proc_pdn_connectivity->default_ebi, ue_id,
   ...
   * Create a new EPS bearer context transaction and starts the timer, since no
   * further CN operation necessary for dedicated bearers.
   */
  if (pti != PROCEDURE_TRANSACTION_IDENTITY_UNASSIGNED) {
    esm_proc_bearer_context =
        _esm_proc_get_bearer_context_procedure(ue_id, pti, ESM_EBI_UNASSIGNED);
    /** If no procedure.. reject the message. */
    if (!esm_proc_bearer_context) {
      OAILOG_ERROR(LOG_NAS_EMM,
                   "EMMCN-SAP  - "
                   "No ESM procedure for the received bearer modification "
    ...
    OAILOG_INFO(
        ...
        ". Starting its timer. Continuing to process the UBR.\n",
        pti, ue_id);
    ...
    esm_proc_bearer_context->esm_base_proc.esm_proc_timer.id =
        nas_esm_timer_start(mme_config.nas_config.t3486_sec, 0 /*usec*/,
                            (void *)&esm_proc_bearer_context->esm_base_proc);
    esm_proc_bearer_context->esm_base_proc.timeout_notif =
        _modify_eps_bearer_context_t3486_handler;
  } else {
    /** Check if this a retry (initially triggered by core network: PTI needs to
     * be 0), search for a bearer context modification procedure. */
    if (retry) {
      nas_esm_proc_bearer_context_t *esm_proc_bearer_context =
          _esm_proc_get_bearer_context_procedure(ue_id, pti,
                                                 bc_tbu->eps_bearer_id);
      if (esm_proc_bearer_context) {
        /** Retry the procedure. */
        esm_cause_t esm_cause = _modify_eps_bearer_context_t3486_handler(
            esm_proc_bearer_context, esm_rsp_msg,
            NULL); /**< The callback handler for ll cannot be changed. */
        *retx_count = esm_proc_bearer_context->esm_base_proc.retx_count;
        OAILOG_INFO(
            LOG_NAS_EMM,
            "EMMCN-SAP  - "
            "A bearer context modification procedure for UE " MME_UE_S1AP_ID_FMT
            " (pti=%d) already exists. Result of resending the message %d. \n",
            ue_id, pti, esm_cause);
  ...
  if (!esm_proc_bearer_context) {
    ...
    esm_proc_bearer_context = _esm_proc_create_bearer_context_procedure(
        ue_id, pti, linked_ebi, pdn_cid, bc_tbu->eps_bearer_id, INVALID_TEID,
        mme_config.nas_config.t3486_sec, 0 /*usec*/,
        _modify_eps_bearer_context_t3486_handler);
    if (!esm_proc_bearer_context) {
      OAILOG_ERROR(LOG_NAS_ESM,
                   "ESM-PROC  - Error creating a new procedure for the bearer "
                   "context (ebi=%d) for UE " MME_UE_S1AP_ID_FMT ". \n",
  ...
  esm_cause = mme_app_esm_modify_bearer_context(
      ue_id, bc_tbu->eps_bearer_id, NULL, ESM_EBR_MODIFY_PENDING,
      bc_tbu->bearer_level_qos, bc_tbu->tft, apn_ambr);
  if (esm_cause != ESM_CAUSE_SUCCESS) {
    OAILOG_ERROR(LOG_NAS_ESM,
                 "ESM-PROC  - Error validating the bearer context modification "
                 "procedure UE " MME_UE_S1AP_ID_FMT " (ebi=%d). \n",
                 ue_id, bc_tbu->eps_bearer_id);
    /** Send the error code depending on retry. */
    if (retry) {
      OAILOG_WARNING(LOG_NAS_ESM,
                     "ESM-PROC  - Retry for UBR of UE " MME_UE_S1AP_ID_FMT
                     " (ebi=%d). Rejecting UBR.  \n",
                     ue_id, bc_tbu->eps_bearer_id);
      ...
      OAILOG_WARNING(LOG_NAS_ESM,
                     "ESM-PROC  - No retry for UBR of UE " MME_UE_S1AP_ID_FMT
  ...
  if (bc_tbu->bearer_level_qos) {
    memcpy((void *)&esm_proc_bearer_context->bearer_level_qos,
           bc_tbu->bearer_level_qos, sizeof(bearer_qos_t));

    OAILOG_INFO(LOG_NAS_ESM,
                "ESM-PROC  - Verified EPS bearer context modification "
    ...
    qos_params_to_eps_qos(
        bc_tbu->bearer_level_qos->qci, bc_tbu->bearer_level_qos->mbr.br_dl,
        bc_tbu->bearer_level_qos->mbr.br_ul,
        bc_tbu->bearer_level_qos->gbr.br_dl,
        bc_tbu->bearer_level_qos->gbr.br_ul, &eps_qos1, false);
    eps_qos = &eps_qos1;
  }

  esm_send_modify_eps_bearer_context_request(
      pti, bc_tbu->eps_bearer_id, esm_rsp_msg, eps_qos,
...
esm_cause_t esm_proc_modify_eps_bearer_context_accept(
  ...
  esm_cause = mme_app_finalize_bearer_context(
      ue_id, esm_bearer_procedure->pdn_cid, esm_bearer_procedure->linked_ebi,
      ebi, &esm_bearer_procedure->apn_ambr,
  ...
  if (esm_cause == ESM_CAUSE_SUCCESS) {
    /**
     * The ESM procedure was terminated implicitly but the bearer context
     * remained (old status was same (ESM_ACTIVE). We changed the status of the
     * ESM bearer context, so the ESM reply came before any E-RAB failures, if
     * any.
     *
...
void esm_proc_modify_eps_bearer_context_reject(
   ...
   * Will also check if the bearer exists. If not (E-RAB Setup Failure), the
   * message will be dropped.
   */
  if (esm_cause == ESM_CAUSE_INVALID_EPS_BEARER_IDENTITY) {
    OAILOG_WARNING(LOG_NAS_ESM,
                   "ESM-PROC  - Deactivating the ESM bearer locally. "
                   "(ue_id=" MME_UE_S1AP_ID_FMT ", ebi=%d)\n",
                   ue_id, ebi);
    mme_app_release_bearer_context(ue_id, NULL, ESM_EBI_UNASSIGNED, ebi);
  } else {
    /* Finalize the bearer with the old values. */
    mme_app_finalize_bearer_context(ue_id, esm_bearer_procedure->pdn_cid,
                                    esm_bearer_procedure->linked_ebi, ebi, NULL,
                                    NULL, NULL, NULL);
  }
  /*
   * Stop the ESM procedure.
  ...
  /*
   * Inform the MME_APP only if it came as an ESM response, the bearer existed
   * and no E-RAB failure was received yet (else the ESM procedure would be
   * released by MME_APP).
   */
  nas_itti_modify_eps_bearer_ctx_rej(
      ue_id, ebi, esm_cause); /**< The SAE-GW should release the cause, too. */
  OAILOG_FUNC_OUT(LOG_NAS_ESM);
}
...
static esm_cause_t _modify_eps_bearer_context_t3486_handler(
  ...
  if (esm_base_proc->retx_count < EPS_BEARER_CONTEXT_MODIFY_COUNTER_MAX) {
    ...
    mme_app_get_pdn_context(
        esm_base_proc->ue_id, esm_proc_bearer_context->pdn_cid,
    ...
    if (pdn_context) {
      ...
      bearer_context_new_t *bearer_context = mme_app_get_session_bearer_context(
          pdn_context, esm_proc_bearer_context->bearer_ebi);
      if (bearer_context) {
        /** Always set the ll-handler as E-RAB modify (overwrite it). */
        if (ll_handler_arg) {
          memcpy(&ll_handler_arg->bearer_level_qos,
                 &bearer_context->bearer_level_qos,
                 sizeof(bearer_context->bearer_level_qos));
          ...
          OAILOG_WARNING(LOG_NAS_ESM,
                         "ESM-PROC  - Resetting ERAB modification values for "
        ...
        qos_params_to_eps_qos(bearer_context->bearer_level_qos.qci,
                              bearer_context->bearer_level_qos.mbr.br_dl,
                              bearer_context->bearer_level_qos.mbr.br_ul,
                              bearer_context->bearer_level_qos.gbr.br_dl,
                              bearer_context->bearer_level_qos.gbr.br_ul,
                              &eps_qos, false);
        esm_send_modify_eps_bearer_context_request(
            esm_base_proc->pti, bearer_context->ebi, esm_resp_msg,
            &bearer_context->bearer_level_qos,
        ...
        nas_stop_esm_timer(esm_base_proc->ue_id,
                           &esm_base_proc->esm_proc_timer);
  ...
  nas_itti_modify_eps_bearer_ctx_rej(
      ...
      esm_proc_bearer_context->bearer_ebi, ESM_CAUSE_REQUEST_REJECTED_BY_GW);
  /*
   * Release the dedicated EPS bearer context and enter state INACTIVE
   */
  mme_app_finalize_bearer_context(
      esm_proc_bearer_context->esm_base_proc.ue_id,
      esm_proc_bearer_context->pdn_cid, esm_proc_bearer_context->linked_ebi,
