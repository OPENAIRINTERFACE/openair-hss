  ...
  esm_cause_t *esm_cause)
{
  ...
  if (rc != RETURNerror) {
     ...
     */
    rc = esm_proc_eps_bearer_context_deactivate (esm_context, true, ebi, pid, NULL);
 ...
 **                                                                        **
 ** Description: Performs default EPS bearer context rejection procedure  **
 ...
 **                                                                        **
 **      The MME releases the default EPS bearer context. It might be      **
 **      allocated independently of if ACTIVATE DEFAULT EPS BEARER CONTEXT **
 **      REQUEST message was sent or not.                           **
 **                                                                        **
 ** Inputs:  ue_id:      UE local identifier                        **
 **      Others:    None                                       **
 **                                                                        **
 ...
 **                                                                        **
 ***************************************************************************/
int esm_proc_default_eps_bearer_context_failure (esm_context_t * esm_context, pdn_cid_t * const pid, ebi_t *ebi)
{
  OAILOG_FUNC_IN (LOG_NAS_ESM);
  int                                     rc = RETURNerror;
  mme_ue_s1ap_id_t                        ue_id = esm_context->ue_id;
