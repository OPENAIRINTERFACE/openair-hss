...
static esm_cause_t _default_eps_bearer_activate_t3485_handler(
...
void esm_send_activate_default_eps_bearer_context_request(
    ...
    protocol_configuration_options_t *pco, ESM_msg *esm_resp_msg) {
   ...
   */
  esm_resp_msg->activate_default_eps_bearer_context_request
      .protocoldiscriminator = EPS_SESSION_MANAGEMENT_MESSAGE;
  esm_resp_msg->activate_default_eps_bearer_context_request.epsbeareridentity =
      esm_proc_pdn_connectivity->default_ebi;
  esm_resp_msg->activate_default_eps_bearer_context_request.messagetype =
   ...
   * Mandatory - EPS QoS
   */
  esm_resp_msg->activate_default_eps_bearer_context_request.epsqos.qci =
      bearer_level_qos->qci;
  /*
   * Mandatory - Access Point Name
   * No need to copy, because directly encoded.
   */
  esm_resp_msg->activate_default_eps_bearer_context_request.accesspointname =
      bstrcpy(esm_proc_pdn_connectivity->subscribed_apn);
  /*
   * Mandatory - PDN address
   */
  OAILOG_INFO(LOG_NAS_ESM, "ESM-SAP   - pdn_type is %u\n",
              esm_proc_pdn_connectivity->pdn_type);
  esm_resp_msg->activate_default_eps_bearer_context_request.pdnaddress
      .pdntypevalue =
      esm_proc_pdn_connectivity->pdn_type; /**< PDN Type from PAA. */
  esm_resp_msg->activate_default_eps_bearer_context_request.pdnaddress
      .pdnaddressinformation = paa_to_bstring(paa);
  /*
  ...
  if (pco->num_protocol_or_container_id) {
    esm_resp_msg->activate_default_eps_bearer_context_request.presencemask |=
     ...
     * todo: After handover, the received PCOs might have been changed (like DNS
     * address), we need to check them and inform the UE. todo: We also need to
     * be able to ask the SAE-GW for specific PCOs? In handover, any change in
     * the ESM context information has to be forwarded to the UE --> MODIFY EPS
     * BEARER CONTEXT REQUEST!! (probably not possible with TAU accept).
     */
  ...
  ambr_kbps_calc(
      &esm_resp_msg->activate_default_eps_bearer_context_request.apnambr,
      (apn_ambr->br_dl / 1000), (apn_ambr->br_ul / 1000));
...
void esm_proc_default_eps_bearer_context(
  ...
  OAILOG_INFO(LOG_NAS_ESM,
              ...
              ", context_identifier=%d, bc_status %d)\n",
  ...
  if (esm_proc_pdn_connectivity->is_attach) {
    ...
    OAILOG_INFO(LOG_NAS_ESM,
                "ESM-PROC  - Starting T3485 for Default EPS bearer context "
                "activation (ue_id=" MME_UE_S1AP_ID_FMT
                ", context_identifier=%d)\n",
                ue_id, esm_proc_pdn_connectivity->pdn_cid);
    /** Stop any timer if running. */
    nas_stop_esm_timer(
        ue_id, &esm_proc_pdn_connectivity->esm_base_proc.esm_proc_timer);
    /** Start the T3485 timer for additional PDN connectivity. */
    esm_proc_pdn_connectivity->esm_base_proc.esm_proc_timer.id =
        nas_esm_timer_start(
            mme_config.nas_config.t3485_sec, 0 /*usec*/,
            (nas_esm_proc_t *)
                esm_proc_pdn_connectivity); /**< Address field should be big
                                               enough to save an ID. */
    esm_proc_pdn_connectivity->esm_base_proc.timeout_notif =
  ...
  if (pdn_context) {
    ...
    if (default_bc) {
      ...
      esm_send_activate_default_eps_bearer_context_request(
          esm_proc_pdn_connectivity, &pdn_context->subscribed_apn_ambr,
          &default_bc->bearer_level_qos, pdn_context->paa, pdn_context->pco,
          esm_rsp_msg);
    }
  }
  if (esm_proc_pdn_connectivity->bc_status)
    _esm_proc_free_pdn_connectivity_procedure(
        &esm_proc_pdn_connectivity); /**< Procedure is completed. */
  OAILOG_FUNC_OUT(LOG_NAS_ESM);
}
...
void esm_proc_default_eps_bearer_context_accept(
   ...
   * The CN state & FTEID will be set by the MME_APP layer in independently of
   * this. The PCO of the ESM procedure should be updated.
   */
  ...
  if (rc != RETURNok) {
    OAILOG_WARNING(LOG_NAS_ESM,
                   "ESM-PROC  - Default EPS bearer context activation accept, "
                   "could not be processed for UE (ue_id=" MME_UE_S1AP_ID_FMT
                   ", ebi=%d). "
...
static esm_cause_t _default_eps_bearer_activate_t3485_handler(
  ...
  if (esm_base_proc->retx_count < DEFAULT_EPS_BEARER_ACTIVATE_COUNTER_MAX) {
    OAILOG_INFO(LOG_NAS_ESM,
                "ESM-PROC  - Retransmitting Default EPS bearer context "
                "activation request to UE (ue_id=" MME_UE_S1AP_ID_FMT
                ", ebi=%d)\n",
    ...
    mme_app_get_pdn_context(
        esm_base_proc->ue_id, esm_proc_pdn_connectivity->pdn_cid,
        esm_proc_pdn_connectivity->default_ebi,
    ...
    if (pdn_context) {
      bearer_context_new_t *bearer_context = mme_app_get_session_bearer_context(
          pdn_context, esm_proc_pdn_connectivity->default_ebi);
      if (bearer_context) {
        ...
        if (!(bearer_context->bearer_state & BEARER_STATE_ENB_CREATED)) {
          ...
          OAILOG_WARNING(
              LOG_NAS_ESM,
              "ESM-PROC  - Default EPS bearer context not established on the "
        ...
        esm_send_activate_default_eps_bearer_context_request(
            esm_proc_pdn_connectivity, &pdn_context->subscribed_apn_ambr,
            &bearer_context->bearer_level_qos, pdn_context->paa,
            pdn_context->pco, esm_resp_msg);
        OAILOG_INFO(
            LOG_NAS_ESM,
            "ESM-PROC  - Successfully retransmitted Default EPS bearer context "
            "activation request to UE (ue_id=" MME_UE_S1AP_ID_FMT ", ebi=%d)\n",
            esm_proc_pdn_connectivity->esm_base_proc.ue_id,
            esm_proc_pdn_connectivity->default_ebi);
  ...
  OAILOG_INFO(LOG_NAS_ESM,
              "ESM-PROC  - Max timeout occurred or could not retransmit "
              "Default EPS bearer context activation request to UE. Rejecting "
              "PDN context activation. "
              "(ue_id=" MME_UE_S1AP_ID_FMT ", ebi=%d)\n",
              esm_base_proc->ue_id, esm_proc_pdn_connectivity->default_ebi);
  /*
   * Prepare the reject message.
   */
  esm_send_pdn_connectivity_reject(esm_proc_pdn_connectivity->esm_base_proc.pti,
                                   esm_resp_msg, ESM_CAUSE_NETWORK_FAILURE);
  /*
   * Release the transactional PDN connectivity procedure.
   */
  esm_proc_pdn_connectivity_failure(esm_base_proc->ue_id,
