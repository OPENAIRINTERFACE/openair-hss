...
esm_cause_t esm_recv_status(const mme_ue_s1ap_id_t ue_id, proc_tid_t pti,
                            ebi_t ebi, const esm_status_msg *msg) {
  OAILOG_FUNC_IN(LOG_NAS_ESM);
  esm_cause_t esm_cause = ESM_CAUSE_SUCCESS;

  OAILOG_INFO(LOG_NAS_ESM,
              "ESM-SAP   - Received ESM status message (pti=%d, ebi=%d) for "
...
esm_cause_t esm_recv_pdn_connectivity_request(
    bool *is_attach, const mme_ue_s1ap_id_t ue_id, const imsi_t *imsi,
    proc_tid_t pti, ebi_t ebi, tai_t *visited_tai,
   ...
   * No procedure should exist at all (also no network triggered procedure
   * without PTI. Not checking any bearer context procedure at this step.
   */
  ...
  if (esm_proc_pdn_connectivity) {
    OAILOG_INFO(LOG_NAS_ESM,
                "ESM-SAP   - Found a PDN connectivity procedure (pti=%d) for "
                "UE " MME_UE_S1AP_ID_FMT ".\n",
                esm_proc_pdn_connectivity->esm_base_proc.pti, ue_id);
    /** We have another transactional procedure. If the PTIs don't match, reject
     * the message. */
    if (esm_proc_pdn_connectivity->esm_base_proc.pti == pti) {
      /* We have a duplicate PDN connectivity request. */
      DevAssert(!*is_attach);
      OAILOG_INFO(LOG_NAS_ESM,
                  "ESM-SAP   - A PDN connectivity request (pti=%d) for "
                  "UE " MME_UE_S1AP_ID_FMT
                  " is already received. \n"
                  " Continuing to check the new procedure (pti=%d).\n",
                  esm_proc_pdn_connectivity->esm_base_proc.pti, ue_id, pti);
      esm_cause_t esm_cause = esm_proc_pdn_connectivity_retx(
          ue_id, esm_proc_pdn_connectivity, esm_rsp_msg);
      if (esm_cause == ESM_CAUSE_SUCCESS && esm_rsp_msg->header.message_type) {
        /** Restart the T3485 timer and resend the
         * ACTIVATE_DEFAULT_EPS_BEARER_CONTEXT message. */
        esm_proc_default_eps_bearer_context(ue_id, esm_rsp_msg,
                                            esm_proc_pdn_connectivity);
      }
      OAILOG_FUNC_RETURN(LOG_NAS_ESM, esm_cause);
    } else {
      OAILOG_ERROR(LOG_NAS_ESM,
                   "ESM-SAP   - Already an existing UE triggered ESM PDN "
                   "connectivity procedure (pti=%d) for UE " MME_UE_S1AP_ID_FMT
                   " (no pdn context)."
                   "Removing old procedure and rejecting request for new "
                   "procedure (pti=%d).\n",
                   esm_proc_pdn_connectivity->esm_base_proc.pti, ue_id, pti);
  ...
  nas_esm_proc_bearer_context_t *esm_proc_bearer_context =
      _esm_proc_get_bearer_context_procedure(
          ue_id, PROCEDURE_TRANSACTION_IDENTITY_UNASSIGNED, ESM_EBI_UNASSIGNED);
  if (esm_proc_bearer_context) {
    OAILOG_ERROR(LOG_NAS_ESM,
                 "ESM-SAP   - An Bearer Context procedure is ongoing. Ignoring "
                 "received PDN connectivity request for UE " MME_UE_S1AP_ID_FMT
                 ". \n",
                 ue_id);
  ...
  if (msg->requesttype == REQUEST_TYPE_INITIAL_REQUEST) {
    ...
    pdn_request_type = -1;
    OAILOG_ERROR(LOG_NAS_ESM,
                 "ESM-SAP   - Invalid PDN request type "
                 "(INITIAL/HANDOVER/EMERGENCY) (%d) for UE " MME_UE_S1AP_ID_FMT
                 ".\n",
                 ue_id);
  ...
  if (pdn_request_type != ESM_PDN_REQUEST_INITIAL) {
    OAILOG_ERROR(LOG_NAS_ESM,
                 "ESM-SAP   - No other request type than INITIAL_REQUEST is "
                 "supported for PDN connectivity procedure. "
                 "Rejecting the PDN connectivity procedure  (req_type=%d)."
                 "(ue_id=%d, pti=%d)\n",
  ...
  if (mme_app_select_apn(imsi64, msg->accesspointname, &apn_configuration) ==
      RETURNerror) {
    DevAssert(msg->accesspointname);
    OAILOG_ERROR(
        LOG_NAS_ESM,
        "ESM-SAP   - No APN configuration could be found for APN \"%s\". "
        "Rejecting the PDN connectivity procedure."
        "(ue_id=%d, pti=%d)\n",
  ...
  *is_attach = ue_context->privates.fields.mm_state == UE_UNREGISTERED;
  ...
  if (!*is_attach && !(msg->presencemask &
                       PDN_CONNECTIVITY_REQUEST_ACCESS_POINT_NAME_PRESENT)) {
    OAILOG_ERROR(LOG_NAS_ESM,
                 "ESM-SAP   - No APN name present for extra PDN connectivity "
                 "request. Rejecting the PDN connectivity procedure."
                 "(ue_id=%d, pti=%d)\n",
                 ue_id, pti);
    OAILOG_FUNC_RETURN(LOG_NAS_ESM, ESM_CAUSE_UNKNOWN_ACCESS_POINT_NAME);
  }
  ...
  /**
   * Create a new ESM procedure for PDN Connectivity. Not important if we
   * continue with ESM information or PDN connectivity, since this function will
   * handle the received messages (no specific callback needed).
   */
  DevAssert(!esm_proc_pdn_connectivity);
  OAILOG_DEBUG(LOG_NAS_ESM,
               "ESM-SAP   - No ESM procedure for UE " MME_UE_S1AP_ID_FMT
               " exists. Proceeding with handling the new ESM request (pti=%d) "
               "for PDN connectivity.\n",
               ue_id, pti);
  esm_proc_pdn_connectivity =
      _esm_proc_create_pdn_connectivity_procedure(ue_id, imsi, pti);
  ...
  if (msg->presencemask &
    ...
    if (!mme_config.nas_config.disable_esm_information &&
        msg->esminformationtransferflag) {
      /**
      ...
      OAILOG_WARNING(
          LOG_NAS_ESM,
          "ESM-SAP   - Ignoring received ESM Information Transfer flag. "
          "(ue_id=%d, pti=%d)\n",
          ue_id, pti);
  ...
  if (!esm_proc_pdn_connectivity->subscribed_apn) {
    /** No APN Name received from UE. We will used the default one from the
     * subscription data. */
    OAILOG_INFO(
        LOG_NAS_ESM,
        "ESM-SAP   - No APN name received from UE to establish PDN connection "
        "and subscription exists. "
        "Will attach to the default APN in the subscription information. "
        "(ue_id=%d, pti=%d)\n",
        ue_id, pti);
  }
  ...
  if (msg->presencemask &
    ...
    copy_protocol_configuration_options(&esm_proc_pdn_connectivity->pco,
                                        &msg->protocolconfigurationoptions);
  }
  if (!apn_configuration) { /**< We should have an APN identifier by now. If no
                               APN-Config is available, request subscription
                               data. */
    OAILOG_INFO(LOG_NAS_ESM,
                "ESM-SAP   - Getting subscription profile for IMSI " IMSI_64_FMT
                ". "
                "(ue_id=%d, pti=%d)\n",
                imsi64, ue_id, pti);
    nas_itti_pdn_config_req(ue_id, imsi,
                            esm_proc_pdn_connectivity->request_type,
                            &esm_proc_pdn_connectivity->visited_tai.plmn);
    /*
  ...
  OAILOG_INFO(LOG_NAS_ESM,
              "ESM-SAP   - Found a valid (default) APN configuration (cid=%d). "
              "Updating the UE context, if it is an attach procedure. Else, "
              "continuing with the PDN Connectivity procedure."
              "(ue_id=%d, pti=%d)\n",
  ...
  if (!esm_proc_pdn_connectivity->subscribed_apn) {
    esm_proc_pdn_connectivity->subscribed_apn =
        blk2bstr(apn_configuration->service_selection,
                 apn_configuration
                     ->service_selection_length); /**< Set the APN-NI from the
                                                     service selection. */
  }
  ...
  if (esm_proc_pdn_connectivity->is_attach) {
    subscription_data_t *subscription_data =
        mme_ue_subscription_data_exists_imsi(&mme_app_desc.mme_ue_contexts,
                                             imsi64);
    if (mme_app_update_ue_subscription(
            esm_proc_pdn_connectivity->esm_base_proc.ue_id,
            subscription_data) == RETURNerror) {
      OAILOG_ERROR(LOG_NAS_ESM,
                   "ESM-SAP   - Error while trying to update the subscription "
                   "data for the UE_context for APN \"%s\" (cid=%d). "
                   "(ue_id=" MME_UE_S1AP_ID_FMT ", pti=%d)\n",
                   bdata(esm_proc_pdn_connectivity->subscribed_apn),
                   esm_proc_pdn_connectivity->pdn_cid, ue_id, pti);
      /** Remove the procedure and send a pdn rejection back, which will trigger
       * the attach reject. */
      _esm_proc_free_pdn_connectivity_procedure(&esm_proc_pdn_connectivity);
      OAILOG_FUNC_RETURN(LOG_NAS_ESM, ESM_CAUSE_REQUEST_REJECTED_BY_GW);
    }
  ...
  /*
   * Establish the PDN Connectivity, using the default APN-QoS values received
   * in the subscription information, which will be processed in the MME_APP
   * layer inside the UE_CONTEXT. Just trigger PDN connectivity here.
   *
   ...
   */
  esm_cause_t esm_cause = esm_proc_pdn_connectivity_request(
      ue_id, imsi, visited_tai, esm_proc_pdn_connectivity, apn_configuration);
  //      (pco.num_protocol_or_container_id ) ? &pco:NULL);
  if (esm_cause != ESM_CAUSE_SUCCESS) {
    OAILOG_ERROR(LOG_NAS_ESM,
                 "ESM-SAP   - Error while trying to establish an APN session "
                 "for APN \"%s\" (cid=%d). "
                 "(ue_id=%d, pti=%d)\n",
                 bdata(esm_proc_pdn_connectivity->subscribed_apn),
                 esm_proc_pdn_connectivity->pdn_cid, ue_id, pti);
...
esm_cause_t esm_recv_information_response(
  ...
  nas_esm_proc_pdn_connectivity_t *esm_proc_pdn_connectivity =
      _esm_proc_get_pdn_connectivity_procedure(ue_id, pti);
  if (!esm_proc_pdn_connectivity) {
    OAILOG_WARNING(LOG_NAS_ESM,
                   "ESM-SAP   - No procedure for UE " MME_UE_S1AP_ID_FMT
                   " found. Ignoring the received ESM information request.\n",
                   ue_id);
    OAILOG_FUNC_RETURN(LOG_NAS_ESM, ESM_CAUSE_SUCCESS);
  }

  /** We have another transactional procedure. If the PTIs don't match, reject
   * the message and remove the procedure. */
  if (esm_proc_pdn_connectivity->esm_base_proc.pti != pti) {
    _esm_proc_free_pdn_connectivity_procedure(&esm_proc_pdn_connectivity);
    /** Not removing the associated PDN context yet. */
    OAILOG_ERROR(LOG_NAS_ESM,
                 "ESM-SAP   - Already an existing UE triggered ESM procedure "
                 "(pti=%d, type=%d) for UE " MME_UE_S1AP_ID_FMT
                 "."
                 " Rejecting request for new procedure (pti=%d).\n",
                 esm_proc_pdn_connectivity->esm_base_proc.pti,
                 esm_proc_pdn_connectivity->esm_base_proc.type, ue_id, pti);
    OAILOG_FUNC_RETURN(LOG_NAS_ESM, ESM_CAUSE_INVALID_PTI_VALUE);
  }
  ...
  nas_esm_proc_bearer_context_t *esm_proc_bearer_context =
      _esm_proc_get_bearer_context_procedure(
          ue_id, PROCEDURE_TRANSACTION_IDENTITY_UNASSIGNED, ESM_EBI_UNASSIGNED);
  if (esm_proc_bearer_context) {
    /** A network initiated procedure exists. Not allowed (not removing the
     * procedure). */
    OAILOG_INFO(LOG_NAS_ESM,
                "ESM-SAP   - UE initiated ESM information procedure (pti=%d, "
                "type=%d) for UE " MME_UE_S1AP_ID_FMT
                " collided with network initiated bearer procedure (pti=%d).\n",
                esm_proc_pdn_connectivity->esm_base_proc.pti, ue_id,
  ...
  /** Procedure already exists. Check if a response message is appended to the
   * procedure.. Return the procedure. */
  OAILOG_DEBUG(
      LOG_NAS_ESM,
      "ESM-SAP   - Found a valid ESM procedure for UE " MME_UE_S1AP_ID_FMT
      ". Proceeding with handling the new ESM request (pti=%d) for PDN "
      "connectivity.\n",
      ue_id, pti);
  *is_attach = esm_proc_pdn_connectivity->is_attach;
   ...
   */
  apn_configuration_t *apn_configuration = NULL;
  if (mme_app_select_apn(imsi64, esm_proc_pdn_connectivity->subscribed_apn,
                         &apn_configuration) == RETURNerror) {
  ...
  if (!apn_configuration) { /**< Will always be the default configuration, even
                               if a name is given. */
    OAILOG_INFO(LOG_NAS_ESM,
                "ESM-SAP   - Getting subscription profile for IMSI " IMSI_64_FMT
                ". "
   ...
   * Execute the PDN connectivity procedure requested by the UE
   */
  if (esm_proc_pdn_connectivity_request(
          ue_id, &esm_proc_pdn_connectivity->imsi,
          &esm_proc_pdn_connectivity->visited_tai, esm_proc_pdn_connectivity,
          apn_configuration) != ESM_CAUSE_SUCCESS) {
    //      (esm_context->esm_proc_data->pco.num_protocol_or_container_id ) ?
    //      &esm_context->esm_proc_data->pco:NULL);
    OAILOG_ERROR(LOG_NAS_ESM,
                 "ESM-SAP   - Error while trying to establish an APN session "
                 "for APN \"%s\" (cid=%d). "
                 "(ue_id=%d, pti=%d)\n",
...
esm_cause_t esm_recv_pdn_disconnect_request(
   ...
   */
    ...
    /*
     * 3GPP TS 24.301, section 7.3.2, case b
     * * * * Reserved or assigned EPS bearer identity value (Transaction Related
     * messages).
     */
    OAILOG_WARNING(LOG_NAS_ESM,
                   "ESM-SAP   - Invalid EPS bearer identity (ebi=%d)\n", ebi);
  ...
  if (esm_proc_pdn_disconnect) {
    OAILOG_WARNING(LOG_NAS_ESM,
                   "ESM-SAP   - An PDN Connectivity procedure for UE "
                   "ueId " MME_UE_S1AP_ID_FMT " exists. . \n",
                   ue_id);
    if (esm_proc_pdn_disconnect->esm_base_proc.pti != pti) {
      OAILOG_ERROR(LOG_NAS_ESM,
                   ...
                   "."
                   " Rejecting request for new procedure (pti=%d).\n",
                   esm_proc_pdn_disconnect->esm_base_proc.pti,
                   esm_proc_pdn_disconnect->esm_base_proc.type, ue_id, pti);
      OAILOG_FUNC_RETURN(LOG_NAS_ESM, ESM_CAUSE_REQUEST_REJECTED_BY_GW);
    }
    /** Procedure already exists. Check if a response message is appended to the
     * procedure.. Return the procedure. */
    OAILOG_WARNING(LOG_NAS_ESM,
                   "ESM-SAP   - Found a valid ESM pdn connectivity procedure "
                   "(pti=%d) for UE " MME_UE_S1AP_ID_FMT
                   ". Rehandling the ESM disconnection procedure. \n",
                   pti, ue_id);
    /*
     * Return deactivate EPS bearer context request message for the default
     * bearer of the pdn connection. Sending the ESM Request directly, makes
     * retransmission handling easier. We are not interested in the outcome of
     * the Delete Session Response.
     */
     ...
     * Restart the timer.
     */
    nas_stop_esm_timer(ue_id,
                       &esm_proc_pdn_disconnect->esm_base_proc.esm_proc_timer);
    /** Start the T3485 timer for additional PDN connectivity. */
    esm_proc_pdn_disconnect->esm_base_proc.esm_proc_timer.id =
        nas_esm_timer_start(
            mme_config.nas_config.t3495_sec, 0,
            (nas_esm_proc_t *)
                esm_proc_pdn_disconnect); /**< Address field should be big
                                             enough to save an ID. */
    /* Don't change the timeout handler (should still be
  ...
  if (esm_proc_bearer_context) {
    ...
    OAILOG_INFO(LOG_NAS_ESM,
                ...
                " collided with network initiated bearer procedure (pti=0).\n",
                pti, ue_id);
   ...
   * Validating the message in the context of the UE and finding the PDN context
   * to remove (validating not last PDN). Setting the ESM contexts of the bearer
   * into INACTIVE_PENDING state (not caring about the CN states). Triggering a
   * Delete Session Request for the PDN.
   */
  // todo: PCOs @ esm disconnect
  esm_cause_t esm_cause = esm_proc_pdn_disconnect_request(
      ue_id, pti, PDN_CONTEXT_IDENTIFIER_UNASSIGNED,
...
esm_cause_t esm_recv_activate_default_eps_bearer_context_accept(
  ...
  if (!esm_pdn_connectivity_proc) {
    OAILOG_WARNING(LOG_NAS_ESM,
                   "ESM-SAP   - No procedure for UE " MME_UE_S1AP_ID_FMT
                   " found. Ignoring the received ESM message ADBA.\n",
                   ue_id);
    OAILOG_FUNC_RETURN(LOG_NAS_ESM, ESM_CAUSE_SUCCESS);
  }
  ...
  if (pending_qos) {
    OAILOG_WARNING(LOG_NAS_EMM,
                   "EMM-PROC  - UE for ueId " MME_UE_S1AP_ID_FMT
                   " has a pending QoS procedure waiting the ESM PDN "
...
esm_cause_t esm_recv_activate_dedicated_eps_bearer_context_accept(
  ...
  if (!esm_bearer_procedure) {
    OAILOG_ERROR(
        LOG_NAS_ESM,
        "ESM-PROC  - No ESM bearer procedure exists for successfully activated "
        "dedicated bearer (ebi=%d, pti=%d) for UE " MME_UE_S1AP_ID_FMT ". \n",
        ebi, pti, ue_id);
   ...
   * * * * by the UE
   */
  esm_cause = esm_proc_dedicated_eps_bearer_context_accept(
      ue_id, ebi, esm_bearer_procedure);
  OAILOG_FUNC_RETURN(LOG_NAS_ESM, esm_cause);
}
...
esm_cause_t esm_recv_activate_dedicated_eps_bearer_context_reject(
  ...
  if (!esm_proc_bearer_context) {
    OAILOG_WARNING(LOG_NAS_ESM,
                   "ESM-PROC  - No EPS bearer context modification procedure "
                   "exists for pti=%d"
  ...
  esm_proc_dedicated_eps_bearer_context_reject(
      ue_id, ebi, esm_proc_bearer_context, msg->esmcause);
  OAILOG_FUNC_RETURN(LOG_NAS_ESM, ESM_CAUSE_SUCCESS);
}
...
esm_cause_t esm_recv_modify_eps_bearer_context_accept(
  ...
  if (!esm_bearer_procedure) {
    OAILOG_ERROR(
        LOG_NAS_ESM,
        "ESM-PROC  - No ESM bearer procedure exists for successfully modified "
        "dedicated bearer (ebi=%d, pti=%d) for UE " MME_UE_S1AP_ID_FMT ". \n",
        ebi, pti, ue_id);
    OAILOG_FUNC_RETURN(LOG_NAS_ESM, ESM_CAUSE_REQUEST_REJECTED_UNSPECIFIED);
  }
   ...
   * * * * by the UE
   */
  esm_cause = esm_proc_modify_eps_bearer_context_accept(ue_id, ebi,
                                                        esm_bearer_procedure);
  OAILOG_FUNC_RETURN(LOG_NAS_ESM, esm_cause);
}
...
esm_cause_t esm_recv_modify_eps_bearer_context_reject(
   ...
   * * * *  accepted by the UE
   */
  esm_proc_modify_eps_bearer_context_reject(ue_id, ebi, esm_proc_bearer_context,
                                            msg->esmcause);
  OAILOG_FUNC_RETURN(LOG_NAS_ESM, ESM_CAUSE_SUCCESS);
}
 ...
 **                                                                        **
 ***************************************************************************/
esm_cause_t esm_recv_bearer_resource_allocation(
    mme_ue_s1ap_id_t ue_id, proc_tid_t pti, ebi_t ebi,
   ...
   * * * *  accepted by the UE
   */
  esm_cause_t esm_cause = esm_proc_bearer_resource_allocation_request(
      ue_id, pti, ebi, &msg->trafficflowaggregate);
  OAILOG_FUNC_RETURN(LOG_NAS_ESM, esm_cause);
}
 ...
 **                                                                        **
 ***************************************************************************/
esm_cause_t esm_recv_bearer_resource_modification(
    mme_ue_s1ap_id_t ue_id, proc_tid_t pti, ebi_t ebi,
  ...
  nas_esm_proc_bearer_context_t *esm_proc_bearer_context =
      _esm_proc_get_bearer_context_procedure(
          ue_id, PROCEDURE_TRANSACTION_IDENTITY_UNASSIGNED, ebi);
  if (esm_proc_bearer_context) {
    /** If there is a procedure already existing, check the PTI. */
    if (esm_proc_bearer_context->esm_base_proc.pti == pti) {
      /** Ignore the currently received message, no matter if there is a change
       * in the message or not. */
      OAILOG_INFO(
          LOG_NAS_ESM,
          "ESM-PROC  - Ignoring received Bearer Resource Modification Request "
          "(ebi=%d ue_id=" MME_UE_S1AP_ID_FMT
          ", pti=%d), "
      ...
      OAILOG_INFO(
          LOG_NAS_ESM,
          "ESM-PROC  - Rejecting received Bearer Resource Modification Request "
          "(ebi=%d ue_id=" MME_UE_S1AP_ID_FMT
          ", pti=%d), "
          "because there already is a ESM procedure for this bearer with "
          "pti=%d. \n",
   ...
   * * * *  accepted by the UE
   */
  esm_cause = esm_proc_bearer_resource_modification_request(
      ue_id, pti, ebi, msg->esmcause, &msg->trafficflowaggregate,
      &msg->requiredtrafficflowqos);
  OAILOG_FUNC_RETURN(LOG_NAS_ESM, esm_cause);
}
...
esm_cause_t esm_recv_deactivate_eps_bearer_context_accept(
  ...
  if (!esm_base_proc) {
    ...
    if (!esm_base_proc) {
      OAILOG_WARNING(LOG_NAS_ESM,
                     "ESM-SAP   - No ESM procedure (neither bearer nor pdn) "
                     "could be found for UE " MME_UE_S1AP_ID_FMT " (pti=%d)\n",
                     ue_id, pti);
      OAILOG_FUNC_RETURN(LOG_NAS_ESM, ESM_CAUSE_INVALID_PTI_VALUE);
    }
  }
  if (esm_base_proc->type == ESM_PROC_PDN_CONTEXT) {
    ...
    mme_app_esm_delete_pdn_context(
        ue_id, esm_proc_pdn_connectivity->subscribed_apn,
    ...
    mme_app_release_bearer_context(ue_id, NULL,
                                   esm_proc_bearer_context->linked_ebi, ebi);
    ...
    OAILOG_WARNING(
        LOG_NAS_ESM,
        "ESM-SAP   - We released  the dedicated EBI. Responding with delete "
        "bearer response back. (ebi=%d) for UE " MME_UE_S1AP_ID_FMT ". \n",
        ebi, ue_id);
    /** Respond per bearer. */
