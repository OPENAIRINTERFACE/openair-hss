...
/*
   ...
   --------------------------------------------------------------------------
*/
static esm_cause_t _bearer_resource_modification_timeout_handler(
 ...
 **                                                                        **
 ***************************************************************************/
void esm_send_bearer_resource_modification_reject(pti_t pti, ESM_msg *esm_msg,
   ...
   */
  esm_msg->bearer_resource_modification_reject.protocoldiscriminator =
      EPS_SESSION_MANAGEMENT_MESSAGE;
  esm_msg->bearer_resource_modification_reject.epsbeareridentity =
      EPS_BEARER_IDENTITY_UNASSIGNED;
  esm_msg->bearer_resource_modification_reject.messagetype =
      BEARER_RESOURCE_MODIFICATION_REJECT;
  esm_msg->bearer_resource_modification_reject.proceduretransactionidentity =
      pti;
  /*
  ...
  OAILOG_DEBUG(
      LOG_NAS_ESM,
      "ESM-SAP   - Send Bearer Resource Modification Reject message "
      "(pti=%d, ebi=%d)\n",
      esm_msg->bearer_resource_modification_reject.proceduretransactionidentity,
      esm_msg->bearer_resource_modification_reject.epsbeareridentity);
...
esm_cause_t esm_proc_bearer_resource_modification_request(
  ...
  OAILOG_INFO(LOG_NAS_ESM,
              ...
              "(ebi=%d, ue_id=" MME_UE_S1AP_ID_FMT ", pti=%d, esm_cause=%d)\n",
              ebi, ue_id, pti, esm_cause_received);

  memset(&flow_qos, 0, sizeof(flow_qos_t));

  /** Process the message. */
  if (new_flow_qos && new_flow_qos->qci &&
      (new_flow_qos->bitRatesPresent | new_flow_qos->bitRatesExtPresent)) {
    if (new_flow_qos->bitRatesPresent) {
      flow_qos.gbr.br_dl =
          eps_qos_bit_rate_value(new_flow_qos->bitRates.guarBitRateForDL);
      flow_qos.gbr.br_ul =
          eps_qos_bit_rate_value(new_flow_qos->bitRates.guarBitRateForUL);
      flow_qos.mbr.br_dl =
          eps_qos_bit_rate_value(new_flow_qos->bitRates.maxBitRateForDL);
      flow_qos.mbr.br_ul =
          eps_qos_bit_rate_value(new_flow_qos->bitRates.maxBitRateForUL);
    }
    if (new_flow_qos->bitRatesExtPresent) {
      flow_qos.gbr.br_dl +=
          eps_qos_bit_rate_ext_value(new_flow_qos->bitRates.guarBitRateForDL);
      flow_qos.gbr.br_ul +=
          eps_qos_bit_rate_ext_value(new_flow_qos->bitRates.guarBitRateForUL);
      flow_qos.mbr.br_dl +=
          eps_qos_bit_rate_ext_value(new_flow_qos->bitRates.maxBitRateForDL);
      flow_qos.mbr.br_ul +=
          eps_qos_bit_rate_ext_value(new_flow_qos->bitRates.maxBitRateForUL);
    }
    flow_qos.qci = new_flow_qos->qci; /**< If not changed, must be the QCI of
                                         the current bearer. */
  }

  /** Verify the received the bearer resource modification request. */
  esm_cause = mme_app_validate_bearer_resource_modification(
      ue_id, ebi, &linked_ebi, tad, flow_qos.qci ? &flow_qos : NULL);
  if (esm_cause != ESM_CAUSE_SUCCESS) {
    /** If the error is due QoS, we have a valid TFT. And can continue to
     * process it. */
    if (esm_cause != ESM_CAUSE_SYNTACTICAL_ERROR_IN_THE_TFT_OPERATION &&
      ...
      OAILOG_ERROR(LOG_NAS_ESM,
                   "ESM-PROC  - Failed to verify the received Bearer Resource "
                   "Modification Request "
                   "(ebi=%d ue_id=" MME_UE_S1AP_ID_FMT
                   ", pti=%d) with esm_cause %d. \n",
  ...
  nas_esm_proc_bearer_context_t *esm_proc_bearer_context =
      _esm_proc_create_bearer_context_procedure(
          ue_id, pti, linked_ebi, PDN_CONTEXT_IDENTIFIER_UNASSIGNED, ebi,
          INVALID_TEID, 3, 0, _bearer_resource_modification_timeout_handler);
  if (!esm_proc_bearer_context) {
    OAILOG_ERROR(LOG_NAS_ESM,
                 "ESM-PROC  - Error creating bearer context ESM procedure for "
                 "the received Bearer Resource Modification Request "
  ...
  if (esm_cause != ESM_CAUSE_SUCCESS) {
    ...
    if (esm_cause == ESM_CAUSE_SYNTACTICAL_ERROR_IN_THE_TFT_OPERATION) {
      ...
      OAILOG_ERROR(LOG_NAS_ESM,
                   "ESM-PROC  - Since we have a valid TFT operation, ignoring "
                   "the received flow-qos modification error"
                   "(ebi=%d ue_id=" MME_UE_S1AP_ID_FMT
                   ", pti=%d) with esm_cause %d. \n",
                   ebi, ue_id, pti, esm_cause);
      /** Continue, but, in addition to those filters which are in the req/res
       * message, we may also have filters in the TFT.. Need to remove them in
       * the UE, as well.. */
  ...
  OAILOG_INFO(LOG_NAS_ESM,
              "ESM-PROC  - Successfully verified the received Bearer Resource "
              "Modification Request "
              "(ebi=%d ue_id=" MME_UE_S1AP_ID_FMT ", pti=%d). \n",
              ebi, ue_id, pti);
  /** Don't start the timer on the ESM procedure. Trigger a Delete Bearer
   * Command message. */
  nas_itti_s11_bearer_resource_cmd(pti, esm_proc_bearer_context->linked_ebi,
                                   esm_proc_bearer_context->mme_s11_teid,
                                   &esm_proc_bearer_context->saegw_s11_fteid,
                                   ebi, tad, &flow_qos);
...
void esm_proc_bearer_resource_failure(mme_ue_s1ap_id_t ue_id,
  ...
  OAILOG_INFO(LOG_NAS_ESM,
              "ESM-PROC  - Bearer Resource Failure Indication "
  ...
  if (esm_proc_bearer_context) {
    ...
    esm_send_bearer_resource_modification_reject(
        esm_proc_bearer_context->esm_base_proc.pti, esm_rsp_msg,
    ...
    OAILOG_WARNING(LOG_NAS_ESM,
                   "ESM-PROC  - Removed the current ESM procedure for the "
                   "received Bearer Resource Failure Indication "
                   "(ue_id=" MME_UE_S1AP_ID_FMT ", pti=%d)\n",
                   ue_id, pti);
    OAILOG_FUNC_OUT(LOG_MME_APP);
  }
  OAILOG_WARNING(LOG_NAS_ESM,
                 "ESM-PROC  - No ESM procedure could be found for the received "
...
static esm_cause_t _bearer_resource_modification_timeout_handler(
   ...
   */
  esm_send_bearer_resource_modification_reject(esm_base_proc->pti, esm_resp_msg,
