    ...
    switch (ITTI_MSG_ID(received_message_p)) {
      ...
      case NAS_PDN_DISCONNECT_RSP: {
        nas_esm_proc_pdn_disconnect_res(
            &NAS_PDN_DISCONNECT_RSP(received_message_p));
      } break;

      /** Messages sent directly from MME_APP to NAS_ESM layer for S11 session
       * responses. */
      case NAS_ACTIVATE_EPS_BEARER_CTX_REQ: {
        nas_esm_proc_activate_eps_bearer_ctx(
            &NAS_ACTIVATE_EPS_BEARER_CTX_REQ(received_message_p));
      } break;

      case NAS_MODIFY_EPS_BEARER_CTX_REQ: {
        nas_esm_proc_modify_eps_bearer_ctx(
            &NAS_MODIFY_EPS_BEARER_CTX_REQ(received_message_p));
      } break;

      case NAS_DEACTIVATE_EPS_BEARER_CTX_REQ: {
        nas_esm_proc_deactivate_eps_bearer_ctx(
            &NAS_DEACTIVATE_EPS_BEARER_CTX_REQ(received_message_p));
      } break;

      /** S11 Message. */
      case S11_BEARER_RESOURCE_FAILURE_INDICATION: {
        nas_esm_proc_bearer_resource_failure_indication(
            &S11_BEARER_RESOURCE_FAILURE_INDICATION(received_message_p));
      } break;
