...
/*
   Timer handlers
*/
static esm_cause_t _dedicated_eps_bearer_activate_t3485_handler(
...
void esm_send_activate_dedicated_eps_bearer_context_request(
   ...
   */
  esm_msg->activate_dedicated_eps_bearer_context_request.protocoldiscriminator =
      EPS_SESSION_MANAGEMENT_MESSAGE;
  esm_msg->activate_dedicated_eps_bearer_context_request.epsbeareridentity =
      ebi;
  esm_msg->activate_dedicated_eps_bearer_context_request.messagetype =
  ...
  esm_msg->activate_dedicated_eps_bearer_context_request
      .linkedepsbeareridentity = linked_ebi;
  /*
   * Mandatory - EPS QoS
   */
  memcpy((void *)&esm_msg->activate_dedicated_eps_bearer_context_request.epsqos,
         qos, sizeof(EpsQualityOfService));
  /*
   * Mandatory - traffic flow template
   */
  if (tft) {
    memcpy(&esm_msg->activate_dedicated_eps_bearer_context_request.tft, tft,
  ...
  //    sizeof(protocol_configuration_options_t)); msg->presencemask |=
...
esm_cause_t esm_proc_dedicated_eps_bearer_context(
  ...
  if (!pdn_context) {
    OAILOG_ERROR(
        LOG_NAS_EMM,
        "EMMCN-SAP  - "
        "No PDN context was found for UE " MME_UE_S1AP_ID_FMT
        " for cid %d and default ebi %d to assign dedicated bearers.\n",
  ...
  /** Check if the dedicated bearer id has already been set, if so try to find a
   * procedure and trigger the message again. */
  if (bc_tbc->eps_bearer_id && retry) {
    ...
    if (esm_proc_bearer_context) {
      esm_cause_t esm_cause = _dedicated_eps_bearer_activate_t3485_handler(
          esm_proc_bearer_context, esm_rsp_msg, NULL);
      OAILOG_INFO(
          LOG_NAS_EMM,
          "EMMCN-SAP  - "
          "A bearer context activation procedure for UE " MME_UE_S1AP_ID_FMT
          " (pti=%d, ebi=%d) already exists. "
   ...
   * This should only be for dedicated bearers (todo: handover with dedicated
   * bearers). In case of handover, this should also work.
   */
  // todo: PCO handling
  traffic_flow_template_t *tft = bc_tbc->tft;
  esm_cause = mme_app_register_dedicated_bearer_context(
      ue_id, ESM_EBR_ACTIVE_PENDING, pdn_context->context_identifier,
      pdn_context->default_ebi, bc_tbc,
      ESM_EBI_UNASSIGNED); /**< We set the ESM state directly to active in
                              handover. */
  if (esm_cause != ESM_CAUSE_SUCCESS) {
    OAILOG_ERROR(
        LOG_NAS_ESM,
        "ESM-PROC  - Error assigning bearer context for ue " MME_UE_S1AP_ID_FMT
        ". \n",
        ue_id);
   ...
   */
  nas_esm_proc_bearer_context_t *esm_proc_bearer_context =
      _esm_proc_create_bearer_context_procedure(
          ue_id, pti, linked_ebi, pdn_cid, bc_tbc->eps_bearer_id,
          bc_tbc->s1u_sgw_fteid.teid, mme_config.nas_config.t3485_sec,
          0 /*usec*/, _dedicated_eps_bearer_activate_t3485_handler);
  if (!esm_proc_bearer_context) {
    ...
    mme_app_release_bearer_context(ue_id, &pdn_context->context_identifier,
  ...
  qos_params_to_eps_qos(
      bc_tbc->bearer_level_qos.qci, bc_tbc->bearer_level_qos.mbr.br_dl,
      bc_tbc->bearer_level_qos.mbr.br_ul, bc_tbc->bearer_level_qos.gbr.br_dl,
      bc_tbc->bearer_level_qos.gbr.br_ul, &eps_qos, false);
  esm_send_activate_dedicated_eps_bearer_context_request(
      PROCEDURE_TRANSACTION_IDENTITY_UNASSIGNED, bc_tbc->eps_bearer_id,
      esm_rsp_msg, linked_ebi, &eps_qos, tft, &bc_tbc->pco);
  OAILOG_FUNC_RETURN(LOG_NAS_ESM, ESM_CAUSE_SUCCESS);
}
...
esm_cause_t esm_proc_dedicated_eps_bearer_context_accept(
  ...
  if (esm_cause == ESM_CAUSE_SUCCESS) {
    ...
    nas_itti_activate_eps_bearer_ctx_cnf(ue_id, ebi,
                                         esm_bearer_procedure->s1u_saegw_teid);
  } else {
    nas_itti_activate_eps_bearer_ctx_rej(
        ue_id, esm_bearer_procedure->s1u_saegw_teid, esm_cause);
  }
...
void esm_proc_dedicated_eps_bearer_context_reject(
   ...
   */
  mme_app_release_bearer_context(ue_id, &esm_bearer_procedure->pdn_cid,
                                 esm_bearer_procedure->linked_ebi, ebi);
...
static esm_cause_t _dedicated_eps_bearer_activate_t3485_handler(
  ...
  if (esm_base_proc->retx_count < DEDICATED_EPS_BEARER_ACTIVATE_COUNTER_MAX) {
    ...
    if (pdn_context) {
      ...
      if (bearer_context) {
        /** Check if the default bearer has been created on the eNB side, if not
         * set the ll-handler to something else. */
        if (!(bearer_context->bearer_state & BEARER_STATE_ENB_CREATED)) {
          OAILOG_WARNING(
              LOG_NAS_ESM,
              "ESM-PROC  - Dedicated EPS bearer context not established on the "
              "eNB side yet (ue_id=" MME_UE_S1AP_ID_FMT ", ebi=%d).\n",
          ...
          if (ll_handler_arg) {
            ...
            OAILOG_WARNING(
                LOG_NAS_ESM,
                "ESM-PROC  - Dedicated EPS bearer context not established on "
                "the eNB side yet (ue_id=" MME_UE_S1AP_ID_FMT
                ", ebi=%d), but altering ll callback handler.\n",
                esm_base_proc->ue_id, bearer_context->ebi);
          }
        }
        ...
        qos_params_to_eps_qos(bearer_context->bearer_level_qos.qci,
                              ...
                              &eps_qos, false);
        esm_send_activate_dedicated_eps_bearer_context_request(
            PROCEDURE_TRANSACTION_IDENTITY_UNASSIGNED, bearer_context->ebi,
            esm_resp_msg, bearer_context->linked_ebi,
            &bearer_context->bearer_level_qos,
        ...
        esm_base_proc->esm_proc_timer.id = nas_esm_timer_start(
            mme_config.nas_config.t3485_sec, 0 /*usec*/, (void *)esm_base_proc);
  ...
  nas_itti_activate_eps_bearer_ctx_rej(esm_base_proc->ue_id, s1u_saegw_teid,
  ...
  mme_app_release_bearer_context(
      esm_base_proc->ue_id, &esm_proc_bearer_context->pdn_cid,
      esm_proc_bearer_context->linked_ebi, esm_proc_bearer_context->bearer_ebi);
