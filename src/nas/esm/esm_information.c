...
static esm_cause_t _esm_information_t3489_handler(nas_esm_proc_t *esm_base_proc,
                                                  ESM_msg *esm_rsp_msg);

static void esm_send_esm_information_request(pti_t pti, ebi_t ebi,
                                             ESM_msg *esm_req_msg);
...
static void esm_send_esm_information_request(pti_t pti, ebi_t ebi,
   ...
   */
  esm_req_msg->esm_information_request.protocoldiscriminator =
  ...
  OAILOG_NOTICE(
      ...
      esm_req_msg->esm_information_request.proceduretransactionidentity,
      esm_req_msg->esm_information_request.epsbeareridentity);
...
void esm_proc_esm_information_request(
   ...
   * start timer T3489
   */
  esm_send_esm_information_request(esm_proc_pdn_connectivity->esm_base_proc.pti,
  ...
  esm_proc_pdn_connectivity->esm_base_proc.esm_proc_timer.id =
      nas_esm_timer_start(mme_config.nas_config.t3489_sec, 0 /*usec*/,
                          (nas_esm_proc_t *)esm_proc_pdn_connectivity);
...
void esm_proc_esm_information_response(
  ...
  /** Process the result. */
  if (esm_information_resp->presencemask &
    ...
    esm_proc_pdn_connectivity->subscribed_apn =
        bstrcpy(esm_information_resp->accesspointname);
  } else {
    /** No APN Name received from UE. We will used the default one from the
     * subscription data. */
    OAILOG_ERROR(
        LOG_NAS_ESM,
        "ESM-SAP   - No APN name received in the ESM information response "
        "(ue_id=%d, pti=%d).\n",
        ue_id, pti);
  }
  ...
  if (esm_information_resp->presencemask &
    ...
    copy_protocol_configuration_options(
        &esm_proc_pdn_connectivity->pco,
        &esm_information_resp->protocolconfigurationoptions);
  }
...
static esm_cause_t _esm_information_t3489_handler(nas_esm_proc_t *esm_base_proc,
  ...
  if (esm_base_proc->retx_count < ESM_INFORMATION_COUNTER_MAX) {
     ...
     */
    esm_send_pdn_connectivity_reject(esm_base_proc->pti, esm_rsp_msg,
