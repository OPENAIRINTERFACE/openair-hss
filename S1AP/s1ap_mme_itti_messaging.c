  ...
  const uint32_t length)
{
  ...
  MSC_LOG_TX_MESSAGE (MSC_S1AP_MME, MSC_NAS_MME, NULL, 0, "0 NAS_UPLINK_DATA_IND ue_id " S1AP_UE_ID_FMT " len %u", NAS_UL_DATA_IND (message_p).UEid, NAS_UL_DATA_IND (message_p).nasMsg.length);
  ...
  nas_error_code_t error_code)
{
  ...
  MSC_LOG_TX_MESSAGE (MSC_S1AP_MME, MSC_NAS_MME, NULL, 0, "0 NAS_DOWNLINK_DATA_CNF ue_id " S1AP_UE_ID_FMT " errCode %u", NAS_DL_DATA_CNF (message_p).UEid, NAS_DL_DATA_CNF (message_p).errCode);
  return itti_send_msg_to_task (TASK_NAS_MME, INSTANCE_DEFAULT, message_p);
}
