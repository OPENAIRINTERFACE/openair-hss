  ...
  const struct emm_data_context_s * const elm_pP)
{
  if (elm_pP ) {
    ...
    LOG_INFO (LOG_NAS_EMM, "         imsi:             %s      (The IMSI provided by the UE or the MME)\n", imsi_str);
    ...
    LOG_INFO (LOG_NAS_EMM, "         guti:             %s      (The GUTI assigned to the UE)\n", guti_str);
    ...
    if (elm_pP->security ) {
      ...
      LOG_INFO (LOG_NAS_EMM, "             kasme: %s     (ASME security key (native context))\n", key_string);
      ...
      LOG_INFO (LOG_NAS_EMM, "             knas_enc: %s     (NAS cyphering key)\n", key_string);
      ...
      LOG_INFO (LOG_NAS_EMM, "             knas_int: %s     (NAS integrity key)\n", key_string);
      LOG_INFO (LOG_NAS_EMM, "             dl_count.overflow: %u\n", elm_pP->security->dl_count.overflow);
      LOG_INFO (LOG_NAS_EMM, "             dl_count.seq_num:  %u\n", elm_pP->security->dl_count.seq_num);
      LOG_INFO (LOG_NAS_EMM, "             ul_count.overflow: %u\n", elm_pP->security->ul_count.overflow);
      LOG_INFO (LOG_NAS_EMM, "             ul_count.seq_num:  %u\n", elm_pP->security->ul_count.seq_num);
      LOG_INFO (LOG_NAS_EMM, "             TODO  capability");
      LOG_INFO (LOG_NAS_EMM, "             selected_algorithms.encryption:  %x\n", elm_pP->security->selected_algorithms.encryption);
      LOG_INFO (LOG_NAS_EMM, "             selected_algorithms.integrity:   %x\n", elm_pP->security->selected_algorithms.integrity);
    } else {
      LOG_INFO (LOG_NAS_EMM, "         No security context\n");
    }

    LOG_INFO (LOG_NAS_EMM, "         _emm_fsm_status     %u\n", elm_pP->_emm_fsm_status);
