  ...
  uint32_t len)
{
  ...
  *(buffer + encoded) = ((encode_u8_linked_eps_bearer_identity (&bearer_resource_allocation_request->linkedepsbeareridentity) & 0x0f) << 4) | 0x00;
  ...
  if ((encode_result = encode_traffic_flow_template (&bearer_resource_allocation_request->trafficflowaggregate, 0, buffer + encoded, len - encoded)) < 0)  //Return in case of error
