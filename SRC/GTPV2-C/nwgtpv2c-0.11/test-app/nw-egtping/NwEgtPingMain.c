  ...
  char *argv[])
{
  ...
  if ((argc == 2) && ((strcmp ("--help", argv[i]) == 0)
                      || (strcmp (argv[i], "-h") == 0)))
    return NW_FAILURE;
  ...
  char *argv[])
{
  ...
  rc = nwGtpv2cUlpPing (&ulpObj, inet_addr (egtPing.targetIpStr), egtPing.pingCount, egtPing.pingInterval, 2, 3);
  NW_ASSERT (NW_OK == rc);
  /*---------------------------------------------------------------------------
