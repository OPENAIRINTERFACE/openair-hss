  ...
  nwUdpDataIndicationCallbackData) {
    ...
    if (bytesRead > 0) {
      ...
      NW_LOG (NW_LOG_LEVEL_DEBG, "Received UDP message of size %u from peer %u.%u.%u.%u:%u", bytesRead,
              (peer.sin_addr.s_addr & 0x000000ff), (peer.sin_addr.s_addr & 0x0000ff00) >> 8, (peer.sin_addr.s_addr & 0x00ff0000) >> 16, (peer.sin_addr.s_addr & 0xff000000) >> 24, ntohs (peer.sin_port));
...
/*---------------------------------------------------------------------------
  ...
  uint32_t peerPort) {
    ...
    NW_LOG (NW_LOG_LEVEL_DEBG, "Sending %u bytes of data to %u.%u.%u.%u:%u", dataSize, (peerIp & 0xff000000) >> 24, (peerIp & 0x00ff0000) >> 16, (peerIp & 0x0000ff00) >> 8, (peerIp & 0x000000ff), peerPort);
