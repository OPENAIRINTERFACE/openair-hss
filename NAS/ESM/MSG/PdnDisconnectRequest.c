  ...
  uint32_t len)
{
  ...
  while (len - decoded > 0) {
    ...
    case PDN_DISCONNECT_REQUEST_PROTOCOL_CONFIGURATION_OPTIONS_IEI:
      if ((decoded_result = decode_protocol_configuration_options (&pdn_disconnect_request->protocolconfigurationoptions, PDN_DISCONNECT_REQUEST_PROTOCOL_CONFIGURATION_OPTIONS_IEI, buffer + decoded, len - decoded)) <= 0)
        return decoded_result;
  ...
  uint32_t len)
{
  ...
  *(buffer + encoded) = (encode_u8_linked_eps_bearer_identity (&pdn_disconnect_request->linkedepsbeareridentity) & 0x0f);
  ...
  if ((pdn_disconnect_request->presencemask & PDN_DISCONNECT_REQUEST_PROTOCOL_CONFIGURATION_OPTIONS_PRESENT)
    ...
    if ((encode_result = encode_protocol_configuration_options (&pdn_disconnect_request->protocolconfigurationoptions, PDN_DISCONNECT_REQUEST_PROTOCOL_CONFIGURATION_OPTIONS_IEI, buffer + encoded, len - encoded)) < 0)
