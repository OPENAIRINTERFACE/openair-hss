  ...
  const network_tft_t * tft)
{
  ...
  if (pid < ESM_DATA_PDN_MAX) {
     ...
     */
      ...
      LOG_TRACE (WARNING, "ESM-PROC  - The total number of active EPS" "bearers is exceeded");
    } else {
      /*
      ...
      if (is_default) {
        ...
        if (pdn->bearer[bid] != NULL) {
          LOG_TRACE (ERROR, "ESM-PROC  - A default EPS bearer context " "is already allocated");
          LOG_FUNC_RETURN (ESM_EBI_UNASSIGNED);
        }
      } else {
        /*
  ...
  if (bid < ESM_DATA_EPS_BEARER_MAX) {
    ...
    LOG_TRACE (WARNING, "ESM-PROC  - Failed to create new EPS bearer " "context (ebi=%d)", ebi);
  }
  ...
  int *bid)
{
  ...
  if (found) {
    ...
    if (pdn->bearer[*bid]->bid != *bid) {
      LOG_TRACE (ERROR, "ESM-PROC  - EPS bearer identifier %d is " "not valid", *bid);
      LOG_FUNC_RETURN (ESM_EBI_UNASSIGNED);
    }
    ...
    if (*bid == 0) {
      ...
      for (i = 1; pdn->n_bearers > 0; i++) {
        if (pdn->bearer[i]) {
          LOG_TRACE (WARNING, "ESM-PROC  - Release EPS bearer context " "(ebi=%d)", pdn->bearer[i]->ebi);
