  ...
  int length)
{
  ...
  default:
    bytes = 0;
    LOG_TRACE (WARNING, "NET-API   - AS message 0x%x is not valid", msg->msgID);
    break;
  }
  ...
  LOG_TRACE (WARNING, "NET-API   - Failed to decode AS message 0x%x", msg->msgID);
  ...
  int length)
{
  ...
  LOG_TRACE (WARNING, "NET-API   - Failed to encode AS message 0x%x", msg->msgID);
  LOG_FUNC_RETURN (RETURNerror);
}
