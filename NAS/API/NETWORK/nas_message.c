  ...
  void *security)
{
  ...
  LOG_TRACE (DEBUG, "Incremented emm_security_context.dl_count.seq_num -> %u", emm_security_context->dl_count.seq_num);
  ...
  if (size < 0) {
    ...
    if (emm_security_context) {
      ...
      if (mac == msg->header.message_authentication_code) {
        ...
        LOG_TRACE (DEBUG, "msg->header.message_authentication_code = %04X != computed = %04X", msg->header.message_authentication_code, mac);
      }
    }
  ...
  void *security)
{
  ...
  if (size < 0) {
    ...
    if (bytes > 0) {
       ...
       * Compute the NAS message authentication code
       */
      LOG_TRACE (DEBUG, "offset %d = %d - %d, hdr encode = %d, length = %d bytes = %d", offset, size, sizeof (uint8_t), size, length, bytes);
  ...
  if (header->protocol_discriminator == EPS_MOBILITY_MANAGEMENT_MESSAGE) {
    if (header->security_header_type != SECURITY_HEADER_TYPE_NOT_PROTECTED) {
      ...
      if (length < NAS_MESSAGE_SECURITY_HEADER_SIZE) {
         ...
         */
        LOG_TRACE (WARNING, "NET-API   - The size of the header (%u) " "exceeds the buffer length (%u)", NAS_MESSAGE_SECURITY_HEADER_SIZE, length);
        LOG_FUNC_RETURN (-1);
      }
  ...
  int length)
{
  ...
  if (header->protocol_discriminator == EPS_MOBILITY_MANAGEMENT_MESSAGE) {
     ...
     */
    LOG_TRACE (WARNING, "NET-API   - Protocol discriminator 0x%x is " "not supported", header->protocol_discriminator);
  }
  ...
  nas_message_decode_status_t * status)
{
  ...
  case SECURITY_HEADER_TYPE_INTEGRITY_PROTECTED:
  case SECURITY_HEADER_TYPE_INTEGRITY_PROTECTED_NEW:
    LOG_TRACE (DEBUG, "No decryption of message length %u according to security header type 0x%02x", length, security_header_type);
    ...
    if (NULL != emm_security_context) {
      switch (emm_security_context->selected_algorithms.encryption) {
        ...
        case NAS_SECURITY_ALGORITHMS_EEA0:
          LOG_TRACE (DEBUG, "NAS_SECURITY_ALGORITHMS_EEA0 dir %d ul_count.seq_num %d dl_count.seq_num %d", direction, emm_security_context->ul_count.seq_num, emm_security_context->dl_count.seq_num);
          memcpy (dest, src, length);
          /*
           * Decode the first octet (security header type or EPS bearer identity,
           * * * * and protocol discriminator)
        ...
        default:
          LOG_TRACE (ERROR, "Unknown Cyphering protection algorithm %d", emm_security_context->selected_algorithms.encryption);
          memcpy (dest, src, length);
          /*
           * Decode the first octet (security header type or EPS bearer identity,
           * * * * and protocol discriminator)
      ...
      LOG_TRACE (ERROR, "No security context");
      LOG_FUNC_RETURN (0);
    }
  ...
  default:
    LOG_TRACE (ERROR, "Unknown security header type %u", security_header_type);
    LOG_FUNC_RETURN (0);
  };
}
  ...
  const emm_security_context_t * const emm_security_context)
{
  ...
  if (!emm_security_context) {
    LOG_TRACE (ERROR, "No security context set for encryption protection algorithm");
    LOG_FUNC_RETURN (0);
  }
  ...
  case SECURITY_HEADER_TYPE_INTEGRITY_PROTECTED:
  case SECURITY_HEADER_TYPE_INTEGRITY_PROTECTED_NEW:
    LOG_TRACE (DEBUG, "No encryption of message according to security header type 0x%02x", security_header_type);
    memcpy (dest, src, length);
    LOG_FUNC_RETURN (length);
    break;
  ...
  const emm_security_context_t * const emm_security_context)
{
  ...
  case NAS_SECURITY_ALGORITHMS_EIA1:{
      ...
      LOG_TRACE (DEBUG, "NAS_SECURITY_ALGORITHMS_EIA1 returned MAC %x.%x.%x.%x(%u) for length %d direction %d, count %d", mac[0], mac[1], mac[2], mac[3], *((uint32_t *) & mac), length, direction, count);
      mac32 = (uint32_t *) & mac;
  ...
  case NAS_SECURITY_ALGORITHMS_EIA2:{
      ...
      LOG_TRACE (DEBUG, "NAS_SECURITY_ALGORITHMS_EIA2 returned MAC %x.%x.%x.%x(%u) for length %d direction %d, count %d", mac[0], mac[1], mac[2], mac[3], *((uint32_t *) & mac), length, direction, count);
      mac32 = (uint32_t *) & mac;
      LOG_FUNC_RETURN (ntohl (*mac32));
    }
    break;
  ...
  default:
    LOG_TRACE (ERROR, "Unknown integrity protection algorithm %d", emm_security_context->selected_algorithms.integrity);
    break;
  }
