...
STAILQ_HEAD (
  ...
  udp_socket_desc_s) udp_socket_list;
     ...
     static
     struct udp_socket_desc_s               *udp_eNB_get_socket_desc (
  task_id_t task_id);
  ...
  void *args_p);
     ...
     int                                     udp_enb_init (
  const Enb_properties_t * enb_config_p);
  ...
  uint32_t length)
{
  ...
  if (sendto (sd, (void *)buffer, (size_t) length, 0, (struct sockaddr *)&to, to_length) < 0) {
    LOG_E (UDP_, "[SD %d] Failed to send data to " IPV4_ADDR " on port %d, buffer size %u\n", sd, IPV4_ADDR_FORMAT (address), port, length);
    return -1;
  }
#if LOG_UDP > 0
  LOG_I (UDP_, "[SD %d] Successfully sent to " IPV4_ADDR " on port %d, buffer size %u, buffer address %x\n", sd, IPV4_ADDR_FORMAT (address), port, length, buffer);
#endif
  return 0;
}
  ...
  struct udp_socket_desc_s *udp_sock_pP)
{
  ...
  if (1) {
    ...
    if ((n = recvfrom (udp_sock_pP->sd, l_buffer, sizeof (l_buffer), 0, (struct sockaddr *)&addr, &from_len)) < 0) {
  ...
  while (1) {
    ...
    if (received_message_p != NULL) {
      ...
      case UDP_INIT:{
          ...
          udp_eNB_create_socket (udp_init_p->port, udp_init_p->address, ITTI_MSG_ORIGIN_ID (received_message_p));
        }
        break;
      ...
      case UDP_DATA_REQ:{
          ...
          if (udp_sock_p == NULL) {
            LOG_E (UDP_, "Failed to retrieve the udp socket descriptor " "associated with task %d\n", ITTI_MSG_ORIGIN_ID (received_message_p));
            pthread_mutex_unlock (&udp_socket_list_mutex);
          ...
          if (bytes_written != udp_data_req_p->buffer_length) {
            LOG_E (UDP_, "There was an error while writing to socket %u rc %d" "(%d:%s) May be normal if GTPU kernel module loaded on same host (may NF_DROP IP packet)\n", udp_sd, bytes_written, errno, strerror (errno));
          }
